# 每日 GitHub 动态 (2026-04-18)

我关注用户的今日公开动态 (每15分钟更新)。

## 今日动态

### [0xbug](https://github.com/0xbug)
- 🌟 👤 [0xbug](https://github.com/0xbug) Starred [nats-io/nats.py](https://github.com/nats-io/nats.py)
  > Python3 client for NATS
- 🌟 👤 [0xbug](https://github.com/0xbug) Starred [modelcontextprotocol/go-sdk](https://github.com/modelcontextprotocol/go-sdk)
  > The official Go SDK for Model Context Protocol servers and clients. Maintained in collaboration with...

### [1c7](https://github.com/1c7)
- 🌟 👤 [1c7](https://github.com/1c7) Starred [clash-verge-rev/clash-verge-rev](https://github.com/clash-verge-rev/clash-verge-rev)
  > A modern GUI client based on Tauri, designed to run in Windows, macOS and Linux for tailored proxy e...

### [34306](https://github.com/34306)
- 🌟 👤 [34306](https://github.com/34306) Starred [rooootdev/lara](https://github.com/rooootdev/lara)
  > WIP darksword kexploit implement

### [AEnjoy](https://github.com/AEnjoy)
- 🌟 👤 [AEnjoy](https://github.com/AEnjoy) Starred [arduino/go-apt-client](https://github.com/arduino/go-apt-client)

### [Abyss-W4tcher](https://github.com/Abyss-W4tcher)
- 🌟 👤 [Abyss-W4tcher](https://github.com/Abyss-W4tcher) Starred [Abyss-W4tcher/volatility3-symbols](https://github.com/Abyss-W4tcher/volatility3-symbols)
  > Collection of Linux and macOS Volatility3 Intermediate Symbol Files (ISF), suitable for memory analy...

### [Adrikikicp2](https://github.com/Adrikikicp2)
- 🍴 👤 [Adrikikicp2](https://github.com/Adrikikicp2) Forked [AndresTraks/HlslDecompiler](https://github.com/AndresTraks/HlslDecompiler) to [Adrikikicp2/HlslDecompiler-builder](https://github.com/Adrikikicp2/HlslDecompiler-builder)
  > Decompiles Shader Model 1–4 shaders into HLSL code (work in progress)

### [Aira-Sakuranomiya](https://github.com/Aira-Sakuranomiya)
- 🌟 👤 [Aira-Sakuranomiya](https://github.com/Aira-Sakuranomiya) Starred [hakadao/ArknightsParallaxCarousel](https://github.com/hakadao/ArknightsParallaxCarousel)
  > 明日方舟官網3D視差輪播效果實現

### [AlexiaChen](https://github.com/AlexiaChen)
- 🌟 👤 [AlexiaChen](https://github.com/AlexiaChen) Starred [GMWalletApp/epusdt](https://github.com/GMWalletApp/epusdt)
  > 开源优雅的跨平台收款网关 Easy Payment USDT——epsdt
- 🌟 👤 [AlexiaChen](https://github.com/AlexiaChen) Starred [xataio/xata](https://github.com/xataio/xata)
  > Open source, cloud native, Postgres platform with copy-on-write branching and scale-to-zero
- 🌟 👤 [AlexiaChen](https://github.com/AlexiaChen) Starred [bwjoke/BTC-Trading-Since-2020](https://github.com/bwjoke/BTC-Trading-Since-2020)
  > Public BTC trading context since 2020.

### [BeneficialCode](https://github.com/BeneficialCode)
- 🌟 👤 [BeneficialCode](https://github.com/BeneficialCode) Starred [komimoe/Arkari](https://github.com/komimoe/Arkari)
  > Yet another llvm based obfuscator based on goron.

### [CCint3](https://github.com/CCint3)
- 🌟 👤 [CCint3](https://github.com/CCint3) Starred [baulk/git-minimal](https://github.com/baulk/git-minimal)
  > Git on Linux build script

### [CasterWx](https://github.com/CasterWx)
- 🌟 👤 [CasterWx](https://github.com/CasterWx) Starred [xixu-me/awesome-persona-distill-skills](https://github.com/xixu-me/awesome-persona-distill-skills)
  > Curated list of Agent Skills centered on people, relationships, commemorative scenes, and methodolog...

### [Chordp](https://github.com/Chordp)
- 🌟 👤 [Chordp](https://github.com/Chordp) Starred [SimoneAvogadro/android-reverse-engineering-skill](https://github.com/SimoneAvogadro/android-reverse-engineering-skill)
  > Claude Code skill to support Android app's reverse engineering
- 🌟 👤 [Chordp](https://github.com/Chordp) Starred [Donchitos/Claude-Code-Game-Studios](https://github.com/Donchitos/Claude-Code-Game-Studios)
  > Turn Claude Code into a full game dev studio — 49 AI agents, 72 workflow skills, and a complete coor...
- 🌟 👤 [Chordp](https://github.com/Chordp) Starred [jamiepine/voicebox](https://github.com/jamiepine/voicebox)
  > The open-source voice synthesis studio

### [Col-E](https://github.com/Col-E)
- 🌟 👤 [Col-E](https://github.com/Col-E) Starred [3000IQPlay/obfuscator-list](https://github.com/3000IQPlay/obfuscator-list)
  > List of all the somewhat popular and non-popular Java obfuscators on the Internet.

### [Cracked5pider](https://github.com/Cracked5pider)
- 🌟 👤 [Cracked5pider](https://github.com/Cracked5pider) Starred [ArduPilot/ardupilot](https://github.com/ArduPilot/ardupilot)
  > ArduPlane, ArduCopter, ArduRover, ArduSub source

### [ErodedElk](https://github.com/ErodedElk)
- 🌟 👤 [ErodedElk](https://github.com/ErodedElk) Starred [P4nda0s/IDA-NO-MCP](https://github.com/P4nda0s/IDA-NO-MCP)
  > Say goodbye to the complex, verbose, and laggy interaction mode of IDA Pro MCP

### [FBLeee](https://github.com/FBLeee)
- 🌟 👤 [FBLeee](https://github.com/FBLeee) Starred [guheng-re/unflat](https://github.com/guheng-re/unflat)

### [Firebasky](https://github.com/Firebasky)
- 🌟 👤 [Firebasky](https://github.com/Firebasky) Starred [wang-h/werss](https://github.com/wang-h/werss)
  > WeRSS - AI Powered 微信公众号热度分析系统，支持微信公众号定时采集、文章导出、关键词提取、主题聚类分析、可视化、文章导出、飞书/钉钉/微信消息支持，附带外部可访问API/MCP及AI...

### [Germey](https://github.com/Germey)
- 🌟 👤 [Germey](https://github.com/Germey) Starred [AceDataCloud/X402Client](https://github.com/AceDataCloud/X402Client)
  > X402 payment protocol client for AceDataCloud APIs

### [Gracker](https://github.com/Gracker)
- 🌟 👤 [Gracker](https://github.com/Gracker) Starred [wellwind/claude-code-from-source-zh-tw](https://github.com/wellwind/claude-code-from-source-zh-tw)

### [Henryhaohao](https://github.com/Henryhaohao)
- 🌟 👤 [Henryhaohao](https://github.com/Henryhaohao) Starred [S-Trespassing/claw-in-chrome](https://github.com/S-Trespassing/claw-in-chrome)
  > 这是一个放在 Chrome 侧边栏里的助手扩展。解除了Claude in Chrome的登录限制和套餐限制,支持自定义模型供应商,并且支持编辑原插件不支持编辑的参数,让您的大模型在该插件中有更好的表现...

### [Lakr233](https://github.com/Lakr233)
- 🚀 👤 [Lakr233](https://github.com/Lakr233) Made [Lakr233/bgclick-rev-skill](https://github.com/Lakr233/bgclick-rev-skill) public

### [MartinKayJr](https://github.com/MartinKayJr)
- 🌟 👤 [MartinKayJr](https://github.com/MartinKayJr) Starred [Jeric-X/SyncClipboard](https://github.com/Jeric-X/SyncClipboard)
  > 跨平台剪贴板同步、历史记录管理工具 / Cross-platform cipboard syncing, history management tool
- 🌟 👤 [MartinKayJr](https://github.com/MartinKayJr) Starred [Tools-cx-app/meta-magic_mount-rs](https://github.com/Tools-cx-app/meta-magic_mount-rs)
  > An implementation of a metamodule using Magic Mount, Based on MKSU, Template is from meta-overlay.
- 🌟 👤 [MartinKayJr](https://github.com/MartinKayJr) Starred [OukaroMF/OukaroManager](https://github.com/OukaroMF/OukaroManager)
  > A KernelSU module that gives you a simple WebUI to convert regular Android apps into system apps — n...

### [MolotovCherry](https://github.com/MolotovCherry)
- 🌟 👤 [MolotovCherry](https://github.com/MolotovCherry) Starred [lkroll/zed-qml](https://github.com/lkroll/zed-qml)
  > QML language support for Zed

### [Mr-xn](https://github.com/Mr-xn)
- 🍴 👤 [Mr-xn](https://github.com/Mr-xn) Forked [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) to [Mr-xn/CLIProxyAPI](https://github.com/Mr-xn/CLIProxyAPI)
  > Wrap Gemini CLI, Antigravity, ChatGPT Codex, Claude Code as an OpenAI/Gemini/Claude/Codex compatible...
- 🍴 👤 [Mr-xn](https://github.com/Mr-xn) Forked [PicGo/Awesome-PicGo](https://github.com/PicGo/Awesome-PicGo) to [Mr-xn/Awesome-PicGo](https://github.com/Mr-xn/Awesome-PicGo)
  > A collection of awesome projects using PicGo.
- 🌟 👤 [Mr-xn](https://github.com/Mr-xn) Starred [ccbkkb/MicroWARP](https://github.com/ccbkkb/MicroWARP)
  > 🚀 An 800KB RAM ultra-lightweight Cloudflare WARP SOCKS5 proxy in Docker. 仅需 800KB 内存的纯内核态 Cloudflare...

### [OnesoftQwQ](https://github.com/OnesoftQwQ)
- 🍴 👤 [OnesoftQwQ](https://github.com/OnesoftQwQ) Forked [JohanChane/clashtui](https://github.com/JohanChane/clashtui) to [OnesoftQwQ/clashtui](https://github.com/OnesoftQwQ/clashtui)
  > Mihomo (Clash.Meta) TUI Client

### [P001water](https://github.com/P001water)
- 🚀 👤 [P001water](https://github.com/P001water) Made [SecuraPath/DaofaGui](https://github.com/SecuraPath/DaofaGui) public
  > 一套把 FOFA 查询、模板复用、批量执行、结果导出与排错流程收进桌面端的工作台
- 🌟 👤 [P001water](https://github.com/P001water) Starred [P001water/P001water](https://github.com/P001water/P001water)
- 🌟 👤 [P001water](https://github.com/P001water) Starred [SecuraPath/DaofaGui](https://github.com/SecuraPath/DaofaGui)
  > 一套把 FOFA 查询、模板复用、批量执行、结果导出与排错流程收进桌面端的工作台

### [P4nda0s](https://github.com/P4nda0s)
- 🌟 👤 [P4nda0s](https://github.com/P4nda0s) Starred [P4nda0s/kpm-panda-hide](https://github.com/P4nda0s/kpm-panda-hide)
  > (kpm) (for Android hackers)Hide your frida and debuggers

### [PIKACHUIM](https://github.com/PIKACHUIM)
- 🌟 👤 [PIKACHUIM](https://github.com/PIKACHUIM) Starred [buptczq/WinCryptSSHAgent](https://github.com/buptczq/WinCryptSSHAgent)
  > Using a Yubikey for SSH Authentication on Windows Seamlessly

### [SeeFlowerX](https://github.com/SeeFlowerX)
- 🌟 👤 [SeeFlowerX](https://github.com/SeeFlowerX) Starred [Lakr233/bgclick-rev-skill](https://github.com/Lakr233/bgclick-rev-skill)

### [SimonTheCoder](https://github.com/SimonTheCoder)
- 🌟 👤 [SimonTheCoder](https://github.com/SimonTheCoder) Starred [lsdefine/GenericAgent](https://github.com/lsdefine/GenericAgent)
  > Self-evolving agent: grows skill tree from 3.3K-line seed, achieving full system control with 6x les...

### [Solaree](https://github.com/Solaree)
- 🌟 👤 [Solaree](https://github.com/Solaree) Starred [Solaree/solana-frontend-course](https://github.com/Solaree/solana-frontend-course)
  > A 4-lecture hands-on module for frontend developers transitioning from Web2 to building production-r...

### [SummerSec](https://github.com/SummerSec)
- 🌟 👤 [SummerSec](https://github.com/SummerSec) Starred [jamiepine/voicebox](https://github.com/jamiepine/voicebox)
  > The open-source voice synthesis studio
- 🌟 👤 [SummerSec](https://github.com/SummerSec) Starred [browser-use/video-use](https://github.com/browser-use/video-use)

### [WenHao2130](https://github.com/WenHao2130)
- 🍴 👤 [WenHao2130](https://github.com/WenHao2130) Forked [WenHao-Dev/twrp_device_oppo_taylor](https://github.com/WenHao-Dev/twrp_device_oppo_taylor) to [WenHao-Dev/twrp_device_oppo_luwu](https://github.com/WenHao-Dev/twrp_device_oppo_luwu)
  > A TWRP device tree for OPPO Reno10 (PHW110)

### [XiaoliChan](https://github.com/XiaoliChan)
- 🌟 👤 [XiaoliChan](https://github.com/XiaoliChan) Starred [mandiant/gopacket](https://github.com/mandiant/gopacket)
  > Gopacket is a clean Go implementation of Impacket, a library intended for working with network proto...

### [YuKongA](https://github.com/YuKongA)
- 🌟 👤 [YuKongA](https://github.com/YuKongA) Starred [tailscale/tailscale-rs](https://github.com/tailscale/tailscale-rs)
  > Rust implementation of Tailscale (preview, experimental)
- 🌟 👤 [YuKongA](https://github.com/YuKongA) Starred [tailscale/tailscale-android](https://github.com/tailscale/tailscale-android)
  > Tailscale Android Client

### [alexander-hanel](https://github.com/alexander-hanel)
- 🌟 👤 [alexander-hanel](https://github.com/alexander-hanel) Starred [clearbluejar/pyghidra-mcp](https://github.com/clearbluejar/pyghidra-mcp)
  > Python Command-Line Ghidra MCP

### [androidmalin](https://github.com/androidmalin)
- 🌟 👤 [androidmalin](https://github.com/androidmalin) Starred [warpdotdev/Warp](https://github.com/warpdotdev/Warp)
  > Warp is the agentic development environment, built for coding with multiple AI agents.

### [ant4g0nist](https://github.com/ant4g0nist)
- 🌟 👤 [ant4g0nist](https://github.com/ant4g0nist) Starred [Unity-Technologies/ml-agents](https://github.com/Unity-Technologies/ml-agents)
  > The Unity Machine Learning Agents Toolkit (ML-Agents) is an open-source project that enables games a...

### [bb33bb](https://github.com/bb33bb)
- 🌟 👤 [bb33bb](https://github.com/bb33bb) Starred [bb33bb/cheat-engine-mac](https://github.com/bb33bb/cheat-engine-mac)
  > Cheat Engine. A development environment focused on modding
- 🍴 👤 [bb33bb](https://github.com/bb33bb) Forked [LLiloMaster/iPhone16_bootrom](https://github.com/LLiloMaster/iPhone16_bootrom) to [bb33bb/iPhone16_bootrom](https://github.com/bb33bb/iPhone16_bootrom)

### [chancerly](https://github.com/chancerly)
- 🌟 👤 [chancerly](https://github.com/chancerly) Starred [nowsecure/r2frida](https://github.com/nowsecure/r2frida)
  > Radare2 and Frida better together.

### [cn-kali-team](https://github.com/cn-kali-team)
- 🌟 👤 [cn-kali-team](https://github.com/cn-kali-team) Starred [cloudwego/pilota](https://github.com/cloudwego/pilota)
  > A thrift and protobuf implementation in pure rust with high performance and extensibility.
- 🌟 👤 [cn-kali-team](https://github.com/cn-kali-team) Starred [tailscale/tailscale-rs](https://github.com/tailscale/tailscale-rs)
  > Rust implementation of Tailscale (preview, experimental)

### [codingsf](https://github.com/codingsf)
- 🌟 👤 [codingsf](https://github.com/codingsf) Starred [pikvm/ustreamer](https://github.com/pikvm/ustreamer)
  > µStreamer - Lightweight and fast MJPEG-HTTP streamer
- 🌟 👤 [codingsf](https://github.com/codingsf) Starred [MisterY52/apex_dma_kvm_pub](https://github.com/MisterY52/apex_dma_kvm_pub)
  > Apex Legends QEMU/KVM hack

### [deathmemory](https://github.com/deathmemory)
- 🌟 👤 [deathmemory](https://github.com/deathmemory) Starred [virjar/team-talk](https://github.com/virjar/team-talk)
  > 团队协作沟通软件，打算做成开源免费的钉钉/飞书，预期使用5年时间完成这个巨大的目标。

### [doodlewind](https://github.com/doodlewind)
- 🌟 👤 [doodlewind](https://github.com/doodlewind) Starred [heygen-com/hyperframes](https://github.com/heygen-com/hyperframes)
  > Write HTML. Render video. Built for agents.

### [dwisiswant0](https://github.com/dwisiswant0)
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [mandiant/gopacket](https://github.com/mandiant/gopacket)
  > Gopacket is a clean Go implementation of Impacket, a library intended for working with network proto...

### [dzxpert](https://github.com/dzxpert)
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [KelvinMsft/ThreadSpy](https://github.com/KelvinMsft/ThreadSpy)

### [fanzhidongyzby](https://github.com/fanzhidongyzby)
- 🌟 👤 [fanzhidongyzby](https://github.com/fanzhidongyzby) Starred [SJTU-IPADS/SkVM](https://github.com/SJTU-IPADS/SkVM)
  > The Language Virtual Machine for Agent Skills
- 🌟 👤 [fanzhidongyzby](https://github.com/fanzhidongyzby) Starred [affaan-m/everything-claude-code](https://github.com/affaan-m/everything-claude-code)
  > The agent harness performance optimization system. Skills, instincts, memory, security, and research...
- 🍴 👤 [fanzhidongyzby](https://github.com/fanzhidongyzby) Forked [EvoMap/evolver](https://github.com/EvoMap/evolver) to [fanzhidongyzby/evolver](https://github.com/fanzhidongyzby/evolver)
  > The GEP-Powered Self-Evolution Engine for AI Agents. Genome Evolution Protocol. | evomap.ai

### [fjh658](https://github.com/fjh658)
- 🌟 👤 [fjh658](https://github.com/fjh658) Starred [yarasaa/Clippy](https://github.com/yarasaa/Clippy)
  > Clipboard Manager for macOS

### [fnmsd](https://github.com/fnmsd)
- 🌟 👤 [fnmsd](https://github.com/fnmsd) Starred [zhukunpenglinyutong/desktop-cc-gui](https://github.com/zhukunpenglinyutong/desktop-cc-gui)
  > CC GUI 客户端（专为开发者打造的VibeCoding平台）

### [fr0zenrain](https://github.com/fr0zenrain)
- 🌟 👤 [fr0zenrain](https://github.com/fr0zenrain) Starred [ZhangHanDong/harness-engineering-from-cc-to-ai-coding](https://github.com/ZhangHanDong/harness-engineering-from-cc-to-ai-coding)
  > Harness Engineering From Claude Code source code to AI Coding

### [gabime](https://github.com/gabime)
- 🌟 👤 [gabime](https://github.com/gabime) Starred [smol-machines/smolvm](https://github.com/smol-machines/smolvm)
  > Tool to build & run portable, lightweight, self-contained virtual machines.
- 🌟 👤 [gabime](https://github.com/gabime) Starred [NetBSDfr/smolBSD](https://github.com/NetBSDfr/smolBSD)
  > smolBSD is a tiny BSD UNIX (NetBSD) system creation tool, primarily aimed at building modern, lightw...

### [gal2xy](https://github.com/gal2xy)
- 🌟 👤 [gal2xy](https://github.com/gal2xy) Starred [kdrag0n/safetynet-fix](https://github.com/kdrag0n/safetynet-fix)
  > Google SafetyNet attestation workarounds for Magisk
- 🌟 👤 [gal2xy](https://github.com/gal2xy) Starred [ebrasha/Abdal-DroidGuard](https://github.com/ebrasha/Abdal-DroidGuard)
  > Abdal DroidGuard is an advanced and powerful Android application hardening tool developed by  Ebrahi...
- 🍴 👤 [gal2xy](https://github.com/gal2xy) Forked [ebrasha/Abdal-DroidGuard](https://github.com/ebrasha/Abdal-DroidGuard) to [gal2xy/Abdal-DroidGuard](https://github.com/gal2xy/Abdal-DroidGuard)
  > Abdal DroidGuard is an advanced and powerful Android application hardening tool developed by  Ebrahi...

### [gmh5225](https://github.com/gmh5225)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [smol-machines/smolvm](https://github.com/smol-machines/smolvm) to [gmh5225/smolvm](https://github.com/gmh5225/smolvm)
  > Tool to build & run portable, lightweight, self-contained virtual machines.
- 🌟 👤 [gmh5225](https://github.com/gmh5225) Starred [smol-machines/smolvm](https://github.com/smol-machines/smolvm)
  > Tool to build & run portable, lightweight, self-contained virtual machines.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [KeServiceDescriptorTable/win32k-hook](https://github.com/KeServiceDescriptorTable/win32k-hook) to [gmh5225/win32k-hook](https://github.com/gmh5225/win32k-hook)
  > hooking win32k syscalls before they go into win32k drivers.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [KeServiceDescriptorTable/cormem.sys-vulnerable-driver](https://github.com/KeServiceDescriptorTable/cormem.sys-vulnerable-driver) to [gmh5225/cormem.sys-vulnerable-driver](https://github.com/gmh5225/cormem.sys-vulnerable-driver)
  > cormem.sys is a vulnerable driver by Teledyne Digital Imaging Inc.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [KeServiceDescriptorTable/vulnerable-drivers](https://github.com/KeServiceDescriptorTable/vulnerable-drivers) to [gmh5225/vulnerable-drivers](https://github.com/gmh5225/vulnerable-drivers)
  > vulnerable drivers for windows machines.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [KeServiceDescriptorTable/roak](https://github.com/KeServiceDescriptorTable/roak) to [gmh5225/roak](https://github.com/gmh5225/roak)
  > roak - Windows 10/11 rootkit
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [EvoMap/evolver](https://github.com/EvoMap/evolver) to [gmh5225/evolver](https://github.com/gmh5225/evolver)
  > The GEP-Powered Self-Evolution Engine for AI Agents. Genome Evolution Protocol. | evomap.ai
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [anthropics/claude-desktop-buddy](https://github.com/anthropics/claude-desktop-buddy) to [gmh5225/claude-desktop-buddy](https://github.com/gmh5225/claude-desktop-buddy)
  > Reference and an example for the Bluetooth API for makers in Claude Cowork & Claude Code Desktop
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [tailscale/tsidp](https://github.com/tailscale/tsidp) to [gmh5225/tsidp](https://github.com/gmh5225/tsidp)
  > A simple OIDC / OAuth Identity Provider (IdP) server for your tailnet.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [GRVYDEV/marky](https://github.com/GRVYDEV/marky) to [gmh5225/marky](https://github.com/gmh5225/marky)
  > A lightweight easy to use markdown viewer
- 🌟 👤 [gmh5225](https://github.com/gmh5225) Starred [morpho-org/metamorpho](https://github.com/morpho-org/metamorpho)
  > MetaMorpho
- 🌟 👤 [gmh5225](https://github.com/gmh5225) Starred [GreatV/oar-ocr](https://github.com/GreatV/oar-ocr)
  > An Optical Character Recognition (OCR) and Document Layout Analysis library written in Rust.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [bryankeller/wiiMac](https://github.com/bryankeller/wiiMac) to [gmh5225/wiiMac](https://github.com/gmh5225/wiiMac)
  > A Mac OS X bootloader for the Nintendo Wii.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [bwjoke/BTC-Trading-Since-2020](https://github.com/bwjoke/BTC-Trading-Since-2020) to [gmh5225/BTC-Trading-Since-2020](https://github.com/gmh5225/BTC-Trading-Since-2020)
  > Public BTC trading context since 2020.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [wuwangzhang1216/abliterix](https://github.com/wuwangzhang1216/abliterix) to [gmh5225/abliterix](https://github.com/gmh5225/abliterix)
  > Automated alignment adjustment for LLMs — direct steering, LoRA, and MoE expert-granular abliteratio...

### [guiying712](https://github.com/guiying712)
- 🌟 👤 [guiying712](https://github.com/guiying712) Starred [mayukh4/linux-android](https://github.com/mayukh4/linux-android)
  > A script that sets up linux on your anroid device via termux

### [hackcatml](https://github.com/hackcatml)
- 🌟 👤 [hackcatml](https://github.com/hackcatml) Starred [wuwangzhang1216/abliterix](https://github.com/wuwangzhang1216/abliterix)
  > Automated alignment adjustment for LLMs — direct steering, LoRA, and MoE expert-granular abliteratio...

### [hanc00l](https://github.com/hanc00l)
- 🌟 👤 [hanc00l](https://github.com/hanc00l) Starred [wgpsec/AboutSecurity](https://github.com/wgpsec/AboutSecurity)
  > Everything for pentest. | 用于渗透测试的 payload 和 bypass 字典.

### [happydog-intj](https://github.com/happydog-intj)
- 🌟 👤 [happydog-intj](https://github.com/happydog-intj) Starred [googleworkspace/cli](https://github.com/googleworkspace/cli)
  > Google Workspace CLI — one command-line tool for Drive, Gmail, Calendar, Sheets, Docs, Chat, Admin, ...

### [hyuunnn](https://github.com/hyuunnn)
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [tobyilee/book-writer](https://github.com/tobyilee/book-writer)
  > Automated book-writing harness with Toby-style voice (11 agents + 10 skills, EPUB output)

### [i64](https://github.com/i64)
- 🌟 👤 [i64](https://github.com/i64) Starred [plabayo/rama](https://github.com/plabayo/rama)
  > modular service framework to move and transform network packets

### [imlk0](https://github.com/imlk0)
- 🌟 👤 [imlk0](https://github.com/imlk0) Starred [supabase/supabase](https://github.com/supabase/supabase)
  > The Postgres development platform. Supabase gives you a dedicated Postgres database to build your we...

### [jianglin-code](https://github.com/jianglin-code)
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [libai1943/UAV-Path-Planning](https://github.com/libai1943/UAV-Path-Planning)
  > Unmanned Combat Aerial Vehicles Path Planning Using a Novel Probability Density Model Based on Artif...
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [libai2020/Single_Vehicle_Parking_Trajectory_Planning](https://github.com/libai2020/Single_Vehicle_Parking_Trajectory_Planning)
  > 第二章. 混合A*做路径决策，S-T图搜索做速度决策，数值求解最优控制问题做轨迹优化
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [libai2020/Sequential_Trajectory_Decision](https://github.com/libai2020/Sequential_Trajectory_Decision)
  > 第三章3.1.4小节.基于序贯计算的多车轨迹决策方法

### [joxeankoret](https://github.com/joxeankoret)
- 🍴 👤 [joxeankoret](https://github.com/joxeankoret) Forked [marioballano/cheapbin](https://github.com/marioballano/cheapbin) to [joxeankoret/cheapbin](https://github.com/joxeankoret/cheapbin)
  > Listen to your binaries

### [kagancapar](https://github.com/kagancapar)
- 🌟 👤 [kagancapar](https://github.com/kagancapar) Starred [svnscha/mcp-windbg](https://github.com/svnscha/mcp-windbg)
  > Model Context Protocol for WinDBG
- 🍴 👤 [kagancapar](https://github.com/kagancapar) Forked [svnscha/mcp-windbg](https://github.com/svnscha/mcp-windbg) to [kagancapar/mcp-windbg](https://github.com/kagancapar/mcp-windbg)
  > Model Context Protocol for WinDBG

### [kasnria001](https://github.com/kasnria001)
- 🌟 👤 [kasnria001](https://github.com/kasnria001) Starred [P4nda0s/IDA-NO-MCP](https://github.com/P4nda0s/IDA-NO-MCP)
  > Say goodbye to the complex, verbose, and laggy interaction mode of IDA Pro MCP
- 🌟 👤 [kasnria001](https://github.com/kasnria001) Starred [P4nda0s/reverse-skills](https://github.com/P4nda0s/reverse-skills)
  > Reverse engineering skills for Claude Code | 逆向工程 Claude Code Skills 插件
- 🌟 👤 [kasnria001](https://github.com/kasnria001) Starred [zyb2ww/BOOT.XF.2.1](https://github.com/zyb2ww/BOOT.XF.2.1)

### [keesj](https://github.com/keesj)
- 🍴 👤 [keesj](https://github.com/keesj) Forked [AlexsJones/llmserve](https://github.com/AlexsJones/llmserve) to [keesj/llmserve-fork](https://github.com/keesj/llmserve-fork)
  > A simple TUI for serving local LLM models. Pick a model, pick a backend, serve it

### [kingking888](https://github.com/kingking888)
- 🌟 👤 [kingking888](https://github.com/kingking888) Starred [blacktop/ida-mcp-rs](https://github.com/blacktop/ida-mcp-rs)
  > Headless IDA Pro MCP Server
- 🌟 👤 [kingking888](https://github.com/kingking888) Starred [SuZiXunYue/ida-script-mcp](https://github.com/SuZiXunYue/ida-script-mcp)
- 🌟 👤 [kingking888](https://github.com/kingking888) Starred [Captain-AI-Hub/IDA-MCP](https://github.com/Captain-AI-Hub/IDA-MCP)
  > IDA MCP

### [kylesmile1103](https://github.com/kylesmile1103)
- 🌟 👤 [kylesmile1103](https://github.com/kylesmile1103) Starred [crisng95/flowkit](https://github.com/crisng95/flowkit)
  > AI agent can create video content better than you. No reason why you do it manually in google flow!

### [linuxmobile](https://github.com/linuxmobile)
- 🌟 👤 [linuxmobile](https://github.com/linuxmobile) Starred [ZenNotes/zennotes](https://github.com/ZenNotes/zennotes)
  > Keyboard-first local Markdown notes with Vim motions, diagrams, and MCP integration.

### [linxid](https://github.com/linxid)
- 🌟 👤 [linxid](https://github.com/linxid) Starred [NVlabs/OmniVinci](https://github.com/NVlabs/OmniVinci)
  > OmniVinci is an omni-modal LLM for joint understanding of vision, audio, and language.

### [lixeon](https://github.com/lixeon)
- 🌟 👤 [lixeon](https://github.com/lixeon) Starred [Fincept-Corporation/FinceptTerminal](https://github.com/Fincept-Corporation/FinceptTerminal)
  > FinceptTerminal is a modern finance application offering advanced market analytics, investment resea...

### [luoyesiqiu](https://github.com/luoyesiqiu)
- 🌟 👤 [luoyesiqiu](https://github.com/luoyesiqiu) Starred [DingTalk-Real-AI/dingtalk-openclaw-connector](https://github.com/DingTalk-Real-AI/dingtalk-openclaw-connector)
  > Official OpenClaw DingTalk channel plugin | 钉钉官方 OpenClaw 插件

### [madeye](https://github.com/madeye)
- 🌟 👤 [madeye](https://github.com/madeye) Starred [madeye/meow-ios](https://github.com/madeye/meow-ios)
  > iOS port of meow — VPN/proxy client for iOS 26, powered by mihomo-rust
- 🌟 👤 [madeye](https://github.com/madeye) Starred [uditgoenka/autoresearch](https://github.com/uditgoenka/autoresearch)
  > Claude Autoresearch Skill — Autonomous goal-directed iteration for Claude Code. Inspired by Karpathy...

### [mav8557](https://github.com/mav8557)
- 🌟 👤 [mav8557](https://github.com/mav8557) Starred [mandiant/gopacket](https://github.com/mandiant/gopacket)
  > Gopacket is a clean Go implementation of Impacket, a library intended for working with network proto...

### [mcr](https://github.com/mcr)
- 🌟 👤 [mcr](https://github.com/mcr) Starred [loopbackio/loopback-next](https://github.com/loopbackio/loopback-next)
  > LoopBack makes it easy to build modern API applications that require complex integrations.

### [mcxiaoke](https://github.com/mcxiaoke)
- 🌟 👤 [mcxiaoke](https://github.com/mcxiaoke) Starred [HIllya51/LunaTranslator](https://github.com/HIllya51/LunaTranslator)
  > 视觉小说翻译器 / Visual Novel Translator

### [mgeeky](https://github.com/mgeeky)
- 🌟 👤 [mgeeky](https://github.com/mgeeky) Starred [DylanDavis1/DSCourier](https://github.com/DylanDavis1/DSCourier)

### [mingheyan](https://github.com/mingheyan)
- 🌟 👤 [mingheyan](https://github.com/mingheyan) Starred [iBotPeaches/Apktool](https://github.com/iBotPeaches/Apktool)
  > A tool for reverse engineering Android apk files

### [moonD4rk](https://github.com/moonD4rk)
- 🌟 👤 [moonD4rk](https://github.com/moonD4rk) Starred [mandiant/gopacket](https://github.com/mandiant/gopacket)
  > Gopacket is a clean Go implementation of Impacket, a library intended for working with network proto...

### [mzfr](https://github.com/mzfr)
- 🌟 👤 [mzfr](https://github.com/mzfr) Starred [heygen-com/hyperframes](https://github.com/heygen-com/hyperframes)
  > Write HTML. Render video. Built for agents.

### [not1cyyy](https://github.com/not1cyyy)
- 🚀 👤 [not1cyyy](https://github.com/not1cyyy) Made [not1cyyy/4chanity](https://github.com/not1cyyy/4chanity) public
  > MAKE 4CHAN GREAT AGAIN !!!

### [oblique](https://github.com/oblique)
- 🌟 👤 [oblique](https://github.com/oblique) Starred [VHSgunzo/uruntime](https://github.com/VHSgunzo/uruntime)
  > Universal RunImage and AppImage runtime with SquashFS and DwarFS supports
- 🌟 👤 [oblique](https://github.com/oblique) Starred [kem-a/AppManager](https://github.com/kem-a/AppManager)
  > MacOS style AppImage installer and management application

### [secretsquirrel](https://github.com/secretsquirrel)
- 🌟 👤 [secretsquirrel](https://github.com/secretsquirrel) Starred [frank2/limbos-gate](https://github.com/frank2/limbos-gate)
  > Hell's Gate, but make it 32-bit!

### [shareef12](https://github.com/shareef12)
- 🌟 👤 [shareef12](https://github.com/shareef12) Starred [ergrelet/windiff](https://github.com/ergrelet/windiff)
  > Web-based tool that allows comparing symbol, type and syscall information of Microsoft Windows binar...
- 🍴 👤 [shareef12](https://github.com/shareef12) Forked [hxyulin/hadris](https://github.com/hxyulin/hadris) to [shareef12/hadris](https://github.com/shareef12/hadris)
  > A filesystem crate, written in rust. Designed to be fast, portable, and supports no-std environments...

### [sisoma2](https://github.com/sisoma2)
- 🌟 👤 [sisoma2](https://github.com/sisoma2) Starred [Alishahryar1/free-claude-code](https://github.com/Alishahryar1/free-claude-code)
  > Use claude-code for free in the terminal, VSCode extension or via discord like openclaw

### [sotasan](https://github.com/sotasan)
- 🌟 👤 [sotasan](https://github.com/sotasan) Starred [twpayne/chezmoi](https://github.com/twpayne/chezmoi)
  > Manage your dotfiles across multiple diverse machines, securely.
- 🌟 👤 [sotasan](https://github.com/sotasan) Starred [tauri-apps/tauri](https://github.com/tauri-apps/tauri)
  > Build smaller, faster, and more secure desktop and mobile applications with a web frontend.

### [ssut](https://github.com/ssut)
- 🌟 👤 [ssut](https://github.com/ssut) Starred [hypery11/flipper-tesla-fsd](https://github.com/hypery11/flipper-tesla-fsd)
  > Tesla Mod — Flipper Zero & ESP32. Nag killer, FSD region unlock, track mode, BMS dashboard, blind sp...

### [swisskyrepo](https://github.com/swisskyrepo)
- 🌟 👤 [swisskyrepo](https://github.com/swisskyrepo) Starred [symgraph/GhidrAssistMCP](https://github.com/symgraph/GhidrAssistMCP)
  > An MCP extension for Ghidra
- 🌟 👤 [swisskyrepo](https://github.com/swisskyrepo) Starred [mandiant/gopacket](https://github.com/mandiant/gopacket)
  > Gopacket is a clean Go implementation of Impacket, a library intended for working with network proto...

### [wszqkzqk](https://github.com/wszqkzqk)
- 🌟 👤 [wszqkzqk](https://github.com/wszqkzqk) Starred [SPONGEMM/SPONGE](https://github.com/SPONGEMM/SPONGE)
  > Main Repository of SPONGE
- 🍴 👤 [wszqkzqk](https://github.com/wszqkzqk) Forked [SPONGEMM/SPONGE](https://github.com/SPONGEMM/SPONGE) to [wszqkzqk/SPONGE](https://github.com/wszqkzqk/SPONGE)
  > Main Repository of SPONGE

### [xinyu2428](https://github.com/xinyu2428)
- 🌟 👤 [xinyu2428](https://github.com/xinyu2428) Starred [guimaizi/facai](https://github.com/guimaizi/facai)
  > facai网安工具

### [xsser](https://github.com/xsser)
- 🌟 👤 [xsser](https://github.com/xsser) Starred [decoderesearch/SAELens](https://github.com/decoderesearch/SAELens)
  > Training Sparse Autoencoders on Language Models

### [yanyaoli](https://github.com/yanyaoli)
- 🌟 👤 [yanyaoli](https://github.com/yanyaoli) Starred [showlab/ShowUI](https://github.com/showlab/ShowUI)
  > [CVPR 2025] Open-source, End-to-end, Vision-Language-Action model for GUI Agent & Computer Use.

### [yhs0602](https://github.com/yhs0602)
- 🍴 👤 [yhs0602](https://github.com/yhs0602) Forked [chrisryugj/korean-law-mcp](https://github.com/chrisryugj/korean-law-mcp) to [yhs0602/korean-law-mcp](https://github.com/yhs0602/korean-law-mcp)
  > 국가법령정보MCP | 법제처 41개 API → 16개 MCP 도구. 법령·판례·조례·조약을 AI로 검색·조회·분석 + LLM 환각 방지 인용 검증 | 41 Korean legal ...

### [yhy0](https://github.com/yhy0)
- 🌟 👤 [yhy0](https://github.com/yhy0) Starred [APIs-guru/graphql-voyager](https://github.com/APIs-guru/graphql-voyager)
  > 🛰️ Represent any GraphQL API as an interactive graph

### [yuan-xy](https://github.com/yuan-xy)
- 🌟 👤 [yuan-xy](https://github.com/yuan-xy) Starred [mikedh/trimesh](https://github.com/mikedh/trimesh)
  > Python library for loading and using triangular meshes.
- 🌟 👤 [yuan-xy](https://github.com/yuan-xy) Starred [elalish/manifold](https://github.com/elalish/manifold)
  > Geometry library for topological robustness
- 🌟 👤 [yuan-xy](https://github.com/yuan-xy) Starred [wjakob/nanobind](https://github.com/wjakob/nanobind)
  > nanobind: tiny and efficient C++/Python bindings
- 🌟 👤 [yuan-xy](https://github.com/yuan-xy) Starred [cnr-isti-vclab/vcglib](https://github.com/cnr-isti-vclab/vcglib)
  > The VCGlib is a C++, templated, no dependency, library for manipulation, processing and cleaning of ...

### [zenghongtu](https://github.com/zenghongtu)
- 🌟 👤 [zenghongtu](https://github.com/zenghongtu) Starred [hanshuaikang/nezha](https://github.com/hanshuaikang/nezha)
  >  Run multiple AI coding agents across projects (Claude Code and Codex)


---
*最后更新于 2026-04-18 10:40:27 UTC*
*历史记录保存在 `archive` 目录中。*
