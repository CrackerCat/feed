# 每日 GitHub 动态 (2026-05-04)

我关注用户的今日公开动态 (每15分钟更新)。

## 今日动态

### [1c7](https://github.com/1c7)
- 🌟 👤 [1c7](https://github.com/1c7) Starred [nexu-io/open-design](https://github.com/nexu-io/open-design)
  > 🎨 Local-first, open-source alternative to Anthropic's Claude Design. ⚡ 19 Skills · ✨ 71 brand-grade ...

### [AndnixSH](https://github.com/AndnixSH)
- 🌟 👤 [AndnixSH](https://github.com/AndnixSH) Starred [overlord3850/dnspy-mcp](https://github.com/overlord3850/dnspy-mcp)
  > A Claude Code MCP that provides dnSpy-equivalent .NET analysis.
- 🍴 👤 [AndnixSH](https://github.com/AndnixSH) Forked [AgentSmithers/DnSpy-MCPserver-Extension](https://github.com/AgentSmithers/DnSpy-MCPserver-Extension) to [AndnixSH/DnSpy-MCPserver-Extension](https://github.com/AndnixSH/DnSpy-MCPserver-Extension)
  > AgentSmithers DnSpyEx MCPServer extension
- 🍴 👤 [AndnixSH](https://github.com/AndnixSH) Forked [overlord3850/dnspy-mcp](https://github.com/overlord3850/dnspy-mcp) to [AndnixSH/dnspy-mcp](https://github.com/AndnixSH/dnspy-mcp)
  > A Claude Code MCP that provides dnSpy-equivalent .NET analysis.

### [Ar3h](https://github.com/Ar3h)
- 🌟 👤 [Ar3h](https://github.com/Ar3h) Starred [TauricResearch/TradingAgents](https://github.com/TauricResearch/TradingAgents)
  > TradingAgents: Multi-Agents LLM Financial Trading Framework

### [Brinmon](https://github.com/Brinmon)
- 🌟 👤 [Brinmon](https://github.com/Brinmon) Starred [Logkiss/Rat-winos4.0-gh0st](https://github.com/Logkiss/Rat-winos4.0-gh0st)
  > 免杀远控木马源码整理开源(银狐 winos 大灰狼  gh0st)  Rat 
- 🌟 👤 [Brinmon](https://github.com/Brinmon) Starred [gmh5225/awesome-game-security](https://github.com/gmh5225/awesome-game-security)
  > awesome game security [Welcome to PR]

### [CC11001100](https://github.com/CC11001100)
- 🍴 👤 [CC11001100](https://github.com/CC11001100) Forked [NationalSecurityAgency/ghidra](https://github.com/NationalSecurityAgency/ghidra) to [android-security-engineer/AI-ghidra](https://github.com/android-security-engineer/AI-ghidra)
  > Ghidra is a software reverse engineering (SRE) framework
- 🍴 👤 [CC11001100](https://github.com/CC11001100) Forked [rednaga/APKiD](https://github.com/rednaga/APKiD) to [android-security-engineer/AI-APKiD](https://github.com/android-security-engineer/AI-APKiD)
  > Android Application Identifier for Packers, Protectors, Obfuscators and Oddities - PEiD for Android
- 🍴 👤 [CC11001100](https://github.com/CC11001100) Forked [dwisiswant0/apkleaks](https://github.com/dwisiswant0/apkleaks) to [android-security-engineer/AI-apkleaks](https://github.com/android-security-engineer/AI-apkleaks)
  > Scanning APK file for URIs, endpoints & secrets.
- 🍴 👤 [CC11001100](https://github.com/CC11001100) Forked [pxb1988/dex2jar](https://github.com/pxb1988/dex2jar) to [android-security-engineer/AI-dex2jar](https://github.com/android-security-engineer/AI-dex2jar)
  > Tools to work with android .dex and java .class files

### [ChiChou](https://github.com/ChiChou)
- 🌟 👤 [ChiChou](https://github.com/ChiChou) Starred [jonhoo/inferno](https://github.com/jonhoo/inferno)
  > A Rust port of FlameGraph

### [Colton1skees](https://github.com/Colton1skees)
- 🌟 👤 [Colton1skees](https://github.com/Colton1skees) Starred [d-kfmnn/talisman2](https://github.com/d-kfmnn/talisman2)

### [DarkFunct](https://github.com/DarkFunct)
- 🍴 👤 [DarkFunct](https://github.com/DarkFunct) Forked [sansan0/TrendRadar](https://github.com/sansan0/TrendRadar) to [DarkFunct/trendradar](https://github.com/DarkFunct/trendradar)
  > ⭐AI-driven public opinion & trend monitor with multi-platform aggregation, RSS, and smart alerts.🎯 告...

### [DarkLineX](https://github.com/DarkLineX)
- 🌟 👤 [DarkLineX](https://github.com/DarkLineX) Starred [AICodo/pve-emu-realpc](https://github.com/AICodo/pve-emu-realpc)
  > PVE Debain Ubuntu ArchLinux virtual machine emulates a physical machine（pve Debian Ubuntu ArchLinux虚...

### [Dump-GUY](https://github.com/Dump-GUY)
- 🌟 👤 [Dump-GUY](https://github.com/Dump-GUY) Starred [dotnet/LLVMSharp](https://github.com/dotnet/LLVMSharp)
  > LLVM bindings for .NET Standard written in C# using ClangSharp
- 🌟 👤 [Dump-GUY](https://github.com/Dump-GUY) Starred [microsoft/win32metadata](https://github.com/microsoft/win32metadata)
  > Tooling to generate metadata for Win32 APIs in the Windows SDK.
- 🌟 👤 [Dump-GUY](https://github.com/Dump-GUY) Starred [dotnet/ClangSharp](https://github.com/dotnet/ClangSharp)
  > Clang bindings for .NET written in C#

### [Exo1i](https://github.com/Exo1i)
- 🍴 👤 [Exo1i](https://github.com/Exo1i) Forked [rmuxnet/xiaomi_sm8250_kernel_e404](https://github.com/rmuxnet/xiaomi_sm8250_kernel_e404) to [Exo1i/xiaomi_sm8250_kernel_e404](https://github.com/Exo1i/xiaomi_sm8250_kernel_e404)
  > FORCE PUSH WARNING ! --- E404 Kernel for MiKona

### [FeeiCN](https://github.com/FeeiCN)
- 🌟 👤 [FeeiCN](https://github.com/FeeiCN) Starred [mdx-js/mdx](https://github.com/mdx-js/mdx)
  > Markdown for the component era

### [Grigory-Rylov](https://github.com/Grigory-Rylov)
- 🌟 👤 [Grigory-Rylov](https://github.com/Grigory-Rylov) Starred [jarczakpawel/OrcaSlicer-bambulab](https://github.com/jarczakpawel/OrcaSlicer-bambulab)
  > This is the end....

### [Howard20181](https://github.com/Howard20181)
- 🌟 👤 [Howard20181](https://github.com/Howard20181) Starred [Zarosmm/obs-bilibili-stream](https://github.com/Zarosmm/obs-bilibili-stream)
  > 这是一个为 OBS Studio 开发的插件，用于简化在 Bilibili 平台上的直播流程。插件支持扫码登录 Bilibili，更新直播间信息，并获取 RTMP 推流地址和推流码。
- 🌟 👤 [Howard20181](https://github.com/Howard20181) Starred [Radekyspec/StartLive](https://github.com/Radekyspec/StartLive)
  > 绕过B站直播姬获取推流地址
- 🌟 👤 [Howard20181](https://github.com/Howard20181) Starred [Rsplwe/bili-live-hime](https://github.com/Rsplwe/bili-live-hime)
  > bilibili官方直播姬的轻量化替代工具，实现推流地址获取、直播间管理、实时弹幕与礼物推送、观众统计。支持 Windows / Linux / macOS。

### [JasonWei512](https://github.com/JasonWei512)
- 🌟 👤 [JasonWei512](https://github.com/JasonWei512) Starred [MagicJinn/MrBeastify-Youtube](https://github.com/MagicJinn/MrBeastify-Youtube)
  > Is your browser experience boring? This extension adds the famous youtuber MrBeast to every thumbnai...
- 🌟 👤 [JasonWei512](https://github.com/JasonWei512) Starred [dlpufan/bilibili-tim](https://github.com/dlpufan/bilibili-tim)
  > 这是一款专为 Bilibili 用户设计的趣味增强工具。将你浏览的 B 站视频封面中的人物，自动替换（或叠加）为“影视飓风” Tim 的经典人像。

### [Jefino9488](https://github.com/Jefino9488)
- 🍴 👤 [Jefino9488](https://github.com/Jefino9488) Forked [johnfanv2/LenovoLegionLinux](https://github.com/johnfanv2/LenovoLegionLinux) to [Jefino9488/LenovoLegionLinux](https://github.com/Jefino9488/LenovoLegionLinux)
  > Driver and tools for controlling Lenovo Legion laptops in Linux including fan control and power mode...

### [KHwang9883](https://github.com/KHwang9883)
- 🌟 👤 [KHwang9883](https://github.com/KHwang9883) Starred [AlkaidLab/moonlight-harmony](https://github.com/AlkaidLab/moonlight-harmony)

### [KuLiPai](https://github.com/KuLiPai)
- 🌟 👤 [KuLiPai](https://github.com/KuLiPai) Starred [JustLikeCheese/LGBT-Prompt](https://github.com/JustLikeCheese/LGBT-Prompt)
  > 利用 AI 对 LGBT 弱势群体的 "同情偏向" 让他执行原先会被拒绝的请求

### [Kwansy98](https://github.com/Kwansy98)
- 🌟 👤 [Kwansy98](https://github.com/Kwansy98) Starred [noahware/binprotect](https://github.com/noahware/binprotect)
  > x64 PE bin2bin obfuscator which doesn't add a section to the binary

### [KyuubiRan](https://github.com/KyuubiRan)
- 🌟 👤 [KyuubiRan](https://github.com/KyuubiRan) Starred [OpenYSM/YSMParser](https://github.com/OpenYSM/YSMParser)
  > A Parser to recover Yes-Steve-Model Encrypted Binaries.

### [LaPhilosophie](https://github.com/LaPhilosophie)
- 🌟 👤 [LaPhilosophie](https://github.com/LaPhilosophie) Starred [Ar9av/PaperOrchestra](https://github.com/Ar9av/PaperOrchestra)
  > An automated AI research-paper writer based off Google's PaperOrchestra paper's implementation throu...

### [LabGuy94](https://github.com/LabGuy94)
- 🌟 👤 [LabGuy94](https://github.com/LabGuy94) Starred [llvm/llvm-project](https://github.com/llvm/llvm-project)
  > The LLVM Project is a collection of modular and reusable compiler and toolchain technologies.

### [Lakr233](https://github.com/Lakr233)
- 🌟 👤 [Lakr233](https://github.com/Lakr233) Starred [SaseQ/discord-mcp](https://github.com/SaseQ/discord-mcp)
  > A MCP server for the Discord integration. Enable your AI assistants to seamlessly interact with Disc...

### [Lil-Ran](https://github.com/Lil-Ran)
- 🌟 👤 [Lil-Ran](https://github.com/Lil-Ran) Starred [ret2shell/ret2shell](https://github.com/ret2shell/ret2shell)
  > A feature-riches CTF challenge platform!

### [LoseNine](https://github.com/LoseNine)
- 🌟 👤 [LoseNine](https://github.com/LoseNine) Starred [LoseNine/Firefox-FingerPrint-Analyzer](https://github.com/LoseNine/Firefox-FingerPrint-Analyzer)
  > 一个面向网站指纹风控研究与辅助 JS 逆向学术分析的桌面工具，一键分析web页面指纹风控，可直接将全量日志交给AI进行JS补环境，专用于网站指纹搜集研究和辅助JS补环境。

### [Mandi-Sa](https://github.com/Mandi-Sa)
- 🌟 👤 [Mandi-Sa](https://github.com/Mandi-Sa) Starred [researchxxl/syncthing-android](https://github.com/researchxxl/syncthing-android)
  > Syncthing-Fork - A Syncthing Wrapper for Android.

### [MiroKaku](https://github.com/MiroKaku)
- 🌟 👤 [MiroKaku](https://github.com/MiroKaku) Starred [nexu-io/open-design](https://github.com/nexu-io/open-design)
  > 🎨 Local-first, open-source alternative to Anthropic's Claude Design. ⚡ 19 Skills · ✨ 71 brand-grade ...

### [Moe-hacker](https://github.com/Moe-hacker)
- 🌟 👤 [Moe-hacker](https://github.com/Moe-hacker) Starred [TechUnRestricted/WinDiskWriter](https://github.com/TechUnRestricted/WinDiskWriter)
  > 🖥 Windows Bootable USB creator for macOS. 🛠 Patches Windows 11 to bypass TPM and Secure Boot require...

### [Mufanc](https://github.com/Mufanc)
- 🌟 👤 [Mufanc](https://github.com/Mufanc) Starred [sahaj-b/ghostty-cursor-shaders](https://github.com/sahaj-b/ghostty-cursor-shaders)
  > Custom cursor shaders for ghostty (trails and ripple/pulse effects)

### [NativeStar](https://github.com/NativeStar)
- 🌟 👤 [NativeStar](https://github.com/NativeStar) Starred [websockets/ws](https://github.com/websockets/ws)
  > Simple to use, blazing fast and thoroughly tested WebSocket client and server for Node.js

### [Nukem9](https://github.com/Nukem9)
- 🍴 👤 [Nukem9](https://github.com/Nukem9) Forked [gottyduke/DKUtil](https://github.com/gottyduke/DKUtil) to [Nukem9/DKUtil](https://github.com/Nukem9/DKUtil)
  > Some utility headers to help with x64 native plugin development

### [PIKACHUIM](https://github.com/PIKACHUIM)
- 🌟 👤 [PIKACHUIM](https://github.com/PIKACHUIM) Starred [sudoevolve/EUI-NEO](https://github.com/sudoevolve/EUI-NEO)
  > EUI-NEO is a cross-platform, high-performance, low-overhead C++17 UI framework built on OpenGL and G...

### [Prslc](https://github.com/Prslc)
- 🍴 👤 [Prslc](https://github.com/Prslc) Forked [bmax121/KernelPatch](https://github.com/bmax121/KernelPatch) to [Prslc/KernelPatch](https://github.com/Prslc/KernelPatch)
  > Patching and hooking the Linux kernel with only a stripped Linux kernel image.

### [R0rt1z2](https://github.com/R0rt1z2)
- 🌟 👤 [R0rt1z2](https://github.com/R0rt1z2) Starred [AngeloJacobo/DDR3-Notes](https://github.com/AngeloJacobo/DDR3-Notes)
  > My notes for DDR3 SDRAM controller

### [S3cur3Th1sSh1t](https://github.com/S3cur3Th1sSh1t)
- 🌟 👤 [S3cur3Th1sSh1t](https://github.com/S3cur3Th1sSh1t) Starred [watchtowrlabs/watchTowr-vs-cPanel-WHM-AuthBypass-to-RCE.py](https://github.com/watchtowrlabs/watchTowr-vs-cPanel-WHM-AuthBypass-to-RCE.py)

### [SakuraKoi](https://github.com/SakuraKoi)
- 🍴 👤 [SakuraKoi](https://github.com/SakuraKoi) Forked [Joshcraft2002/Dumplings-Delight-Rewrapped](https://github.com/Joshcraft2002/Dumplings-Delight-Rewrapped) to [NyakoRandomForks/Dumplings-Delight-Rewrapped](https://github.com/NyakoRandomForks/Dumplings-Delight-Rewrapped)
- 🍴 👤 [SakuraKoi](https://github.com/SakuraKoi) Forked [Neon402/VoxyServer-1.21.1](https://github.com/Neon402/VoxyServer-1.21.1) to [NyakoRandomForks/VoxyServer-1.21.1](https://github.com/NyakoRandomForks/VoxyServer-1.21.1)
  > VoxyServer mod backport for 1.21.1
- 🍴 👤 [SakuraKoi](https://github.com/SakuraKoi) Forked [Creators-of-Aeronautics/Simulated-Project](https://github.com/Creators-of-Aeronautics/Simulated-Project) to [NyakoRandomForks/Simulated-Project](https://github.com/NyakoRandomForks/Simulated-Project)
  > Build anything from airships to planes and cars!
- 🍴 👤 [SakuraKoi](https://github.com/SakuraKoi) Forked [m3t4f1v3/voxy](https://github.com/m3t4f1v3/voxy) to [SakuraKoi/voxy](https://github.com/SakuraKoi/voxy)
  > An LoD rendering mod for minecraft
- 🍴 👤 [SakuraKoi](https://github.com/SakuraKoi) Forked [meansabine/neo-voxy](https://github.com/meansabine/neo-voxy) to [NyakoRandomForks/neo-voxy](https://github.com/NyakoRandomForks/neo-voxy)
  > Neo-Forge Port of Voxy (Very Early Alpha)
- 🍴 👤 [SakuraKoi](https://github.com/SakuraKoi) Forked [m3t4f1v3/voxy](https://github.com/m3t4f1v3/voxy) to [NyakoRandomForks/voxy](https://github.com/NyakoRandomForks/voxy)
  > An LoD rendering mod for minecraft

### [SherlockChiang](https://github.com/SherlockChiang)
- 🌟 👤 [SherlockChiang](https://github.com/SherlockChiang) Starred [google/magika](https://github.com/google/magika)
  > Fast and accurate AI powered file content types detection 
- 🌟 👤 [SherlockChiang](https://github.com/SherlockChiang) Starred [lcandy2/enable-chrome-ai](https://github.com/lcandy2/enable-chrome-ai)
  > Enable Gemini in Chrome, AI Powered History search, DevTools Al Innovations in Google Chrome without...

### [Solaree](https://github.com/Solaree)
- 🌟 👤 [Solaree](https://github.com/Solaree) Starred [mitmproxy/android-unpinner](https://github.com/mitmproxy/android-unpinner)
  > Remove Certificate Pinning from APKs
- 🌟 👤 [Solaree](https://github.com/Solaree) Starred [httptoolkit/frida-interception-and-unpinning](https://github.com/httptoolkit/frida-interception-and-unpinning)
  > Frida scripts to rewrite mobile applications at runtime to directly MitM all HTTPS traffic
- 🌟 👤 [Solaree](https://github.com/Solaree) Starred [PlayCover/PlayCover](https://github.com/PlayCover/PlayCover)
  > Community fork of PlayCover

### [Soulghost](https://github.com/Soulghost)
- 🌟 👤 [Soulghost](https://github.com/Soulghost) Starred [a16z-infra/ai-town](https://github.com/a16z-infra/ai-town)
  > A MIT-licensed, deployable starter kit for building and customizing your own version of AI town - a ...
- 🌟 👤 [Soulghost](https://github.com/Soulghost) Starred [KsanaDock/Microverse](https://github.com/KsanaDock/Microverse)
  > A god-simulation sandbox game built on Godot 4 as a multi-agent AI social simulation system. In this...

### [SsageParuders](https://github.com/SsageParuders)
- 🌟 👤 [SsageParuders](https://github.com/SsageParuders) Starred [0Chencc/clawgod](https://github.com/0Chencc/clawgod)
  > This is NOT a third-party Claude Code client. ClawGod is a runtime patch applied on top of the offic...
- 🌟 👤 [SsageParuders](https://github.com/SsageParuders) Starred [Acmesec/PromptJailbreakManual](https://github.com/Acmesec/PromptJailbreakManual)
  > Prompt越狱手册

### [Th4nat0s](https://github.com/Th4nat0s)
- 🌟 👤 [Th4nat0s](https://github.com/Th4nat0s) Starred [D4-project/Plum-Island](https://github.com/D4-project/Plum-Island)
  > Proactive Land Uncovering & Monitoring

### [ThatNotEasy](https://github.com/ThatNotEasy)
- 🌟 👤 [ThatNotEasy](https://github.com/ThatNotEasy) Starred [MobSF/Mobile-Security-Framework-MobSF](https://github.com/MobSF/Mobile-Security-Framework-MobSF)
  > Mobile Security Framework (MobSF) is an automated, all-in-one mobile application (Android/iOS/Window...
- 🍴 👤 [ThatNotEasy](https://github.com/ThatNotEasy) Forked [Genymobile/scrcpy](https://github.com/Genymobile/scrcpy) to [ThatNotEasy/scrcpy](https://github.com/ThatNotEasy/scrcpy)
  > Display and control your Android device

### [ThunderCls](https://github.com/ThunderCls)
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [xkqg/MatPlotLibNet](https://github.com/xkqg/MatPlotLibNet)
  > A .NET 10 / .NET 8 charting library inspired by matplotlib. Fluent API, dependency injection, parall...

### [WAY29](https://github.com/WAY29)
- 🌟 👤 [WAY29](https://github.com/WAY29) Starred [public-apis/public-apis](https://github.com/public-apis/public-apis)
  > A collective list of free APIs
- 🍴 👤 [WAY29](https://github.com/WAY29) Forked [public-apis/public-apis](https://github.com/public-apis/public-apis) to [WAY29/public-apis](https://github.com/WAY29/public-apis)
  > A collective list of free APIs

### [YuKongA](https://github.com/YuKongA)
- 🌟 👤 [YuKongA](https://github.com/YuKongA) Starred [freestylefly/awesome-gpt-image-2](https://github.com/freestylefly/awesome-gpt-image-2)
  > Prompt as Code | GPT-Image2 工业级提示词引擎与模板库，370+ 个案例逆向工程，20+ 套工业级模板，持续更新中

### [ameetsaahu](https://github.com/ameetsaahu)
- 🍴 👤 [ameetsaahu](https://github.com/ameetsaahu) Forked [enovella/TEE-reversing](https://github.com/enovella/TEE-reversing) to [ameetsaahu/TEE-reversing](https://github.com/ameetsaahu/TEE-reversing)
  >  A curated list of public TEE resources for learning how to reverse-engineer and achieve trusted cod...

### [androidmalin](https://github.com/androidmalin)
- 🌟 👤 [androidmalin](https://github.com/androidmalin) Starred [guofei9987/blind_watermark](https://github.com/guofei9987/blind_watermark)
  > Blind&Invisible Watermark ，图片盲水印，提取水印无须原图！

### [ant4g0nist](https://github.com/ant4g0nist)
- 🚀 👤 [ant4g0nist](https://github.com/ant4g0nist) Made [ant4g0nist/pyre](https://github.com/ant4g0nist/pyre) public
  > Ghidra decompiler in your browser

### [arpruss](https://github.com/arpruss)
- 🍴 👤 [arpruss](https://github.com/arpruss) Forked [Davidslv/rogue](https://github.com/Davidslv/rogue) to [arpruss/rogue](https://github.com/arpruss/rogue)
  > Original Rogue Game (5.4.4)

### [banteg](https://github.com/banteg)
- 🌟 👤 [banteg](https://github.com/banteg) Starred [morpho-org/safer](https://github.com/morpho-org/safer)
  > Allows to easily create tx to be signed through CLI and submitted on a Safe
- 🍴 👤 [banteg](https://github.com/banteg) Forked [Kenix3/libultraship](https://github.com/Kenix3/libultraship) to [banteg/libultraship](https://github.com/banteg/libultraship)
  > Porting games to the PC

### [boy-hack](https://github.com/boy-hack)
- 🌟 👤 [boy-hack](https://github.com/boy-hack) Starred [finvfamily/finshare](https://github.com/finvfamily/finshare)
  >  专业的金融数据获取工具库 - A Professional Financial Data Fetching Toolkit for   Python
- 🌟 👤 [boy-hack](https://github.com/boy-hack) Starred [fengwm64/aistock-api-cf](https://github.com/fengwm64/aistock-api-cf)
  > A 股数据 API，基于 Cloudflare Workers 构建，提供股票基本信息、股票实时行情、指数实时行情、盈利预测、热门人气榜、新闻头条等接口。

### [byt3n33dl3](https://github.com/byt3n33dl3)
- 🌟 👤 [byt3n33dl3](https://github.com/byt3n33dl3) Starred [OWASP/cve-lite-cli](https://github.com/OWASP/cve-lite-cli)
  > Fast, developer-friendly JS/TS dependency vulnerability scanner with local lockfile scanning, OSV ma...
- 🌟 👤 [byt3n33dl3](https://github.com/byt3n33dl3) Starred [XsanFlip/poc-cpanel-cve-2026-41940](https://github.com/XsanFlip/poc-cpanel-cve-2026-41940)
- 🌟 👤 [byt3n33dl3](https://github.com/byt3n33dl3) Starred [LongWayHomie/PolyEngine](https://github.com/LongWayHomie/PolyEngine)
  > PolyEngine is an evasive PE packer designed for CTF challenges and low-level Windows security educat...
- 🌟 👤 [byt3n33dl3](https://github.com/byt3n33dl3) Starred [ChiChou/ios-debug.py](https://github.com/ChiChou/ios-debug.py)
  > Shortcut to automate your iproxy, debugserver, lldb workflow

### [ch4ncellor](https://github.com/ch4ncellor)
- 🌟 👤 [ch4ncellor](https://github.com/ch4ncellor) Starred [btcpayserver/btcpayserver](https://github.com/btcpayserver/btcpayserver)
  > Accept Bitcoin payments. Free, open-source & self-hosted, Bitcoin payment processor. 

### [cmzy](https://github.com/cmzy)
- 🌟 👤 [cmzy](https://github.com/cmzy) Starred [erweixin/RaTeX](https://github.com/erweixin/RaTeX)
  > >99.5% KaTeX syntax coverage. LaTeX math renderer in pure Rust. No JavaScript, no WebView, no DOM. O...

### [cn-kali-team](https://github.com/cn-kali-team)
- 🌟 👤 [cn-kali-team](https://github.com/cn-kali-team) Starred [RustCrypto/utils](https://github.com/RustCrypto/utils)
  > Utility crates used in RustCrypto

### [codecat](https://github.com/codecat)
- 🍴 👤 [codecat](https://github.com/codecat) Forked [rodrigocfd/winlamb](https://github.com/rodrigocfd/winlamb) to [SeraphSecure/winlamb](https://github.com/SeraphSecure/winlamb)
  > A lightweight modern C++11 library for Win32 API, using lambdas to handle Windows messages.

### [ctkqiang](https://github.com/ctkqiang)
- 🌟 👤 [ctkqiang](https://github.com/ctkqiang) Starred [Tencent-TDS/KuiklyUI](https://github.com/Tencent-TDS/KuiklyUI)
  > A Kotlin Multiplatform UI framework from Tencent TDS — high-performance, one codebase for six platfo...
- 🍴 👤 [ctkqiang](https://github.com/ctkqiang) Forked [Tencent-TDS/KuiklyUI](https://github.com/Tencent-TDS/KuiklyUI) to [ctkqiang/KuiklyUI](https://github.com/ctkqiang/KuiklyUI)
  > A Kotlin Multiplatform UI framework from Tencent TDS — high-performance, one codebase for six platfo...

### [d3adzo](https://github.com/d3adzo)
- 🌟 👤 [d3adzo](https://github.com/d3adzo) Starred [d3vzer0/KubePy-Hound](https://github.com/d3vzer0/KubePy-Hound)
  > A Python utility to dump Kubernetes resources and store them in BloodHound's OpenGraph
- 🌟 👤 [d3adzo](https://github.com/d3adzo) Starred [MorDavid/vCenterHound](https://github.com/MorDavid/vCenterHound)
  > Collect infrastructure and permissions data from vCenter and export it as a BloodHound‑compatible gr...

### [dzxpert](https://github.com/dzxpert)
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [waryas/xign_poc_april_2026](https://github.com/waryas/xign_poc_april_2026)
  > Use XIGNCODE3 driver to cheat

### [einverne](https://github.com/einverne)
- 🌟 👤 [einverne](https://github.com/einverne) Starred [farion1231/cc-switch](https://github.com/farion1231/cc-switch)
  > A cross-platform desktop All-in-One assistant tool for Claude Code, Codex, OpenCode, openclaw & Gemi...
- 🌟 👤 [einverne](https://github.com/einverne) Starred [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)
  > Sub2API-CRS2 一站式开源中转服务，让 Claude、Openai 、Gemini、Antigravity订阅统一接入，支持拼车共享，更高效分摊成本，原生工具无缝使用。

### [enginelesscc](https://github.com/enginelesscc)
- 🍴 👤 [enginelesscc](https://github.com/enginelesscc) Forked [microsoft/PowerToys](https://github.com/microsoft/PowerToys) to [enginelesscc/PowerToys](https://github.com/enginelesscc/PowerToys)
  > Microsoft PowerToys is a collection of utilities that supercharge productivity and customization on ...

### [enovella](https://github.com/enovella)
- 🌟 👤 [enovella](https://github.com/enovella) Starred [android1337/brkida](https://github.com/android1337/brkida)
  > C++ macro for x64 programs that breaks ida hex-rays decompiler tool.

### [everettjf](https://github.com/everettjf)
- 🌟 👤 [everettjf](https://github.com/everettjf) Starred [cirruslabs/tart](https://github.com/cirruslabs/tart)
  > macOS and Linux VMs on Apple Silicon to use in CI and other automations
- 🌟 👤 [everettjf](https://github.com/everettjf) Starred [nexu-io/open-design](https://github.com/nexu-io/open-design)
  > 🎨 Local-first, open-source alternative to Anthropic's Claude Design. ⚡ 19 Skills · ✨ 71 brand-grade ...
- 🌟 👤 [everettjf](https://github.com/everettjf) Starred [tddworks/baguette](https://github.com/tddworks/baguette)
  > Headless iOS Simulator manager/farm + host-side input injection for iOS 26 — taps, swipes, multi-fin...

### [exp-sky](https://github.com/exp-sky)
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [m-bain/whisperX](https://github.com/m-bain/whisperX)
  > WhisperX:  Automatic Speech Recognition with Word-level Timestamps (& Diarization)
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [Softcatala/whisper-ctranslate2](https://github.com/Softcatala/whisper-ctranslate2)
  > Whisper command line client compatible with original OpenAI client based on CTranslate2.

### [extremecoders-re](https://github.com/extremecoders-re)
- 🌟 👤 [extremecoders-re](https://github.com/extremecoders-re) Starred [ace-step/ACE-Step-1.5](https://github.com/ace-step/ACE-Step-1.5)
  > The most powerful local music generation model that outperforms almost all commercial alternatives, ...

### [feicong](https://github.com/feicong)
- 🌟 👤 [feicong](https://github.com/feicong) Starred [burrowers/garble](https://github.com/burrowers/garble)
  > Obfuscate Go builds
- 🍴 👤 [feicong](https://github.com/feicong) Forked [burrowers/garble](https://github.com/burrowers/garble) to [feicong/garble](https://github.com/feicong/garble)
  > Obfuscate Go builds

### [fengjixuchui](https://github.com/fengjixuchui)
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [ciembor/agent-rules-books](https://github.com/ciembor/agent-rules-books) to [fengjixuchui/agent-rules-books](https://github.com/fengjixuchui/agent-rules-books)
  > Ready-to-use Codex, Cursor, and Claude Code rules inspired by classic software engineering books.

### [ggggmllll](https://github.com/ggggmllll)
- 🌟 👤 [ggggmllll](https://github.com/ggggmllll) Starred [J5now/JDex2](https://github.com/J5now/JDex2)
  > 基于Xposed / Lsposed的主动调用抽取壳脱壳工具

### [gmh5225](https://github.com/gmh5225)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [iDescriptor/iDescriptor](https://github.com/iDescriptor/iDescriptor) to [gmh5225/iDescriptor](https://github.com/gmh5225/iDescriptor)
  > A free, open-source, and cross-platform iDevice management tool
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [BlackBeltTechnology/pi-agent-dashboard](https://github.com/BlackBeltTechnology/pi-agent-dashboard) to [gmh5225/pi-agent-dashboard](https://github.com/gmh5225/pi-agent-dashboard)
  > Real-time web dashboard for pi coding-agent sessions. Multi-session view, live chat mirroring, integ...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [nullthrix/BugBounty-Methodology](https://github.com/nullthrix/BugBounty-Methodology) to [gmh5225/BugBounty-Methodology](https://github.com/gmh5225/BugBounty-Methodology)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [raullenchai/Rapid-MLX](https://github.com/raullenchai/Rapid-MLX) to [gmh5225/Rapid-MLX](https://github.com/gmh5225/Rapid-MLX)
  > The fastest local AI engine for Apple Silicon. 4.2x faster than Ollama, 0.08s cached TTFT, 100% tool...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [aattaran/deepclaude](https://github.com/aattaran/deepclaude) to [gmh5225/deepclaude-](https://github.com/gmh5225/deepclaude-)
  > Use Claude Code's autonomous agent loop with DeepSeek V4 Pro, OpenRouter, or any Anthropic-compatibl...
- 🌟 👤 [gmh5225](https://github.com/gmh5225) Starred [aattaran/deepclaude](https://github.com/aattaran/deepclaude)
  > Use Claude Code's autonomous agent loop with DeepSeek V4 Pro, OpenRouter, or any Anthropic-compatibl...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [bint-disasm/binbit](https://github.com/bint-disasm/binbit) to [gmh5225/binbit](https://github.com/gmh5225/binbit)
  > bintbit is a bitvector theory SMT solver and CDCL SAT solver
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [ant4g0nist/pyre](https://github.com/ant4g0nist/pyre) to [gmh5225/pyre](https://github.com/gmh5225/pyre)
  > Ghidra decompiler in your browser
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [d-kfmnn/talisman2](https://github.com/d-kfmnn/talisman2) to [gmh5225/talisman2](https://github.com/gmh5225/talisman2)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [ace-step/ACE-Step-1.5](https://github.com/ace-step/ACE-Step-1.5) to [gmh5225/ACE-Step-1.5](https://github.com/gmh5225/ACE-Step-1.5)
  > The most powerful local music generation model that outperforms almost all commercial alternatives, ...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [zakirkun/deep-eye](https://github.com/zakirkun/deep-eye) to [gmh5225/deep-eye](https://github.com/gmh5225/deep-eye)
  > An advanced AI-driven vulnerability scanner and penetration testing tool that integrates multiple AI...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [antichainalysis/xmr-churner](https://github.com/antichainalysis/xmr-churner) to [gmh5225/xmr-churner](https://github.com/gmh5225/xmr-churner)
  > moneroc - The proper way to churn Monero
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [vixhal-baraiya/16bit-cpu](https://github.com/vixhal-baraiya/16bit-cpu) to [gmh5225/16bit-cpu](https://github.com/gmh5225/16bit-cpu)
  > 16-bit CPU emulator from scratch in pure C
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [vixhal-baraiya/SuckCode](https://github.com/vixhal-baraiya/SuckCode) to [gmh5225/SuckCode](https://github.com/gmh5225/SuckCode)
  > A Minimal Alternative to Claude Code, Written in Python

### [hanbings](https://github.com/hanbings)
- 🌟 👤 [hanbings](https://github.com/hanbings) Starred [chakra-ui/ark](https://github.com/chakra-ui/ark)
  > Unstyled, accessible UI components for your design System. Works in React, Vue, Solid, and Svelte.
- 🌟 👤 [hanbings](https://github.com/hanbings) Starred [4ier/notion-cli](https://github.com/4ier/notion-cli)
  > Work seamlessly with Notion from the command line. Full Notion API coverage in a single binary.

### [happydog-intj](https://github.com/happydog-intj)
- 🌟 👤 [happydog-intj](https://github.com/happydog-intj) Starred [ErlichLiu/DeepClaude](https://github.com/ErlichLiu/DeepClaude)
  > Unleash Next-Level AI! 🚀  💻 Code Generation: DeepSeek r1 + Claude 3.7 Sonnet - Unparalleled Performa...
- 🌟 👤 [happydog-intj](https://github.com/happydog-intj) Starred [rasbt/LLMs-from-scratch](https://github.com/rasbt/LLMs-from-scratch)
  > Implement a ChatGPT-like LLM in PyTorch from scratch, step by step

### [hayasec](https://github.com/hayasec)
- 🌟 👤 [hayasec](https://github.com/hayasec) Starred [browser-use/browser-harness](https://github.com/browser-use/browser-harness)
  > Browser Harness | Self-healing harness that enables LLMs to complete any task.

### [henrypp](https://github.com/henrypp)
- 🌟 👤 [henrypp](https://github.com/henrypp) Starred [dail8859/NotepadNext](https://github.com/dail8859/NotepadNext)
  > A cross-platform, reimplementation of Notepad++

### [hhhaiai](https://github.com/hhhaiai)
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [LoseNine/Firefox-FingerPrint-Analyzer](https://github.com/LoseNine/Firefox-FingerPrint-Analyzer)
  > 一个面向网站指纹风控研究与辅助 JS 逆向学术分析的桌面工具，一键分析web页面指纹风控，可直接将全量日志交给AI进行JS补环境，专用于网站指纹搜集研究和辅助JS补环境。
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [zhongruan0522/new-api](https://github.com/zhongruan0522/new-api)
  > 二开针对部分CodingPlan自用适配
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [nexu-io/open-design](https://github.com/nexu-io/open-design)
  > 🎨 Local-first, open-source alternative to Anthropic's Claude Design. ⚡ 19 Skills · ✨ 71 brand-grade ...
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [nuxt-modules/og-image](https://github.com/nuxt-modules/og-image)
  > Generate OG Images with Vue templates in Nuxt.

### [hyuunnn](https://github.com/hyuunnn)
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [penxle/typie](https://github.com/penxle/typie)
  > 작가를 위한 올인원 글쓰기 도구
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [aattaran/deepclaude](https://github.com/aattaran/deepclaude)
  > Use Claude Code's autonomous agent loop with DeepSeek V4 Pro, OpenRouter, or any Anthropic-compatibl...
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [microsoft/win32metadata](https://github.com/microsoft/win32metadata)
  > Tooling to generate metadata for Win32 APIs in the Windows SDK.

### [imj01y](https://github.com/imj01y)
- 🌟 👤 [imj01y](https://github.com/imj01y) Starred [discourse/discourse](https://github.com/discourse/discourse)
  > A platform for community discussion. Free, open, simple.

### [iscle](https://github.com/iscle)
- 🌟 👤 [iscle](https://github.com/iscle) Starred [AngeloJacobo/DDR3-Notes](https://github.com/AngeloJacobo/DDR3-Notes)
  > My notes for DDR3 SDRAM controller

### [jaredrummler](https://github.com/jaredrummler)
- 🌟 👤 [jaredrummler](https://github.com/jaredrummler) Starred [manthrax/JSlug](https://github.com/manthrax/JSlug)
  > Javascript implementation of Slug font loading and rendering, for THREEJS
- 🌟 👤 [jaredrummler](https://github.com/jaredrummler) Starred [battlesquid/gh-skyline](https://github.com/battlesquid/gh-skyline)
  > Your Contribution Story in 3D

### [jonpalmisc](https://github.com/jonpalmisc)
- 🌟 👤 [jonpalmisc](https://github.com/jonpalmisc) Starred [hzeller/rpi-rgb-led-matrix](https://github.com/hzeller/rpi-rgb-led-matrix)
  > Controlling up to three chains of 64x64, 32x32, 16x32 or similar RGB LED displays using Raspberry Pi...

### [jpillora](https://github.com/jpillora)
- 🌟 👤 [jpillora](https://github.com/jpillora) Starred [chrishutchinson/claude-receipts](https://github.com/chrishutchinson/claude-receipts)
  > Bring receipts from your Claude Code sessions

### [kasnria001](https://github.com/kasnria001)
- 🌟 👤 [kasnria001](https://github.com/kasnria001) Starred [x64dbg/ScyllaHide](https://github.com/x64dbg/ScyllaHide)
  > Advanced usermode anti-anti-debugger. Forked from https://bitbucket.org/NtQuery/scyllahide

### [kkent030315](https://github.com/kkent030315)
- 🌟 👤 [kkent030315](https://github.com/kkent030315) Starred [wellbia/simple_rustlibs](https://github.com/wellbia/simple_rustlibs)

### [lcxl](https://github.com/lcxl)
- 🌟 👤 [lcxl](https://github.com/lcxl) Starred [walkinglabs/hands-on-modern-rl](https://github.com/walkinglabs/hands-on-modern-rl)
  > 🚀 An open-source, hands-on curriculum bridging the gap from basic RL concepts to LLM alignment, RLVR...

### [libra146](https://github.com/libra146)
- 🌟 👤 [libra146](https://github.com/libra146) Starred [mattpocock/skills](https://github.com/mattpocock/skills)
  > Skills for Real Engineers. Straight from my .claude directory.
- 🌟 👤 [libra146](https://github.com/libra146) Starred [mindfold-ai/Trellis](https://github.com/mindfold-ai/Trellis)
  > The best agent harness.
- 🌟 👤 [libra146](https://github.com/libra146) Starred [liuzhengdongfortest/CodeStable](https://github.com/liuzhengdongfortest/CodeStable)

### [lizhangqu](https://github.com/lizhangqu)
- 🌟 👤 [lizhangqu](https://github.com/lizhangqu) Starred [glados-network/GLaDOS](https://github.com/glados-network/GLaDOS)
  > status of GLaDOS

### [ltlly](https://github.com/ltlly)
- 🌟 👤 [ltlly](https://github.com/ltlly) Starred [openai/codex-plugin-cc](https://github.com/openai/codex-plugin-cc)
  > Use Codex from Claude Code to review code or delegate tasks.

### [mav8557](https://github.com/mav8557)
- 🌟 👤 [mav8557](https://github.com/mav8557) Starred [KonaeAkira/raphael-rs](https://github.com/KonaeAkira/raphael-rs)
  > Crafting macro solver for Final Fantasy XIV

### [mgeeky](https://github.com/mgeeky)
- 🌟 👤 [mgeeky](https://github.com/mgeeky) Starred [wudidike/pentest_skill](https://github.com/wudidike/pentest_skill)
  > Black-box web penetration testing automation framework for AI Agents
- 🌟 👤 [mgeeky](https://github.com/mgeeky) Starred [blackarrowsec/talks](https://github.com/blackarrowsec/talks)
  > Public talks, workshops and research presentations from BlackArrow

### [mingheyan](https://github.com/mingheyan)
- 🌟 👤 [mingheyan](https://github.com/mingheyan) Starred [openssl/openssl](https://github.com/openssl/openssl)
  > General purpose TLS and crypto library
- 🌟 👤 [mingheyan](https://github.com/mingheyan) Starred [weidai11/cryptopp](https://github.com/weidai11/cryptopp)
  > free C++ class library of cryptographic schemes
- 🌟 👤 [mingheyan](https://github.com/mingheyan) Starred [ReversecLabs/android-keystore-audit](https://github.com/ReversecLabs/android-keystore-audit)
- 🌟 👤 [mingheyan](https://github.com/mingheyan) Starred [LoseNine/Firefox-FingerPrint-Analyzer](https://github.com/LoseNine/Firefox-FingerPrint-Analyzer)
  > 一个面向网站指纹风控研究与辅助 JS 逆向学术分析的桌面工具，一键分析web页面指纹风控，可直接将全量日志交给AI进行JS补环境，专用于网站指纹搜集研究和辅助JS补环境。

### [mitsuakki](https://github.com/mitsuakki)
- 🌟 👤 [mitsuakki](https://github.com/mitsuakki) Starred [raullenchai/Rapid-MLX](https://github.com/raullenchai/Rapid-MLX)
  > The fastest local AI engine for Apple Silicon. 4.2x faster than Ollama, 0.08s cached TTFT, 100% tool...
- 🌟 👤 [mitsuakki](https://github.com/mitsuakki) Starred [0xeb/libghidra](https://github.com/0xeb/libghidra)
  > SDK for automating Ghidra from Python, Rust, and C++. Decompile, rename, annotate, inspect symbols/t...

### [mrexodia](https://github.com/mrexodia)
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [bint-disasm/bint-disasm.github.io](https://github.com/bint-disasm/bint-disasm.github.io)
  > web frontend for the bin't binary analysis tool 
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [Michaelliv/pi-goal](https://github.com/Michaelliv/pi-goal)
  > Persistent autonomous goals for pi
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [bint-disasm/binbit](https://github.com/bint-disasm/binbit)
  > bintbit is a bitvector theory SMT solver and CDCL SAT solver

### [natsumerinchan](https://github.com/natsumerinchan)
- 🌟 👤 [natsumerinchan](https://github.com/natsumerinchan) Starred [evdenis/kexec](https://github.com/evdenis/kexec)
  > Kexec Magisk Module

### [neil-wu](https://github.com/neil-wu)
- 🌟 👤 [neil-wu](https://github.com/neil-wu) Starred [darrylmorley/whatcable](https://github.com/darrylmorley/whatcable)
  > macOS menu bar app that tells you, in plain English, what each USB-C cable plugged into your Mac can...

### [nico](https://github.com/nico)
- 🌟 👤 [nico](https://github.com/nico) Starred [raiyanyahya/how-to-train-your-gpt](https://github.com/raiyanyahya/how-to-train-your-gpt)
  > Build a modern LLM from scratch. Every line commented. Explained like we are five.

### [odzhan](https://github.com/odzhan)
- 🌟 👤 [odzhan](https://github.com/odzhan) Starred [codewhitesec/Lastenzug](https://github.com/codewhitesec/Lastenzug)
  > Socks4a proxy leveraging PIC, Websockets and static obfuscation on assembly level
- 🌟 👤 [odzhan](https://github.com/odzhan) Starred [haproxy/haproxy](https://github.com/haproxy/haproxy)
  > HAProxy Load Balancer's development branch (mirror of git.haproxy.org)
- 🌟 👤 [odzhan](https://github.com/odzhan) Starred [nicocha30/ligolo-ng](https://github.com/nicocha30/ligolo-ng)
  > An advanced, yet simple, tunneling/pivoting tool that uses a TUN interface.
- 🌟 👤 [odzhan](https://github.com/odzhan) Starred [klsecservices/rpivot](https://github.com/klsecservices/rpivot)
  > socks4 reverse proxy for penetration testing
- 🌟 👤 [odzhan](https://github.com/odzhan) Starred [Kudaes/Puzzle](https://github.com/Kudaes/Puzzle)
  > Set of PoC to abuse Windows minifilters functionality

### [pbek](https://github.com/pbek)
- 🌟 👤 [pbek](https://github.com/pbek) Starred [Mic92/nixfmt-rs](https://github.com/Mic92/nixfmt-rs)
  > A from-scratch Rust reimplementation of nixfmt that produces byte-identical output to the Haskell or...

### [phith0n](https://github.com/phith0n)
- 🌟 👤 [phith0n](https://github.com/phith0n) Starred [colinhacks/zod](https://github.com/colinhacks/zod)
  > TypeScript-first schema validation with static type inference

### [qinlili23333](https://github.com/qinlili23333)
- 🌟 👤 [qinlili23333](https://github.com/qinlili23333) Starred [wrongly-cuddly-obsession/NTSB_FOIA_MU5735](https://github.com/wrongly-cuddly-obsession/NTSB_FOIA_MU5735)

### [rankalpha](https://github.com/rankalpha)
- 🌟 👤 [rankalpha](https://github.com/rankalpha) Starred [pd4d10/debugtron](https://github.com/pd4d10/debugtron)
  > Debug in-production Electron based app

### [ravindu644](https://github.com/ravindu644)
- 🌟 👤 [ravindu644](https://github.com/ravindu644) Starred [RuriOSS/ruri](https://github.com/RuriOSS/ruri)
  > A zipped Linux container runtime that zips (Lightweight User-friendly Linux-container Implementation...

### [re-zero001](https://github.com/re-zero001)
- 🌟 👤 [re-zero001](https://github.com/re-zero001) Starred [OpenYSM/YSMParser](https://github.com/OpenYSM/YSMParser)
  > A Parser to recover Yes-Steve-Model Encrypted Binaries.

### [ricardojoserf](https://github.com/ricardojoserf)
- 🚀 👤 [ricardojoserf](https://github.com/ricardojoserf) Made [ricardojoserf/BACnet-scan](https://github.com/ricardojoserf/BACnet-scan) public
  > Tool to actively scan the network for BACnet/IP and BACnet/SC

### [rmuxnet](https://github.com/rmuxnet)
- 🍴 👤 [rmuxnet](https://github.com/rmuxnet) Forked [kvsnr113/xiaomi_sm8250_kernel_e404](https://github.com/kvsnr113/xiaomi_sm8250_kernel_e404) to [rmuxnet/xiaomi_sm8250_kernel_e404](https://github.com/rmuxnet/xiaomi_sm8250_kernel_e404)
  > FORCE PUSH WARNING ! --- E404 Kernel for MiKona

### [roadwy](https://github.com/roadwy)
- 🌟 👤 [roadwy](https://github.com/roadwy) Starred [zhom/donutbrowser](https://github.com/zhom/donutbrowser)
  > Simple Yet Powerful Anti-Detect Browser 🍩

### [rogertan30](https://github.com/rogertan30)
- 🌟 👤 [rogertan30](https://github.com/rogertan30) Starred [Binlogo/Design-Patterns-In-Swift-CN](https://github.com/Binlogo/Design-Patterns-In-Swift-CN)
  > 设计模式（Swift 5.0 实现）- 中文版

### [s4n7h0](https://github.com/s4n7h0)
- 🌟 👤 [s4n7h0](https://github.com/s4n7h0) Starred [zerostring-tech/promptchef](https://github.com/zerostring-tech/promptchef)
  > A community library of AI prompts and skills — search, copy, and contribute.

### [sakana164](https://github.com/sakana164)
- 🌟 👤 [sakana164](https://github.com/sakana164) Starred [j-hc/revanced-magisk-module](https://github.com/j-hc/revanced-magisk-module)
  > Extensive ReVanced builder. Builds both modules and APKs. Updated daily.

### [surendrajat](https://github.com/surendrajat)
- 🌟 👤 [surendrajat](https://github.com/surendrajat) Starred [GunjanGrunge/WestBenga-zECI-Live-Results](https://github.com/GunjanGrunge/WestBenga-zECI-Live-Results)

### [survivalizeed](https://github.com/survivalizeed)
- 🌟 👤 [survivalizeed](https://github.com/survivalizeed) Starred [trailofbits/CoBRA](https://github.com/trailofbits/CoBRA)
  > Coefficient-Based Reconstruction of Arithmetic — a Mixed Boolean-Arithmetic (MBA) expression simplif...

### [swisskyrepo](https://github.com/swisskyrepo)
- 🌟 👤 [swisskyrepo](https://github.com/swisskyrepo) Starred [califio/skills](https://github.com/califio/skills)
  > Claude Code plugins and Codex skills from Calif.io for AI-assisted security research and code auditi...

### [taisuii](https://github.com/taisuii)
- 🌟 👤 [taisuii](https://github.com/taisuii) Starred [cnaiart/Chat-Painting-System](https://github.com/cnaiart/Chat-Painting-System)
  > AIGC聊天绘画系统「PHP源码版」，基于前后端分离架构以及Vue3、uni-app、ThinkPHP6.x、PHP8.0技术栈开发，包含PC端、H5端、小程序端、APP端。ChatAI聚合对接国内外...
- 🌟 👤 [taisuii](https://github.com/taisuii) Starred [dungngminh/simutil](https://github.com/dungngminh/simutil)
  > Cross platform utility TUI app for launching iOS simulators / Android emulators, discover physical d...

### [thatqier](https://github.com/thatqier)
- 🌟 👤 [thatqier](https://github.com/thatqier) Starred [agiapp/BRPickerView](https://github.com/agiapp/BRPickerView)
  > BRPickerView 封装的是iOS中常用的选择器组件，主要包括：日期选择器（支持年月日、年月等15种日期样式选择，支持设置星期、至今等）、地址选择器（支持省市区、省市、省三种地区选择）、自定义字...

### [vaibhavpandeyvpz](https://github.com/vaibhavpandeyvpz)
- 🌟 👤 [vaibhavpandeyvpz](https://github.com/vaibhavpandeyvpz) Starred [Portkey-AI/gateway](https://github.com/Portkey-AI/gateway)
  > A blazing fast AI Gateway with integrated guardrails. Route to 1,600+ LLMs, 50+ AI Guardrails with 1...

### [wbaby](https://github.com/wbaby)
- 🍴 👤 [wbaby](https://github.com/wbaby) Forked [waryas/xign_poc_april_2026](https://github.com/waryas/xign_poc_april_2026) to [wbaby/xign_poc_april_2026](https://github.com/wbaby/xign_poc_april_2026)
  > Use XIGNCODE3 driver to cheat

### [wszxl516](https://github.com/wszxl516)
- 🌟 👤 [wszxl516](https://github.com/wszxl516) Starred [pikasTech/PikaPython](https://github.com/pikasTech/PikaPython)
  > An ultra-lightweight Python interpreter that runs with only 4KB of RAM, zero dependencies. It is rea...

### [x0rz4](https://github.com/x0rz4)
- 🌟 👤 [x0rz4](https://github.com/x0rz4) Starred [Mouseww/anything-analyzer](https://github.com/Mouseww/anything-analyzer)
  > 全能协议分析工具：浏览器抓包 + MITM 代理 + 指纹伪装 + AI 分析 + MCP Server 无缝对接 AI Agent/IDE   |  All-in-one protocol anal...

### [xiaoweigege](https://github.com/xiaoweigege)
- 🌟 👤 [xiaoweigege](https://github.com/xiaoweigege) Starred [nexu-io/open-design](https://github.com/nexu-io/open-design)
  > 🎨 Local-first, open-source alternative to Anthropic's Claude Design. ⚡ 19 Skills · ✨ 71 brand-grade ...
- 🌟 👤 [xiaoweigege](https://github.com/xiaoweigege) Starred [ruvnet/ruflo](https://github.com/ruvnet/ruflo)
  > 🌊 The leading agent orchestration platform for Claude. Deploy intelligent multi-agent swarms, coordi...

### [yhs0602](https://github.com/yhs0602)
- 🌟 👤 [yhs0602](https://github.com/yhs0602) Starred [Q00/ouroboros](https://github.com/Q00/ouroboros)
  > Agent OS: Stop prompting. Start specifying.
- 🌟 👤 [yhs0602](https://github.com/yhs0602) Starred [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills)
  > A single CLAUDE.md file to improve Claude Code behavior, derived from Andrej Karpathy's observations...

### [yifengyou](https://github.com/yifengyou)
- 🌟 👤 [yifengyou](https://github.com/yifengyou) Starred [lefeck/ubuntu-autoinstall-generator-tools](https://github.com/lefeck/ubuntu-autoinstall-generator-tools)
  > Generate a fully-automated Ubuntu ISO for unattended installations.

### [zhanghecn](https://github.com/zhanghecn)
- 🌟 👤 [zhanghecn](https://github.com/zhanghecn) Starred [sdyckjq-lab/llm-wiki-skill](https://github.com/sdyckjq-lab/llm-wiki-skill)
  > 基于 Karpathy llm-wiki 方法论的个人知识库构建 Skill，支持多平台！

### [zhongyang219](https://github.com/zhongyang219)
- 🌟 👤 [zhongyang219](https://github.com/zhongyang219) Starred [jaywcjlove/awesome-mac](https://github.com/jaywcjlove/awesome-mac)
  >  This project is dedicated to collecting high-quality macOS software and organizing them systematic...
- 🌟 👤 [zhongyang219](https://github.com/zhongyang219) Starred [samzong/MacMusicPlayer](https://github.com/samzong/MacMusicPlayer)
  > A clean, lightweight music player for macOS.


---
*最后更新于 2026-05-04 16:45:35 UTC*
*历史记录保存在 `archive` 目录中。*
