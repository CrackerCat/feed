# 每日 GitHub 动态 (2026-07-31)

我关注用户的今日公开动态 (每15分钟更新)。

## 今日动态

### [0xCD4](https://github.com/0xCD4)
- 🌟 👤 [0xCD4](https://github.com/0xCD4) Starred [0xCD4/n8n-ai-agent-security-lab](https://github.com/0xCD4/n8n-ai-agent-security-lab)
  > CSINT Research side project for static and staging-safe security regression checks of n8n AI workflo...

### [0xbug](https://github.com/0xbug)
- 🌟 👤 [0xbug](https://github.com/0xbug) Starred [evilrabbit/lifeline](https://github.com/evilrabbit/lifeline)
  > A timeline component for the stories that unfold over time. Ships as a shadcn registry.

### [1c7](https://github.com/1c7)
- 🌟 👤 [1c7](https://github.com/1c7) Starred [QwenAudio/qwen-audio-agent](https://github.com/QwenAudio/qwen-audio-agent)
  > A realtime voice runtime that keeps Agents talking, working, and present.  Real-time Voice Runtime f...

### [5ec1cff](https://github.com/5ec1cff)
- 🌟 👤 [5ec1cff](https://github.com/5ec1cff) Starred [BuSung-dev/Root-My-Galaxy](https://github.com/BuSung-dev/Root-My-Galaxy)
  > KSU installer for supported Samsung Galaxy firmware with CVE-2026-43499
- 🌟 👤 [5ec1cff](https://github.com/5ec1cff) Starred [BuSung-dev/Root-My-Galaxy-Payloads](https://github.com/BuSung-dev/Root-My-Galaxy-Payloads)
  > Signed device profiles, mobile exploit payloads, and KernelSU artifacts for Root My Galaxy

### [AbhiTheModder](https://github.com/AbhiTheModder)
- 🚀 👤 [AbhiTheModder](https://github.com/AbhiTheModder) Made [Student-Activities-IITM-BS/student-activities-app](https://github.com/Student-Activities-IITM-BS/student-activities-app) public
  > A calm, native home for the IIT Madras BS community.

### [AndnixSH](https://github.com/AndnixSH)
- 🌟 👤 [AndnixSH](https://github.com/AndnixSH) Starred [Droppers/SingleFileExtractor](https://github.com/Droppers/SingleFileExtractor)
  >  A tool for extracting contents (assemblies, configuration, etc.) from a single-file application to ...

### [AnkioTomas](https://github.com/AnkioTomas)
- 🌟 👤 [AnkioTomas](https://github.com/AnkioTomas) Starred [gavamedia/deltafin](https://github.com/gavamedia/deltafin)
  > Run full Kimi K3 on a single device. And an OpenAI-compatible API server for local chat and coding a...
- 🌟 👤 [AnkioTomas](https://github.com/AnkioTomas) Starred [0rangec3t/Black-cat](https://github.com/0rangec3t/Black-cat)
  > Claude Code RedTeam Skill — Hypothesis-Driven Cognitive Architecture。一个假设-证据驱动的红队skill

### [Asabeneh](https://github.com/Asabeneh)
- 🌟 👤 [Asabeneh](https://github.com/Asabeneh) Starred [gravitational/teleport](https://github.com/gravitational/teleport)
  > The easiest, and most secure way to access and protect all of your infrastructure.

### [BlackHole1](https://github.com/BlackHole1)
- 🌟 👤 [BlackHole1](https://github.com/BlackHole1) Starred [github/gh-stack](https://github.com/github/gh-stack)
  > GitHub Stacked PRs

### [BruceLi20110501](https://github.com/BruceLi20110501)
- 🌟 👤 [BruceLi20110501](https://github.com/BruceLi20110501) Starred [ZYS-Create1024/APC-Injector](https://github.com/ZYS-Create1024/APC-Injector)
  > Kernel APC Injection Example (with Whitelist System) 内核APC注入示例(带白名单系统)

### [Cxk4ng](https://github.com/Cxk4ng)
- 🌟 👤 [Cxk4ng](https://github.com/Cxk4ng) Starred [roothide/Bootstrap](https://github.com/roothide/Bootstrap)
  > A full featured bootstrap for iOS 15.0-17.0 A8-A17Pro & M1+M2 using roothide

### [Dere3046](https://github.com/Dere3046)
- 🌟 👤 [Dere3046](https://github.com/Dere3046) Starred [RVC-Boss/GPT-SoVITS](https://github.com/RVC-Boss/GPT-SoVITS)
  > 1 min voice data can also be used to train a good TTS model! (few shot voice cloning)

### [EgeBalci](https://github.com/EgeBalci)
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [egebilecen/yugami](https://github.com/egebilecen/yugami)
  > A x64 binary packer written in Rust.
- 🍴 👤 [EgeBalci](https://github.com/EgeBalci) Forked [egebilecen/yugami](https://github.com/egebilecen/yugami) to [EgeBalci/yugami](https://github.com/EgeBalci/yugami)
  > A x64 binary packer written in Rust.
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [simplifaisoul/osiris](https://github.com/simplifaisoul/osiris)
  > Open Source Global Intelligence Platform - Real-Time OSINT Dashboard - A Palantir Alternative -     ...
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [Mr-In4inci3le/sliver-gui](https://github.com/Mr-In4inci3le/sliver-gui)
  > A desktop operator console for Sliver C2, built with Wails. Provides a native, lightweight GUI inter...
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [planetaryescape/mxr](https://github.com/planetaryescape/mxr)
  > Local-first, keyboard-native terminal email client
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [LizardByte/Sunshine](https://github.com/LizardByte/Sunshine)
  > Self-hosted game stream host for Moonlight.
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [meli/meli](https://github.com/meli/meli)
  > terminal mail client, mirror of https://git.meli-email.org/meli/meli.git https://crates.io/crates/me...
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [utkusen/sast-skills](https://github.com/utkusen/sast-skills)
  > Collection of agent skills that turn your AI coder into a SAST scanner
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [radareorg/r2ai](https://github.com/radareorg/r2ai)
  > LLM-based reversing for radare2

### [EggUncle](https://github.com/EggUncle)
- 🌟 👤 [EggUncle](https://github.com/EggUncle) Starred [OrangesHuang/Resonance](https://github.com/OrangesHuang/Resonance)
  > 同频（Resonance）是一个本地化的 ETF「国家队」资金监控系统。它通过价格位置、份额流向、交易方向、成交额热度、融资杠杆五个指标各自亮灯，当同色灯≥3盏时判定为「危险共振」或「机会共振」，量化...

### [ErodedElk](https://github.com/ErodedElk)
- 🌟 👤 [ErodedElk](https://github.com/ErodedElk) Starred [tw93/Mole](https://github.com/tw93/Mole)
  > 🐹 Clean, uninstall, analyze, optimize, and monitor your Mac from the terminal.

### [FBLeee](https://github.com/FBLeee)
- 🌟 👤 [FBLeee](https://github.com/FBLeee) Starred [taisuii/rusda](https://github.com/taisuii/rusda)
  > 对frida 的patch，产物可直接下载Release

### [FeJQ](https://github.com/FeJQ)
- 🌟 👤 [FeJQ](https://github.com/FeJQ) Starred [every-app/open-seo](https://github.com/every-app/open-seo)
  > Open source alternative to Semrush and Ahrefs

### [Felix3322](https://github.com/Felix3322)
- 🌟 👤 [Felix3322](https://github.com/Felix3322) Starred [Anowhere/WindowsKernelBook](https://github.com/Anowhere/WindowsKernelBook)
  > 《Windows 内核安全编程技术实践》 系列丛书，探索 Anti RootKit 反内核工具核心原理与技术实现细节。
- 🌟 👤 [Felix3322](https://github.com/Felix3322) Starred [DATA-GaMi/WindowsKernelBook](https://github.com/DATA-GaMi/WindowsKernelBook)
  > 《Windows 内核安全编程技术实践》 系列丛书，探索 Anti RootKit 反内核工具核心原理与技术实现细节。

### [Firebasky](https://github.com/Firebasky)
- 🌟 👤 [Firebasky](https://github.com/Firebasky) Starred [Zer0SumGam3/CVE-2026-66066-POC](https://github.com/Zer0SumGam3/CVE-2026-66066-POC)
  > PoC for CVE-2026-66066 in Ruby on Rails
- 🍴 👤 [Firebasky](https://github.com/Firebasky) Forked [Zer0SumGam3/CVE-2026-66066-POC](https://github.com/Zer0SumGam3/CVE-2026-66066-POC) to [Firebasky/CVE-2026-66066-POC](https://github.com/Firebasky/CVE-2026-66066-POC)
  > PoC for CVE-2026-66066 in Ruby on Rails

### [FunnyWolf](https://github.com/FunnyWolf)
- 🌟 👤 [FunnyWolf](https://github.com/FunnyWolf) Starred [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph)
  > Pre-indexed code knowledge graph, auto syncs on code changes, for Claude Code, Codex, Gemini, Cursor...

### [HdShare](https://github.com/HdShare)
- 🌟 👤 [HdShare](https://github.com/HdShare) Starred [REAndroid/ARSCLib](https://github.com/REAndroid/ARSCLib)
  > Android binary resources read/write library
- 🍴 👤 [HdShare](https://github.com/HdShare) Forked [YunJavaPro/Silk-Codec-Android](https://github.com/YunJavaPro/Silk-Codec-Android) to [HdShare/Silk-Codec-Android](https://github.com/HdShare/Silk-Codec-Android)
  > Android 平台 Silk 音频编解码工具

### [Howard20181](https://github.com/Howard20181)
- 🍴 👤 [Howard20181](https://github.com/Howard20181) Forked [5ec1cff/MyInjector](https://github.com/5ec1cff/MyInjector) to [Howard20181/MyInjector](https://github.com/Howard20181/MyInjector)
- 🌟 👤 [Howard20181](https://github.com/Howard20181) Starred [Howard20181/MyInjector](https://github.com/Howard20181/MyInjector)

### [J5now](https://github.com/J5now)
- 🌟 👤 [J5now](https://github.com/J5now) Starred [bluearchive-cafe/bluearchive-cafe](https://github.com/bluearchive-cafe/bluearchive-cafe)
  > 蔚蓝咖啡厅 - 非官方的《蔚蓝档案》日服汉化项目，致力于为玩家提供优质、全面的日服汉化服务！
- 🍴 👤 [J5now](https://github.com/J5now) Forked [Perfare/Zygisk-Il2CppDumper](https://github.com/Perfare/Zygisk-Il2CppDumper) to [J5now/Zygisk-Il2CppDumper](https://github.com/J5now/Zygisk-Il2CppDumper)
  > Using Zygisk to dump il2cpp data at runtime

### [JasonWei512](https://github.com/JasonWei512)
- 🌟 👤 [JasonWei512](https://github.com/JasonWei512) Starred [crazypeace/xray-vless-reality](https://github.com/crazypeace/xray-vless-reality)
  > Xray, VLESS_Reality模式 极简一键脚本
- 🌟 👤 [JasonWei512](https://github.com/JasonWei512) Starred [2dust/v2rayN](https://github.com/2dust/v2rayN)
  > A GUI client for Windows, Linux and macOS, support Xray and sing-box and others
- 🌟 👤 [JasonWei512](https://github.com/JasonWei512) Starred [XTLS/RealiTLScanner](https://github.com/XTLS/RealiTLScanner)
  > A TLS server scanner for Reality
- 🌟 👤 [JasonWei512](https://github.com/JasonWei512) Starred [V2RaySSR/RealityChecker](https://github.com/V2RaySSR/RealityChecker)
  > Reality协议目标网站检测工具 - 一键检测网站是否适合作为Reality协议目标，支持TLS检测、地理位置分析、CDN识别等功能

### [Jirubizu](https://github.com/Jirubizu)
- 🌟 👤 [Jirubizu](https://github.com/Jirubizu) Starred [emrearmagan/atlas.nvim](https://github.com/emrearmagan/atlas.nvim)
  > A Neovim plugin to browse and manage pull requests and issues across GitHub, GitLab, Bitbucket, and ...

### [JnuSimba](https://github.com/JnuSimba)
- 🌟 👤 [JnuSimba](https://github.com/JnuSimba) Starred [bojieli/ai-agent-book](https://github.com/bojieli/ai-agent-book)
  > 《深入理解 AI Agent：设计原理与工程实践》（李博杰 著）开源主仓库：全书正文、编译版 PDF 与按章配套代码

### [JunioJsv](https://github.com/JunioJsv)
- 🌟 👤 [JunioJsv](https://github.com/JunioJsv) Starred [nvpro-samples/nvpro_core2](https://github.com/nvpro-samples/nvpro_core2)
  > A comprehensive collection of modular libraries for high-performance graphics development

### [KHwang9883](https://github.com/KHwang9883)
- 🌟 👤 [KHwang9883](https://github.com/KHwang9883) Starred [DiscuzTeam/DiscuzX](https://github.com/DiscuzTeam/DiscuzX)
  > Official Mirror Repository of Discuz!, the most popular community forum building system in China.

### [Keno](https://github.com/Keno)
- 🍴 👤 [Keno](https://github.com/Keno) Forked [hashicorp/terraform-provider-aws](https://github.com/hashicorp/terraform-provider-aws) to [KenoAIStaging/terraform-provider-aws](https://github.com/KenoAIStaging/terraform-provider-aws)
  > The AWS Provider enables Terraform to manage AWS resources.

### [KpLi0rn](https://github.com/KpLi0rn)
- 🌟 👤 [KpLi0rn](https://github.com/KpLi0rn) Starred [Kaimen-Inc/Co-Scientist](https://github.com/Kaimen-Inc/Co-Scientist)
  > Open Source Reimplementation of Google Deepmind's Co-Scientist

### [KyuubiRan](https://github.com/KyuubiRan)
- 🌟 👤 [KyuubiRan](https://github.com/KyuubiRan) Starred [go-gitea/gitea](https://github.com/go-gitea/gitea)
  > Git with a cup of tea! Painless self-hosted all-in-one software development service, including Git h...

### [MBRO95](https://github.com/MBRO95)
- 🌟 👤 [MBRO95](https://github.com/MBRO95) Starred [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)
  > The agent that grows with you

### [Moe-hacker](https://github.com/Moe-hacker)
- 🌟 👤 [Moe-hacker](https://github.com/Moe-hacker) Starred [arnav-kr/Bluke](https://github.com/arnav-kr/Bluke)
  > Bluke turns your phone into a Bluetooth mechanical keyboard, a Gamepad and a Trackpad
- 🌟 👤 [Moe-hacker](https://github.com/Moe-hacker) Starred [hjr265/ukbd](https://github.com/hjr265/ukbd)
  > Virtual Bluetooth keyboard and trackpad combo for Android

### [Monoceros406](https://github.com/Monoceros406)
- 🌟 👤 [Monoceros406](https://github.com/Monoceros406) Starred [cobo666/windowsbooks](https://github.com/cobo666/windowsbooks)
  > Important and interesting books about system programming for Windows OS.
- 🌟 👤 [Monoceros406](https://github.com/Monoceros406) Starred [huihuilikaile/CTF-BTFly](https://github.com/huihuilikaile/CTF-BTFly)
  > 一个基于pi的自动化ctf agent解题工具

### [Mrmiaomrzh](https://github.com/Mrmiaomrzh)
- 🍴 👤 [Mrmiaomrzh](https://github.com/Mrmiaomrzh) Forked [cuandada/Reverse-engineering](https://github.com/cuandada/Reverse-engineering) to [Utility-Software-Collection/Reverse-engineering](https://github.com/Utility-Software-Collection/Reverse-engineering)
  > The results of reverse engineering analysis archives, unpacking, decompiling, decryption of protecti...
- 🌟 👤 [Mrmiaomrzh](https://github.com/Mrmiaomrzh) Starred [HSSkyBoy/NyaEeMC](https://github.com/HSSkyBoy/NyaEeMC)
  > 服务端插件 - MoveLight移动光源

### [Mufanc](https://github.com/Mufanc)
- 🌟 👤 [Mufanc](https://github.com/Mufanc) Starred [SeeFlowerX/unwinddaemon](https://github.com/SeeFlowerX/unwinddaemon)

### [NiTianErXing666](https://github.com/NiTianErXing666)
- 🌟 👤 [NiTianErXing666](https://github.com/NiTianErXing666) Starred [xianwan1314/CVE-2026-43499-Poc-Analysis](https://github.com/xianwan1314/CVE-2026-43499-Poc-Analysis)
  > Vulnerability analysis and Proof of Concept (PoC) for CVE-2026-43499 affecting Xiaomi devices. For e...

### [P001water](https://github.com/P001water)
- 🌟 👤 [P001water](https://github.com/P001water) Starred [swisskyrepo/SMBLoot](https://github.com/swisskyrepo/SMBLoot)
  > SMBLoot is a terminal interface for browsing SMB shares, reading remote files, downloading and uploa...

### [PIKACHUIM](https://github.com/PIKACHUIM)
- 🌟 👤 [PIKACHUIM](https://github.com/PIKACHUIM) Starred [M2Team/NanaBox](https://github.com/M2Team/NanaBox)
  > The third-party lightweight XAML-based out-of-box-experience oriented Hyper-V virtualization softwar...

### [PalmDevs](https://github.com/PalmDevs)
- 🌟 👤 [PalmDevs](https://github.com/PalmDevs) Starred [LaurieWired/CallMeMaybe](https://github.com/LaurieWired/CallMeMaybe)
  > Runtime reflection library built on C++26 static reflection

### [Skulltrail192](https://github.com/Skulltrail192)
- 🌟 👤 [Skulltrail192](https://github.com/Skulltrail192) Starred [diakin/reactosVG14](https://github.com/diakin/reactosVG14)
  > A free Windows-compatible Operating System

### [Slzdude](https://github.com/Slzdude)
- 🌟 👤 [Slzdude](https://github.com/Slzdude) Starred [openai/codex-security](https://github.com/openai/codex-security)
  > OpenAI's Codex Security CLI and TypeScript SDK for finding, validating, and fixing security vulnerab...

### [SodaMilk233](https://github.com/SodaMilk233)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [AYssu/fscan-ui](https://github.com/AYssu/fscan-ui)
  > FastScan V3跨平台源码，预发布版本，前期使用react搭建过一版，本次为跨平台重构！
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [HimitsuShell/HimitsuObfuscator](https://github.com/HimitsuShell/HimitsuObfuscator)
  > a lightweight llvm-17 obfuscator for any linux

### [SummerSec](https://github.com/SummerSec)
- 🌟 👤 [SummerSec](https://github.com/SummerSec) Starred [acepanel/panel](https://github.com/acepanel/panel)
  > ⚡ AcePanel - Enterprise server operation and maintenance management panel
- 🌟 👤 [SummerSec](https://github.com/SummerSec) Starred [FlowiseAI/Flowise](https://github.com/FlowiseAI/Flowise)
  > Build AI Agents, Visually

### [T5ive](https://github.com/T5ive)
- 🌟 👤 [T5ive](https://github.com/T5ive) Starred [drumih/turbo-fieldfare](https://github.com/drumih/turbo-fieldfare)
  > Gemma 4 26B-A4B inference in ~2 GB of RAM on any M-series MacBook
- 🌟 👤 [T5ive](https://github.com/T5ive) Starred [citrolabs/ego-lite](https://github.com/citrolabs/ego-lite)
  > The fastest browser for AI agents to run browser automation, built for sharing your logged-in browse...
- 🌟 👤 [T5ive](https://github.com/T5ive) Starred [different-ai/openwork](https://github.com/different-ai/openwork)
  > The open-source alternative to Claude Cowork (powered by opencode)

### [TUGOhost](https://github.com/TUGOhost)
- 🌟 👤 [TUGOhost](https://github.com/TUGOhost) Starred [xuchonglang/investing-for-beginners](https://github.com/xuchonglang/investing-for-beginners)
  > 小隐寺投资百科官方公开索引：美股、期权与加密货币知识框架

### [TideSec](https://github.com/TideSec)
- 🌟 👤 [TideSec](https://github.com/TideSec) Starred [Autumn-27/ARTEX](https://github.com/Autumn-27/ARTEX)
  > AI 自主渗透测试系统

### [Uevo001](https://github.com/Uevo001)
- 🍴 👤 [Uevo001](https://github.com/Uevo001) Forked [Liebesfreud/Komari-Material](https://github.com/Liebesfreud/Komari-Material) to [Uevo001/Komari-Material](https://github.com/Uevo001/Komari-Material)
  > A Material Design 3 theme for Komari Monitor

### [VoidSec](https://github.com/VoidSec)
- 🌟 👤 [VoidSec](https://github.com/VoidSec) Starred [MiaAI-Lab/sparkDash](https://github.com/MiaAI-Lab/sparkDash)
  > sparkDash ⚡ — Multi-DGX Spark Monitoring Dashboard
- 🍴 👤 [VoidSec](https://github.com/VoidSec) Forked [MiaAI-Lab/sparkDash](https://github.com/MiaAI-Lab/sparkDash) to [VoidSec/sparkDash](https://github.com/VoidSec/sparkDash)
  > sparkDash ⚡ — Multi-DGX Spark Monitoring Dashboard

### [X1r0z](https://github.com/X1r0z)
- 🌟 👤 [X1r0z](https://github.com/X1r0z) Starred [openclaw/mcporter](https://github.com/openclaw/mcporter)
  > Call MCPs via TypeScript, masquerading as simple TypeScript API. Or package them as cli.

### [Xieansecn](https://github.com/Xieansecn)
- 🌟 👤 [Xieansecn](https://github.com/Xieansecn) Starred [PhoenixHairpin/mcp-termux](https://github.com/PhoenixHairpin/mcp-termux)
  > MCP RE v7.0 — Android逆向工程一体化MCP服务器 | 73个工具 | stackplz eBPF + paradise内存 + radare2静态分析 | ARM64 Root

### [YuKongA](https://github.com/YuKongA)
- 🌟 👤 [YuKongA](https://github.com/YuKongA) Starred [fuqiuluo/amice](https://github.com/fuqiuluo/amice)
  > 🍂 Rust LLVM obfuscation framework with instruction-level VMP/code virtualization for Android, C/C++ ...

### [amalmurali47](https://github.com/amalmurali47)
- 🌟 👤 [amalmurali47](https://github.com/amalmurali47) Starred [openai/codex-security](https://github.com/openai/codex-security)
  > OpenAI's Codex Security CLI and TypeScript SDK for finding, validating, and fixing security vulnerab...

### [ba0gu0](https://github.com/ba0gu0)
- 🌟 👤 [ba0gu0](https://github.com/ba0gu0) Starred [AaronL725/grok-register](https://github.com/AaronL725/grok-register)
  > 【已存活三个月+，无封号】批量注册稳定 Grok 账号，可直接导入 grok2api 和 cpa 使用，并且可以与cloudflare无限域名邮箱等联动。通过1000+账号连续注册测试。

### [binbin594738977](https://github.com/binbin594738977)
- 🌟 👤 [binbin594738977](https://github.com/binbin594738977) Starred [Mygod/VPNHotspot](https://github.com/Mygod/VPNHotspot)
  > Share your VPN connection over hotspot or repeater! (root required)
- 🌟 👤 [binbin594738977](https://github.com/binbin594738977) Starred [chen08209/FlClash](https://github.com/chen08209/FlClash)
  > A multi-platform proxy client based on ClashMeta,simple and easy to use, open-source and ad-free.

### [byt3bl33d3r](https://github.com/byt3bl33d3r)
- 🌟 👤 [byt3bl33d3r](https://github.com/byt3bl33d3r) Starred [AminBlg/SimpleEnglish](https://github.com/AminBlg/SimpleEnglish)
  > Agent skill: make LLMs write docs in ASD-STE100 Simplified Technical English — no AI slop

### [carlospolop](https://github.com/carlospolop)
- 🍴 👤 [carlospolop](https://github.com/carlospolop) Forked [chigwell/telegram-mcp](https://github.com/chigwell/telegram-mcp) to [carlospolop/telegram-mcp](https://github.com/carlospolop/telegram-mcp)
  > Telegram MCP server powered by Telethon to let MCP clients read chats, manage groups, and send/modif...

### [cctv18](https://github.com/cctv18)
- 🌟 👤 [cctv18](https://github.com/cctv18) Starred [seakee/CPA-Manager-Plus](https://github.com/seakee/CPA-Manager-Plus)
  > A self-hosted CPA / CLIProxyAPI management panel and AI gateway observability dashboard for requests...
- 🌟 👤 [cctv18](https://github.com/cctv18) Starred [router-for-me/Cli-Proxy-API-Management-Center](https://github.com/router-for-me/Cli-Proxy-API-Management-Center)
  > This is a WebUI interface based on CLI-Proxy-API, designed to simplify configuration modifications a...

### [chizhanyuefeng](https://github.com/chizhanyuefeng)
- 🌟 👤 [chizhanyuefeng](https://github.com/chizhanyuefeng) Starred [CMU-MultiComp-Lab/OpenFace-3.0](https://github.com/CMU-MultiComp-Lab/OpenFace-3.0)
  > OpenFace 3.0 – open-source toolkit for facial landmark detection, action unit detection, eye-gaze es...

### [cn-kali-team](https://github.com/cn-kali-team)
- 🌟 👤 [cn-kali-team](https://github.com/cn-kali-team) Starred [Helicone/ai-gateway](https://github.com/Helicone/ai-gateway)
  > The fastest, lightest, and easiest-to-integrate AI gateway on the market. Fully open-sourced.
- 🌟 👤 [cn-kali-team](https://github.com/cn-kali-team) Starred [tinyhumansai/openhuman](https://github.com/tinyhumansai/openhuman)
  > Your Personal AI super intelligence. A brain that builds a local-first memory of your life, a fantas...
- 🌟 👤 [cn-kali-team](https://github.com/cn-kali-team) Starred [Helicone/helicone](https://github.com/Helicone/helicone)
  > 🧊 Open source LLM observability platform. One line of code to monitor, evaluate, and experiment. YC ...

### [codezjx](https://github.com/codezjx)
- 🌟 👤 [codezjx](https://github.com/codezjx) Starred [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)
  > Sub2API 一站式开源中转服务，让 Claude、Openai 、Gemini、Grok订阅统一接入，支持拼车共享，更高效分摊成本，原生工具无缝使用。

### [ctkqiang](https://github.com/ctkqiang)
- 🍴 👤 [ctkqiang](https://github.com/ctkqiang) Forked [CrackerCat/ghostlock-rothko](https://github.com/CrackerCat/ghostlock-rothko) to [ctkqiang/ghostlock-rothko](https://github.com/ctkqiang/ghostlock-rothko)
  > CVE-2026-43499 GhostLock exploit for Redmi K70 Ultra (rothko) - data-only physmap overwrite
- 🌟 👤 [ctkqiang](https://github.com/ctkqiang) Starred [ctkqiang/ghostlock-rothko](https://github.com/ctkqiang/ghostlock-rothko)
  > CVE-2026-43499 GhostLock exploit for Redmi K70 Ultra (rothko) - data-only physmap overwrite
- 🌟 👤 [ctkqiang](https://github.com/ctkqiang) Starred [bytedance/flowgram.ai](https://github.com/bytedance/flowgram.ai)
  > FlowGram is an extensible workflow development framework with built-in canvas, form, variable, and m...
- 🍴 👤 [ctkqiang](https://github.com/ctkqiang) Forked [bytedance/flowgram.ai](https://github.com/bytedance/flowgram.ai) to [ctkqiang/flowgram.ai](https://github.com/ctkqiang/flowgram.ai)
  > FlowGram is an extensible workflow development framework with built-in canvas, form, variable, and m...

### [cuiliang](https://github.com/cuiliang)
- 🌟 👤 [cuiliang](https://github.com/cuiliang) Starred [shuimowang/QuickerLink](https://github.com/shuimowang/QuickerLink)
  > Open-source Android client for Quicker actions, screen, clipboard, and small files over LAN WSS

### [cwuom](https://github.com/cwuom)
- 🌟 👤 [cwuom](https://github.com/cwuom) Starred [chenmozhijin/LDDC](https://github.com/chenmozhijin/LDDC)
  > 简单易用的精准歌词(逐字歌词/卡拉OK歌词)下载匹配工具|A simple and user-friendly tool for downloading and matching precise ly...

### [darkskygit](https://github.com/darkskygit)
- 🌟 👤 [darkskygit](https://github.com/darkskygit) Starred [Eatgrapes/Mocari](https://github.com/Eatgrapes/Mocari)
  > A pure Rust Live2D/Cubism runtime experiment

### [doodlewind](https://github.com/doodlewind)
- 🌟 👤 [doodlewind](https://github.com/doodlewind) Starred [siwei-yuan/coral](https://github.com/siwei-yuan/coral)
  > Agent Swarms that evolve by changing code. Every iteration committed. Every change auditable.

### [duanluan](https://github.com/duanluan)
- 🍴 👤 [duanluan](https://github.com/duanluan) Forked [luoxingran/dolby_beta](https://github.com/luoxingran/dolby_beta) to [duanluan/dolby](https://github.com/duanluan/dolby)
  > 杜比大喇叭的β版迎来了重大的革新，合并了UnblockMusic Pro的所有功能且更加强大，同时UnblockMusicPro_Xposed项目将会停止维护，让我们欢送这位老朋友！

### [dzxpert](https://github.com/dzxpert)
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [yxlnqs/intel-full-emu-I225](https://github.com/yxlnqs/intel-full-emu-I225)
  > DMA Firmware or sth idek  | if you dont know what you do then dont change anything on the bar contro...

### [easychen](https://github.com/easychen)
- 🌟 👤 [easychen](https://github.com/easychen) Starred [cloudflarebase/cloudflarebase](https://github.com/cloudflarebase/cloudflarebase)
  > The open-source Firebase for Cloudflare.
- 🌟 👤 [easychen](https://github.com/easychen) Starred [QwenAudio/qwen-audio-agent](https://github.com/QwenAudio/qwen-audio-agent)
  > A realtime voice runtime that keeps Agents talking, working, and present.  Real-time Voice Runtime f...
- 🌟 👤 [easychen](https://github.com/easychen) Starred [oomol-lab/open-connector](https://github.com/oomol-lab/open-connector)
  > Open-source auth gateway connecting 1000+ SaaS providers to AI agents through SDK, CLI, MCP, HTTP, a...

### [ejpir](https://github.com/ejpir)
- 🍴 👤 [ejpir](https://github.com/ejpir) Forked [earendil-works/pi](https://github.com/earendil-works/pi) to [ejpir/pi](https://github.com/ejpir/pi)
  > AI agent toolkit: unified LLM API, agent loop, TUI, coding agent CLI

### [ergrelet](https://github.com/ergrelet)
- 🌟 👤 [ergrelet](https://github.com/ergrelet) Starred [fvrmatteo/DrillAndJoin](https://github.com/fvrmatteo/DrillAndJoin)
  > AI implementation of the "Drill & Join: A Method for Exact Inductive Program Synthesis" paper by Rem...

### [eteran](https://github.com/eteran)
- 🌟 👤 [eteran](https://github.com/eteran) Starred [astral-sh/ruff](https://github.com/astral-sh/ruff)
  > An extremely fast Python linter and code formatter, written in Rust.

### [everettjf](https://github.com/everettjf)
- 🌟 👤 [everettjf](https://github.com/everettjf) Starred [QwenAudio/qwen-audio-agent](https://github.com/QwenAudio/qwen-audio-agent)
  > A realtime voice runtime that keeps Agents talking, working, and present.  Real-time Voice Runtime f...

### [feicong](https://github.com/feicong)
- 🌟 👤 [feicong](https://github.com/feicong) Starred [LeoChen-CoreMind/VMPacker](https://github.com/LeoChen-CoreMind/VMPacker)
  > ARM64 ELF Virtual Machine Protection System
- 🌟 👤 [feicong](https://github.com/feicong) Starred [revercc/VMPacker](https://github.com/revercc/VMPacker)
  > ARM64 ELF Virtual Machine Protection System

### [fengjixuchui](https://github.com/fengjixuchui)
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [Infatoshi/netherite](https://github.com/Infatoshi/netherite) to [fengjixuchui/netherite](https://github.com/fengjixuchui/netherite)
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [cli0xfa/mmio-for-byovd](https://github.com/cli0xfa/mmio-for-byovd) to [fengjixuchui/mmio-for-byovd](https://github.com/fengjixuchui/mmio-for-byovd)
  > Header-only x64 Windows kernel memory access helper for BYOVD toolchains: System CR3 scan, LA57-awar...

### [flostyen](https://github.com/flostyen)
- 🌟 👤 [flostyen](https://github.com/flostyen) Starred [M3NIX/ssh-tui-rs](https://github.com/M3NIX/ssh-tui-rs)
  > Simple SSH TUI built with ratatui to navigate .ssh/config saved connections

### [for-A1kaid](https://github.com/for-A1kaid)
- 🌟 👤 [for-A1kaid](https://github.com/for-A1kaid) Starred [moyan222/Browser-BucketScan](https://github.com/moyan222/Browser-BucketScan)

### [fvrmatteo](https://github.com/fvrmatteo)
- 🚀 👤 [fvrmatteo](https://github.com/fvrmatteo) Made [fvrmatteo/AutomaticAbstractionForCongruences](https://github.com/fvrmatteo/AutomaticAbstractionForCongruences) public
  > AI implementation of the "Automatic Abstraction for Congruences" and "Automatic Abstraction for Inte...
- 🚀 👤 [fvrmatteo](https://github.com/fvrmatteo) Made [fvrmatteo/DrillAndJoin](https://github.com/fvrmatteo/DrillAndJoin) public
  > AI implementation of the "Drill & Join: A Method for Exact Inductive Program Synthesis" paper by Rem...
- 🌟 👤 [fvrmatteo](https://github.com/fvrmatteo) Starred [razordemo/insert-no-coins](https://github.com/razordemo/insert-no-coins)
  > "Insert No Coins" 64k intro
- 🌟 👤 [fvrmatteo](https://github.com/fvrmatteo) Starred [razordemo/the-scene-is-dead](https://github.com/razordemo/the-scene-is-dead)
  > sources of the 64k intro "THE SCENE IS DEAD"

### [fynch3r](https://github.com/fynch3r)
- 🌟 👤 [fynch3r](https://github.com/fynch3r) Starred [xuchonglang/investing-for-beginners](https://github.com/xuchonglang/investing-for-beginners)
  > 小隐寺投资百科官方公开索引：美股、期权与加密货币知识框架

### [gh0stkey](https://github.com/gh0stkey)
- 🌟 👤 [gh0stkey](https://github.com/gh0stkey) Starred [anomalyco/models.dev](https://github.com/anomalyco/models.dev)
  > An open-source database of AI models.

### [gmh5225](https://github.com/gmh5225)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [pashov/ai-web3-security](https://github.com/pashov/ai-web3-security) to [gmh5225/ai-web3-security](https://github.com/gmh5225/ai-web3-security)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [notahacker8/RobloxCheats](https://github.com/notahacker8/RobloxCheats) to [gmh5225/RobloxCheats](https://github.com/gmh5225/RobloxCheats)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [daimon3332/address](https://github.com/daimon3332/address) to [gmh5225/address](https://github.com/gmh5225/address)
  > A self-hosted address and synthetic test-profile generator for 27 countries and regions, built from ...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [digimata/quill](https://github.com/digimata/quill) to [gmh5225/quill--](https://github.com/gmh5225/quill--)
  > Ultra-minimalist macOS recording + transcription.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [shinthink/CVE-2026-57827](https://github.com/shinthink/CVE-2026-57827) to [gmh5225/CVE-2026-57827](https://github.com/gmh5225/CVE-2026-57827)
  > CVE-2026-57827 — RSFiles! Joomla Component Unauthenticated File Upload RCE. Split-controller upload ...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [mutinylaboratories/ghidra_svr_bridge](https://github.com/mutinylaboratories/ghidra_svr_bridge) to [gmh5225/ghidra_svr_bridge](https://github.com/gmh5225/ghidra_svr_bridge)
  > Binary Ninja plugin that syncs analysis bidirectionally with a Ghidra Server repository, via a Java ...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [sumeshi/agentlogparser-rs](https://github.com/sumeshi/agentlogparser-rs) to [gmh5225/agentlogparser-rs](https://github.com/gmh5225/agentlogparser-rs)
  > A library for parsing AI agent artifacts in JSON, JSONL, or CSV format.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [HimitsuShell/HimitsuObfuscator](https://github.com/HimitsuShell/HimitsuObfuscator) to [gmh5225/HimitsuObfuscator](https://github.com/gmh5225/HimitsuObfuscator)
  > a lightweight llvm-17 obfuscator for any linux
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [Frontle-Foundation/BottomSheet](https://github.com/Frontle-Foundation/BottomSheet) to [gmh5225/BottomSheet](https://github.com/gmh5225/BottomSheet)
  > Bottom sheet UI available in Vanilla JS
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [HimitsuShell/HimitsuShell](https://github.com/HimitsuShell/HimitsuShell) to [gmh5225/HimitsuShell](https://github.com/gmh5225/HimitsuShell)
  > shell script protector (obfuscation, embedded interpreter, anti-tamper, DRM) - linux, IoT
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [itemVerse/ItemVerse-UnitySDK](https://github.com/itemVerse/ItemVerse-UnitySDK) to [gmh5225/ItemVerse-UnitySDK](https://github.com/gmh5225/ItemVerse-UnitySDK)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [AminBlg/SimpleEnglish](https://github.com/AminBlg/SimpleEnglish) to [gmh5225/SimpleEnglish](https://github.com/gmh5225/SimpleEnglish)
  > Agent skill: make LLMs write docs in ASD-STE100 Simplified Technical English — no AI slop
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [HexaEngine/Hexa.NET.ImGui](https://github.com/HexaEngine/Hexa.NET.ImGui) to [gmh5225/Hexa.NET.ImGui](https://github.com/gmh5225/Hexa.NET.ImGui)
  > A .NET wrapper for the Dear ImGui.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [fvrmatteo/DrillAndJoin](https://github.com/fvrmatteo/DrillAndJoin) to [gmh5225/DrillAndJoin](https://github.com/gmh5225/DrillAndJoin)
  > AI implementation of the "Drill & Join: A Method for Exact Inductive Program Synthesis" paper by Rem...

### [hahwul](https://github.com/hahwul)
- 🌟 👤 [hahwul](https://github.com/hahwul) Starred [google/gvisor](https://github.com/google/gvisor)
  > Application Kernel for Containers
- 🍴 👤 [hahwul](https://github.com/hahwul) Forked [sdogruyol/gcry](https://github.com/sdogruyol/gcry) to [hahwul/gcry](https://github.com/hahwul/gcry)
  > Crystal’s GC, written in Crystal.
- 🌟 👤 [hahwul](https://github.com/hahwul) Starred [sdogruyol/fast-http-server](https://github.com/sdogruyol/fast-http-server)
  > Super fast, zero configuration command line HTTP Server.
- 🌟 👤 [hahwul](https://github.com/hahwul) Starred [sdogruyol/tren](https://github.com/sdogruyol/tren)
  > Use SQL as first-class Crystal methods

### [halfkiss](https://github.com/halfkiss)
- 🌟 👤 [halfkiss](https://github.com/halfkiss) Starred [valinet/ExplorerPatcher](https://github.com/valinet/ExplorerPatcher)
  > This project aims to enhance the working environment on Windows
- 🌟 👤 [halfkiss](https://github.com/halfkiss) Starred [TranslucentTB/TranslucentTB](https://github.com/TranslucentTB/TranslucentTB)
  > A lightweight utility that makes the Windows taskbar translucent/transparent.

### [hhhaiai](https://github.com/hhhaiai)
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [YuJunZhiXue/Cline-proxy](https://github.com/YuJunZhiXue/Cline-proxy)
  > 一个使用go写的简略的反代CLine的网关

### [hoochanlon](https://github.com/hoochanlon)
- 🍴 👤 [hoochanlon](https://github.com/hoochanlon) Forked [ricocc/uiineed-todo-list](https://github.com/ricocc/uiineed-todo-list) to [hoochanlon/todo](https://github.com/hoochanlon/todo)
  > Todo List Online - Minimalist, No-Login Required Web Todo App
- 🍴 👤 [hoochanlon](https://github.com/hoochanlon) Forked [GoogleChromeLabs/squoosh](https://github.com/GoogleChromeLabs/squoosh) to [hoochanlon/tuya](https://github.com/hoochanlon/tuya)
  > Make images smaller using best-in-class codecs, right in the browser.

### [huntergregal](https://github.com/huntergregal)
- 🌟 👤 [huntergregal](https://github.com/huntergregal) Starred [8051Enthusiast/allyourbase](https://github.com/8051Enthusiast/allyourbase)
  > Finds the base address of a firmware by comparing string addresses with target pointer addresses

### [hyuunnn](https://github.com/hyuunnn)
- 🚀 👤 [hyuunnn](https://github.com/hyuunnn) Made [hyuunnn/jadxvim](https://github.com/hyuunnn/jadxvim) public
- 🚀 👤 [hyuunnn](https://github.com/hyuunnn) Made [hyuunnn/jadx-slides](https://github.com/hyuunnn/jadx-slides) public

### [iAklis](https://github.com/iAklis)
- 🌟 👤 [iAklis](https://github.com/iAklis) Starred [huggingface/speech-to-speech](https://github.com/huggingface/speech-to-speech)
  > Build local voice agents with open-source models

### [jianglin-code](https://github.com/jianglin-code)
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [FossifyOrg/Contacts](https://github.com/FossifyOrg/Contacts)
  > Easy and quick contact management with no ads, handles groups and favorites too.

### [jonpalmisc](https://github.com/jonpalmisc)
- 🌟 👤 [jonpalmisc](https://github.com/jonpalmisc) Starred [baochip/baochip-1x](https://github.com/baochip/baochip-1x)
  > Baochip 1x Silicon

### [jpillora](https://github.com/jpillora)
- 🌟 👤 [jpillora](https://github.com/jpillora) Starred [bashalarmistalt/decimen-optical-transfer](https://github.com/bashalarmistalt/decimen-optical-transfer)
- 🌟 👤 [jpillora](https://github.com/jpillora) Starred [denoland/celld](https://github.com/denoland/celld)
  > self-hosted, distributed Durable Objects

### [junknet](https://github.com/junknet)
- 🍴 👤 [junknet](https://github.com/junknet) Forked [1jehuang/jcode](https://github.com/1jehuang/jcode) to [junknet/jcode](https://github.com/junknet/jcode)
  > The most RAM efficient harness

### [jweny](https://github.com/jweny)
- 🌟 👤 [jweny](https://github.com/jweny) Starred [nexu-io/open-design](https://github.com/nexu-io/open-design)
  > 🎨 The open-source Claude Design alternative. 🖥️ Local-first desktop app. 🖼️ Your coding agent become...

### [keleus](https://github.com/keleus)
- 🌟 👤 [keleus](https://github.com/keleus) Starred [rohitg00/ai-engineering-from-scratch](https://github.com/rohitg00/ai-engineering-from-scratch)
  > Learn it. Build it. Ship it for others.

### [kmkz](https://github.com/kmkz)
- 🍴 👤 [kmkz](https://github.com/kmkz) Forked [rapid7/metasploit-framework](https://github.com/rapid7/metasploit-framework) to [kmkz/metasploit-framework](https://github.com/kmkz/metasploit-framework)
  > Metasploit Framework

### [liuhuanyong](https://github.com/liuhuanyong)
- 🌟 👤 [liuhuanyong](https://github.com/liuhuanyong) Starred [liuhuanyong/FDEGreenBook](https://github.com/liuhuanyong/FDEGreenBook)
  > 一个自包含的交互式网站，把 FDE（Forward Deployed Engineer，前线部署工程师）讲透：教程 + 游戏 + 画像测试三合一。

### [luzhixing12345](https://github.com/luzhixing12345)
- 🚀 👤 [luzhixing12345](https://github.com/luzhixing12345) Made [luzhixing12345/pic-handle](https://github.com/luzhixing12345/pic-handle) public
  > 在线图片处理（裁剪/尺寸/压缩/证件照）

### [m4ll0k](https://github.com/m4ll0k)
- 🌟 👤 [m4ll0k](https://github.com/m4ll0k) Starred [sunblaze-ucb/cybergym](https://github.com/sunblaze-ucb/cybergym)
  > CyberGym is a large-scale, high-quality cybersecurity evaluation framework designed to rigorously as...
- 🍴 👤 [m4ll0k](https://github.com/m4ll0k) Forked [sunblaze-ucb/cybergym](https://github.com/sunblaze-ucb/cybergym) to [m4ll0k/cybergym](https://github.com/m4ll0k/cybergym)
  > CyberGym is a large-scale, high-quality cybersecurity evaluation framework designed to rigorously as...

### [mingheyan](https://github.com/mingheyan)
- 🌟 👤 [mingheyan](https://github.com/mingheyan) Starred [Facetomyself/ruyi-mcp](https://github.com/Facetomyself/ruyi-mcp)
  > Community MCP server for ruyiPage browser automation, network inspection, fingerprint analysis, and ...

### [moehw](https://github.com/moehw)
- 🌟 👤 [moehw](https://github.com/moehw) Starred [theodorebeaupre-prog/garmin-gcd-toolkit](https://github.com/theodorebeaupre-prog/garmin-gcd-toolkit)
  > Reverse-engineering toolkit for Garmin .GCD firmware containers — documented format + Python CLI to ...

### [mustime](https://github.com/mustime)
- 🌟 👤 [mustime](https://github.com/mustime) Starred [earendil-works/pi](https://github.com/earendil-works/pi)
  > AI agent toolkit: unified LLM API, agent loop, TUI, coding agent CLI

### [mzfr](https://github.com/mzfr)
- 🌟 👤 [mzfr](https://github.com/mzfr) Starred [Noelo-Lab/kuna](https://github.com/Noelo-Lab/kuna)
  > An agent-first decompiler designed to be refined by other agents. Kuna is written in Rust and was or...

### [nafiez](https://github.com/nafiez)
- 🚀 👤 [nafiez](https://github.com/nafiez) Made [nafiez/Metasploit-CVE-2026-54121-Certighost](https://github.com/nafiez/Metasploit-CVE-2026-54121-Certighost) public
  > A Metasploit auxiliary module that escalates from any low-privileged domain user to full domain comp...

### [natsumerinchan](https://github.com/natsumerinchan)
- 🌟 👤 [natsumerinchan](https://github.com/natsumerinchan) Starred [1wc10086/Han1mePlus](https://github.com/1wc10086/Han1mePlus)
  > 一个完全由 Dart 和 Flutter 构建的 Hanime1 第三方客户端

### [nblog](https://github.com/nblog)
- 🌟 👤 [nblog](https://github.com/nblog) Starred [openai/codex-security](https://github.com/openai/codex-security)
  > OpenAI's Codex Security CLI and TypeScript SDK for finding, validating, and fixing security vulnerab...

### [nek0y4nsu](https://github.com/nek0y4nsu)
- 🌟 👤 [nek0y4nsu](https://github.com/nek0y4nsu) Starred [PeerDB-io/peerdb](https://github.com/PeerDB-io/peerdb)
  > Fast, Simple and a cost effective tool to replicate data from Postgres to Data Warehouses, Queues an...

### [netxfly](https://github.com/netxfly)
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [block/buzz](https://github.com/block/buzz)
  > A hive mind communication platform
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [physiclaw/PhysiClaw](https://github.com/physiclaw/PhysiClaw)
  > The AI agent that interacts with you in the real world.
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [Autumn-27/ARTEX](https://github.com/Autumn-27/ARTEX)
  > AI 自主渗透测试系统
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [Autumn-27/ScopeSentry](https://github.com/Autumn-27/ScopeSentry)
  > ScopeSentry-Cyberspace mapping, subdomain enumeration, port scanning, sensitive information discover...

### [noxke](https://github.com/noxke)
- 🌟 👤 [noxke](https://github.com/noxke) Starred [UiIsBack/GhostHook](https://github.com/UiIsBack/GhostHook)
  > GhostHook is a webhook spammer which spams discord webhooks with a message of your choice and it's v...

### [npjohnson](https://github.com/npjohnson)
- 🌟 👤 [npjohnson](https://github.com/npjohnson) Starred [codewalkerster/platform_hardware_wifi_qualcomm_drivers_qca6174](https://github.com/codewalkerster/platform_hardware_wifi_qualcomm_drivers_qca6174)
- 🌟 👤 [npjohnson](https://github.com/npjohnson) Starred [codewalkerster/vendor_amlogic_wifi](https://github.com/codewalkerster/vendor_amlogic_wifi)

### [null-dev](https://github.com/null-dev)
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [xtclovver/RKNHardering](https://github.com/xtclovver/RKNHardering)
  > Обнаружние VPN конфигураторов на пользовательских устройствах с дальнейшей целью защиты от обнаружен...
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [lukaspieper/Gcam-Services-Provider](https://github.com/lukaspieper/Gcam-Services-Provider)
  > App faking only the absolute necessary Apis to use Gcam without Play Services

### [oleavr](https://github.com/oleavr)
- 🍴 👤 [oleavr](https://github.com/oleavr) Forked [picolibc/picolibc](https://github.com/picolibc/picolibc) to [frida/picolibc](https://github.com/frida/picolibc)
  > picolibc - a C library designed for embedded 32- and 64- bit systems.

### [oobbb](https://github.com/oobbb)
- 🌟 👤 [oobbb](https://github.com/oobbb) Starred [dannyisbad/kilij](https://github.com/dannyisbad/kilij)
  > LLVM 20 code obfuscation framework. 12 IR passes and a bytecode VM with three execution modes.

### [penguin-wwy](https://github.com/penguin-wwy)
- 🌟 👤 [penguin-wwy](https://github.com/penguin-wwy) Starred [NVlabs/CuTe](https://github.com/NVlabs/CuTe)
  > Reference implementation and examples of the CuTe Layout representation and algebra.

### [plusls](https://github.com/plusls)
- 🌟 👤 [plusls](https://github.com/plusls) Starred [openclaw/mcporter](https://github.com/openclaw/mcporter)
  > Call MCPs via TypeScript, masquerading as simple TypeScript API. Or package them as cli.

### [polymorf](https://github.com/polymorf)
- 🍴 👤 [polymorf](https://github.com/polymorf) Forked [antirez/ds4](https://github.com/antirez/ds4) to [polymorf/ds4](https://github.com/polymorf/ds4)
  > DeepSeek 4 Flash and PRO local inference engine for Metal, CUDA and ROCm

### [progmboy](https://github.com/progmboy)
- 🌟 👤 [progmboy](https://github.com/progmboy) Starred [huggingface/speech-to-speech](https://github.com/huggingface/speech-to-speech)
  > Build local voice agents with open-source models
- 🌟 👤 [progmboy](https://github.com/progmboy) Starred [Noelo-Lab/kuna](https://github.com/Noelo-Lab/kuna)
  > An agent-first decompiler designed to be refined by other agents. Kuna is written in Rust and was or...

### [pwh-pwh](https://github.com/pwh-pwh)
- 🌟 👤 [pwh-pwh](https://github.com/pwh-pwh) Starred [Nutlope/hallmark](https://github.com/Nutlope/hallmark)
  > Anti-AI-slop design skill for Claude Code, Cursor, and Codex.
- 🌟 👤 [pwh-pwh](https://github.com/pwh-pwh) Starred [superdesigndev/superdesign-skill](https://github.com/superdesigndev/superdesign-skill)
  > The design skill for Claude Code, Cursor and any coding agent. Stop shipping AI-slop UI: turn it int...

### [quanyeyang](https://github.com/quanyeyang)
- 🌟 👤 [quanyeyang](https://github.com/quanyeyang) Starred [Functionhx/rebuttal-reader](https://github.com/Functionhx/rebuttal-reader)
  > Read public peer-review conversations from reviewer concern to author rebuttal and final decision.
- 🌟 👤 [quanyeyang](https://github.com/quanyeyang) Starred [bsc-loca/sargantana](https://github.com/bsc-loca/sargantana)
- 🍴 👤 [quanyeyang](https://github.com/quanyeyang) Forked [bsc-loca/sargantana](https://github.com/bsc-loca/sargantana) to [quanyeyang/sargantana](https://github.com/quanyeyang/sargantana)

### [r00tuser111](https://github.com/r00tuser111)
- 🌟 👤 [r00tuser111](https://github.com/r00tuser111) Starred [0rangec3t/Black-cat](https://github.com/0rangec3t/Black-cat)
  > Claude Code RedTeam Skill — Hypothesis-Driven Cognitive Architecture。一个假设-证据驱动的红队skill

### [ret2ldz](https://github.com/ret2ldz)
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [googleprojectzero/Jackalope](https://github.com/googleprojectzero/Jackalope)
  > Binary, coverage-guided fuzzer for Windows, macOS, Linux and Android

### [roadwy](https://github.com/roadwy)
- 🌟 👤 [roadwy](https://github.com/roadwy) Starred [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph)
  > Pre-indexed code knowledge graph, auto syncs on code changes, for Claude Code, Codex, Gemini, Cursor...

### [sekaiacg](https://github.com/sekaiacg)
- 🌟 👤 [sekaiacg](https://github.com/sekaiacg) Starred [JingMatrix/Demo](https://github.com/JingMatrix/Demo)
  > A demo app to detect Zygote injections
- 🌟 👤 [sekaiacg](https://github.com/sekaiacg) Starred [JingMatrix/LSPatch](https://github.com/JingMatrix/LSPatch)
  > A non-root Xposed framework extending from LSPosed

### [semlinker](https://github.com/semlinker)
- 🌟 👤 [semlinker](https://github.com/semlinker) Starred [huggingface/speech-to-speech](https://github.com/huggingface/speech-to-speech)
  > Build local voice agents with open-source models

### [sgnls](https://github.com/sgnls)
- 🍴 👤 [sgnls](https://github.com/sgnls) Forked [Floorp-Projects/Floorp](https://github.com/Floorp-Projects/Floorp) to [sgnls/Floorp](https://github.com/sgnls/Floorp)
  > All of source code of Floorp 12, the most Advanced and Fastest Firefox derivative 🦊

### [sh1ma](https://github.com/sh1ma)
- 🌟 👤 [sh1ma](https://github.com/sh1ma) Starred [marp-team/marp-cli](https://github.com/marp-team/marp-cli)
  > A CLI interface for Marp and Marpit based converters
- 🌟 👤 [sh1ma](https://github.com/sh1ma) Starred [mattmc3/antidote](https://github.com/mattmc3/antidote)
  > https://antidote.sh - the cure to slow zsh plugin management

### [shunf4](https://github.com/shunf4)
- 🍴 👤 [shunf4](https://github.com/shunf4) Forked [sanyok12345/teleproto](https://github.com/sanyok12345/teleproto) to [ProjectKoto/teleproto](https://github.com/ProjectKoto/teleproto)
  > Telegram MTProto API client library written in TypeScript

### [sin5678](https://github.com/sin5678)
- 🌟 👤 [sin5678](https://github.com/sin5678) Starred [bkerler/edl](https://github.com/bkerler/edl)
  > Inofficial Qualcomm Firehose / Sahara / Streaming / Diag Tools :)

### [sonyps5201314](https://github.com/sonyps5201314)
- 🍴 👤 [sonyps5201314](https://github.com/sonyps5201314) Forked [mrexodia/ida-pro-mcp](https://github.com/mrexodia/ida-pro-mcp) to [sonyps5201314/ida-pro-mcp](https://github.com/sonyps5201314/ida-pro-mcp)
  > AI-powered reverse engineering assistant that bridges IDA Pro with language models through MCP.

### [sotasan](https://github.com/sotasan)
- 🌟 👤 [sotasan](https://github.com/sotasan) Starred [agavra/tuicr](https://github.com/agavra/tuicr)
  > a code review TUI with vim keybindings
- 🌟 👤 [sotasan](https://github.com/sotasan) Starred [openai/whisper](https://github.com/openai/whisper)
  > Robust Speech Recognition via Large-Scale Weak Supervision

### [stuxnet147](https://github.com/stuxnet147)
- 🌟 👤 [stuxnet147](https://github.com/stuxnet147) Starred [stablyai/orca](https://github.com/stablyai/orca)
  > Orca is the ADE for working with a fleet of parallel agents. Run any coding agent with your own subs...
- 🌟 👤 [stuxnet147](https://github.com/stuxnet147) Starred [SoftSec-KAIST/BinKit](https://github.com/SoftSec-KAIST/BinKit)
  > Binary Code Similarity Analysis (BCSA) Benchmark

### [suifei](https://github.com/suifei)
- 🌟 👤 [suifei](https://github.com/suifei) Starred [openai/codex-security](https://github.com/openai/codex-security)
  > OpenAI's Codex Security CLI and TypeScript SDK for finding, validating, and fixing security vulnerab...

### [thatstraw](https://github.com/thatstraw)
- 🍴 👤 [thatstraw](https://github.com/thatstraw) Forked [learnhouse/learnhouse](https://github.com/learnhouse/learnhouse) to [thatstraw/learnhouse](https://github.com/thatstraw/learnhouse)
  > The Next-gen Open Source learning platform for everyone ✨

### [ttttmr](https://github.com/ttttmr)
- 🌟 👤 [ttttmr](https://github.com/ttttmr) Starred [pingdotgg/t3code](https://github.com/pingdotgg/t3code)

### [uforever](https://github.com/uforever)
- 🍴 👤 [uforever](https://github.com/uforever) Forked [SukiSU-Ultra/SukiSU-Ultra](https://github.com/SukiSU-Ultra/SukiSU-Ultra) to [uforever/SukiSU-Ultra](https://github.com/uforever/SukiSU-Ultra)
  > Kernel-based Android Root Solution & KPM

### [volodymyrsmirnov](https://github.com/volodymyrsmirnov)
- 🌟 👤 [volodymyrsmirnov](https://github.com/volodymyrsmirnov) Starred [ScrapeCreators/social-media-research-skills](https://github.com/ScrapeCreators/social-media-research-skills)
  > Social media research skills for AI agents powered by ScrapeCreators

### [wa1ki0g](https://github.com/wa1ki0g)
- 🌟 👤 [wa1ki0g](https://github.com/wa1ki0g) Starred [xai-org/x-algorithm](https://github.com/xai-org/x-algorithm)
  > Algorithm powering the For You feed on X

### [wangshub](https://github.com/wangshub)
- 🌟 👤 [wangshub](https://github.com/wangshub) Starred [mattpocock/skills](https://github.com/mattpocock/skills)
  > Skills for Real Engineers. Straight from my .agents directory.

### [waruqi](https://github.com/waruqi)
- 🌟 👤 [waruqi](https://github.com/waruqi) Starred [espressif/esp32-arduino-lib-builder](https://github.com/espressif/esp32-arduino-lib-builder)

### [whoshuu](https://github.com/whoshuu)
- 🌟 👤 [whoshuu](https://github.com/whoshuu) Starred [TheSnakeFang/rlviz](https://github.com/TheSnakeFang/rlviz)
  > Visualize and compare agent rollouts.

### [williballenthin](https://github.com/williballenthin)
- 🌟 👤 [williballenthin](https://github.com/williballenthin) Starred [HullaBrian/ttd-capa](https://github.com/HullaBrian/ttd-capa)
  > Capability extraction from Time Travel Debugging Traces

### [winezer0](https://github.com/winezer0)
- 🍴 👤 [winezer0](https://github.com/winezer0) Forked [zzet/gortex](https://github.com/zzet/gortex) to [winezer0/gortex](https://github.com/winezer0/gortex)
  > High-performance code-intelligence engine for AI agents and IDE, supports 257 languages, multi repos...

### [wishihab](https://github.com/wishihab)
- 🌟 👤 [wishihab](https://github.com/wishihab) Starred [monteslu/qrshare](https://github.com/monteslu/qrshare)
  > an offline way to share files with QR codes, WASM, and usermedia

### [wkbin](https://github.com/wkbin)
- 🌟 👤 [wkbin](https://github.com/wkbin) Starred [M-YiXi/writeGOD](https://github.com/M-YiXi/writeGOD)
  > 这是一个基于 知识图谱 + 向量检索 的作家辅助工具。它将你的素材和小说文本转化为可视化的概念网络，帮你理清人物关系、追踪伏笔、检测逻辑漏洞。对于一些超大型百万字的小说非常有帮助，可以回顾到整本书的大...

### [wnxd](https://github.com/wnxd)
- 🍴 👤 [wnxd](https://github.com/wnxd) Forked [daijro/camoufox](https://github.com/daijro/camoufox) to [wnxd/camoufox](https://github.com/wnxd/camoufox)
  > 🦊 Anti-detect browser

### [wy876](https://github.com/wy876)
- 🌟 👤 [wy876](https://github.com/wy876) Starred [huanhq99/FlixPilot](https://github.com/huanhq99/FlixPilot)
  > 你的全能 Emby / 流媒体管理中枢

### [x-tools-author](https://github.com/x-tools-author)
- 🌟 👤 [x-tools-author](https://github.com/x-tools-author) Starred [YorkWaugh/AutoCAD_Official_Links](https://github.com/YorkWaugh/AutoCAD_Official_Links)
  > AutoCAD Simplified Chinese Official Links Collection

### [ybdt](https://github.com/ybdt)
- 🌟 👤 [ybdt](https://github.com/ybdt) Starred [JLospinoso/gargoyle](https://github.com/JLospinoso/gargoyle)
  > Historical Windows temporal memory-state research artifact for studying time-bound memory observatio...
- 🌟 👤 [ybdt](https://github.com/ybdt) Starred [tijme/kong-loader](https://github.com/tijme/kong-loader)
  > Using Just In Time (JIT) instruction decryption, this shellcode loader ensures that only the current...
- 🌟 👤 [ybdt](https://github.com/ybdt) Starred [tijme/amd-ryzen-master-driver-v17-exploit](https://github.com/tijme/amd-ryzen-master-driver-v17-exploit)
  > Cobalt Strike (CS) Beacon Object File (BOF) for kernel exploitation using AMD's Ryzen Master Driver ...
- 🌟 👤 [ybdt](https://github.com/ybdt) Starred [ScriptIdiot/sleepmask_PatchlessHook](https://github.com/ScriptIdiot/sleepmask_PatchlessHook)
  > Code snippets to add on top of cobalt strike sleep mask to achieve patchless hook on AMSI and ETW

### [yifengyou](https://github.com/yifengyou)
- 🌟 👤 [yifengyou](https://github.com/yifengyou) Starred [zhangyantao/opensm](https://github.com/zhangyantao/opensm)
  > 开源国密SM2/SM3/SM4算法，基于GmSSL，基于OpenSSL1.1.0

### [yujincheng08](https://github.com/yujincheng08)
- 🌟 👤 [yujincheng08](https://github.com/yujincheng08) Starred [cherryand1112-creator/IPTV-JAPAN-M3U](https://github.com/cherryand1112-creator/IPTV-JAPAN-M3U)
  > 只为分享能够持续有效的日本电视台m3u直播源，尽我所能做到每月更新

### [yux1azhengye](https://github.com/yux1azhengye)
- 🌟 👤 [yux1azhengye](https://github.com/yux1azhengye) Starred [Zer0SumGam3/CVE-2026-66066-POC](https://github.com/Zer0SumGam3/CVE-2026-66066-POC)
  > PoC for CVE-2026-66066 in Ruby on Rails
- 🍴 👤 [yux1azhengye](https://github.com/yux1azhengye) Forked [Zer0SumGam3/CVE-2026-66066-POC](https://github.com/Zer0SumGam3/CVE-2026-66066-POC) to [yux1azhengye/CVE-2026-66066-POC](https://github.com/yux1azhengye/CVE-2026-66066-POC)
  > PoC for CVE-2026-66066 in Ruby on Rails
- 🌟 👤 [yux1azhengye](https://github.com/yux1azhengye) Starred [PortSwigger/mcp-server](https://github.com/PortSwigger/mcp-server)
  > MCP Server for Burp
- 🍴 👤 [yux1azhengye](https://github.com/yux1azhengye) Forked [PortSwigger/mcp-server](https://github.com/PortSwigger/mcp-server) to [yux1azhengye/mcp-server](https://github.com/yux1azhengye/mcp-server)
  > MCP Server for Burp

### [yuzheng14](https://github.com/yuzheng14)
- 🌟 👤 [yuzheng14](https://github.com/yuzheng14) Starred [AetherKiri/AetherKiri](https://github.com/AetherKiri/AetherKiri)
  > A cross-platform KiriKiri2 game emulator using Godot 4 as the rendering layer.

### [zacbre](https://github.com/zacbre)
- 🌟 👤 [zacbre](https://github.com/zacbre) Starred [Technerd-SG/hassio-diivoo2mqtt](https://github.com/Technerd-SG/hassio-diivoo2mqtt)

### [zgy0x01](https://github.com/zgy0x01)
- 🌟 👤 [zgy0x01](https://github.com/zgy0x01) Starred [eltavine/Duck-Detector-Refactoring](https://github.com/eltavine/Duck-Detector-Refactoring)
  > Android environment integrity inspection tool for root, hook, bootloader, SELinux, virtualization, a...

### [zhangyoufu](https://github.com/zhangyoufu)
- 🌟 👤 [zhangyoufu](https://github.com/zhangyoufu) Starred [Nishikinonakai/fix-ios-apps-not-launching-on-apple-silicon-mac](https://github.com/Nishikinonakai/fix-ios-apps-not-launching-on-apple-silicon-mac)
  > Fix: iOS apps won't launch / do nothing when clicked on Apple Silicon Macs (M1-M4) — EXEC_EXIT_REASO...

### [zhengjim](https://github.com/zhengjim)
- 🌟 👤 [zhengjim](https://github.com/zhengjim) Starred [shimmeris/SCFProxy](https://github.com/shimmeris/SCFProxy)
  > A proxy tool based on cloud function.

### [zhongyang219](https://github.com/zhongyang219)
- 🌟 👤 [zhongyang219](https://github.com/zhongyang219) Starred [rui314/chibicc](https://github.com/rui314/chibicc)
  > A small C compiler


---
*最后更新于 2026-07-31 17:36:20 UTC*
*历史记录保存在 `archive` 目录中。*
