# 每日 GitHub 动态 (2026-09-13)

我关注用户的今日公开动态 (每15分钟更新)。

## 今日动态

### [0xBakeer](https://github.com/0xBakeer)
- 🌟 👤 [0xBakeer](https://github.com/0xBakeer) Starred [alesha-pro/atlas](https://github.com/alesha-pro/atlas)
  > Interactive canvas for taking an LLM apart tensor by tensor: measured INT8/INT4/FP8 error, distribut...

### [1193776794](https://github.com/1193776794)
- 🌟 👤 [1193776794](https://github.com/1193776794) Starred [yizhiyonggangdexiaojia/xjvpn](https://github.com/yizhiyonggangdexiaojia/xjvpn)
  > Root Android transparent proxy client powered by Mihomo, with per-app routing and domain auditing.

### [2977094657](https://github.com/2977094657)
- 🌟 👤 [2977094657](https://github.com/2977094657) Starred [deeplethe/utopia](https://github.com/deeplethe/utopia)
  > World's first open-source enterprise world model.

### [34306](https://github.com/34306)
- 🌟 👤 [34306](https://github.com/34306) Starred [Xplo8E/Liter8](https://github.com/Xplo8E/Liter8)
  > experimental - ryt now wip, unstable

### [ABI-ZTROS](https://github.com/ABI-ZTROS)
- 🚀 👤 [ABI-ZTROS](https://github.com/ABI-ZTROS) Made [ABI-ZTROS/SceneTTC](https://github.com/ABI-ZTROS/SceneTTC) public
- 🚀 👤 [ABI-ZTROS](https://github.com/ABI-ZTROS) Made [ABI-ZTROS/SceneTweak](https://github.com/ABI-ZTROS/SceneTweak) public
  > Dynamic rule-based Xposed hook engine for SceneTTC

### [DarkLineX](https://github.com/DarkLineX)
- 🌟 👤 [DarkLineX](https://github.com/DarkLineX) Starred [MG1937/ASC](https://github.com/MG1937/ASC)
  > ASC is a super FAST Android decompiler front-end designed for Agents/Mobile Researchers.

### [DerekSelander](https://github.com/DerekSelander)
- 🌟 👤 [DerekSelander](https://github.com/DerekSelander) Starred [EthanArbuckle/ios1-restore-tool](https://github.com/EthanArbuckle/ios1-restore-tool)

### [DiamondHunters](https://github.com/DiamondHunters)
- 🌟 👤 [DiamondHunters](https://github.com/DiamondHunters) Starred [frida/frida-core](https://github.com/frida/frida-core)
  > Frida core library

### [Dr-TSNG](https://github.com/Dr-TSNG)
- 🚀 👤 [Dr-TSNG](https://github.com/Dr-TSNG) Made [Dr-TSNG/altdb](https://github.com/Dr-TSNG/altdb) public
  > A KernelSU module that provides alternative adb server over local network

### [Dump-GUY](https://github.com/Dump-GUY)
- 🌟 👤 [Dump-GUY](https://github.com/Dump-GUY) Starred [Stirling-Tools/Stirling-PDF](https://github.com/Stirling-Tools/Stirling-PDF)
  > #1 PDF Application on GitHub that lets you edit PDFs on any device anywhere

### [Ercilan](https://github.com/Ercilan)
- 🌟 👤 [Ercilan](https://github.com/Ercilan) Starred [rustdesk/rustdesk](https://github.com/rustdesk/rustdesk)
  > An open-source remote desktop application designed for self-hosting, as an alternative to TeamViewer...

### [Erich8200](https://github.com/Erich8200)
- 🌟 👤 [Erich8200](https://github.com/Erich8200) Starred [Sakura999999999/Practice_kern](https://github.com/Sakura999999999/Practice_kern)
  > Linux Kernel Exploitation Assignments
- 🍴 👤 [Erich8200](https://github.com/Erich8200) Forked [Sakura999999999/Practice_kern](https://github.com/Sakura999999999/Practice_kern) to [Erich8200/Practice_kern](https://github.com/Erich8200/Practice_kern)
  > Linux Kernel Exploitation Assignments

### [ErodedElk](https://github.com/ErodedElk)
- 🌟 👤 [ErodedElk](https://github.com/ErodedElk) Starred [MG1937/ASC](https://github.com/MG1937/ASC)
  > ASC is a super FAST Android decompiler front-end designed for Agents/Mobile Researchers.

### [Fansirsqi](https://github.com/Fansirsqi)
- 🌟 👤 [Fansirsqi](https://github.com/Fansirsqi) Starred [ssyclr/zygisk-mcp-il2cpp](https://github.com/ssyclr/zygisk-mcp-il2cpp)
  > 一个MCP服务

### [FiYHer](https://github.com/FiYHer)
- 🌟 👤 [FiYHer](https://github.com/FiYHer) Starred [avaneev/avir](https://github.com/avaneev/avir)
  > High-quality pro HDR image resizing / scaling C++ library, including a very fast, precise, SIMD Lanc...

### [Fplyth0ner-Combie](https://github.com/Fplyth0ner-Combie)
- 🌟 👤 [Fplyth0ner-Combie](https://github.com/Fplyth0ner-Combie) Starred [doki-byte/EasyShell](https://github.com/doki-byte/EasyShell)
  > EasyShell - A New C2 Framework

### [GangJust](https://github.com/GangJust)
- 🌟 👤 [GangJust](https://github.com/GangJust) Starred [samoylenkodmitry/Cranpose](https://github.com/samoylenkodmitry/Cranpose)
  > Cranpose is a Jetpack Compose-inspired declarative Rust UI framework. https://crates.io/crates/cranp...

### [GitSquared](https://github.com/GitSquared)
- 🌟 👤 [GitSquared](https://github.com/GitSquared) Starred [facebook/stylex](https://github.com/facebook/stylex)
  > StyleX is the styling system for ambitious user interfaces.

### [JingMatrix](https://github.com/JingMatrix)
- 🍴 👤 [JingMatrix](https://github.com/JingMatrix) Forked [worldcoin/walletkit](https://github.com/worldcoin/walletkit) to [JingMatrix/walletkit](https://github.com/JingMatrix/walletkit)
  > WalletKit enables wallets to use World ID.

### [KHwang9883](https://github.com/KHwang9883)
- 🌟 👤 [KHwang9883](https://github.com/KHwang9883) Starred [op7418/Humanizer-zh](https://github.com/op7418/Humanizer-zh)
  > Humanizer 的汉化版本，Claude Code Skills，旨在消除文本中 AI 生成的痕迹。

### [Konano](https://github.com/Konano)
- 🌟 👤 [Konano](https://github.com/Konano) Starred [justhil/evid-wall](https://github.com/justhil/evid-wall)
  > 灰测路由证据链墙与公开证据提交页

### [M0Rf30](https://github.com/M0Rf30)
- 🍴 👤 [M0Rf30](https://github.com/M0Rf30) Forked [markqvist/RNode_Firmware](https://github.com/markqvist/RNode_Firmware) to [M0Rf30/RNode_Firmware](https://github.com/M0Rf30/RNode_Firmware)
  > RNode is an open, free and flexible digital radio interface with many uses
- 🍴 👤 [M0Rf30](https://github.com/M0Rf30) Forked [markqvist/Reticulum](https://github.com/markqvist/Reticulum) to [M0Rf30/Reticulum](https://github.com/M0Rf30/Reticulum)
  > The cryptography-based networking stack for building unstoppable networks with LoRa, Packet Radio, W...
- 🌟 👤 [M0Rf30](https://github.com/M0Rf30) Starred [liberatedsystems/RNode_Firmware_CE](https://github.com/liberatedsystems/RNode_Firmware_CE)
  > RNode is an open, free and flexible digital radio interface with many uses

### [MikaCybertron](https://github.com/MikaCybertron)
- 🌟 👤 [MikaCybertron](https://github.com/MikaCybertron) Starred [josskixg/renjana](https://github.com/josskixg/renjana)
  > Android virtual container app — run multiple isolated instances of any app, each with its own storag...
- 🍴 👤 [MikaCybertron](https://github.com/MikaCybertron) Forked [josskixg/renjana](https://github.com/josskixg/renjana) to [MikaCybertron/renjana](https://github.com/MikaCybertron/renjana)
  > Android virtual container app — run multiple isolated instances of any app, each with its own storag...

### [Monster-GM](https://github.com/Monster-GM)
- 🌟 👤 [Monster-GM](https://github.com/Monster-GM) Starred [rooootdev/lara](https://github.com/rooootdev/lara)
  > iOS Toolbox using the DarkSword kexploit. iOS 17.0 - iOS 18.7.1 & iOS 26.0.x, excluding M5 and A19.
- 🌟 👤 [Monster-GM](https://github.com/Monster-GM) Starred [0xjohnnydev/lightsaber](https://github.com/0xjohnnydev/lightsaber)
  > iOS 18.4-18.6.2 userland exploit chain with JS injection into SpringBoard and other processes. Deriv...
- 🌟 👤 [Monster-GM](https://github.com/Monster-GM) Starred [kaffeindecaf/W0lfSword](https://github.com/kaffeindecaf/W0lfSword)
  > iOS kernel exploit toolkit | DarkSword R/W | SSV Bypass | XPF offset verification

### [MrWQ](https://github.com/MrWQ)
- 🌟 👤 [MrWQ](https://github.com/MrWQ) Starred [aldegad/sprite-gen](https://github.com/aldegad/sprite-gen)
  > Generate clean 2D game sprites & animation atlases — component-row pipeline: state rows, alpha clean...

### [Seyud](https://github.com/Seyud)
- 🌟 👤 [Seyud](https://github.com/Seyud) Starred [google/artemis](https://github.com/google/artemis)
  > ARTEMIS turns natural-language instructions into reliable Android automation. It automates end-to-en...
- 🌟 👤 [Seyud](https://github.com/Seyud) Starred [Dr-TSNG/altdb](https://github.com/Dr-TSNG/altdb)
  > A KernelSU module that provides alternative adb server over local network

### [Silentsoul04](https://github.com/Silentsoul04)
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [BehiSecc/HuntProxy](https://github.com/BehiSecc/HuntProxy) to [Silentsoul04/HuntProxy](https://github.com/Silentsoul04/HuntProxy)
  > A web security workbench built for AI agents.
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [BehiSecc/HuntProxy](https://github.com/BehiSecc/HuntProxy)
  > A web security workbench built for AI agents.

### [Skorpion96](https://github.com/Skorpion96)
- 🌟 👤 [Skorpion96](https://github.com/Skorpion96) Starred [kitsumed/ShizuCallRecorder](https://github.com/kitsumed/ShizuCallRecorder)
  > ShizuCallRecorder empowers ADB through Shizuku to record phone calls on non-rooted device!

### [Sndav](https://github.com/Sndav)
- 🌟 👤 [Sndav](https://github.com/Sndav) Starred [3641397194-wq/gpt6-Astra](https://github.com/3641397194-wq/gpt6-Astra)
  > gpt6 Astra-claude-grok4.6-deepseekv4pro-glm5.3-gemini破甲越狱 · 冷咖啡 BREAK//OPEN 工作台

### [Syncabler](https://github.com/Syncabler)
- 🌟 👤 [Syncabler](https://github.com/Syncabler) Starred [gem5/gem5](https://github.com/gem5/gem5)
  > The official repository for the gem5 computer-system architecture simulator.
- 🍴 👤 [Syncabler](https://github.com/Syncabler) Forked [gem5/gem5](https://github.com/gem5/gem5) to [Syncabler/gem5](https://github.com/Syncabler/gem5)
  > The official repository for the gem5 computer-system architecture simulator.

### [T5ive](https://github.com/T5ive)
- 🌟 👤 [T5ive](https://github.com/T5ive) Starred [t8y2/dbx](https://github.com/t8y2/dbx)
  > 20 MB lightweight cross-platform database client for 90+ databases, including MySQL, PostgreSQL, SQL...

### [ThunderCls](https://github.com/ThunderCls)
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [codedogQBY/ReadAny](https://github.com/codedogQBY/ReadAny)
  > AI-powered cross-platform e-book reader with semantic search, RAG chat, local vector store, notes, T...
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [AdguardTeam/AdGuardHome](https://github.com/AdguardTeam/AdGuardHome)
  > Network-wide ads & trackers blocking DNS server
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [0xERR0R/blocky](https://github.com/0xERR0R/blocky)
  > Fast and lightweight DNS proxy as ad-blocker for local network with many features
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [TechnitiumSoftware/DnsServer](https://github.com/TechnitiumSoftware/DnsServer)
  > Technitium DNS Server
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [stumpapp/stump](https://github.com/stumpapp/stump)
  > A free and open source comics, manga and digital book server with OPDS support
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [NVIDIA-RTX/Streamline](https://github.com/NVIDIA-RTX/Streamline)
  > Streamline Integration Framework
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [sdli1995/dlssg_for_sm86](https://github.com/sdli1995/dlssg_for_sm86)
  > Here is a dlssg for RTX30 Series GPU 

### [Tokeii0](https://github.com/Tokeii0)
- 🌟 👤 [Tokeii0](https://github.com/Tokeii0) Starred [Tcotl/AgentCapture](https://github.com/Tcotl/AgentCapture)
  > 针对 AI 自动化渗透 Agent 的新一代反制蜜罐，通过反向代理将API密饵载入真实业务、反向提示词注入等方式反制自动化渗透 Agent，实现多款主流通用Agent的反制上线控制。

### [VonChange](https://github.com/VonChange)
- 🌟 👤 [VonChange](https://github.com/VonChange) Starred [tronikos/androidtvremote2](https://github.com/tronikos/androidtvremote2)
  > A Python library implementing the Android TV Remote protocol v2
- 🌟 👤 [VonChange](https://github.com/VonChange) Starred [LadybirdBrowser/ladybird](https://github.com/LadybirdBrowser/ladybird)
  > Truly independent web browser

### [WAY29](https://github.com/WAY29)
- 🌟 👤 [WAY29](https://github.com/WAY29) Starred [Novals83/5min-btc-polymarket](https://github.com/Novals83/5min-btc-polymarket)
  > OpenClaw skill for BTC 5-minute Polymarket momentum trading with configurable risk controls and opti...

### [XiaoliChan](https://github.com/XiaoliChan)
- 🌟 👤 [XiaoliChan](https://github.com/XiaoliChan) Starred [Idov31/Silverseal](https://github.com/Idov31/Silverseal)
  > Silverseal is a Linux framework containing a bootkit, rootkit loader and a rootkit

### [Xieansecn](https://github.com/Xieansecn)
- 🌟 👤 [Xieansecn](https://github.com/Xieansecn) Starred [win32ss/supermium](https://github.com/win32ss/supermium)
  > Chromium fork for Windows XP/2003 and up

### [YinMo19](https://github.com/YinMo19)
- 🌟 👤 [YinMo19](https://github.com/YinMo19) Starred [directwire/Daub](https://github.com/directwire/Daub)
  > Headless calibrated-brush renderer: strokes JSON to PNG + layered .kra + layered .psd + reveal video...

### [Youlor](https://github.com/Youlor)
- 🌟 👤 [Youlor](https://github.com/Youlor) Starred [rootSunc/CNEquity](https://github.com/rootSunc/CNEquity)
  > 中国股票数据基础设施，覆盖行情、研报、资金面、筹码、公告、龙虎榜、ETF/期权、舆情互动、基本面、行业板块、宏观与风险事件等核心数据。42+A股数据集，日更、自托管、MCP 原生，零注册、零 API ...

### [alex193a](https://github.com/alex193a)
- 🌟 👤 [alex193a](https://github.com/alex193a) Starred [Dr-TSNG/altdb](https://github.com/Dr-TSNG/altdb)
  > A KernelSU module that provides alternative adb server over local network

### [anantshri](https://github.com/anantshri)
- 🍴 👤 [anantshri](https://github.com/anantshri) Forked [diegosouzapw/OmniRoute](https://github.com/diegosouzapw/OmniRoute) to [anantshri/OmniRoute](https://github.com/anantshri/OmniRoute)
  > Never stop coding. Free MIT AI gateway: one endpoint, 352 providers (150+ free), 1200+ models Kimi, ...

### [aviraxp](https://github.com/aviraxp)
- 🌟 👤 [aviraxp](https://github.com/aviraxp) Starred [yume-chan/ya-webadb](https://github.com/yume-chan/ya-webadb)
  > ADB in your browser

### [byt3bl33d3r](https://github.com/byt3bl33d3r)
- 🌟 👤 [byt3bl33d3r](https://github.com/byt3bl33d3r) Starred [ZhengyiLuo/AgentsDock](https://github.com/ZhengyiLuo/AgentsDock)

### [cctv18](https://github.com/cctv18)
- 🌟 👤 [cctv18](https://github.com/cctv18) Starred [router-for-me/EasyCLIProxyAPI](https://github.com/router-for-me/EasyCLIProxyAPI)
  > A desktop GUI for CLIProxyAPI and a tool for automatically configuring popular AI agents.

### [cli0xfa](https://github.com/cli0xfa)
- 🌟 👤 [cli0xfa](https://github.com/cli0xfa) Starred [vladelaina/BongoCat](https://github.com/vladelaina/BongoCat)
  > 🩷 💘C × SDL3 × OpenGL — Three Mysterious Forces, United as One! Bong~ Bongocat!!!Don't be shy, Join H...

### [cozis](https://github.com/cozis)
- 🍴 👤 [cozis](https://github.com/cozis) Forked [GJDuck/e9patch](https://github.com/GJDuck/e9patch) to [cozis/e9patch](https://github.com/cozis/e9patch)
  > A powerful static binary rewriting tool
- 🌟 👤 [cozis](https://github.com/cozis) Starred [penberg/titania](https://github.com/penberg/titania)
  > Project Titania is a complete large language model system, from transformer to transistor, simple en...

### [ctkqiang](https://github.com/ctkqiang)
- 🌟 👤 [ctkqiang](https://github.com/ctkqiang) Starred [mqz0211/termux](https://github.com/mqz0211/termux)
  > Learn ethical hacking using Termux on Android. This guide covers 100+ essential Linux commands, tool...

### [darkskygit](https://github.com/darkskygit)
- 🌟 👤 [darkskygit](https://github.com/darkskygit) Starred [krkrsdl3/krkrsdl3](https://github.com/krkrsdl3/krkrsdl3)
  > 多平台支持的krkr视觉小说引擎。

### [dartcv](https://github.com/dartcv)
- 🌟 👤 [dartcv](https://github.com/dartcv) Starred [zhihu/Matisse](https://github.com/zhihu/Matisse)
  > :fireworks: A well-designed local image and video selector for Android

### [dwisiswant0](https://github.com/dwisiswant0)
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [LMMS/tap-plugins](https://github.com/LMMS/tap-plugins)
  > Manually updated mirror of https://git.hq.sig7.se/tap-plugins.git
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [unicorn-engine/unicorn](https://github.com/unicorn-engine/unicorn)
  > Unicorn CPU emulator framework (ARM, AArch64, M68K, Mips, Sparc, PowerPC, RiscV, S390x, TriCore, X86...
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [yifanzhang-pro/recurrent-looped-tranformer](https://github.com/yifanzhang-pro/recurrent-looped-tranformer)
  > Official Project Page for Recurrent Looped Transformer (RLT)
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [pizlonator/yosh](https://github.com/pizlonator/yosh)
  > Yo shell - a LLM enabled fork of bash
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [Vercantez/openuikit](https://github.com/Vercantez/openuikit)
  > iOS apps on Linux: OpenUIKit + SwiftUI + Foundation + machorun Mach-O loader — full monorepo

### [dzxpert](https://github.com/dzxpert)
- 🍴 👤 [dzxpert](https://github.com/dzxpert) Forked [PentestPad/larascan](https://github.com/PentestPad/larascan) to [dzxpert/larascan](https://github.com/dzxpert/larascan)
  > Laravel PHP framework black-box vulnerability scanner
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [ma1co/fwtool.py](https://github.com/ma1co/fwtool.py)
  > A tool to unpack Sony camera firmware images
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [ma1co/Sony-PMCA-RE](https://github.com/ma1co/Sony-PMCA-RE)
  > Reverse Engineering Sony Digital Cameras

### [easychen](https://github.com/easychen)
- 🌟 👤 [easychen](https://github.com/easychen) Starred [basketikun/infinite-canvas](https://github.com/basketikun/infinite-canvas)
  > 面向 AI 创作的开源无限画布工作台，集成 AI 生图、参考图编辑、视频生成、Agent 智能助手、画布编排、对话创作、提示词库与素材管理等能力，支持可视化创作流程与多 Agent 协同工作。兼容 O...

### [enginelesscc](https://github.com/enginelesscc)
- 🍴 👤 [enginelesscc](https://github.com/enginelesscc) Forked [ramensoftware/windhawk-mods](https://github.com/ramensoftware/windhawk-mods) to [enginelesscc/windhawk-mods](https://github.com/enginelesscc/windhawk-mods)
  > The official collection of Windhawk mods

### [eteran](https://github.com/eteran)
- 🌟 👤 [eteran](https://github.com/eteran) Starred [markel1974/gowolfenstein](https://github.com/markel1974/gowolfenstein)
  > a go wolfenstein rich clone

### [fatalSec](https://github.com/fatalSec)
- 🍴 👤 [fatalSec](https://github.com/fatalSec) Forked [radareorg/r2flutter](https://github.com/radareorg/r2flutter) to [fatalSec/r2flutter](https://github.com/fatalSec/r2flutter)
  > Dart/Flutter support for radare2

### [feicong](https://github.com/feicong)
- 🌟 👤 [feicong](https://github.com/feicong) Starred [jiji262/douyin-downloader](https://github.com/jiji262/douyin-downloader)
  > A practical Douyin downloader for both single-item and profile batch downloads, with progress displa...

### [fengjixuchui](https://github.com/fengjixuchui)
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [Idov31/Silverseal](https://github.com/Idov31/Silverseal) to [fengjixuchui/Silverseal](https://github.com/fengjixuchui/Silverseal)
  > Silverseal is a Linux framework containing a bootkit, rootkit loader and a rootkit
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [winterknife/EVENSTAR](https://github.com/winterknife/EVENSTAR) to [fengjixuchui/EVENSTAR](https://github.com/fengjixuchui/EVENSTAR)
  > Intel 64/Windows low-level experiments

### [fnmsd](https://github.com/fnmsd)
- 🌟 👤 [fnmsd](https://github.com/fnmsd) Starred [Zeejay0/gathered-scenes-zine-skill](https://github.com/Zeejay0/gathered-scenes-zine-skill)

### [foospidy](https://github.com/foospidy)
- 🌟 👤 [foospidy](https://github.com/foospidy) Starred [Ar9av/obsidian-wiki](https://github.com/Ar9av/obsidian-wiki)
  > Framework for AI agents to build and maintain a digital brain through Obsidian wiki

### [gal2xy](https://github.com/gal2xy)
- 🌟 👤 [gal2xy](https://github.com/gal2xy) Starred [tcc0lin/awsome-magisk](https://github.com/tcc0lin/awsome-magisk)
  > 关于Magisk生态的研究

### [hackcatml](https://github.com/hackcatml)
- 🌟 👤 [hackcatml](https://github.com/hackcatml) Starred [slee-persis/GVS5H](https://github.com/slee-persis/GVS5H)
  > GVS5H: Five Qwen3.8-27B Models Match Claude Fable 5 on LiveCodeBench Hard  |  Fable 5 Level Coding f...

### [hacksysteam](https://github.com/hacksysteam)
- 🌟 👤 [hacksysteam](https://github.com/hacksysteam) Starred [Runfusion/Fusion](https://github.com/Runfusion/Fusion)
  > Your Software Factory - build faster and better with multi node agents that work 24/7
- 🌟 👤 [hacksysteam](https://github.com/hacksysteam) Starred [nashsu/llm_wiki](https://github.com/nashsu/llm_wiki)
  > LLM Wiki is a cross-platform desktop application that turns your documents into an organized, interl...
- 🌟 👤 [hacksysteam](https://github.com/hacksysteam) Starred [microsoft/AI-Engineering-Coach](https://github.com/microsoft/AI-Engineering-Coach)
  > better agentic engineering

### [hyuunnn](https://github.com/hyuunnn)
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [bojieli/ai-infra-book](https://github.com/bojieli/ai-infra-book)
  > 《深入理解 AI Infra：量化分析与系统设计》（李博杰 著）开源书稿：从硬件约束和模型架构出发，量化推导 LLM 推理与训练系统设计。含全书正文、PDF、配套计算工具与实验

### [hzqst](https://github.com/hzqst)
- 🍴 👤 [hzqst](https://github.com/hzqst) Forked [getpaseo/paseo](https://github.com/getpaseo/paseo) to [hzqst/paseo](https://github.com/hzqst/paseo)
  > Orchestrate multiple coding agents from desktop and mobile

### [imj01y](https://github.com/imj01y)
- 🌟 👤 [imj01y](https://github.com/imj01y) Starred [tisfeng/Easydict](https://github.com/tisfeng/Easydict)
  > 一个简洁优雅的词典翻译 macOS App。开箱即用，支持离线 OCR 识别，支持有道词典，🍎 苹果系统词典，🍎 苹果系统翻译，OpenAI，Gemini，DeepL，Google，Bing，腾讯，百...

### [interference-security](https://github.com/interference-security)
- 🌟 👤 [interference-security](https://github.com/interference-security) Starred [zh54321/SharePointDumper](https://github.com/zh54321/SharePointDumper)
  > PowerShell SharePoint extraction + auditing tool for red/blue/purple teams. Enumerates all SharePoin...

### [j0nk0](https://github.com/j0nk0)
- 🌟 👤 [j0nk0](https://github.com/j0nk0) Starred [EvoMap/AutoResearch](https://github.com/EvoMap/AutoResearch)
  > AI/ML research agents from idea to paper-ready evidence. An EvoMap open-source project.
- 🌟 👤 [j0nk0](https://github.com/j0nk0) Starred [openai/symphony](https://github.com/openai/symphony)
  > Symphony turns project work into isolated, autonomous implementation runs, allowing teams to manage ...
- 🌟 👤 [j0nk0](https://github.com/j0nk0) Starred [gepa-ai/gepa](https://github.com/gepa-ai/gepa)
  > Optimize prompts, code, and more with AI-powered Reflective Optimization
- 🌟 👤 [j0nk0](https://github.com/j0nk0) Starred [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness)
  > DeepSeek Harness: Everything is a Plugin.
- 🌟 👤 [j0nk0](https://github.com/j0nk0) Starred [debpalash/VoiceStudio](https://github.com/debpalash/VoiceStudio)
  > VoiceStudio is the open-source, fully-local ElevenLabs alternative — voice cloning, voice design, vi...

### [jpacg](https://github.com/jpacg)
- 🌟 👤 [jpacg](https://github.com/jpacg) Starred [yizhiyonggangdexiaojia/xjvpn](https://github.com/yizhiyonggangdexiaojia/xjvpn)
  > Root Android transparent proxy client powered by Mihomo, with per-app routing and domain auditing.

### [k0mraid3](https://github.com/k0mraid3)
- 🌟 👤 [k0mraid3](https://github.com/k0mraid3) Starred [nuloperrito/KDZTool](https://github.com/nuloperrito/KDZTool)
  > Extract the contents of LG's KDZ firmware and repack them into a new KDZ firmware in a native single...
- 🌟 👤 [k0mraid3](https://github.com/k0mraid3) Starred [openlgtv/epk2extract](https://github.com/openlgtv/epk2extract)
  > Extraction tool for LG, Hisense, Sharp, Philips/TPV, Thompson and similar TVs/Embedded Devices

### [kaganisildak](https://github.com/kaganisildak)
- 🌟 👤 [kaganisildak](https://github.com/kaganisildak) Starred [ergrelet/unlicense](https://github.com/ergrelet/unlicense)
  > Dynamic unpacker and import fixer for Themida/WinLicense 2.x and 3.x.

### [kmiit](https://github.com/kmiit)
- 🍴 👤 [kmiit](https://github.com/kmiit) Forked [go-gitea/gitea](https://github.com/go-gitea/gitea) to [kmiit/gitea](https://github.com/kmiit/gitea)
  > Git with a cup of tea! Painless self-hosted all-in-one software development service, including Git h...

### [kujian](https://github.com/kujian)
- 🌟 👤 [kujian](https://github.com/kujian) Starred [mattpocock/skills](https://github.com/mattpocock/skills)
  > Skills for Real Engineers. Straight from my .agents directory.

### [lanyi233](https://github.com/lanyi233)
- 🌟 👤 [lanyi233](https://github.com/lanyi233) Starred [Jursin/ilife798](https://github.com/Jursin/ilife798)
  > 慧生活798第三方客户端

### [leleliu008](https://github.com/leleliu008)
- 🌟 👤 [leleliu008](https://github.com/leleliu008) Starred [JustVugg/colibri](https://github.com/JustVugg/colibri)
  > Run frontier MoE models on hardware you already own — pure C, zero deps, experts streamed from disk....

### [liuzjMr](https://github.com/liuzjMr)
- 🍴 👤 [liuzjMr](https://github.com/liuzjMr) Forked [yi1108/printfilm](https://github.com/yi1108/printfilm) to [liuzjMr/printfilm](https://github.com/liuzjMr/printfilm)
  > PRINTFILM：AI 科普视频与漫剧创作平台

### [m4ll0k](https://github.com/m4ll0k)
- 🍴 👤 [m4ll0k](https://github.com/m4ll0k) Forked [Mathieu2301/TradingView-API](https://github.com/Mathieu2301/TradingView-API) to [m4ll0k/TradingView-API](https://github.com/m4ll0k/TradingView-API)
  > 📈 Get real-time stocks from TradingView

### [mak7im01](https://github.com/mak7im01)
- 🌟 👤 [mak7im01](https://github.com/mak7im01) Starred [savo-o/resona](https://github.com/savo-o/resona)
  > an open source soundcloud client for android. 

### [mgeeky](https://github.com/mgeeky)
- 🌟 👤 [mgeeky](https://github.com/mgeeky) Starred [malmoeb/presentations](https://github.com/malmoeb/presentations)
  > Presentations from yours truely

### [mingzun09](https://github.com/mingzun09)
- 🍴 👤 [mingzun09](https://github.com/mingzun09) Forked [serein-213/android_gki_kernel_5.15_common](https://github.com/serein-213/android_gki_kernel_5.15_common) to [mingzun09/android_gki_kernel_5.15_common](https://github.com/mingzun09/android_gki_kernel_5.15_common)
  > android13-5.15 通用内核，已测试于 Xiaomi 13最新系统

### [muellan](https://github.com/muellan)
- 🍴 👤 [muellan](https://github.com/muellan) Forked [ashuttl/omarchy-linecast](https://github.com/ashuttl/omarchy-linecast) to [muellan/omarchy-linecast](https://github.com/muellan/omarchy-linecast)
  > Omarchy bar-widget plugin for linecast — weather, sun, moon, and tide panels

### [nek0y4nsu](https://github.com/nek0y4nsu)
- 🌟 👤 [nek0y4nsu](https://github.com/nek0y4nsu) Starred [tiwe0/r0dump](https://github.com/tiwe0/r0dump)
- 🍴 👤 [nek0y4nsu](https://github.com/nek0y4nsu) Forked [tiwe0/r0dump](https://github.com/tiwe0/r0dump) to [nek0y4nsu/r0dump](https://github.com/nek0y4nsu/r0dump)

### [null-dev](https://github.com/null-dev)
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [tmo1/sms-ie](https://github.com/tmo1/sms-ie)
  > SMS Import / Export is a simple Android app that imports and exports SMS and MMS messages, call logs...
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [Grumbel/evtest-qt](https://github.com/Grumbel/evtest-qt)
  > Linux Joystick Tester for Qt
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [polychromatic/polychromatic](https://github.com/polychromatic/polychromatic)
  > RGB lighting management software for GNU/Linux. Powered by OpenRazer.
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [sezanzeb/input-remapper](https://github.com/sezanzeb/input-remapper)
  > 🎮 ⌨ An easy to use tool to change the behaviour of your input devices.
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [NVIDIA/nvidia-container-toolkit](https://github.com/NVIDIA/nvidia-container-toolkit)
  > Build and run containers leveraging NVIDIA GPUs
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [containerd/nerdctl](https://github.com/containerd/nerdctl)
  > contaiNERD CTL - Docker-compatible CLI for containerd, with support for Compose, Rootless, eStargz, ...
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [NVIDIA/gpu-driver-container](https://github.com/NVIDIA/gpu-driver-container)
  > The NVIDIA GPU driver container allows the provisioning of the NVIDIA driver through the use of cont...

### [patois](https://github.com/patois)
- 🌟 👤 [patois](https://github.com/patois) Starred [systemfreund/supersonic.el](https://github.com/systemfreund/supersonic.el)
  > Music player package for Emacs compatible with servers implementing the Subsonic API, such as Navidr...

### [qazbnm456](https://github.com/qazbnm456)
- 🌟 👤 [qazbnm456](https://github.com/qazbnm456) Starred [kasif-dekel/maplestory-jump-quest-fuzzing](https://github.com/kasif-dekel/maplestory-jump-quest-fuzzing)

### [rmuxnet](https://github.com/rmuxnet)
- 🍴 👤 [rmuxnet](https://github.com/rmuxnet) Forked [torvalds/linux](https://github.com/torvalds/linux) to [rmuxnet/linux-7.xx](https://github.com/rmuxnet/linux-7.xx)
  > Linux kernel source tree

### [saileaxh](https://github.com/saileaxh)
- 🌟 👤 [saileaxh](https://github.com/saileaxh) Starred [huoji120/code_review_agent](https://github.com/huoji120/code_review_agent)
  > 简洁高效的代码审计agent

### [shmilylty](https://github.com/shmilylty)
- 🌟 👤 [shmilylty](https://github.com/shmilylty) Starred [Southseast/Zend_Decode](https://github.com/Southseast/Zend_Decode)
  > 一个缝合的Zend批量解密脚本。

### [sidex15](https://github.com/sidex15)
- 🌟 👤 [sidex15](https://github.com/sidex15) Starred [midori01/susfs4ksu](https://github.com/midori01/susfs4ksu)

### [sotasan](https://github.com/sotasan)
- 🌟 👤 [sotasan](https://github.com/sotasan) Starred [adefossez/demucs](https://github.com/adefossez/demucs)
  > Code for the paper Hybrid Spectrogram and Waveform Source Separation
- 🌟 👤 [sotasan](https://github.com/sotasan) Starred [spotify/basic-pitch](https://github.com/spotify/basic-pitch)
  > A lightweight yet powerful audio-to-MIDI converter with pitch bend detection
- 🌟 👤 [sotasan](https://github.com/sotasan) Starred [yt-dlp/yt-dlp](https://github.com/yt-dlp/yt-dlp)
  > A feature-rich command-line audio/video downloader

### [tfoldi](https://github.com/tfoldi)
- 🍴 👤 [tfoldi](https://github.com/tfoldi) Forked [dusty-nv/jetson-containers](https://github.com/dusty-nv/jetson-containers) to [tfoldi/jetson-containers](https://github.com/tfoldi/jetson-containers)
  > Machine Learning Containers for NVIDIA Jetson and JetPack-L4T

### [tiwe0](https://github.com/tiwe0)
- 🌟 👤 [tiwe0](https://github.com/tiwe0) Starred [boxproxy/box](https://github.com/boxproxy/box)
  > 安卓代理模块，使用方法看wiki

### [w00tzenheimer](https://github.com/w00tzenheimer)
- 🌟 👤 [w00tzenheimer](https://github.com/w00tzenheimer) Starred [s3team/uroboros](https://github.com/s3team/uroboros)
  > Infrastructure for Reassembleable Disassembling and Transformation
- 🌟 👤 [w00tzenheimer](https://github.com/w00tzenheimer) Starred [strongcourage/uafuzz](https://github.com/strongcourage/uafuzz)
  > UAFuzz: Binary-level Directed Fuzzing for Use-After-Free Vulnerabilities

### [wangshub](https://github.com/wangshub)
- 🌟 👤 [wangshub](https://github.com/wangshub) Starred [bones-studio/seed-viewer](https://github.com/bones-studio/seed-viewer)
  > A web application for browsing and viewing the BONES-SEED motion capture dataset

### [wbaby](https://github.com/wbaby)
- 🍴 👤 [wbaby](https://github.com/wbaby) Forked [z-mio/parse_hub_bot](https://github.com/z-mio/parse_hub_bot) to [wbaby/parse_hub_bot](https://github.com/wbaby/parse_hub_bot)
  > Telegram 多平台聚合解析机器人

### [wesmar](https://github.com/wesmar)
- 🍴 👤 [wesmar](https://github.com/wesmar) Forked [namjaejeon/linux-ntfs](https://github.com/namjaejeon/linux-ntfs) to [wesmar/linux-ntfs](https://github.com/wesmar/linux-ntfs)
  > NTFS(NTFS PLUS) for Linux (Backport for low kernel version support)

### [wy876](https://github.com/wy876)
- 🌟 👤 [wy876](https://github.com/wy876) Starred [mibgb65-cloud/OmniMail](https://github.com/mibgb65-cloud/OmniMail)
  > Self-hosted multi-domain webmail on Cloudflare Workers, D1, R2, Queues, Email Routing, and Resend.

### [x-spy](https://github.com/x-spy)
- 🍴 👤 [x-spy](https://github.com/x-spy) Forked [obsproject/obs-studio](https://github.com/obsproject/obs-studio) to [x-spy/obs-studio](https://github.com/x-spy/obs-studio)
  > OBS Studio - Free and open source software for live streaming and screen recording

### [x7dbg](https://github.com/x7dbg)
- 🌟 👤 [x7dbg](https://github.com/x7dbg) Starred [Lordog/dive-into-llms](https://github.com/Lordog/dive-into-llms)
  > 《动手学大模型Dive into LLMs》系列编程实践教程

### [yanyaoli](https://github.com/yanyaoli)
- 🌟 👤 [yanyaoli](https://github.com/yanyaoli) Starred [melgarafael/DeskcommCRM](https://github.com/melgarafael/DeskcommCRM)
  > Open-source AI sales OS — self-hosted CRM with native AI agents + WhatsApp (WAHA). Open alternative ...
- 🌟 👤 [yanyaoli](https://github.com/yanyaoli) Starred [ayghri/i-have-adhd](https://github.com/ayghri/i-have-adhd)
  > A skill to stop your coding agent from burying the answer. ADHD-friendly output.
- 🌟 👤 [yanyaoli](https://github.com/yanyaoli) Starred [coreyhaines31/marketingskills](https://github.com/coreyhaines31/marketingskills)
  > Marketing skills for Claude Code and AI agents. CRO, copywriting, SEO, analytics, and growth enginee...

### [yinsel](https://github.com/yinsel)
- 🍴 👤 [yinsel](https://github.com/yinsel) Forked [JetBrains/fernflower](https://github.com/JetBrains/fernflower) to [yinsel/fernflower](https://github.com/yinsel/fernflower)
  > Decompiler from Java bytecode to Java, used in IntelliJ IDEA.
- 🌟 👤 [yinsel](https://github.com/yinsel) Starred [seakee/CPA-Manager-Plus](https://github.com/seakee/CPA-Manager-Plus)
  > A self-hosted CPA / CLIProxyAPI management panel and AI gateway observability dashboard for requests...
- 🍴 👤 [yinsel](https://github.com/yinsel) Forked [Le1a/JarLibsConsolidator](https://github.com/Le1a/JarLibsConsolidator) to [yinsel/JarLibsConsolidator](https://github.com/yinsel/JarLibsConsolidator)
  > 一个IDEA插件：一键收集项目中所有jar包依赖的工具插件。遍历项目目录收集所有jar文件，复制到all-in-one文件夹，并自动添加为项目库。

### [ys1231](https://github.com/ys1231)
- 🌟 👤 [ys1231](https://github.com/ys1231) Starred [Asterisk4Magisk/bpf2socks](https://github.com/Asterisk4Magisk/bpf2socks)

### [yzddmr6](https://github.com/yzddmr6)
- 🍴 👤 [yzddmr6](https://github.com/yzddmr6) Forked [sm8450-mainline/linux](https://github.com/sm8450-mainline/linux) to [yzddmr6/linux-sm8450-liuqin](https://github.com/yzddmr6/linux-sm8450-liuqin)
  > Linux kernel source tree

### [zensu357](https://github.com/zensu357)
- 🌟 👤 [zensu357](https://github.com/zensu357) Starred [Dr-TSNG/altdb](https://github.com/Dr-TSNG/altdb)
  > A KernelSU module that provides alternative adb server over local network

### [zzyccs](https://github.com/zzyccs)
- 🌟 👤 [zzyccs](https://github.com/zzyccs) Starred [DeusData/codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp)
  > High-performance code intelligence MCP server. Indexes codebases into a persistent knowledge graph —...


---
*最后更新于 2026-09-13 17:55:36 UTC*
*历史记录保存在 `archive` 目录中。*
