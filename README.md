# 每日 GitHub 动态 (2026-07-03)

我关注用户的今日公开动态 (每15分钟更新)。

## 今日动态

### [0x1306a94](https://github.com/0x1306a94)
- 🌟 👤 [0x1306a94](https://github.com/0x1306a94) Starred [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki)
  > OpenWiki is a CLI that writes and maintains agent documentation for your codebase.
- 🌟 👤 [0x1306a94](https://github.com/0x1306a94) Starred [DnV1eX/LiquidGlassKit](https://github.com/DnV1eX/LiquidGlassKit)
  > Backport of Apple’s Liquid Glass system for iOS 13–18 with native API parity. Includes a reimplement...

### [0xbinder](https://github.com/0xbinder)
- 🌟 👤 [0xbinder](https://github.com/0xbinder) Starred [DreadedLama/Simple-Dialer](https://github.com/DreadedLama/Simple-Dialer)
  > A handy phone call manager with phonebook, number blocking and multi-SIM support that uses truecalle...

### [0xdad0](https://github.com/0xdad0)
- 🌟 👤 [0xdad0](https://github.com/0xdad0) Starred [lautarovculic/ioscpy](https://github.com/lautarovculic/ioscpy)
  > A CLI that mirrors and controls a jailbroken iPhone over USB.
- 🌟 👤 [0xdad0](https://github.com/0xdad0) Starred [Younesfdj/gitfut](https://github.com/Younesfdj/gitfut)
  > Your GitHub stats, turned into a World-Cup-style player card 

### [1193776794](https://github.com/1193776794)
- 🌟 👤 [1193776794](https://github.com/1193776794) Starred [kenzok8/small](https://github.com/kenzok8/small)
  >  homeprxoy mihomo sing-box dae passwall
- 🌟 👤 [1193776794](https://github.com/1193776794) Starred [lidongyooo/GumTrace](https://github.com/lidongyooo/GumTrace)
  > 基于 Frida Gum (Stalker) 引擎的 ARM64 动态指令追踪工具，支持 Android 和 iOS 平台。

### [16hex-Dumb](https://github.com/16hex-Dumb)
- 🌟 👤 [16hex-Dumb](https://github.com/16hex-Dumb) Starred [iternal-technologies-partners/blockify-agentic-data-optimization](https://github.com/iternal-technologies-partners/blockify-agentic-data-optimization)
  > Blockify: Agentic data optimization for enterprise RAG and agentic search. Blockify replaces naive c...

### [20000s](https://github.com/20000s)
- 🌟 👤 [20000s](https://github.com/20000s) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 
- 🌟 👤 [20000s](https://github.com/20000s) Starred [canyie/TransitionPlayer](https://github.com/canyie/TransitionPlayer)
  > CVE-2026-0091, play with an issue in android window management to perform arbitrary code execution i...

### [Aar0n3906](https://github.com/Aar0n3906)
- 🌟 👤 [Aar0n3906](https://github.com/Aar0n3906) Starred [gojue/ecapture](https://github.com/gojue/ecapture)
  > Capturing SSL/TLS plaintext without a CA certificate using eBPF. Supported on Linux/Android kernels ...

### [AlexiaChen](https://github.com/AlexiaChen)
- 🌟 👤 [AlexiaChen](https://github.com/AlexiaChen) Starred [rapidfuzz/RapidFuzz](https://github.com/rapidfuzz/RapidFuzz)
  > Rapid fuzzy string matching in Python using various string metrics
- 🌟 👤 [AlexiaChen](https://github.com/AlexiaChen) Starred [rapidfuzz/rapidfuzz-cpp](https://github.com/rapidfuzz/rapidfuzz-cpp)
  > Rapid fuzzy string matching in C++ using the Levenshtein Distance
- 🌟 👤 [AlexiaChen](https://github.com/AlexiaChen) Starred [yuki-koyama/tinycolormap](https://github.com/yuki-koyama/tinycolormap)
  > A header-only, single-file library for colormaps written in C++11

### [AndnixSH](https://github.com/AndnixSH)
- 🌟 👤 [AndnixSH](https://github.com/AndnixSH) Starred [ispointer/RePairip](https://github.com/ispointer/RePairip)

### [Ares-X](https://github.com/Ares-X)
- 🌟 👤 [Ares-X](https://github.com/Ares-X) Starred [Yu9191/wloc](https://github.com/Yu9191/wloc)
  > 修改 Apple 网络定位（gs-loc）返回坐标 · 支持 Surge / Quantumult X / Loon / Stash · 快捷指令一键设置/恢复定位

### [BeneficialCode](https://github.com/BeneficialCode)
- 🌟 👤 [BeneficialCode](https://github.com/BeneficialCode) Starred [mguentner/cannelloni](https://github.com/mguentner/cannelloni)
  > a SocketCAN over Ethernet tunnel

### [BlackHole1](https://github.com/BlackHole1)
- 🌟 👤 [BlackHole1](https://github.com/BlackHole1) Starred [oomol-lab/open-connector](https://github.com/oomol-lab/open-connector)
  > Open-source auth gateway connecting 1000+ SaaS providers to AI agents through SDK, CLI, MCP, HTTP, a...

### [CHYbeta](https://github.com/CHYbeta)
- 🌟 👤 [CHYbeta](https://github.com/CHYbeta) Starred [kyrolabs/awesome-agents](https://github.com/kyrolabs/awesome-agents)
  > 🤖 Awesome list of AI Agents
- 🌟 👤 [CHYbeta](https://github.com/CHYbeta) Starred [Unclecheng-li/VulnClaw](https://github.com/Unclecheng-li/VulnClaw)
  > 基于 AI Agent + MCP 工具链 + 渗透 Skill 编排， 配合大语言模型， 自然语言输入 → 自动完成「信息收集 → 漏洞发现 → 漏洞利用 → 报告生成」全流程。
- 🌟 👤 [CHYbeta](https://github.com/CHYbeta) Starred [ZephrFish/harness-kit](https://github.com/ZephrFish/harness-kit)
  > A template for building your own AI/LLM harness

### [Ch0pin](https://github.com/Ch0pin)
- 🌟 👤 [Ch0pin](https://github.com/Ch0pin) Starred [lautarovculic/ioscpy](https://github.com/lautarovculic/ioscpy)
  > A CLI that mirrors and controls a jailbroken iPhone over USB.

### [Cracked5pider](https://github.com/Cracked5pider)
- 🌟 👤 [Cracked5pider](https://github.com/Cracked5pider) Starred [msuiche/nanokrnl](https://github.com/msuiche/nanokrnl)
  > The end of an era
- 🌟 👤 [Cracked5pider](https://github.com/Cracked5pider) Starred [dearchap/tinyemu](https://github.com/dearchap/tinyemu)
  > TinyEMU is a system emulator for the RISC-V and x86 architectures. Its purpose is to be small and si...
- 🌟 👤 [Cracked5pider](https://github.com/Cracked5pider) Starred [scumjr/the-sea-watcher](https://github.com/scumjr/the-sea-watcher)
  > Implementation of the SMM rootkit "The Watcher"

### [CyberCraftsmanx](https://github.com/CyberCraftsmanx)
- 🌟 👤 [CyberCraftsmanx](https://github.com/CyberCraftsmanx) Starred [cocomelonc/tiny-shamir](https://github.com/cocomelonc/tiny-shamir)
  > A small, self-contained C implementation of Shamir's Secret Sharing (Adi Shamir, 1979) over GF(2^8)

### [DarkLineX](https://github.com/DarkLineX)
- 🌟 👤 [DarkLineX](https://github.com/DarkLineX) Starred [xiaokanghub/ptehooker](https://github.com/xiaokanghub/ptehooker)
- 🌟 👤 [DarkLineX](https://github.com/DarkLineX) Starred [okhsunrog/vpnhide](https://github.com/okhsunrog/vpnhide)
  > Hide active VPN from selected Android apps (kernel module + LSPosed + Zygisk)

### [Dax89](https://github.com/Dax89)
- 🌟 👤 [Dax89](https://github.com/Dax89) Starred [akustikrausch/yamaha-smaf-player](https://github.com/akustikrausch/yamaha-smaf-player)
  > A dependency-free C++ PLAYER for Yamaha SMAF (.mmf) ringtones: the polyphonic-ringtone format of the...

### [DeEpinGh0st](https://github.com/DeEpinGh0st)
- 🌟 👤 [DeEpinGh0st](https://github.com/DeEpinGh0st) Starred [flankerhqd/jebmcp](https://github.com/flankerhqd/jebmcp)
- 🌟 👤 [DeEpinGh0st](https://github.com/DeEpinGh0st) Starred [0xShe/RTP](https://github.com/0xShe/RTP)
- 🌟 👤 [DeEpinGh0st](https://github.com/DeEpinGh0st) Starred [ekkoo-z/KubeTrail](https://github.com/ekkoo-z/KubeTrail)
  > KubeTrail（云迹） 是一个面向 Kubernetes 授权红队评估与防御验证的容器内态势感知、攻击面发现和 AI 辅助攻防编排工具

### [DerekSelander](https://github.com/DerekSelander)
- 🌟 👤 [DerekSelander](https://github.com/DerekSelander) Starred [Orangera1n/spironolactone](https://github.com/Orangera1n/spironolactone)
  > Something, mayhaps?

### [DoranekoSystems](https://github.com/DoranekoSystems)
- 🌟 👤 [DoranekoSystems](https://github.com/DoranekoSystems) Starred [team-mirai/vibe-coding-workshop](https://github.com/team-mirai/vibe-coding-workshop)
  > 国会議員・議員事務所向け「AIの使い方（バイブコーディング）勉強会」の資料（CC BY 4.0）

### [EgeBalci](https://github.com/EgeBalci)
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [gold-silver-copper/koh](https://github.com/gold-silver-copper/koh)
  > koh — peer-to-peer remote shell, inspired by mosh

### [FBLeee](https://github.com/FBLeee)
- 🌟 👤 [FBLeee](https://github.com/FBLeee) Starred [OLmatter/glm-coding-helper](https://github.com/OLmatter/glm-coding-helper)
  > 智谱 GLM Coding Plan 抢购助手，一键抢购油猴脚本 ，本地 CPU/GPU OCR 自动识别   中文点选验证码，支持多窗口并发、限流重试和支付页保护

### [FldBudin](https://github.com/FldBudin)
- 🌟 👤 [FldBudin](https://github.com/FldBudin) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 
- 🌟 👤 [FldBudin](https://github.com/FldBudin) Starred [maka-agent/maka-agent](https://github.com/maka-agent/maka-agent)
  > Maka — local-first AI desktop assistant

### [FunnyWolf](https://github.com/FunnyWolf)
- 🌟 👤 [FunnyWolf](https://github.com/FunnyWolf) Starred [flyfish-dev/file-viewer](https://github.com/flyfish-dev/file-viewer)
  > Browser-native Office / PDF / CAD / archive viewer for internal web apps, with Vue, React, Svelte, j...

### [GcsSloop](https://github.com/GcsSloop)
- 🌟 👤 [GcsSloop](https://github.com/GcsSloop) Starred [LibreCAD/LibreCAD](https://github.com/LibreCAD/LibreCAD)
  > LibreCAD is a cross-platform 2D CAD program. It can read DXF/DWG, and write DXF/DWG/PDF/SVG files. I...

### [Jirubizu](https://github.com/Jirubizu)
- 🌟 👤 [Jirubizu](https://github.com/Jirubizu) Starred [Jirubizu/reclass-rs](https://github.com/Jirubizu/reclass-rs)

### [JoasASantos](https://github.com/JoasASantos)
- 🌟 👤 [JoasASantos](https://github.com/JoasASantos) Starred [stablyai/orca](https://github.com/stablyai/orca)
  > Orca is the ADE for working with a fleet of parallel agents. Run any coding agent with your own subs...

### [Keno](https://github.com/Keno)
- 🍴 👤 [Keno](https://github.com/Keno) Forked [indygreg/apple-platform-rs](https://github.com/indygreg/apple-platform-rs) to [JuliaCI/apple-platform-rs](https://github.com/JuliaCI/apple-platform-rs)
  > Rust crates supporting Apple platform development
- 🍴 👤 [Keno](https://github.com/Keno) Forked [OpenMathLib/OpenBLAS](https://github.com/OpenMathLib/OpenBLAS) to [KenoAIStaging/OpenBLAS](https://github.com/KenoAIStaging/OpenBLAS)
  > OpenBLAS is an optimized BLAS library based on GotoBLAS2 1.13 BSD version. 

### [Keyird](https://github.com/Keyird)
- 🌟 👤 [Keyird](https://github.com/Keyird) Starred [huggingface/diffusers](https://github.com/huggingface/diffusers)
  > 🤗 Diffusers: State-of-the-art diffusion models for image, video, and audio generation in PyTorch.
- 🌟 👤 [Keyird](https://github.com/Keyird) Starred [HAIGC/Comfyui-HAIGC-PSD](https://github.com/HAIGC/Comfyui-HAIGC-PSD)
  > 保存分层图像为psd格式，适合千问分层图像模型qwen_image_layered出图后保存为psd文件，支持详情页。

### [KuLiPai](https://github.com/KuLiPai)
- 🌟 👤 [KuLiPai](https://github.com/KuLiPai) Starred [SecWiki/android-kernel-exploits](https://github.com/SecWiki/android-kernel-exploits)
  > android kernel exploits漏洞集合 https://www.sec-wiki.com

### [LLeavesG](https://github.com/LLeavesG)
- 🌟 👤 [LLeavesG](https://github.com/LLeavesG) Starred [agentrebench/AgentRE-Bench](https://github.com/agentrebench/AgentRE-Bench)
  > AgentRE-Bench is an agentic benchmark that evaluates state-of-the-art models on long-horizon reverse...

### [LunFengChen](https://github.com/LunFengChen)
- 🌟 👤 [LunFengChen](https://github.com/LunFengChen) Starred [dark-hxx/CLI-Manager](https://github.com/dark-hxx/CLI-Manager)
  > CLI-Manager: 用于集中管理基于CLI 的多个开发项目，解决多窗口切换、重复输入命令的痛点，提升开发工作流效率。
- 🍴 👤 [LunFengChen](https://github.com/LunFengChen) Forked [dark-hxx/CLI-Manager](https://github.com/dark-hxx/CLI-Manager) to [LunFengChen/CLI-Manager](https://github.com/LunFengChen/CLI-Manager)
  > CLI-Manager: 用于集中管理基于CLI 的多个开发项目，解决多窗口切换、重复输入命令的痛点，提升开发工作流效率。
- 🌟 👤 [LunFengChen](https://github.com/LunFengChen) Starred [geekbyter/sidechannel_bypass_kpm](https://github.com/geekbyter/sidechannel_bypass_kpm)
  > something applications can use side channel to check root(hunter)，this kpm can bypass it
- 🍴 👤 [LunFengChen](https://github.com/LunFengChen) Forked [geekbyter/sidechannel_bypass_kpm](https://github.com/geekbyter/sidechannel_bypass_kpm) to [LunFengChen/sidechannel_bypass_kpm](https://github.com/LunFengChen/sidechannel_bypass_kpm)
  > something applications can use side channel to check root(hunter)，this kpm can bypass it
- 🌟 👤 [LunFengChen](https://github.com/LunFengChen) Starred [fuqiuluo/binder-trace](https://github.com/fuqiuluo/binder-trace)
  > 🍂 Android Binder 调用观测工具, 基于内核采集 Binder transaction实现!
- 🍴 👤 [LunFengChen](https://github.com/LunFengChen) Forked [fuqiuluo/binder-trace](https://github.com/fuqiuluo/binder-trace) to [LunFengChen/binder-trace](https://github.com/LunFengChen/binder-trace)
  > 🍂 Android Binder 调用观测工具, 基于内核采集 Binder transaction实现!

### [M0Rf30](https://github.com/M0Rf30)
- 🌟 👤 [M0Rf30](https://github.com/M0Rf30) Starred [akustikrausch/yamaha-smaf-player](https://github.com/akustikrausch/yamaha-smaf-player)
  > A dependency-free C++ PLAYER for Yamaha SMAF (.mmf) ringtones: the polyphonic-ringtone format of the...

### [Mati365](https://github.com/Mati365)
- 🌟 👤 [Mati365](https://github.com/Mati365) Starred [FractalFir/crustc](https://github.com/FractalFir/crustc)
  > Entirety of `rustc`, translated to C. 

### [MhmRdd](https://github.com/MhmRdd)
- 🌟 👤 [MhmRdd](https://github.com/MhmRdd) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 
- 🍴 👤 [MhmRdd](https://github.com/MhmRdd) Forked [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia) to [MhmRdd/CyberMeowfia](https://github.com/MhmRdd/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 

### [MikaCybertron](https://github.com/MikaCybertron)
- 🌟 👤 [MikaCybertron](https://github.com/MikaCybertron) Starred [springmusk026/UnityRenderer](https://github.com/springmusk026/UnityRenderer)
  > Educational project demonstrating IL2CPP runtime interaction from native C++ on Android
- 🍴 👤 [MikaCybertron](https://github.com/MikaCybertron) Forked [springmusk026/UnityRenderer](https://github.com/springmusk026/UnityRenderer) to [MikaCybertron/UnityRenderer](https://github.com/MikaCybertron/UnityRenderer)
  > Educational project demonstrating IL2CPP runtime interaction from native C++ on Android

### [MolotovCherry](https://github.com/MolotovCherry)
- 🌟 👤 [MolotovCherry](https://github.com/MolotovCherry) Starred [qt/qtbridge-rust](https://github.com/qt/qtbridge-rust)
  > Qt Bridge for Rust. Qt Bridges enables developers to provide application logic written in a foreign ...

### [Mr-xn](https://github.com/Mr-xn)
- 🌟 👤 [Mr-xn](https://github.com/Mr-xn) Starred [ZephrFish/harness-kit](https://github.com/ZephrFish/harness-kit)
  > A template for building your own AI/LLM harness
- 🌟 👤 [Mr-xn](https://github.com/Mr-xn) Starred [yArna/isChinaUser](https://github.com/yArna/isChinaUser)
  >  🇨🇳  Determine current user is a Chinese user.
- 🌟 👤 [Mr-xn](https://github.com/Mr-xn) Starred [LinXiaoTao/FuckClaude](https://github.com/LinXiaoTao/FuckClaude)

### [N3rdL0rd](https://github.com/N3rdL0rd)
- 🌟 👤 [N3rdL0rd](https://github.com/N3rdL0rd) Starred [ozkl/doomgeneric](https://github.com/ozkl/doomgeneric)
  > Easily portable doom

### [Naville](https://github.com/Naville)
- 🌟 👤 [Naville](https://github.com/Naville) Starred [gmh5225/ios-location-spoofer](https://github.com/gmh5225/ios-location-spoofer)
  > Standalone iOS app to spoof GPS location without jailbreak. Includes Shadowrocket/Surge/Loon/QX/Stas...
- 🌟 👤 [Naville](https://github.com/Naville) Starred [mekos2772/ios-location-spoofer](https://github.com/mekos2772/ios-location-spoofer)
  > Standalone iOS app to spoof GPS location without jailbreak. Includes Shadowrocket/Surge/Loon/QX/Stas...

### [PangBaiWork](https://github.com/PangBaiWork)
- 🌟 👤 [PangBaiWork](https://github.com/PangBaiWork) Starred [chainreactors/aiscan](https://github.com/chainreactors/aiscan)
  > AI-driven pi-like agent for cyber security — single binary for  pentest, red team, bug  bounty
- 🌟 👤 [PangBaiWork](https://github.com/PangBaiWork) Starred [tyron12233/CodeAssist](https://github.com/tyron12233/CodeAssist)
  > Experimental IDE for building Android applicatons on Android.

### [R0rt1z2](https://github.com/R0rt1z2)
- 🌟 👤 [R0rt1z2](https://github.com/R0rt1z2) Starred [canyie/TransitionPlayer](https://github.com/canyie/TransitionPlayer)
  > CVE-2026-0091, play with an issue in android window management to perform arbitrary code execution i...

### [Resery](https://github.com/Resery)
- 🌟 👤 [Resery](https://github.com/Resery) Starred [canyie/TransitionPlayer](https://github.com/canyie/TransitionPlayer)
  > CVE-2026-0091, play with an issue in android window management to perform arbitrary code execution i...
- 🌟 👤 [Resery](https://github.com/Resery) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 

### [S3cur3Th1sSh1t](https://github.com/S3cur3Th1sSh1t)
- 🌟 👤 [S3cur3Th1sSh1t](https://github.com/S3cur3Th1sSh1t) Starred [vergamota/KslKatz](https://github.com/vergamota/KslKatz)
  > Combining KslDump and GhostKatz to dump LSASS using no-fix KslD.sys memory read to bypass PPL. Extra...
- 🌟 👤 [S3cur3Th1sSh1t](https://github.com/S3cur3Th1sSh1t) Starred [Muz1K1zuM/kslkatz_bof](https://github.com/Muz1K1zuM/kslkatz_bof)
  > Havoc C2 BOF port of the KslD.sys BYOVD technique. Credential extraction from lsass via physical mem...

### [Silentsoul04](https://github.com/Silentsoul04)
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [uphiago/recon-skills](https://github.com/uphiago/recon-skills)
  > 156 offensive security skills for recon and pentest. Field-validated techniques from 600+ targets ac...
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [uphiago/recon-skills](https://github.com/uphiago/recon-skills) to [Silentsoul04/recon-skills](https://github.com/Silentsoul04/recon-skills)
  > 156 offensive security skills for recon and pentest. Field-validated techniques from 600+ targets ac...
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [ajaysenr/HackerOne-Disclosed-Reports](https://github.com/ajaysenr/HackerOne-Disclosed-Reports)
  > A structured, auto-updating archive of disclosed HackerOne reports. Filter by severity, CWE weakness...
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [ajaysenr/HackerOne-Disclosed-Reports](https://github.com/ajaysenr/HackerOne-Disclosed-Reports) to [Silentsoul04/HackerOne-Disclosed-Reports](https://github.com/Silentsoul04/HackerOne-Disclosed-Reports)
  > A structured, auto-updating archive of disclosed HackerOne reports. Filter by severity, CWE weakness...
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [Huu1j/crawl_xz](https://github.com/Huu1j/crawl_xz)
  > 一个批量爬取安全技术社区文章的工具（目前支持先知社区、奇安信攻防社区），支持Markdown、PDF、HTML多种格式输出
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [Huu1j/crawl_xz](https://github.com/Huu1j/crawl_xz) to [Silentsoul04/crawl_xz](https://github.com/Silentsoul04/crawl_xz)
  > 一个批量爬取安全技术社区文章的工具（目前支持先知社区、奇安信攻防社区），支持Markdown、PDF、HTML多种格式输出
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [kuqadk3/CTF-and-Learning](https://github.com/kuqadk3/CTF-and-Learning)
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [kuqadk3/CTF-and-Learning](https://github.com/kuqadk3/CTF-and-Learning) to [Silentsoul04/CTF-and-Learning](https://github.com/Silentsoul04/CTF-and-Learning)
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [github/dmca](https://github.com/github/dmca) to [Silentsoul04/dmca](https://github.com/Silentsoul04/dmca)
  > Repository with text of DMCA takedown notices as received. GitHub does not endorse or adopt any asse...
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [cryusyang/obsidian-notes](https://github.com/cryusyang/obsidian-notes) to [Silentsoul04/obsidian-notescn](https://github.com/Silentsoul04/obsidian-notescn)
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [mindcodings/mpvulnHub](https://github.com/mindcodings/mpvulnHub)
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [mindcodings/mpvulnHub](https://github.com/mindcodings/mpvulnHub) to [Silentsoul04/mpvulnHub](https://github.com/Silentsoul04/mpvulnHub)
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [cujanovic/llm-sast-scanner](https://github.com/cujanovic/llm-sast-scanner)
  > A SAST skill that gives AI coding agents structured vulnerability detection across 103 vulnerability...
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [cujanovic/llm-sast-scanner](https://github.com/cujanovic/llm-sast-scanner) to [Silentsoul04/llm-sast-scanner](https://github.com/Silentsoul04/llm-sast-scanner)
  > A SAST skill that gives AI coding agents structured vulnerability detection across 103 vulnerability...
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [lyonzin/knowledge-rag](https://github.com/lyonzin/knowledge-rag)
  > Local RAG MCP server for Claude Code — hybrid search (semantic + BM25), cross-encoder reranking, 13 ...
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [lyonzin/knowledge-rag](https://github.com/lyonzin/knowledge-rag) to [Silentsoul04/knowledge-rag](https://github.com/Silentsoul04/knowledge-rag)
  > Local RAG MCP server for Claude Code — hybrid search (semantic + BM25), cross-encoder reranking, 13 ...
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [jamesob/local-llm](https://github.com/jamesob/local-llm)
  > Everything I know about running LLMs locally
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [jamesob/local-llm](https://github.com/jamesob/local-llm) to [Silentsoul04/local-llm](https://github.com/Silentsoul04/local-llm)
  > Everything I know about running LLMs locally

### [SodaMilk233](https://github.com/SodaMilk233)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [73kyyy/NexClip](https://github.com/73kyyy/NexClip)
  > NexClip - AI-Powered Video Editor for Android
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [73kyyy/MyVideoEditor](https://github.com/73kyyy/MyVideoEditor)
  > AI-powered video editor for Android
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [gagaword/gagademo](https://github.com/gagaword/gagademo)
  > App课程Demo
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [s-silt/fxapk](https://github.com/s-silt/fxapk)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [111rice111/ImGuiOverlay](https://github.com/111rice111/ImGuiOverlay)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [inliver233/ppcat](https://github.com/inliver233/ppcat)
  > base
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [tianbin0859/mobile-app-crack-toolkit](https://github.com/tianbin0859/mobile-app-crack-toolkit)
  > Mobile App Crack Toolkit v7.0 - Advanced Reverse Engineering Suite
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [ChinaFLTV/OpenHand](https://github.com/ChinaFLTV/OpenHand)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [710399661/APP](https://github.com/710399661/APP)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [zjjhqwqc/ceshi](https://github.com/zjjhqwqc/ceshi)

### [SoyBeanMilkx](https://github.com/SoyBeanMilkx)
- 🌟 👤 [SoyBeanMilkx](https://github.com/SoyBeanMilkx) Starred [jingyaogong/minimind](https://github.com/jingyaogong/minimind)
  > 🧠「大模型」2小时完全从0训练64M的小参数LLM！Train a 64M-parameter LLM from scratch in just 2h!

### [SummerSec](https://github.com/SummerSec)
- 🌟 👤 [SummerSec](https://github.com/SummerSec) Starred [obra/superpowers](https://github.com/obra/superpowers)
  > An agentic skills framework & software development methodology that works.

### [THISISAGOODNAME](https://github.com/THISISAGOODNAME)
- 🌟 👤 [THISISAGOODNAME](https://github.com/THISISAGOODNAME) Starred [erincatto/box3d](https://github.com/erincatto/box3d)
  > Box3D is a 3D physics engine for games

### [TheWildJames](https://github.com/TheWildJames)
- 🍴 👤 [TheWildJames](https://github.com/TheWildJames) Forked [Midrags/SFF](https://github.com/Midrags/SFF) to [TheWildJames/SFF](https://github.com/TheWildJames/SFF)
  > SteaMidra - Advanced Steam game setup and management tool featuring manifest handling, Lua integrati...

### [Twi1ight](https://github.com/Twi1ight)
- 🌟 👤 [Twi1ight](https://github.com/Twi1ight) Starred [deekur/gaokaomath](https://github.com/deekur/gaokaomath)
  > 历年高考数学真题，1952-2026

### [Uevo001](https://github.com/Uevo001)
- 🍴 👤 [Uevo001](https://github.com/Uevo001) Forked [R3pl4c3r/geoip](https://github.com/R3pl4c3r/geoip) to [Uevo001/geoip](https://github.com/Uevo001/geoip)
  > geoip.dat referring to github.com/SukkaLab/ruleset.skk.moe
- 🍴 👤 [Uevo001](https://github.com/Uevo001) Forked [R3pl4c3r/domain-list-custom](https://github.com/R3pl4c3r/domain-list-custom) to [Uevo001/domain-list-custom](https://github.com/Uevo001/domain-list-custom)
  > Domain list referring to github.com/SukkaLab/ruleset.skk.moe . Generate geosite.dat for V2Ray/Xray/M...

### [WangWei-CM](https://github.com/WangWei-CM)
- 🌟 👤 [WangWei-CM](https://github.com/WangWei-CM) Starred [heygen-com/hyperframes](https://github.com/heygen-com/hyperframes)
  > Write HTML. Render video. Built for agents.

### [WitherOrNot](https://github.com/WitherOrNot)
- 🌟 👤 [WitherOrNot](https://github.com/WitherOrNot) Starred [winocm/efi_samples](https://github.com/winocm/efi_samples)

### [XiaoTong6666](https://github.com/XiaoTong6666)
- 🌟 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Starred [ThePedroo/ReZygiskAPI](https://github.com/ThePedroo/ReZygiskAPI)
  > A repository to document the proposal and progress to ReZygisk's API.
- 🌟 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 
- 🌟 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Starred [l2dy/intellij-rust-unofficial-plugin](https://github.com/l2dy/intellij-rust-unofficial-plugin)
  > Unofficial Rust plugin for IntelliJ IDEA Community Edition (fork of intellij-rust)
- 🌟 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Starred [re-zero001/Dobby](https://github.com/re-zero001/Dobby)
  > a lightweight, multi-platform, multi-architecture hook framework.
- 🍴 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Forked [LSPosed/Dobby](https://github.com/LSPosed/Dobby) to [XiaoTong6666/Dobby](https://github.com/XiaoTong6666/Dobby)
  > a lightweight, multi-platform, multi-architecture hook framework.
- 🌟 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Starred [XiaoTong6666/Dobby](https://github.com/XiaoTong6666/Dobby)
  > a lightweight, multi-platform, multi-architecture hook framework.
- 🍴 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Forked [LSPosed/LSPlt](https://github.com/LSPosed/LSPlt) to [XiaoTong6666/LSPlt](https://github.com/XiaoTong6666/LSPlt)
  > Simple PLT hook for Android

### [Xunzhuo](https://github.com/Xunzhuo)
- 🌟 👤 [Xunzhuo](https://github.com/Xunzhuo) Starred [skypilot-org/skypilot](https://github.com/skypilot-org/skypilot)
  > Run, manage, and scale AI workloads on any AI infrastructure. Use one system to access & manage all ...

### [YuHuanTin](https://github.com/YuHuanTin)
- 🌟 👤 [YuHuanTin](https://github.com/YuHuanTin) Starred [ljagiello/ctf-skills](https://github.com/ljagiello/ctf-skills)
  > Agent skills for solving CTF challenges - web exploitation, binary pwn, crypto, reverse engineering,...

### [YuKongA](https://github.com/YuKongA)
- 🌟 👤 [YuKongA](https://github.com/YuKongA) Starred [figma/mcp-server-guide](https://github.com/figma/mcp-server-guide)
  > A guide on how to use the Figma MCP server

### [ZehMatt](https://github.com/ZehMatt)
- 🍴 👤 [ZehMatt](https://github.com/ZehMatt) Forked [Asphaltian/VPhysics-Box3D](https://github.com/Asphaltian/VPhysics-Box3D) to [ZehMatt/VPhysics-Box3D](https://github.com/ZehMatt/VPhysics-Box3D)
  > Vox3D (VPhysics Box3D) is a replacement physics module for the Source Engine.

### [ZephrFish](https://github.com/ZephrFish)
- 🌟 👤 [ZephrFish](https://github.com/ZephrFish) Starred [ioerror/tlsdate](https://github.com/ioerror/tlsdate)
  > secure parasitic rdate replacement

### [aengelke](https://github.com/aengelke)
- 🍴 👤 [aengelke](https://github.com/aengelke) Forked [Phantomical/perf-event-data](https://github.com/Phantomical/perf-event-data) to [aengelke/perf-event-data](https://github.com/aengelke/perf-event-data)
  > Types and traits for parsing records emitted by `perf_event_open`

### [afkT](https://github.com/afkT)
- 🌟 👤 [afkT](https://github.com/afkT) Starred [CherryHQ/cherry-studio](https://github.com/CherryHQ/cherry-studio)
  > AI productivity studio with smart chat, autonomous agents, and 300+ assistants. Unified access to fr...
- 🌟 👤 [afkT](https://github.com/afkT) Starred [nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill)
  > An AI SKILL that provide design intelligence for building professional UI/UX multiple platforms
- 🌟 👤 [afkT](https://github.com/afkT) Starred [DeusData/codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp)
  > High-performance code intelligence MCP server. Indexes codebases into a persistent knowledge graph —...
- 🌟 👤 [afkT](https://github.com/afkT) Starred [openai/codex-plugin-cc](https://github.com/openai/codex-plugin-cc)
  > Use Codex from Claude Code to review code or delegate tasks.
- 🌟 👤 [afkT](https://github.com/afkT) Starred [anthropics/claude-code](https://github.com/anthropics/claude-code)
  > Claude Code is an agentic coding tool that lives in your terminal, understands your codebase, and he...
- 🌟 👤 [afkT](https://github.com/afkT) Starred [tirth8205/code-review-graph](https://github.com/tirth8205/code-review-graph)
  > Local-first code intelligence graph for MCP and CLI. Builds a persistent map of your codebase so AI ...
- 🌟 👤 [afkT](https://github.com/afkT) Starred [mattpocock/skills](https://github.com/mattpocock/skills)
  > Skills for Real Engineers. Straight from my .claude directory.
- 🌟 👤 [afkT](https://github.com/afkT) Starred [microsoft/markitdown](https://github.com/microsoft/markitdown)
  > Python tool for converting files and office documents to Markdown.
- 🌟 👤 [afkT](https://github.com/afkT) Starred [Fission-AI/OpenSpec](https://github.com/Fission-AI/OpenSpec)
  > Spec-driven development (SDD) for AI coding assistants.

### [ant4g0nist](https://github.com/ant4g0nist)
- 🌟 👤 [ant4g0nist](https://github.com/ant4g0nist) Starred [xjffff/FUNCATTN](https://github.com/xjffff/FUNCATTN)
  > Functional Attention: From Pairwise Affinities to Functional Correspondences
- 🌟 👤 [ant4g0nist](https://github.com/ant4g0nist) Starred [cursor/plugins](https://github.com/cursor/plugins)
  > Cursor plugin specification and official plugins
- 🌟 👤 [ant4g0nist](https://github.com/ant4g0nist) Starred [northpolesec/santa](https://github.com/northpolesec/santa)
  > A binary and file access authorization system for macOS.
- 🌟 👤 [ant4g0nist](https://github.com/ant4g0nist) Starred [harbor-framework/harbor](https://github.com/harbor-framework/harbor)
  > Framework for evaluating and improving agents 

### [any86](https://github.com/any86)
- 🌟 👤 [any86](https://github.com/any86) Starred [rust-lang/rust](https://github.com/rust-lang/rust)
  > Empowering everyone to build reliable and efficient software.

### [apkunpacker](https://github.com/apkunpacker)
- 🌟 👤 [apkunpacker](https://github.com/apkunpacker) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 
- 🍴 👤 [apkunpacker](https://github.com/apkunpacker) Forked [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia) to [apkunpacker/CyberMeowfia](https://github.com/apkunpacker/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 
- 🌟 👤 [apkunpacker](https://github.com/apkunpacker) Starred [can1357/pon](https://github.com/can1357/pon)
  > Python 3.14, compiled to metal — JIT & AoT native compiler and runtime in Rust. Cranelift backend, r...
- 🌟 👤 [apkunpacker](https://github.com/apkunpacker) Starred [DeusData/codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp)
  > High-performance code intelligence MCP server. Indexes codebases into a persistent knowledge graph —...

### [b3x206](https://github.com/b3x206)
- 🌟 👤 [b3x206](https://github.com/b3x206) Starred [kovacsv/Online3DViewer](https://github.com/kovacsv/Online3DViewer)
  > A solution to visualize and explore 3D models in your browser.

### [ba0gu0](https://github.com/ba0gu0)
- 🌟 👤 [ba0gu0](https://github.com/ba0gu0) Starred [LinXiaoTao/FuckClaude](https://github.com/LinXiaoTao/FuckClaude)
- 🌟 👤 [ba0gu0](https://github.com/ba0gu0) Starred [uphiago/recon-skills](https://github.com/uphiago/recon-skills)
  > 156 offensive security skills for recon and pentest. Field-validated techniques from 600+ targets ac...

### [backslashxx](https://github.com/backslashxx)
- 🌟 👤 [backslashxx](https://github.com/backslashxx) Starred [hrw/syscalls-table](https://github.com/hrw/syscalls-table)
  > Information about Linux system calls on different architectures

### [bigsinger](https://github.com/bigsinger)
- 🌟 👤 [bigsinger](https://github.com/bigsinger) Starred [SaFo-Lab/DRIFT](https://github.com/SaFo-Lab/DRIFT)
  > [NeurIPS 2025] The official implementation of the paper "DRIFT: Dynamic Rule-Based Defense with Inje...

### [bufferoverflow](https://github.com/bufferoverflow)
- 🌟 👤 [bufferoverflow](https://github.com/bufferoverflow) Starred [robertklep/dsm7-usb-serial-drivers](https://github.com/robertklep/dsm7-usb-serial-drivers)
  > Synology DSM 7 USB serial drivers

### [c1y2m3](https://github.com/c1y2m3)
- 🌟 👤 [c1y2m3](https://github.com/c1y2m3) Starred [larlarua/AutoCVE](https://github.com/larlarua/AutoCVE)
  > Agent-driven automated CVE discovery platform for source code auditing, vulnerability verification, ...
- 🌟 👤 [c1y2m3](https://github.com/c1y2m3) Starred [tuatec/TTToolbox](https://github.com/tuatec/TTToolbox)
  > TTToolbox provides useful helper scripts to automate your character integration workflows in Unreal ...

### [cctv18](https://github.com/cctv18)
- 🌟 👤 [cctv18](https://github.com/cctv18) Starred [yynxxxxx/Codex-5.5-codex-instruct-5.5](https://github.com/yynxxxxx/Codex-5.5-codex-instruct-5.5)

### [chf007](https://github.com/chf007)
- 🌟 👤 [chf007](https://github.com/chf007) Starred [flyfish-dev/file-viewer](https://github.com/flyfish-dev/file-viewer)
  > Browser-native Office / PDF / CAD / archive viewer for internal web apps, with Vue, React, Svelte, j...

### [cinit](https://github.com/cinit)
- 🌟 👤 [cinit](https://github.com/cinit) Starred [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)
  > Wrap Antigravity, ChatGPT Codex, Claude Code, Grok Build as an OpenAI/Gemini/Claude/Codex compatible...

### [clearbluejar](https://github.com/clearbluejar)
- 🌟 👤 [clearbluejar](https://github.com/clearbluejar) Starred [n0qword/win32k-callback-detouring](https://github.com/n0qword/win32k-callback-detouring)
  > Abusing the win32k.sys kernel callback mechanism for arbitrary code execution

### [cn-kali-team](https://github.com/cn-kali-team)
- 🌟 👤 [cn-kali-team](https://github.com/cn-kali-team) Starred [rancher/fleet](https://github.com/rancher/fleet)
  > Deploy workloads from Git to large fleets of Kubernetes clusters

### [codehz](https://github.com/codehz)
- 🌟 👤 [codehz](https://github.com/codehz) Starred [HalFrgrd/flyline](https://github.com/HalFrgrd/flyline)
  > Flyline: a Bash plugin to replace readline for a modern line editing experience: syntax highlighting...

### [codetronik](https://github.com/codetronik)
- 🌟 👤 [codetronik](https://github.com/codetronik) Starred [xingguangcuican6666/AnyCheck](https://github.com/xingguangcuican6666/AnyCheck)
  > Found any su, using any way.

### [cristeigabriela](https://github.com/cristeigabriela)
- 🌟 👤 [cristeigabriela](https://github.com/cristeigabriela) Starred [lowleveldesign/windbg-ext-template](https://github.com/lowleveldesign/windbg-ext-template)
  > A template for creating managed WinDbg extensions

### [ctkqiang](https://github.com/ctkqiang)
- 🌟 👤 [ctkqiang](https://github.com/ctkqiang) Starred [chaitin/SafeLine](https://github.com/chaitin/SafeLine)
  > SafeLine is a self-hosted WAF(Web Application Firewall) / reverse proxy to protect your web apps fro...
- 🍴 👤 [ctkqiang](https://github.com/ctkqiang) Forked [chaitin/SafeLine](https://github.com/chaitin/SafeLine) to [ctkqiang/SafeLine](https://github.com/ctkqiang/SafeLine)
  > SafeLine is a self-hosted WAF(Web Application Firewall) / reverse proxy to protect your web apps fro...

### [darbra](https://github.com/darbra)
- 🌟 👤 [darbra](https://github.com/darbra) Starred [GitKittys/AndroidPrybar](https://github.com/GitKittys/AndroidPrybar)
  > Android Native Trace 与逆向分析工具，用于函数包装执行、指令跟踪、JNI 调用观测与外部跳转控制

### [darkskygit](https://github.com/darkskygit)
- 🍴 👤 [darkskygit](https://github.com/darkskygit) Forked [NullVoxPopuli/disk-perf-git-and-pnpm](https://github.com/NullVoxPopuli/disk-perf-git-and-pnpm) to [darkskygit/disk-perf-git-and-pnpm](https://github.com/darkskygit/disk-perf-git-and-pnpm)
  > Stress test for parallel disk i/o using git and pnpm

### [dartcv](https://github.com/dartcv)
- 🌟 👤 [dartcv](https://github.com/dartcv) Starred [Lixeer/ValleyServer](https://github.com/Lixeer/ValleyServer)
  > 星露谷物语多人游戏专用服务器/Stardew Valley Dedicated Server

### [dodola](https://github.com/dodola)
- 🌟 👤 [dodola](https://github.com/dodola) Starred [facebook/astryx](https://github.com/facebook/astryx)
  > An open source design system that's fully customizable and agent ready
- 🌟 👤 [dodola](https://github.com/dodola) Starred [chthollyphile/folia-major](https://github.com/chthollyphile/folia-major)
  > 专注于绚丽的歌词动画效果的本地音乐/navidrome/第三方网易云播放器

### [doodlewind](https://github.com/doodlewind)
- 🌟 👤 [doodlewind](https://github.com/doodlewind) Starred [visgl/deck.gl](https://github.com/visgl/deck.gl)
  > WebGL2 powered visualization framework
- 🌟 👤 [doodlewind](https://github.com/doodlewind) Starred [screenpipe/screenpipe](https://github.com/screenpipe/screenpipe)
  > YC (S26) | AI that knows what you've seen, said, or heard. Records everything you do, say, hear 24/7...
- 🌟 👤 [doodlewind](https://github.com/doodlewind) Starred [solidjs/solid](https://github.com/solidjs/solid)
  > A declarative, efficient, and flexible JavaScript library for building user interfaces.

### [dstmath](https://github.com/dstmath)
- 🌟 👤 [dstmath](https://github.com/dstmath) Starred [git-ai-project/git-ai](https://github.com/git-ai-project/git-ai)
  > A Git extension for tracking the AI-generated code in your repos

### [dwisiswant0](https://github.com/dwisiswant0)
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [kakkky/scope](https://github.com/kakkky/scope)
  > Scope-bound structured concurrency for Go
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [0xdea/ttyinject-rs](https://github.com/0xdea/ttyinject-rs)
  > Linux local privilege escalation via TIOCSTI tty injection.
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [zalando/skipper](https://github.com/zalando/skipper)
  > An HTTP router and reverse proxy for service composition, including use cases like Kubernetes Ingres...

### [easychen](https://github.com/easychen)
- 🌟 👤 [easychen](https://github.com/easychen) Starred [gluestack/gluestack-ui](https://github.com/gluestack/gluestack-ui)
  > React & React Native Components & Patterns (copy-paste components & patterns crafted with Tailwind C...

### [einverne](https://github.com/einverne)
- 🌟 👤 [einverne](https://github.com/einverne) Starred [stablyai/orca](https://github.com/stablyai/orca)
  > Orca is the ADE for working with a fleet of parallel agents. Run any coding agent with your own subs...

### [eltavine](https://github.com/eltavine)
- 🌟 👤 [eltavine](https://github.com/eltavine) Starred [evil-huawei/evil-huawei](https://github.com/evil-huawei/evil-huawei)
  > Evil Huawei - 华为作过的恶

### [exp-sky](https://github.com/exp-sky)
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [DietrichGebert/ponytail](https://github.com/DietrichGebert/ponytail)
  > Makes your AI agent think like the laziest senior dev in the room. The best code is the code you nev...
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [anthropics/launch-your-agent](https://github.com/anthropics/launch-your-agent)
  > Claude Code skills that take a founder from idea to a live Claude Managed Agent: interview, scope a ...
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [Panniantong/Agent-Reach](https://github.com/Panniantong/Agent-Reach)
  > Give your AI agent eyes to see the entire internet. Read & search Twitter, Reddit, YouTube, GitHub, ...
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [headroomlabs-ai/headroom](https://github.com/headroomlabs-ai/headroom)
  > Compress tool outputs, logs, files, and RAG chunks before they reach the LLM. 60-95% fewer tokens, s...

### [extremecoders-re](https://github.com/extremecoders-re)
- 🌟 👤 [extremecoders-re](https://github.com/extremecoders-re) Starred [TencentCloud/CubeSandbox](https://github.com/TencentCloud/CubeSandbox)
  > Instant, Concurrent, Secure & Lightweight Sandbox for AI Agents.
- 🌟 👤 [extremecoders-re](https://github.com/extremecoders-re) Starred [FractalFir/crustc](https://github.com/FractalFir/crustc)
  > Entirety of `rustc`, translated to C. 
- 🌟 👤 [extremecoders-re](https://github.com/extremecoders-re) Starred [facebook/lexical](https://github.com/facebook/lexical)
  > Lexical is an extensible text editor framework that provides excellent reliability, accessibility an...
- 🌟 👤 [extremecoders-re](https://github.com/extremecoders-re) Starred [skuznetsov/depyo.js](https://github.com/skuznetsov/depyo.js)
  > Python decompiler. Written in JavaScript. Works in NodeJS. Decompiles extremely fast.

### [fengchenzxc](https://github.com/fengchenzxc)
- 🌟 👤 [fengchenzxc](https://github.com/fengchenzxc) Starred [DaoYiSec/SecSkills](https://github.com/DaoYiSec/SecSkills)
  > 收集整理渗透测试、漏洞扫描、代码审计、CTF、逆向、安全研究 等网络安全相关的 Skills和MCP
- 🌟 👤 [fengchenzxc](https://github.com/fengchenzxc) Starred [m-sec-org/BreachWeave](https://github.com/m-sec-org/BreachWeave)
  > 智能渗透Agent Manager/Observer/Solver 多角色架构，基于 pi-mono SDK。

### [fengjixuchui](https://github.com/fengjixuchui)
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [DevlarSchuh/iommu-driver-poc](https://github.com/DevlarSchuh/iommu-driver-poc) to [fengjixuchui/iommu-driver-poc](https://github.com/fengjixuchui/iommu-driver-poc)
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [quarkslab/android-fuzzing](https://github.com/quarkslab/android-fuzzing) to [fengjixuchui/android-fuzzing](https://github.com/fengjixuchui/android-fuzzing)

### [fjh658](https://github.com/fjh658)
- 🌟 👤 [fjh658](https://github.com/fjh658) Starred [3899/SimAdmin](https://github.com/3899/SimAdmin)
  > 🎉面向 Debian 蜂窝 CPE、随身 WiFi、软路由类设备的 SIM/eSIM、蜂窝网络、短信、DDNS和系统状态管理系统。

### [fuqiuluo](https://github.com/fuqiuluo)
- 🌟 👤 [fuqiuluo](https://github.com/fuqiuluo) Starred [J-jaeyoung/bad-epoll](https://github.com/J-jaeyoung/bad-epoll)

### [fynch3r](https://github.com/fynch3r)
- 🌟 👤 [fynch3r](https://github.com/fynch3r) Starred [denoland/deno](https://github.com/denoland/deno)
  > A modern runtime for JavaScript and TypeScript.
- 🌟 👤 [fynch3r](https://github.com/fynch3r) Starred [jiji262/douyin-downloader](https://github.com/jiji262/douyin-downloader)
  > A practical Douyin downloader for both single-item and profile batch downloads, with progress displa...

### [fyrlove](https://github.com/fyrlove)
- 🍴 👤 [fyrlove](https://github.com/fyrlove) Forked [lidongyooo/GumTrace](https://github.com/lidongyooo/GumTrace) to [fyrlove/GumTrace](https://github.com/fyrlove/GumTrace)
  > 基于 Frida Gum (Stalker) 引擎的 ARM64 动态指令追踪工具，支持 Android 和 iOS 平台。

### [gh0stkey](https://github.com/gh0stkey)
- 🌟 👤 [gh0stkey](https://github.com/gh0stkey) Starred [evolus/pencil](https://github.com/evolus/pencil)
  > The Pencil Project's unique mission is to build a free and opensource tool for making diagrams and G...
- 🌟 👤 [gh0stkey](https://github.com/gh0stkey) Starred [dajva/rg.el](https://github.com/dajva/rg.el)
  > Emacs search tool based on ripgrep

### [ghluzhiyuan](https://github.com/ghluzhiyuan)
- 🌟 👤 [ghluzhiyuan](https://github.com/ghluzhiyuan) Starred [Yu9191/wloc](https://github.com/Yu9191/wloc)
  > 修改 Apple 网络定位（gs-loc）返回坐标 · 支持 Surge / Quantumult X / Loon / Stash · 快捷指令一键设置/恢复定位
- 🍴 👤 [ghluzhiyuan](https://github.com/ghluzhiyuan) Forked [Yu9191/wloc](https://github.com/Yu9191/wloc) to [ghluzhiyuan/wloc](https://github.com/ghluzhiyuan/wloc)
  > 修改 Apple 网络定位（gs-loc）返回坐标 · 支持 Surge / Quantumult X / Loon / Stash · 快捷指令一键设置/恢复定位

### [gmh5225](https://github.com/gmh5225)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [FractalFir/crustc](https://github.com/FractalFir/crustc) to [gmh5225/crustc](https://github.com/gmh5225/crustc)
  > Entirety of `rustc`, translated to C. 
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [un4ckn0wl3z/dioprocess-private](https://github.com/un4ckn0wl3z/dioprocess-private) to [gmh5225/dioprocess-private](https://github.com/gmh5225/dioprocess-private)
  > DioProcess — Advanced Windows Internals, Evasion, Rootkit, Hypervisor, SMM & Bootkit Research Framew...
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [skuznetsov/depyo.js](https://github.com/skuznetsov/depyo.js) to [gmh5225/depyo.js](https://github.com/gmh5225/depyo.js)
  > Python decompiler. Written in JavaScript. Works in NodeJS. Decompiles extremely fast.
- 🌟 👤 [gmh5225](https://github.com/gmh5225) Starred [jup-ag/jupiter-amm-interface](https://github.com/jup-ag/jupiter-amm-interface)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [purpshell/meowcaller](https://github.com/purpshell/meowcaller) to [gmh5225/meowcaller](https://github.com/gmh5225/meowcaller)
  > WhatsApp VoIP Go library for whatsmeow

### [guiying712](https://github.com/guiying712)
- 🌟 👤 [guiying712](https://github.com/guiying712) Starred [shy3130/tickflow-stock-panel](https://github.com/shy3130/tickflow-stock-panel)
  > 自托管、零运维的 A 股「选股 + 监控 + 回测」量化工作台 | 基于 TickFlow 数据源  | LLM能力驱使策略定制+个股分析+复盘 | 自由接入第三方个性化扩展数据 | 个人开源 ,不属...

### [handhand](https://github.com/handhand)
- 🌟 👤 [handhand](https://github.com/handhand) Starred [handhand/HackThisAndroid](https://github.com/handhand/HackThisAndroid)
  > android项目，用ndk来实现app保护，同时也提供frida脚本去进行攻击

### [heeeeen](https://github.com/heeeeen)
- 🌟 👤 [heeeeen](https://github.com/heeeeen) Starred [canyie/TransitionPlayer](https://github.com/canyie/TransitionPlayer)
  > CVE-2026-0091, play with an issue in android window management to perform arbitrary code execution i...

### [hellodword](https://github.com/hellodword)
- 🌟 👤 [hellodword](https://github.com/hellodword) Starred [yynxxxxx/Codex-5.5-codex-instruct-5.5](https://github.com/yynxxxxx/Codex-5.5-codex-instruct-5.5)

### [hhhaiai](https://github.com/hhhaiai)
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [ruvnet/ruflo](https://github.com/ruvnet/ruflo)
  > 🌊 The leading agent meta-harness. Deploy intelligent multi-player swarms, coordinate autonomous work...
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [x1aon1ng/Nook](https://github.com/x1aon1ng/Nook)
  > A Frida-Like Style Dynamic instrumentation toolkit for Android

### [hoffstadt](https://github.com/hoffstadt)
- 🌟 👤 [hoffstadt](https://github.com/hoffstadt) Starred [Depersonalizc/volumetric-clouds](https://github.com/Depersonalizc/volumetric-clouds)
  > A real-time GPU volumetric clouds renderer via OpenGL.

### [hyuunnn](https://github.com/hyuunnn)
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [simonw/llm-coding-agent](https://github.com/simonw/llm-coding-agent)
  > A coding agent built on LLM
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [dillon1000/candisk](https://github.com/dillon1000/candisk)
  > macOS disk space visualizer
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [ScalingIntelligence/KernelBench](https://github.com/ScalingIntelligence/KernelBench)
  > KernelBench: Can LLMs Write GPU Kernels? - Benchmark + Toolkit with Torch -> CUDA (+ more DSLs)
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [Michael-A-Kuykendall/shimmy](https://github.com/Michael-A-Kuykendall/shimmy)
  > ⚡ Pure-Rust WebGPU inference engine — OpenAI-API compatible, GGUF native, runs on any GPU. No Python...

### [ibaoger](https://github.com/ibaoger)
- 🍴 👤 [ibaoger](https://github.com/ibaoger) Forked [omegaee/my-fingerprint](https://github.com/omegaee/my-fingerprint) to [ibaoger/my-fingerprint](https://github.com/ibaoger/my-fingerprint)
  > 保护你的浏览器指纹 | Protect Your Browser Fingerprints | Chrome, Edge, Firefox | 扩展 / Extension

### [igorbrigadir](https://github.com/igorbrigadir)
- 🌟 👤 [igorbrigadir](https://github.com/igorbrigadir) Starred [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)
  > The agent that grows with you

### [ioncodes](https://github.com/ioncodes)
- 🌟 👤 [ioncodes](https://github.com/ioncodes) Starred [nesdev-org/MesenCE](https://github.com/nesdev-org/MesenCE)
  > Multi-system emulator (NES, SNES, GB, GBA, PCE, SMS/GG, WS) for Windows, Linux and macOS

### [jianchang512](https://github.com/jianchang512)
- 🌟 👤 [jianchang512](https://github.com/jianchang512) Starred [bilbil0903/uyghur_tts_mms](https://github.com/bilbil0903/uyghur_tts_mms)
- 🍴 👤 [jianchang512](https://github.com/jianchang512) Forked [bilbil0903/uyghur_tts_mms](https://github.com/bilbil0903/uyghur_tts_mms) to [jianchang512/uyghur_tts_mms](https://github.com/jianchang512/uyghur_tts_mms)
- 🌟 👤 [jianchang512](https://github.com/jianchang512) Starred [eslxxx/Uyghur-tts](https://github.com/eslxxx/Uyghur-tts)
- 🍴 👤 [jianchang512](https://github.com/jianchang512) Forked [eslxxx/Uyghur-tts](https://github.com/eslxxx/Uyghur-tts) to [jianchang512/Uyghur-tts](https://github.com/jianchang512/Uyghur-tts)

### [jianglin-code](https://github.com/jianglin-code)
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [triton-lang/triton](https://github.com/triton-lang/triton)
  > Development repository for the Triton language and compiler

### [jiji262](https://github.com/jiji262)
- 🌟 👤 [jiji262](https://github.com/jiji262) Starred [GiggleWang/MobileAgent-Android](https://github.com/GiggleWang/MobileAgent-Android)
  >  An Android-native autonomous agent that uses vision-language models to see and operate your   phone...
- 🌟 👤 [jiji262](https://github.com/jiji262) Starred [AaronL725/grok-register](https://github.com/AaronL725/grok-register)
  > 【已存活两个月+，无封号】批量注册稳定 Grok 账号，可直接导入 grok2api 使用，并且可以与cloudflare无限域名邮箱联动。通过1000+账号连续注册测试。
- 🌟 👤 [jiji262](https://github.com/jiji262) Starred [mm7894215/TokenTracker](https://github.com/mm7894215/TokenTracker)
  > Track token usage across 25 AI coding tools — Claude Code, Codex, Cursor, Gemini, Kiro, OpenCode, An...
- 🌟 👤 [jiji262](https://github.com/jiji262) Starred [yynxxxxx/Codex-5.5-codex-instruct-5.5](https://github.com/yynxxxxx/Codex-5.5-codex-instruct-5.5)

### [jixiaoyong](https://github.com/jixiaoyong)
- 🌟 👤 [jixiaoyong](https://github.com/jixiaoyong) Starred [DeusData/codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp)
  > High-performance code intelligence MCP server. Indexes codebases into a persistent knowledge graph —...
- 🌟 👤 [jixiaoyong](https://github.com/jixiaoyong) Starred [diegosouzapw/OmniRoute](https://github.com/diegosouzapw/OmniRoute)
  > Never stop coding. Free AI gateway: one endpoint, 231+ providers (50+ free), connect Claude Code, Co...

### [jmpews](https://github.com/jmpews)
- 🌟 👤 [jmpews](https://github.com/jmpews) Starred [NodePassProject/Everywhere](https://github.com/NodePassProject/Everywhere)
  > One app. Three networking engines. Your rules.

### [jonatan1024](https://github.com/jonatan1024)
- 🚀 👤 [jonatan1024](https://github.com/jonatan1024) Made [GoldSrc-one/summer-contest-2026](https://github.com/GoldSrc-one/summer-contest-2026) public

### [jpacg](https://github.com/jpacg)
- 🌟 👤 [jpacg](https://github.com/jpacg) Starred [GitKittys/AndroidPrybar](https://github.com/GitKittys/AndroidPrybar)
  > Android Native Trace 与逆向分析工具，用于函数包装执行、指令跟踪、JNI 调用观测与外部跳转控制

### [kaganisildak](https://github.com/kaganisildak)
- 🌟 👤 [kaganisildak](https://github.com/kaganisildak) Starred [project-lambda-zero/AILA](https://github.com/project-lambda-zero/AILA)
  > A Cyber Security AI Framework you need
- 🍴 👤 [kaganisildak](https://github.com/kaganisildak) Forked [project-lambda-zero/AILA](https://github.com/project-lambda-zero/AILA) to [kaganisildak/AILA](https://github.com/kaganisildak/AILA)
  > A Cyber Security AI Framework you need
- 🌟 👤 [kaganisildak](https://github.com/kaganisildak) Starred [ewhauser/gbash](https://github.com/ewhauser/gbash)
  > Pure Go bash implementation for agent sandboxes
- 🌟 👤 [kaganisildak](https://github.com/kaganisildak) Starred [mprahboamey/atom](https://github.com/mprahboamey/atom)
  > optical attention via wave interference in holographic crystals verified to float precision. the mat...

### [kernelzeroday](https://github.com/kernelzeroday)
- 🌟 👤 [kernelzeroday](https://github.com/kernelzeroday) Starred [ctxrs/ctx](https://github.com/ctxrs/ctx)
  > Search the coding agent history already on your machine

### [le31ei](https://github.com/le31ei)
- 🍴 👤 [le31ei](https://github.com/le31ei) Forked [findmover/wxread](https://github.com/findmover/wxread) to [le31ei/wxread](https://github.com/le31ei/wxread)
  > 微信读书刷时长助你成为霸榜“大佬” 

### [leavesCZY](https://github.com/leavesCZY)
- 🌟 👤 [leavesCZY](https://github.com/leavesCZY) Starred [MetaCubeX/ClashMetaForAndroid](https://github.com/MetaCubeX/ClashMetaForAndroid)
  > A rule-based tunnel for Android.

### [lionsoul2014](https://github.com/lionsoul2014)
- 🌟 👤 [lionsoul2014](https://github.com/lionsoul2014) Starred [qedus/osmpbf](https://github.com/qedus/osmpbf)
  > OpenStreetMap PBF file format parser in Go Lang.
- 🌟 👤 [lionsoul2014](https://github.com/lionsoul2014) Starred [paulmach/osm](https://github.com/paulmach/osm)
  > General purpose library for reading, writing and working with OpenStreetMap data

### [llsc12](https://github.com/llsc12)
- 🍴 👤 [llsc12](https://github.com/llsc12) Forked [dolphin-emu/dolphin](https://github.com/dolphin-emu/dolphin) to [llsc12/dolphin](https://github.com/llsc12/dolphin)
  > Dolphin is a GameCube / Wii emulator, allowing you to play games for these two platforms on PC with ...

### [luochuanyuewu](https://github.com/luochuanyuewu)
- 🌟 👤 [luochuanyuewu](https://github.com/luochuanyuewu) Starred [getopenscreen/openscreen](https://github.com/getopenscreen/openscreen)
  > Create stunning demos for free. Open-source, no subscriptions, no watermarks, and free for commercia...

### [luoyesiqiu](https://github.com/luoyesiqiu)
- 🌟 👤 [luoyesiqiu](https://github.com/luoyesiqiu) Starred [P4nda0s/reverse-skills](https://github.com/P4nda0s/reverse-skills)
  > Reverse engineering skills for Claude Code | 逆向工程 Claude Code Skills 插件

### [lxzh](https://github.com/lxzh)
- 🌟 👤 [lxzh](https://github.com/lxzh) Starred [Open-Agriculture/AgIsoStack-plus-plus](https://github.com/Open-Agriculture/AgIsoStack-plus-plus)
  > AgIsoStack++ is the completely free open-source C++ ISOBUS library for everyone
- 🌟 👤 [lxzh](https://github.com/lxzh) Starred [moehman/PoolEditDist](https://github.com/moehman/PoolEditDist)
  > PoolEdit XML Graphical User Interface Editor for creating ISOBUS object pools (dist)
- 🌟 👤 [lxzh](https://github.com/lxzh) Starred [lukashelmke/ISOBUS_Dummy](https://github.com/lukashelmke/ISOBUS_Dummy)
- 🌟 👤 [lxzh](https://github.com/lxzh) Starred [lukashelmke/IsoBus_VT_Client_ESP32_Repo](https://github.com/lukashelmke/IsoBus_VT_Client_ESP32_Repo)
- 🌟 👤 [lxzh](https://github.com/lxzh) Starred [AGRIForward/ROS2ISOBUS](https://github.com/AGRIForward/ROS2ISOBUS)
  > ROS 2 adapters for ISO 11783 / NMEA 2000

### [mdcgqq](https://github.com/mdcgqq)
- 🌟 👤 [mdcgqq](https://github.com/mdcgqq) Starred [UFund-Me/Qbot](https://github.com/UFund-Me/Qbot)
  > [🔥updating ...] AI 自动量化交易机器人(完全本地部署) AI-powered Quantitative Investment Research Platform. 📃 online ...
- 🍴 👤 [mdcgqq](https://github.com/mdcgqq) Forked [UFund-Me/Qbot](https://github.com/UFund-Me/Qbot) to [mdcgqq/Qbot](https://github.com/mdcgqq/Qbot)
  > [🔥updating ...] AI 自动量化交易机器人(完全本地部署) AI-powered Quantitative Investment Research Platform. 📃 online ...

### [mitsuakki](https://github.com/mitsuakki)
- 🍴 👤 [mitsuakki](https://github.com/mitsuakki) Forked [minestrate/php-client](https://github.com/minestrate/php-client) to [SaruMC/php-client](https://github.com/SaruMC/php-client)
  > PHP client for minestrate, especially for @pmmp

### [moonD4rk](https://github.com/moonD4rk)
- 🌟 👤 [moonD4rk](https://github.com/moonD4rk) Starred [lycorp-jp/sim-use](https://github.com/lycorp-jp/sim-use)
  > Give your AI agent eyes and hands on iOS Simulator and Android emulator/devices.

### [mos9527](https://github.com/mos9527)
- 🌟 👤 [mos9527](https://github.com/mos9527) Starred [naelstrof/blender-jiggle-physics](https://github.com/naelstrof/blender-jiggle-physics)
  > A fork of the Wiggle 2 addon with a verlet algorithm, focused on being usable in real-time within th...

### [mq1n](https://github.com/mq1n)
- 🌟 👤 [mq1n](https://github.com/mq1n) Starred [uphiago/recon-skills](https://github.com/uphiago/recon-skills)
  > 156 offensive security skills for recon and pentest. Field-validated techniques from 600+ targets ac...
- 🌟 👤 [mq1n](https://github.com/mq1n) Starred [ogulcancelik/herdr](https://github.com/ogulcancelik/herdr)
  > agent multiplexer that lives in your terminal.

### [mrexodia](https://github.com/mrexodia)
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [mprahboamey/atom](https://github.com/mprahboamey/atom)
  > optical attention via wave interference in holographic crystals verified to float precision. the mat...
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [ewhauser/gbash](https://github.com/ewhauser/gbash)
  > Pure Go bash implementation for agent sandboxes

### [natsumerinchan](https://github.com/natsumerinchan)
- 🌟 👤 [natsumerinchan](https://github.com/natsumerinchan) Starred [Hypostasis-Cat/HypoMux](https://github.com/Hypostasis-Cat/HypoMux)
  > CN Windows 多网卡带宽叠加工具。无需复杂配置，一键聚合多网卡（有线、Wi-Fi网卡、手机热点等），实现物理级多线下载与叠加网速。 EN Windows multi-NIC bandwidth...
- 🌟 👤 [natsumerinchan](https://github.com/natsumerinchan) Starred [Yizhou147/PiliPlus](https://github.com/Yizhou147/PiliPlus)
  > PiliPlus

### [nek0y4nsu](https://github.com/nek0y4nsu)
- 🌟 👤 [nek0y4nsu](https://github.com/nek0y4nsu) Starred [BishopFox/sliver](https://github.com/BishopFox/sliver)
  > Adversary Emulation Framework
- 🍴 👤 [nek0y4nsu](https://github.com/nek0y4nsu) Forked [BishopFox/sliver](https://github.com/BishopFox/sliver) to [nek0y4nsu/sliver](https://github.com/nek0y4nsu/sliver)
  > Adversary Emulation Framework
- 🌟 👤 [nek0y4nsu](https://github.com/nek0y4nsu) Starred [mlua-rs/mlua](https://github.com/mlua-rs/mlua)
  > High level Lua 5.5/5.4/5.3/5.2/5.1 (including LuaJIT) and Luau bindings to Rust with async/await sup...

### [netxfly](https://github.com/netxfly)
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [jiji262/douyin-downloader](https://github.com/jiji262/douyin-downloader)
  > A practical Douyin downloader for both single-item and profile batch downloads, with progress displa...
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [blevesearch/bleve](https://github.com/blevesearch/bleve)
  > A modern text/numeric/geo-spatial/vector indexing library for go

### [nico](https://github.com/nico)
- 🌟 👤 [nico](https://github.com/nico) Starred [apple/container](https://github.com/apple/container)
  > A tool for creating and running Linux containers using lightweight virtual machines on a Mac. It is ...

### [notify-bibi](https://github.com/notify-bibi)
- 🌟 👤 [notify-bibi](https://github.com/notify-bibi) Starred [mattpocock/skills](https://github.com/mattpocock/skills)
  > Skills for Real Engineers. Straight from my .claude directory.

### [null-dev](https://github.com/null-dev)
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [androidx/media](https://github.com/androidx/media)
  > Jetpack Media3 support libraries for media use cases, including ExoPlayer, an extensible media playe...
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [halide/Halide](https://github.com/halide/Halide)
  > a language for fast, portable data-parallel computation
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [antimof/UxPlay](https://github.com/antimof/UxPlay)
  > AirPlay Unix mirroring server
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [VPBox/Dev](https://github.com/VPBox/Dev)
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [selkies-project/selkies](https://github.com/selkies-project/selkies)
  > Open-Source Low-Latency Accelerated Linux WebRTC HTML5 Remote Desktop Streaming Platform for Self-Ho...

### [odzhan](https://github.com/odzhan)
- 🌟 👤 [odzhan](https://github.com/odzhan) Starred [microsoft/krabsetw](https://github.com/microsoft/krabsetw)
  > KrabsETW provides a modern C++ wrapper and a .NET wrapper around the low-level ETW trace consumption...

### [p-ranav](https://github.com/p-ranav)
- 🌟 👤 [p-ranav](https://github.com/p-ranav) Starred [burhop/wright](https://github.com/burhop/wright)
  > A digital engineer, designer, and mechanical analyst

### [pgarba](https://github.com/pgarba)
- 🌟 👤 [pgarba](https://github.com/pgarba) Starred [pgarba/ida-llm-explainer](https://github.com/pgarba/ida-llm-explainer)
  > AI-assisted IDA Pro plugin (local llama.cpp) for function explanations, renames, struct detection, a...

### [pizlonator](https://github.com/pizlonator)
- 🍴 👤 [pizlonator](https://github.com/pizlonator) Forked [Libre-WD-40/git](https://github.com/Libre-WD-40/git) to [pizlonator/git-WD-40](https://github.com/pizlonator/git-WD-40)
  > Git with WD-40 applied

### [plusls](https://github.com/plusls)
- 🌟 👤 [plusls](https://github.com/plusls) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 

### [poorevil](https://github.com/poorevil)
- 🌟 👤 [poorevil](https://github.com/poorevil) Starred [gowvp/owl](https://github.com/gowvp/owl)
  > GB28181 - 2022 NVR | ONVIF/RTMP/RTSP | ZLMediaKit | 本地 YOLO 检测 | 私有部署 | Frigate 中文平替
- 🍴 👤 [poorevil](https://github.com/poorevil) Forked [gowvp/owl](https://github.com/gowvp/owl) to [poorevil/owl](https://github.com/poorevil/owl)
  > GB28181 - 2022 NVR | ONVIF/RTMP/RTSP | ZLMediaKit | 本地 YOLO 检测 | 私有部署 | Frigate 中文平替

### [pwh-pwh](https://github.com/pwh-pwh)
- 🌟 👤 [pwh-pwh](https://github.com/pwh-pwh) Starred [Qeccentric/GlobalTraveling](https://github.com/Qeccentric/GlobalTraveling)
  > 基于 LSPosed 框架的 Android 系统级虚拟定位与环境伪装模块。Fakelocation module based on LSPosed which is available to GMS...

### [pwndorei](https://github.com/pwndorei)
- 🌟 👤 [pwndorei](https://github.com/pwndorei) Starred [SuhJae/koter](https://github.com/SuhJae/koter)
  > 과몰입 한국어 터미널 — 세종대왕님이 보고 계십니다

### [pwnipc](https://github.com/pwnipc)
- 🌟 👤 [pwnipc](https://github.com/pwnipc) Starred [stevenshiau/clonezilla](https://github.com/stevenshiau/clonezilla)
  > Clonezilla is a partition or disk clone tool similar to Norton Ghost®. It saves and restores only us...
- 🌟 👤 [pwnipc](https://github.com/pwnipc) Starred [J-jaeyoung/bad-epoll](https://github.com/J-jaeyoung/bad-epoll)

### [qi4L](https://github.com/qi4L)
- 🌟 👤 [qi4L](https://github.com/qi4L) Starred [realskyrin/capcap](https://github.com/realskyrin/capcap)
  > ⌘⌘ - A lightweight, native macOS screenshot tool that lives in your menu bar. Double-tap ⌘ Command t...
- 🌟 👤 [qi4L](https://github.com/qi4L) Starred [yj94/JADX-NO-MCP](https://github.com/yj94/JADX-NO-MCP)
  > JADX-NO-MCP with GUI and CLI APK analysis, early packer detection, Java call graph export, native JN...

### [r00tuser111](https://github.com/r00tuser111)
- 🌟 👤 [r00tuser111](https://github.com/r00tuser111) Starred [gadievron/raptor](https://github.com/gadievron/raptor)
  > Raptor turns Claude Code into a general-purpose AI offensive/defensive security agent. By using Clau...

### [realoriginal](https://github.com/realoriginal)
- 🌟 👤 [realoriginal](https://github.com/realoriginal) Starred [DevlarSchuh/iommu-driver-poc](https://github.com/DevlarSchuh/iommu-driver-poc)
- 🌟 👤 [realoriginal](https://github.com/realoriginal) Starred [BinaryHardening/cfgrip](https://github.com/BinaryHardening/cfgrip)
- 🌟 👤 [realoriginal](https://github.com/realoriginal) Starred [can1357/pon](https://github.com/can1357/pon)
  > Python 3.14, compiled to metal — JIT & AoT native compiler and runtime in Rust. Cranelift backend, r...
- 🌟 👤 [realoriginal](https://github.com/realoriginal) Starred [heeeyaaaa/vmem-decrypt](https://github.com/heeeyaaaa/vmem-decrypt)
  > Decrypt VMware vTPM-encrypted .vmem/.vmsn/.vmss/.nvram from the VM password, and flatten the .vmem t...

### [redbearder](https://github.com/redbearder)
- 🌟 👤 [redbearder](https://github.com/redbearder) Starred [inventree/InvenTree](https://github.com/inventree/InvenTree)
  > Open Source Inventory Management System

### [ret2ldz](https://github.com/ret2ldz)
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [0xdea/weggli-patterns](https://github.com/0xdea/weggli-patterns)
  > A collection of my weggli patterns to facilitate vulnerability research.
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [0xdea/semgrep-rules](https://github.com/0xdea/semgrep-rules)
  > A collection of my Semgrep rules to facilitate vulnerability research.
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [okankurtuluss/FridaBypassKit](https://github.com/okankurtuluss/FridaBypassKit)
  > Android Universal Anti-Bypass Framework - Root, SSL Pinning, Emulator & Debug Detection Bypass
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [talsec/Free-RASP-Android](https://github.com/talsec/Free-RASP-Android)
  > SDK providing threat detection and security monitoring for Android devices. Shield your app with fre...
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [MagicianGuo/Android-SamsungRoot](https://github.com/MagicianGuo/Android-SamsungRoot)
  > 三星手机root方法

### [roadwy](https://github.com/roadwy)
- 🌟 👤 [roadwy](https://github.com/roadwy) Starred [usestrix/strix](https://github.com/usestrix/strix)
  > Open-source AI penetration testing tool to find and fix your app’s vulnerabilities.

### [rogxo](https://github.com/rogxo)
- 🌟 👤 [rogxo](https://github.com/rogxo) Starred [OpenStarlight/StarlightGUI](https://github.com/OpenStarlight/StarlightGUI)
  > 一个强大而易用的 Windows 内核级工具箱，使用 C++/WinRT WinUI3 编写
- 🌟 👤 [rogxo](https://github.com/rogxo) Starred [xen-project/xen](https://github.com/xen-project/xen)
  > Mirror of the Xen Repository (PRs not accepted see: http://wiki.xenproject.org/wiki/Submitting_Xen_P...

### [sekaiacg](https://github.com/sekaiacg)
- 🌟 👤 [sekaiacg](https://github.com/sekaiacg) Starred [superturtlee/android_kernel_security](https://github.com/superturtlee/android_kernel_security)
  > security patchs for GKI
- 🌟 👤 [sekaiacg](https://github.com/sekaiacg) Starred [superturtlee/superturtlee](https://github.com/superturtlee/superturtlee)
- 🌟 👤 [sekaiacg](https://github.com/sekaiacg) Starred [superturtlee/Droidspaces-rootfs-KDE-builder](https://github.com/superturtlee/Droidspaces-rootfs-KDE-builder)
  > 为Droidspaces用户打造的带支持中文环境的KDE_rootfs,并配合on脚本或systemd快速启动

### [sh1ma](https://github.com/sh1ma)
- 🌟 👤 [sh1ma](https://github.com/sh1ma) Starred [shikijs/shiki](https://github.com/shikijs/shiki)
  > A beautiful yet powerful syntax highlighter

### [shellsec](https://github.com/shellsec)
- 🌟 👤 [shellsec](https://github.com/shellsec) Starred [RPCS3/rpcs3](https://github.com/RPCS3/rpcs3)
  > PlayStation 3 emulator and debugger

### [sin5678](https://github.com/sin5678)
- 🌟 👤 [sin5678](https://github.com/sin5678) Starred [dgoulet/torsocks](https://github.com/dgoulet/torsocks)
  > Library to torify application - NOTE: upstream has been moved to https://gitweb.torproject.org/torso...

### [smallevilbeast](https://github.com/smallevilbeast)
- 🌟 👤 [smallevilbeast](https://github.com/smallevilbeast) Starred [r8e8cd8/jni-forward-unidbg](https://github.com/r8e8cd8/jni-forward-unidbg)
  > Unidbg with real device JNI forwarding agent
- 🌟 👤 [smallevilbeast](https://github.com/smallevilbeast) Starred [NyaMisty/ipatool-py](https://github.com/NyaMisty/ipatool-py)
  > IPATool-py: download ipa easily
- 🌟 👤 [smallevilbeast](https://github.com/smallevilbeast) Starred [0xkuj/IPARanger](https://github.com/0xkuj/IPARanger)
  > GUI Interface for ipatool
- 🌟 👤 [smallevilbeast](https://github.com/smallevilbeast) Starred [aidserv/aid](https://github.com/aidserv/aid)
  > Authorize ios device ,实现爱思助手、鲨漏验机、3utools、imazing一样安装app、iOS私有应用市场

### [snowsnowsnows](https://github.com/snowsnowsnows)
- 🌟 👤 [snowsnowsnows](https://github.com/snowsnowsnows) Starred [can1357/pon](https://github.com/can1357/pon)
  > Python 3.14, compiled to metal — JIT & AoT native compiler and runtime in Rust. Cranelift backend, r...

### [stuxnet147](https://github.com/stuxnet147)
- 🌟 👤 [stuxnet147](https://github.com/stuxnet147) Starred [possible055/relace-mcp](https://github.com/possible055/relace-mcp)
  > Unofficial Relace MCP client with AI features. Personal project; not affiliated with or endorsed by ...

### [surunzi](https://github.com/surunzi)
- 🌟 👤 [surunzi](https://github.com/surunzi) Starred [djblue/tetris](https://github.com/djblue/tetris)
  > An almost complete tetris

### [taisuii](https://github.com/taisuii)
- 🌟 👤 [taisuii](https://github.com/taisuii) Starred [GitKittys/AndroidPrybar](https://github.com/GitKittys/AndroidPrybar)
  > Android Native Trace 与逆向分析工具，用于函数包装执行、指令跟踪、JNI 调用观测与外部跳转控制

### [theapache64](https://github.com/theapache64)
- 🍴 👤 [theapache64](https://github.com/theapache64) Forked [menubar-apps/PullBar](https://github.com/menubar-apps/PullBar) to [theapache64/PullBar](https://github.com/theapache64/PullBar)
  > GitHub pull requests in your menubar!

### [tijme](https://github.com/tijme)
- 🌟 👤 [tijme](https://github.com/tijme) Starred [dockur/windows](https://github.com/dockur/windows)
  > Windows inside a Docker container.
- 🌟 👤 [tijme](https://github.com/tijme) Starred [dockur/windows-arm](https://github.com/dockur/windows-arm)
  > Windows for ARM in a Docker container.

### [tomLadder](https://github.com/tomLadder)
- 🌟 👤 [tomLadder](https://github.com/tomLadder) Starred [metafloor/bwip-js](https://github.com/metafloor/bwip-js)
  > Barcode Writer in Pure JavaScript

### [topcoco](https://github.com/topcoco)
- 🌟 👤 [topcoco](https://github.com/topcoco) Starred [Lightrion/skills](https://github.com/Lightrion/skills)
  > Claude Agent Skills for AOSP platform development and Android app development, powered by the Lightr...
- 🌟 👤 [topcoco](https://github.com/topcoco) Starred [OthmanAdi/planning-with-files](https://github.com/OthmanAdi/planning-with-files)
  > Persistent file-based planning for AI coding agents and long-running agentic tasks. Crash-proof mark...
- 🌟 👤 [topcoco](https://github.com/topcoco) Starred [AVIDS2/memorix](https://github.com/AVIDS2/memorix)
  > Open-source cross-agent memory layer for coding agents via MCP. Compatible with Claude Code, Codex, ...
- 🌟 👤 [topcoco](https://github.com/topcoco) Starred [Edlineas/aivectormemory](https://github.com/Edlineas/aivectormemory)
  > aivectormemory 是一款基于 Model Context Protocol (MCP) 开发的OpenClaw、OpenCode、ClaudeCodeAI记忆管理工具。它专门为 Claud...
- 🌟 👤 [topcoco](https://github.com/topcoco) Starred [thedotmack/claude-mem](https://github.com/thedotmack/claude-mem)
  > Persistent Context Across Sessions for Every Agent –  Captures everything your agent does during ses...

### [tranleduy2000](https://github.com/tranleduy2000)
- 🌟 👤 [tranleduy2000](https://github.com/tranleduy2000) Starred [axkr/sympy2symja](https://github.com/axkr/sympy2symja)
  > SymPy (Python) to Symja (Java) simple math expressions transpiler

### [uvbs](https://github.com/uvbs)
- 🌟 👤 [uvbs](https://github.com/uvbs) Starred [fanchmwrt/fanchmwrt](https://github.com/fanchmwrt/fanchmwrt)
  > An open-source enterprise-grade router system based on OpenWrt.

### [veritas501](https://github.com/veritas501)
- 🌟 👤 [veritas501](https://github.com/veritas501) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 

### [w00tzenheimer](https://github.com/w00tzenheimer)
- 🌟 👤 [w00tzenheimer](https://github.com/w00tzenheimer) Starred [IvanMurzak/Unity-MCP](https://github.com/IvanMurzak/Unity-MCP)
  > AI Skills, MCP Tools, and CLI for Unity Engine. Full AI develop and test loop. Use cli for quick set...

### [wa1ki0g](https://github.com/wa1ki0g)
- 🌟 👤 [wa1ki0g](https://github.com/wa1ki0g) Starred [projectdiscovery/httpx](https://github.com/projectdiscovery/httpx)
  > httpx is a fast and multi-purpose HTTP toolkit that allows running multiple probes using the retryab...

### [winezer0](https://github.com/winezer0)
- 🌟 👤 [winezer0](https://github.com/winezer0) Starred [AmintaCCCP/GithubStarsManager](https://github.com/AmintaCCCP/GithubStarsManager)
  > An app for managing github starred repositories. 
- 🌟 👤 [winezer0](https://github.com/winezer0) Starred [izumi0uu/better-github-stars-manager](https://github.com/izumi0uu/better-github-stars-manager)
  > Manage your GitHub starred repos: tag, note, filter, search across thousands of stars. Chrome MV3 ex...
- 🌟 👤 [winezer0](https://github.com/winezer0) Starred [charmbracelet/crush](https://github.com/charmbracelet/crush)
  > Glamourous agentic coding for all 💘

### [wrvenkat](https://github.com/wrvenkat)
- 🌟 👤 [wrvenkat](https://github.com/wrvenkat) Starred [PolyArch/humanize](https://github.com/PolyArch/humanize)
  > From Automated Idea Factory to Realization

### [wszqkzqk](https://github.com/wszqkzqk)
- 🌟 👤 [wszqkzqk](https://github.com/wszqkzqk) Starred [MoonshotAI/kimi-code](https://github.com/MoonshotAI/kimi-code)
  > Kimi Code CLI  —  The Starting Point for Next-Gen Agents

### [wy876](https://github.com/wy876)
- 🌟 👤 [wy876](https://github.com/wy876) Starred [xiaotuzhenji/xianyu-lite-v2](https://github.com/xiaotuzhenji/xianyu-lite-v2)
  > 闲鱼商品自动发布管理系统-简化版v2

### [xiaoweigege](https://github.com/xiaoweigege)
- 🌟 👤 [xiaoweigege](https://github.com/xiaoweigege) Starred [mobile-next/mobile-mcp](https://github.com/mobile-next/mobile-mcp)
  > Model Context Protocol Server for Mobile Automation and Scraping (iOS, Android, Emulators, Simulator...
- 🌟 👤 [xiaoweigege](https://github.com/xiaoweigege) Starred [Wanghaibo10/Reqable-Mcp](https://github.com/Wanghaibo10/Reqable-Mcp)
  > Reqable mcp工具

### [xsser](https://github.com/xsser)
- 🌟 👤 [xsser](https://github.com/xsser) Starred [can1357/oh-my-pi](https://github.com/can1357/oh-my-pi)
  > ⌥  AI Coding agent for the terminal — hash-anchored edits, optimized tool harness, LSP, Python, brow...

### [xunchahaha](https://github.com/xunchahaha)
- 🌟 👤 [xunchahaha](https://github.com/xunchahaha) Starred [kldhsh123/Afterglow](https://github.com/kldhsh123/Afterglow)
  > 使用社交软件聊天记录结合向量数据库让AI更好的扮演对方的角色，在不微调模型的情况下可以达到可观的效果。把曾经的美好，续成往后的陪伴。

### [yahch](https://github.com/yahch)
- 🌟 👤 [yahch](https://github.com/yahch) Starred [tt-a1i/archify](https://github.com/tt-a1i/archify)
  > Any agent Skill: generate beautiful architecture diagrams with dark/light theme toggle and PNG/JPEG/...

### [yfe404](https://github.com/yfe404)
- 🌟 👤 [yfe404](https://github.com/yfe404) Starred [scrapfly/Antibot-Detector](https://github.com/scrapfly/Antibot-Detector)
  >  Real-time detection of anti-bot systems, CAPTCHAs & fingerprinting techniques. Identifies Cloudflar...
- 🍴 👤 [yfe404](https://github.com/yfe404) Forked [scrapfly/Antibot-Detector](https://github.com/scrapfly/Antibot-Detector) to [yfe404/Antibot-Detector](https://github.com/yfe404/Antibot-Detector)
  >  Real-time detection of anti-bot systems, CAPTCHAs & fingerprinting techniques. Identifies Cloudflar...
- 🌟 👤 [yfe404](https://github.com/yfe404) Starred [makerspet/oomwoo](https://github.com/makerspet/oomwoo)
  > Open-source vacuum robot cleaner

### [yhs0602](https://github.com/yhs0602)
- 🌟 👤 [yhs0602](https://github.com/yhs0602) Starred [rndhouse/mixmod](https://github.com/rndhouse/mixmod)
  > Reduce frontier LLM cost with supervised worker models.

### [yhy0](https://github.com/yhy0)
- 🌟 👤 [yhy0](https://github.com/yhy0) Starred [hangwin/mcp-chrome](https://github.com/hangwin/mcp-chrome)
  > Chrome MCP Server is a Chrome extension-based Model Context Protocol (MCP) server that exposes your ...

### [yinsel](https://github.com/yinsel)
- 🌟 👤 [yinsel](https://github.com/yinsel) Starred [nutstore/obsidian-nutstore-sync](https://github.com/nutstore/obsidian-nutstore-sync)
- 🌟 👤 [yinsel](https://github.com/yinsel) Starred [JosuPalacios99/ADWSHound](https://github.com/JosuPalacios99/ADWSHound)

### [yunsmall](https://github.com/yunsmall)
- 🌟 👤 [yunsmall](https://github.com/yunsmall) Starred [readest/readest](https://github.com/readest/readest)
  > Readest is a modern, feature-rich ebook reader designed for avid readers offering seamless cross-pla...

### [yzddmr6](https://github.com/yzddmr6)
- 🌟 👤 [yzddmr6](https://github.com/yzddmr6) Starred [omnigent-ai/omnigent](https://github.com/omnigent-ai/omnigent)
  > Omnigent is an open-source AI agent framework and meta-harness: orchestrate Claude Code, Codex, Curs...
- 🌟 👤 [yzddmr6](https://github.com/yzddmr6) Starred [TokenRollAI/claude-code-workflow-research](https://github.com/TokenRollAI/claude-code-workflow-research)
  > Research notes and captured artifacts for Claude Code workflow orchestration

### [zer0def](https://github.com/zer0def)
- 🌟 👤 [zer0def](https://github.com/zer0def) Starred [intel/beignet](https://github.com/intel/beignet)
  > Beignet is an open source implementation of the OpenCL specification - a generic compute oriented AP...

### [zetaloop](https://github.com/zetaloop)
- 🌟 👤 [zetaloop](https://github.com/zetaloop) Starred [stats-organization/github-stats-extended](https://github.com/stats-organization/github-stats-extended)
  > Dynamically generate GitHub stats for your READMEs.

### [zhulangpi](https://github.com/zhulangpi)
- 🌟 👤 [zhulangpi](https://github.com/zhulangpi) Starred [ForceInjection/AI-fundamentals](https://github.com/ForceInjection/AI-fundamentals)
  > AI 基础知识 - GPU 架构、CUDA 编程、大模型基础及AI Agent 相关知识。
- 🌟 👤 [zhulangpi](https://github.com/zhulangpi) Starred [NVIDIA/nccl](https://github.com/NVIDIA/nccl)
  > Optimized primitives for collective multi-GPU communication
- 🌟 👤 [zhulangpi](https://github.com/zhulangpi) Starred [NVIDIA/open-gpu-kernel-modules](https://github.com/NVIDIA/open-gpu-kernel-modules)
  > NVIDIA Linux open GPU kernel module source

### [zjw-swun](https://github.com/zjw-swun)
- 🌟 👤 [zjw-swun](https://github.com/zjw-swun) Starred [JetQiao/football-prediction-skill](https://github.com/JetQiao/football-prediction-skill)
  > 竞彩足球智能预测 Skill：Dixon-Coles、赔率价值、赛前情报、回测与离线 HTML 报告

### [zu1k](https://github.com/zu1k)
- 🌟 👤 [zu1k](https://github.com/zu1k) Starred [ricccrd/dd](https://github.com/ricccrd/dd)


---
*最后更新于 2026-07-03 21:06:23 UTC*
*历史记录保存在 `archive` 目录中。*
