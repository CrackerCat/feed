# 每日 GitHub 动态 (2026-07-14)

我关注用户的今日公开动态 (每15分钟更新)。

## 今日动态

### [0x1c1101](https://github.com/0x1c1101)
- 🌟 👤 [0x1c1101](https://github.com/0x1c1101) Starred [crackmesone/crackmes-re-dataset](https://github.com/crackmesone/crackmes-re-dataset)
  > Labeled reverse-engineering dataset over 4,598 crackmes: flags, verifier scripts, and normalized obf...

### [0xSh4dy](https://github.com/0xSh4dy)
- 🌟 👤 [0xSh4dy](https://github.com/0xSh4dy) Starred [w4z3d/cinder](https://github.com/w4z3d/cinder)
  > Cinder is a WIP C compiler

### [0xacb](https://github.com/0xacb)
- 🌟 👤 [0xacb](https://github.com/0xacb) Starred [ethiack/ethibench](https://github.com/ethiack/ethibench)
  > Ethiack AI Pentesting Benchmark Framework

### [0xbinder](https://github.com/0xbinder)
- 🌟 👤 [0xbinder](https://github.com/0xbinder) Starred [shark4ce/Ghidra-SandBlaster-Plugin](https://github.com/shark4ce/Ghidra-SandBlaster-Plugin)
  >  The SandBlaster Plugin is a sophisticated extension for Ghidra that seamlessly integrates the funct...

### [34306](https://github.com/34306)
- 🌟 👤 [34306](https://github.com/34306) Starred [okalachev/flix](https://github.com/okalachev/flix)
  > Making an ESP32-based quadcopter from scratch

### [AlexiaChen](https://github.com/AlexiaChen)
- 🌟 👤 [AlexiaChen](https://github.com/AlexiaChen) Starred [arcships/light-ocr](https://github.com/arcships/light-ocr)
  > C++17 PP-OCRv6 small OCR core with an asynchronous Node-API adapter

### [AlphabugX](https://github.com/AlphabugX)
- 🌟 👤 [AlphabugX](https://github.com/AlphabugX) Starred [can1357/oh-my-pi](https://github.com/can1357/oh-my-pi)
  > ⌥  AI Coding agent for the terminal — hash-anchored edits, optimized tool harness, LSP, Python, brow...

### [Anatdx](https://github.com/Anatdx)
- 🍴 👤 [Anatdx](https://github.com/Anatdx) Forked [ReSukiSU-GKI/GKI_KernelSU_SUSFS](https://github.com/ReSukiSU-GKI/GKI_KernelSU_SUSFS) to [Anatdx/GKI_KernelSU_SUSFS](https://github.com/Anatdx/GKI_KernelSU_SUSFS)
  > 包含 ReSukiSU 和 SUSFS 的GKI内核自动构建仓库，支持5.10 - 6.12的全部内核版本，支持以下可选功能: BBG & DroidSpaces & Extended zram al...

### [AnkioTomas](https://github.com/AnkioTomas)
- 🌟 👤 [AnkioTomas](https://github.com/AnkioTomas) Starred [AnkioTomas/cherry-markdown-next-desktop](https://github.com/AnkioTomas/cherry-markdown-next-desktop)
  > cherry-markdown-next桌面端

### [BlackHole1](https://github.com/BlackHole1)
- 🌟 👤 [BlackHole1](https://github.com/BlackHole1) Starred [cosscom/coss](https://github.com/cosscom/coss)
  > coss.com/ui is the official design system of Cal.com

### [Brinmon](https://github.com/Brinmon)
- 🌟 👤 [Brinmon](https://github.com/Brinmon) Starred [MDX-Tom/gpt-5.6-instruct](https://github.com/MDX-Tom/gpt-5.6-instruct)
  > A Codex CLI jailbreak prompt and test pack for gpt-5.6-sol. 针对 gpt-5.6 系列的 Codex CLI 破甲提示词与测试包。
- 🌟 👤 [Brinmon](https://github.com/Brinmon) Starred [cobusgreyling/loop-engineering](https://github.com/cobusgreyling/loop-engineering)
  > Practical patterns, starters & CLI tools for loop engineering with AI coding agents. Design systems ...

### [ColdWindScholar](https://github.com/ColdWindScholar)
- 🌟 👤 [ColdWindScholar](https://github.com/ColdWindScholar) Starred [Vatuu/silent-hill-decomp](https://github.com/Vatuu/silent-hill-decomp)
  > An in-progress decompilation of the 1.1 US release of Silent Hill on the Playstation 1.
- 🌟 👤 [ColdWindScholar](https://github.com/ColdWindScholar) Starred [LittleBitUA/DownpourRecomp](https://github.com/LittleBitUA/DownpourRecomp)
  > Silent Hill: Downpour — native Windows PC port via static recompilation of the Xbox 360 binary. v1.0...
- 🌟 👤 [ColdWindScholar](https://github.com/ColdWindScholar) Starred [dreamingmoths/memory-of-alessa](https://github.com/dreamingmoths/memory-of-alessa)
  > In-progress matching decompilation of Silent Hill 3

### [Cxk4ng](https://github.com/Cxk4ng)
- 🌟 👤 [Cxk4ng](https://github.com/Cxk4ng) Starred [LunarBar-app/LunarBar](https://github.com/LunarBar-app/LunarBar)
  > A compact lunar calendar for your macOS menu bar.

### [DarkLineX](https://github.com/DarkLineX)
- 🌟 👤 [DarkLineX](https://github.com/DarkLineX) Starred [tc3650/CVE-2026-43499-armv7](https://github.com/tc3650/CVE-2026-43499-armv7)
- 🌟 👤 [DarkLineX](https://github.com/DarkLineX) Starred [huiliyi37/Tianshu-Tui](https://github.com/huiliyi37/Tianshu-Tui)
  > 天枢 (Tianshu) 是一个全功能、高性能的终端编程智能体运行时（TUI）。它跳出了传统 AI 编程助手把大模型仅当成“工具”的局限，基于认知虚拟机 (CVM)、自感知层和信息素（Stigmerg...

### [DavidBuchanan314](https://github.com/DavidBuchanan314)
- 🌟 👤 [DavidBuchanan314](https://github.com/DavidBuchanan314) Starred [kuzeyardabulut/CVE-2024-1065](https://github.com/kuzeyardabulut/CVE-2024-1065)
  > Page Cache Exploit for CVE-2024-1065

### [Dispa1r](https://github.com/Dispa1r)
- 🌟 👤 [Dispa1r](https://github.com/Dispa1r) Starred [bytedance/bhook](https://github.com/bytedance/bhook)
  > :fire: ByteHook is an Android PLT hook library which supports armeabi-v7a, arm64-v8a, x86 and x86_64...
- 🌟 👤 [Dispa1r](https://github.com/Dispa1r) Starred [caprinux/rel-fuscate](https://github.com/caprinux/rel-fuscate)
  > A novel obfuscation technique for ELF programs by abusing symbol resolution metadata to break disass...
- 🌟 👤 [Dispa1r](https://github.com/Dispa1r) Starred [ilyakurdyukov/mediatek_flash](https://github.com/ilyakurdyukov/mediatek_flash)
  > MediaTek firmware dumper for Linux 
- 🌟 👤 [Dispa1r](https://github.com/Dispa1r) Starred [laolanya123/rmcn-physx](https://github.com/laolanya123/rmcn-physx)
  > 失控进化 PhysX：Unity 2022.3 / PhysX 4.1.2 场景遍历、可见性检测与准心碰撞模型绘制
- 🌟 👤 [Dispa1r](https://github.com/Dispa1r) Starred [MobiusM/CVE-2026-43499](https://github.com/MobiusM/CVE-2026-43499)
  > CVE-2026-43499 PoC

### [Dump-GUY](https://github.com/Dump-GUY)
- 🌟 👤 [Dump-GUY](https://github.com/Dump-GUY) Starred [KernelErr/dnSpy.Extension.MCP](https://github.com/KernelErr/dnSpy.Extension.MCP)
  > MCP extension for dnSpy.

### [EgeBalci](https://github.com/EgeBalci)
- 🌟 👤 [EgeBalci](https://github.com/EgeBalci) Starred [sosedoff/pgweb](https://github.com/sosedoff/pgweb)
  > Cross-platform client for PostgreSQL databases

### [FANGG3](https://github.com/FANGG3)
- 🌟 👤 [FANGG3](https://github.com/FANGG3) Starred [ioncodes/chipi](https://github.com/ioncodes/chipi)
  > A declarative instruction decoder and disassembler generator using a custom DSL
- 🍴 👤 [FANGG3](https://github.com/FANGG3) Forked [zhuzhu-Top/demist](https://github.com/zhuzhu-Top/demist) to [FANGG3/demist](https://github.com/FANGG3/demist)

### [Fansirsqi](https://github.com/Fansirsqi)
- 🌟 👤 [Fansirsqi](https://github.com/Fansirsqi) Starred [iniwap/AIWriteX](https://github.com/iniwap/AIWriteX)
  > AIWriteX - 微信公众号全自动AI工具：全网热搜舆情聚合+趋势分析+爆款选题+文章采集+一键生成排版发布 | AI自动配图 | 去AI味、过朱雀检测 | 支持小红书/百家号/头条等多平台 | ...

### [Felix3322](https://github.com/Felix3322)
- 🌟 👤 [Felix3322](https://github.com/Felix3322) Starred [KSwordDEV/KswordFrame](https://github.com/KSwordDEV/KswordFrame)
  > Windows C++ GUI Work Frame
- 🌟 👤 [Felix3322](https://github.com/Felix3322) Starred [ljy2367765883/EDR-DataProtector](https://github.com/ljy2367765883/EDR-DataProtector)
  > An experimental intranet security defense EDR

### [FldBudin](https://github.com/FldBudin)
- 🌟 👤 [FldBudin](https://github.com/FldBudin) Starred [MDX-Tom/gpt-5.6-instruct](https://github.com/MDX-Tom/gpt-5.6-instruct)
  > A Codex CLI jailbreak prompt and test pack for gpt-5.6-sol. 针对 gpt-5.6 系列的 Codex CLI 破甲提示词与测试包。

### [GangJust](https://github.com/GangJust)
- 🌟 👤 [GangJust](https://github.com/GangJust) Starred [MDX-Tom/gpt-5.6-instruct](https://github.com/MDX-Tom/gpt-5.6-instruct)
  > A Codex CLI jailbreak prompt and test pack for gpt-5.6-sol. 针对 gpt-5.6 系列的 Codex CLI 破甲提示词与测试包。

### [GitSquared](https://github.com/GitSquared)
- 🌟 👤 [GitSquared](https://github.com/GitSquared) Starred [vercel/workflow](https://github.com/vercel/workflow)
  > Workflow SDK: Build durable, reliable, and observable apps and AI Agents in TypeScript
- 🌟 👤 [GitSquared](https://github.com/GitSquared) Starred [garden-co/jazz](https://github.com/garden-co/jazz)
  > A local-first relational database for the browser, React Native and your backend.

### [Grigory-Rylov](https://github.com/Grigory-Rylov)
- 🌟 👤 [Grigory-Rylov](https://github.com/Grigory-Rylov) Starred [CGAL/cgal](https://github.com/CGAL/cgal)
  > The public CGAL repository, see the README below
- 🌟 👤 [Grigory-Rylov](https://github.com/Grigory-Rylov) Starred [CommonWealthRobotics/manifold3d-java](https://github.com/CommonWealthRobotics/manifold3d-java)
  > Geometry library for topological robustness
- 🍴 👤 [Grigory-Rylov](https://github.com/Grigory-Rylov) Forked [CommonWealthRobotics/manifold3d-java](https://github.com/CommonWealthRobotics/manifold3d-java) to [Grigory-Rylov/manifold3d-java](https://github.com/Grigory-Rylov/manifold3d-java)
  > Geometry library for topological robustness

### [HdShare](https://github.com/HdShare)
- 🌟 👤 [HdShare](https://github.com/HdShare) Starred [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)
  > Sub2API 一站式开源中转服务，让 Claude、Openai 、Gemini、Grok订阅统一接入，支持拼车共享，更高效分摊成本，原生工具无缝使用。
- 🌟 👤 [HdShare](https://github.com/HdShare) Starred [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)
  > Wrap Antigravity, ChatGPT Codex, Claude Code, Grok Build as an OpenAI/Gemini/Claude/Codex compatible...

### [JoasASantos](https://github.com/JoasASantos)
- 🚀 👤 [JoasASantos](https://github.com/JoasASantos) Made [Red-Team-Leaders/RTL-Labs](https://github.com/Red-Team-Leaders/RTL-Labs) public

### [Knight-ZXW](https://github.com/Knight-ZXW)
- 🍴 👤 [Knight-ZXW](https://github.com/Knight-ZXW) Forked [block/stoic](https://github.com/block/stoic) to [Knight-ZXW/stoic](https://github.com/Knight-ZXW/stoic)
  > Run code within any debuggable Android process, without modifying its APK

### [KyuubiRan](https://github.com/KyuubiRan)
- 🌟 👤 [KyuubiRan](https://github.com/KyuubiRan) Starred [MDX-Tom/gpt-5.6-instruct](https://github.com/MDX-Tom/gpt-5.6-instruct)
  > A Codex CLI jailbreak prompt and test pack for gpt-5.6-sol. 针对 gpt-5.6 系列的 Codex CLI 破甲提示词与测试包。

### [Laurie-Lin](https://github.com/Laurie-Lin)
- 🍴 👤 [Laurie-Lin](https://github.com/Laurie-Lin) Forked [aristocratos/btop](https://github.com/aristocratos/btop) to [Laurie-Lin/btop](https://github.com/Laurie-Lin/btop)
  > A monitor of resources

### [LumitoLuma](https://github.com/LumitoLuma)
- 🍴 👤 [LumitoLuma](https://github.com/LumitoLuma) Forked [ONLYOFFICE/server](https://github.com/ONLYOFFICE/server) to [LumitoLuma/server](https://github.com/LumitoLuma/server)
  > The backend server software layer which is the part of ONLYOFFICE Document Server and is the base fo...

### [M0Rf30](https://github.com/M0Rf30)
- 🌟 👤 [M0Rf30](https://github.com/M0Rf30) Starred [nexu-io/open-design](https://github.com/nexu-io/open-design)
  > 🎨 The open-source Claude Design alternative. 🖥️ Local-first desktop app. 🖼️ Your coding agent become...

### [MartinKayJr](https://github.com/MartinKayJr)
- 🌟 👤 [MartinKayJr](https://github.com/MartinKayJr) Starred [niasand/gg-keeper](https://github.com/niasand/gg-keeper)
  > Giffgaff 保号助手 - 极简流量消耗工具 (120KB/次), forked from dennischancs/gg-keeper
- 🍴 👤 [MartinKayJr](https://github.com/MartinKayJr) Forked [niasand/gg-keeper](https://github.com/niasand/gg-keeper) to [MartinKayJr/gg-keeper](https://github.com/MartinKayJr/gg-keeper)
  > Giffgaff 保号助手 - 极简流量消耗工具 (120KB/次), forked from dennischancs/gg-keeper

### [MlgmXyysd](https://github.com/MlgmXyysd)
- 🌟 👤 [MlgmXyysd](https://github.com/MlgmXyysd) Starred [ginsongsong/Intel_SOC_VCA_kernel](https://github.com/ginsongsong/Intel_SOC_VCA_kernel)
- 🌟 👤 [MlgmXyysd](https://github.com/MlgmXyysd) Starred [chainsx/build](https://github.com/chainsx/build)
  > Armbian Linux Build Framework
- 🌟 👤 [MlgmXyysd](https://github.com/MlgmXyysd) Starred [chainsx/intel-vca2-dkms](https://github.com/chainsx/intel-vca2-dkms)

### [Mufanc](https://github.com/Mufanc)
- 🌟 👤 [Mufanc](https://github.com/Mufanc) Starred [apple/container](https://github.com/apple/container)
  > A tool for creating and running Linux containers using lightweight virtual machines on a Mac. It is ...

### [Nep-Timeline](https://github.com/Nep-Timeline)
- 🌟 👤 [Nep-Timeline](https://github.com/Nep-Timeline) Starred [MinicraftPlus/minicraft-plus-revived](https://github.com/MinicraftPlus/minicraft-plus-revived)
  > Minicraft+, an extension of Notch's original minicraft project, with tons more features.

### [No-Github](https://github.com/No-Github)
- 🌟 👤 [No-Github](https://github.com/No-Github) Starred [anywhere-labs/Agents-Anywhere](https://github.com/anywhere-labs/Agents-Anywhere)
  > Control any coding agent on any device — from your phone.
- 🌟 👤 [No-Github](https://github.com/No-Github) Starred [google/mantis](https://github.com/google/mantis)
  > A modular, stack-agnostic toolkit of security review skills for AI coding agents to autonomously fin...

### [NyaMisty](https://github.com/NyaMisty)
- 🍴 👤 [NyaMisty](https://github.com/NyaMisty) Forked [fairydreaming/llama.cpp](https://github.com/fairydreaming/llama.cpp) to [NyaMisty/llama.cpp](https://github.com/NyaMisty/llama.cpp)
  > LLM inference in C/C++

### [PIKACHUIM](https://github.com/PIKACHUIM)
- 🌟 👤 [PIKACHUIM](https://github.com/PIKACHUIM) Starred [laobamac/MirageWallpaper](https://github.com/laobamac/MirageWallpaper)
  > A live wallpaper engine for macOS, supporting Web/video/scene wallpaper
- 🍴 👤 [PIKACHUIM](https://github.com/PIKACHUIM) Forked [laobamac/MirageWallpaper](https://github.com/laobamac/MirageWallpaper) to [PIKACHUIM/MirageWallpaper](https://github.com/PIKACHUIM/MirageWallpaper)
  > A live wallpaper engine for macOS, supporting Web/video/scene wallpaper

### [PenguinAndy](https://github.com/PenguinAndy)
- 🌟 👤 [PenguinAndy](https://github.com/PenguinAndy) Starred [JustVugg/colibri](https://github.com/JustVugg/colibri)
  > Run GLM-5.2 (744B MoE) on a 25GB-RAM consumer machine — pure C, zero deps, experts streamed from dis...

### [RohitVerma882](https://github.com/RohitVerma882)
- 🌟 👤 [RohitVerma882](https://github.com/RohitVerma882) Starred [Miuzarte/ScrcpyForAndroid](https://github.com/Miuzarte/ScrcpyForAndroid)
  > scrcpy android client (unofficial)
- 🌟 👤 [RohitVerma882](https://github.com/RohitVerma882) Starred [flyfishxu/Kadb](https://github.com/flyfishxu/Kadb)
  > A Kotlin Multiplatform library to connect Android devices directly without ADB binary file.

### [Ryuchen](https://github.com/Ryuchen)
- 🚀 👤 [Ryuchen](https://github.com/Ryuchen) Made [Ryuchen/XA](https://github.com/Ryuchen/XA) public
  > 兴安电竞配套的管理系统

### [SHangwendada](https://github.com/SHangwendada)
- 🌟 👤 [SHangwendada](https://github.com/SHangwendada) Starred [MDX-Tom/gpt-5.6-instruct](https://github.com/MDX-Tom/gpt-5.6-instruct)
  > A Codex CLI jailbreak prompt and test pack for gpt-5.6-sol. 针对 gpt-5.6 系列的 Codex CLI 破甲提示词与测试包。
- 🍴 👤 [SHangwendada](https://github.com/SHangwendada) Forked [MDX-Tom/gpt-5.6-instruct](https://github.com/MDX-Tom/gpt-5.6-instruct) to [SHangwendada/gpt-5.6-instruct](https://github.com/SHangwendada/gpt-5.6-instruct)
  > A Codex CLI jailbreak prompt and test pack for gpt-5.6-sol. 针对 gpt-5.6 系列的 Codex CLI 破甲提示词与测试包。

### [SPuerBRead](https://github.com/SPuerBRead)
- 🌟 👤 [SPuerBRead](https://github.com/SPuerBRead) Starred [deusyu/harness-engineering](https://github.com/deusyu/harness-engineering)
  > Harness Engineering 学习指南 — 从概念理解到独立实践的深度学习档案

### [SakuraKoi](https://github.com/SakuraKoi)
- 🌟 👤 [SakuraKoi](https://github.com/SakuraKoi) Starred [cursey/safetyhook](https://github.com/cursey/safetyhook)
  > C++23 procedure hooking library.

### [Seyud](https://github.com/Seyud)
- 🌟 👤 [Seyud](https://github.com/Seyud) Starred [Anatdx/YukiZygisk](https://github.com/Anatdx/YukiZygisk)
  > A new kernel-level Zygisk implementation designed to explore better, cleaner, and more flexible ways...

### [Skorpion96](https://github.com/Skorpion96)
- 🌟 👤 [Skorpion96](https://github.com/Skorpion96) Starred [impost0r/Rie](https://github.com/impost0r/Rie)
  > CVE-2026-43724 - Apple's published enough advisories about the issue, I'm not getting paid for any o...

### [Sndav](https://github.com/Sndav)
- 🌟 👤 [Sndav](https://github.com/Sndav) Starred [TwoSevenOneT/EDRChoker](https://github.com/TwoSevenOneT/EDRChoker)
  > A tool uses the QoS Policy (Pacer.sys) to throttle Endpoint Detection and Response (EDR) agents from...

### [SodaMilk233](https://github.com/SodaMilk233)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [Is-Daouda/is-Engine-Demo](https://github.com/Is-Daouda/is-Engine-Demo)
  > This version is made for 35 years of Super Mario Bros!
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [djskncxm/DuckVM](https://github.com/djskncxm/DuckVM)
  > 基于文本匹配的vmp还原思路
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [jiangkang11134/SmartCode](https://github.com/jiangkang11134/SmartCode)
  > 根据github开源的模仿Claude Code的项目，对记忆模块进行优化修改

### [SoyBeanMilkx](https://github.com/SoyBeanMilkx)
- 🌟 👤 [SoyBeanMilkx](https://github.com/SoyBeanMilkx) Starred [MobiusM/CVE-2026-43499](https://github.com/MobiusM/CVE-2026-43499)
  > CVE-2026-43499 PoC

### [SsageParuders](https://github.com/SsageParuders)
- 🌟 👤 [SsageParuders](https://github.com/SsageParuders) Starred [x-spy/CVE-2026-43499-popsicle](https://github.com/x-spy/CVE-2026-43499-popsicle)
  > CVE-2026-43499 Implementation for 6.12.23-android16-5-g75e9b1c7ae7c-abogki463945075-4k

### [StanleyNull](https://github.com/StanleyNull)
- 🌟 👤 [StanleyNull](https://github.com/StanleyNull) Starred [MCSManager/MCSManager](https://github.com/MCSManager/MCSManager)
  > Quick deployment, distributed, multi-user, modern management panel for Minecraft and Steam game serv...

### [TUGOhost](https://github.com/TUGOhost)
- 🌟 👤 [TUGOhost](https://github.com/TUGOhost) Starred [JustVugg/colibri](https://github.com/JustVugg/colibri)
  > Run GLM-5.2 (744B MoE) on a 25GB-RAM consumer machine — pure C, zero deps, experts streamed from dis...

### [Tardis07](https://github.com/Tardis07)
- 🌟 👤 [Tardis07](https://github.com/Tardis07) Starred [ttimasdf/pi-maintainer-workflows](https://github.com/ttimasdf/pi-maintainer-workflows)
  > Pi prompt templates for maintaining long-lived soft forks of upstream projects

### [TheWildJames](https://github.com/TheWildJames)
- 🍴 👤 [TheWildJames](https://github.com/TheWildJames) Forked [KernelSU-Next/KernelSU-Next](https://github.com/KernelSU-Next/KernelSU-Next) to [TheWildJames/KernelSU-Next](https://github.com/TheWildJames/KernelSU-Next)
  > An advanced Kernel based root solution for Android
- 🍴 👤 [TheWildJames](https://github.com/TheWildJames) Forked [tymmesyde/Losange](https://github.com/tymmesyde/Losange) to [TheWildJames/Losange](https://github.com/TheWildJames/Losange)
  > A simple Stremio client for GNOME

### [ThomasThelen](https://github.com/ThomasThelen)
- 🌟 👤 [ThomasThelen](https://github.com/ThomasThelen) Starred [apache/ozone](https://github.com/apache/ozone)
  > Scalable, reliable, distributed storage system optimized for data analytics and object store workloa...

### [ThunderCls](https://github.com/ThunderCls)
- 🌟 👤 [ThunderCls](https://github.com/ThunderCls) Starred [tnodir/fort](https://github.com/tnodir/fort)
  > Fort Firewall for Windows

### [TideSec](https://github.com/TideSec)
- 🌟 👤 [TideSec](https://github.com/TideSec) Starred [P4nda0s/reverse-skills](https://github.com/P4nda0s/reverse-skills)
  > Reverse engineering skills for Claude Code | 逆向工程 Claude Code Skills 插件

### [X1r0z](https://github.com/X1r0z)
- 🌟 👤 [X1r0z](https://github.com/X1r0z) Starred [radareorg/radare2](https://github.com/radareorg/radare2)
  > UNIX-like reverse engineering framework and command-line toolset

### [XiaoTong6666](https://github.com/XiaoTong6666)
- 🌟 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Starred [XiaoTong6666/uihelper](https://github.com/XiaoTong6666/uihelper)
- 🌟 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Starred [MDX-Tom/gpt-5.6-instruct](https://github.com/MDX-Tom/gpt-5.6-instruct)
  > A Codex CLI jailbreak prompt and test pack for gpt-5.6-sol. 针对 gpt-5.6 系列的 Codex CLI 破甲提示词与测试包。
- 🌟 👤 [XiaoTong6666](https://github.com/XiaoTong6666) Starred [Kanscape/Hana](https://github.com/Kanscape/Hana)
  > Hanime1 第三方客户端

### [Xieansecn](https://github.com/Xieansecn)
- 🌟 👤 [Xieansecn](https://github.com/Xieansecn) Starred [onnx/models](https://github.com/onnx/models)
  > A collection of pre-trained, state-of-the-art models in the ONNX format 

### [Youlor](https://github.com/Youlor)
- 🌟 👤 [Youlor](https://github.com/Youlor) Starred [NebuSec/CyberMeowfia](https://github.com/NebuSec/CyberMeowfia)
  > PoCs and exploits for CVEs discovered by VEGA. 

### [YuKongA](https://github.com/YuKongA)
- 🌟 👤 [YuKongA](https://github.com/YuKongA) Starred [ImranR98/Obtainium](https://github.com/ImranR98/Obtainium)
  > Get Android app updates straight from the source.

### [aimardcr](https://github.com/aimardcr)
- 🌟 👤 [aimardcr](https://github.com/aimardcr) Starred [littledivy/mimic](https://github.com/littledivy/mimic)
  > Intercept any app, then call it from Python like a library

### [alset0326](https://github.com/alset0326)
- 🌟 👤 [alset0326](https://github.com/alset0326) Starred [alset0326/obsidian-attachment-buttons](https://github.com/alset0326/obsidian-attachment-buttons)
  > An Obsidian plugin that renders every link or embed to a non-markdown attachment as one uniform, nat...

### [antiwar3](https://github.com/antiwar3)
- 🌟 👤 [antiwar3](https://github.com/antiwar3) Starred [KSwordDEV/KSword](https://github.com/KSwordDEV/KSword)
  > KSword 5.1 is an open-source Windows toolkit for ARK, kernel debugging, and system forensics.  中文：KS...

### [apkunpacker](https://github.com/apkunpacker)
- 🌟 👤 [apkunpacker](https://github.com/apkunpacker) Starred [par274/sharpemu](https://github.com/par274/sharpemu)
  > An experimental PlayStation 5 emulator project.

### [asLody](https://github.com/asLody)
- 🌟 👤 [asLody](https://github.com/asLody) Starred [droidrun/mobilerun](https://github.com/droidrun/mobilerun)
  > Automate your mobile devices with natural language commands - an LLM agnostic mobile Agent 🤖

### [ba0gu0](https://github.com/ba0gu0)
- 🌟 👤 [ba0gu0](https://github.com/ba0gu0) Starred [injaneity/pi-computer-use](https://github.com/injaneity/pi-computer-use)
  > Let Pi control your apps on MacOS & Windows
- 🌟 👤 [ba0gu0](https://github.com/ba0gu0) Starred [xiaozhou26/icloud-hme](https://github.com/xiaozhou26/icloud-hme)

### [bigsinger](https://github.com/bigsinger)
- 🌟 👤 [bigsinger](https://github.com/bigsinger) Starred [cynative/cynative](https://github.com/cynative/cynative)
  > Deep cybersecurity research agent for your cloud, code and runtime - ask your infrastructure anythin...

### [bin4re](https://github.com/bin4re)
- 🌟 👤 [bin4re](https://github.com/bin4re) Starred [Ed1s0nZ/CyberStrikeAI](https://github.com/Ed1s0nZ/CyberStrikeAI)
  > The system of action for AI-native cybersecurity—where intent becomes governed execution, evidence b...

### [boy-hack](https://github.com/boy-hack)
- 🌟 👤 [boy-hack](https://github.com/boy-hack) Starred [maojindao55/freebuddy](https://github.com/maojindao55/freebuddy)
  > FreeBuddy: desktop chat workspace for local CLI agents (codex / claude / opencode)

### [breaking153](https://github.com/breaking153)
- 🌟 👤 [breaking153](https://github.com/breaking153) Starred [AHEKOT/ComfyUI_VNCCS](https://github.com/AHEKOT/ComfyUI_VNCCS)
  > Visual Novel Character Creation Suite is a comprehensive tool for creating character sprites for vis...
- 🌟 👤 [breaking153](https://github.com/breaking153) Starred [AHEKOT/VNCCS_Easy-Install](https://github.com/AHEKOT/VNCCS_Easy-Install)
  > Portable VNCCS ComfyUI installer for Windows

### [byt3bl33d3r](https://github.com/byt3bl33d3r)
- 🌟 👤 [byt3bl33d3r](https://github.com/byt3bl33d3r) Starred [JetBrains/youtrackdb](https://github.com/JetBrains/youtrackdb)
  > YouTrackDB is a general-use object-oriented graph database with storage format native to handle grap...
- 🌟 👤 [byt3bl33d3r](https://github.com/byt3bl33d3r) Starred [okalachev/flix](https://github.com/okalachev/flix)
  > Making an ESP32-based quadcopter from scratch

### [can1357](https://github.com/can1357)
- 🌟 👤 [can1357](https://github.com/can1357) Starred [watzon/semantouch](https://github.com/watzon/semantouch)

### [cctv18](https://github.com/cctv18)
- 🌟 👤 [cctv18](https://github.com/cctv18) Starred [MarkCup-Official/Anan-s-Sketchbook-Chat-Box](https://github.com/MarkCup-Official/Anan-s-Sketchbook-Chat-Box)

### [chsbuffer](https://github.com/chsbuffer)
- 🌟 👤 [chsbuffer](https://github.com/chsbuffer) Starred [pandoc-ext/diagram](https://github.com/pandoc-ext/diagram)
  > Generate diagrams from embedded code; supports Mermaid, Dot/GraphViz, PlantUML, Asymptote, D2, CeTZ,...
- 🌟 👤 [chsbuffer](https://github.com/chsbuffer) Starred [mermaid-js/mermaid-cli](https://github.com/mermaid-js/mermaid-cli)
  > Command line tool for the Mermaid library

### [clearbluejar](https://github.com/clearbluejar)
- 🌟 👤 [clearbluejar](https://github.com/clearbluejar) Starred [multica-ai/andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills)
  > A single CLAUDE.md file to improve Claude Code behavior, derived from Andrej Karpathy's observations...

### [cmzy](https://github.com/cmzy)
- 🍴 👤 [cmzy](https://github.com/cmzy) Forked [orottier/web-audio-api-rs](https://github.com/orottier/web-audio-api-rs) to [cmzy/web-audio-api-rs](https://github.com/cmzy/web-audio-api-rs)
  > A Rust implementation of the Web Audio API, for use in non-browser contexts

### [codingsf](https://github.com/codingsf)
- 🌟 👤 [codingsf](https://github.com/codingsf) Starred [gadievron/raptor](https://github.com/gadievron/raptor)
  > Raptor turns Claude Code into a general-purpose AI offensive/defensive security agent. By using Clau...
- 🌟 👤 [codingsf](https://github.com/codingsf) Starred [canyie/CVE-2024-0044](https://github.com/canyie/CVE-2024-0044)
  > RunAsAnyone: PoC and writeup for bypassing the initial patch of CVE-2024-0044, Android run-as any ap...
- 🌟 👤 [codingsf](https://github.com/codingsf) Starred [sarabpal-dev/cheese-cake](https://github.com/sarabpal-dev/cheese-cake)
  > A proof-of-concept for CVE-2025-21479, chained with a Dirty Pagetable technique.

### [cryzlasm](https://github.com/cryzlasm)
- 🌟 👤 [cryzlasm](https://github.com/cryzlasm) Starred [lhish/topic_solve](https://github.com/lhish/topic_solve)
  > 修复Linux.do后台打开标签页不计数的问题

### [darkskygit](https://github.com/darkskygit)
- 🌟 👤 [darkskygit](https://github.com/darkskygit) Starred [LukeFZ/Il2CppInspectorRedux](https://github.com/LukeFZ/Il2CppInspectorRedux)
  > Continuation of Il2CppInspector - A powerful automated tool for reverse engineering Unity IL2CPP bin...

### [delikely](https://github.com/delikely)
- 🌟 👤 [delikely](https://github.com/delikely) Starred [GlasgowEmbedded/glasgow](https://github.com/GlasgowEmbedded/glasgow)
  > Scots Army Knife for electronics

### [dodola](https://github.com/dodola)
- 🌟 👤 [dodola](https://github.com/dodola) Starred [opc8838-hub/cover-flow-showcase](https://github.com/opc8838-hub/cover-flow-showcase)

### [dqzg12300](https://github.com/dqzg12300)
- 🌟 👤 [dqzg12300](https://github.com/dqzg12300) Starred [zgy0x01/QTrace](https://github.com/zgy0x01/QTrace)
  > qbdi based android arm64 trace tool

### [dzxpert](https://github.com/dzxpert)
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [Nuand/bladeRF](https://github.com/Nuand/bladeRF)
  > bladeRF USB 3.0 Superspeed Software Defined Radio Source Code
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [AsahiLinux/muvm](https://github.com/AsahiLinux/muvm)
  > muvm - run programs from your system in a microVM

### [einverne](https://github.com/einverne)
- 🌟 👤 [einverne](https://github.com/einverne) Starred [beancount/beancount](https://github.com/beancount/beancount)
  > Beancount: Double-Entry Accounting from Text Files.

### [elllusion](https://github.com/elllusion)
- 🌟 👤 [elllusion](https://github.com/elllusion) Starred [ALIGN-analoglayout/ALIGN-public](https://github.com/ALIGN-analoglayout/ALIGN-public)
- 🌟 👤 [elllusion](https://github.com/elllusion) Starred [ucb-art/BAG_framework](https://github.com/ucb-art/BAG_framework)
- 🌟 👤 [elllusion](https://github.com/elllusion) Starred [rachelselinar/ReGDS-Logic-Gate-Extraction](https://github.com/rachelselinar/ReGDS-Logic-Gate-Extraction)
  > A custom C++ routine to identify logic gates in the layout extracted netlist (SPICE) of digital circ...
- 🌟 👤 [elllusion](https://github.com/elllusion) Starred [JustVugg/colibri](https://github.com/JustVugg/colibri)
  > Run GLM-5.2 (744B MoE) on a 25GB-RAM consumer machine — pure C, zero deps, experts streamed from dis...

### [es3n1n](https://github.com/es3n1n)
- 🌟 👤 [es3n1n](https://github.com/es3n1n) Starred [otter-sec/rctf](https://github.com/otter-sec/rctf)
  > A CTF platform for all

### [everettjf](https://github.com/everettjf)
- 🌟 👤 [everettjf](https://github.com/everettjf) Starred [injaneity/pi-computer-use](https://github.com/injaneity/pi-computer-use)
  > Let Pi control your apps on MacOS & Windows

### [eversinc33](https://github.com/eversinc33)
- 🌟 👤 [eversinc33](https://github.com/eversinc33) Starred [KernelErr/dnSpy.Extension.MCP](https://github.com/KernelErr/dnSpy.Extension.MCP)
  > MCP extension for dnSpy.

### [exp-sky](https://github.com/exp-sky)
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [anysearch-ai/anysearch-skill](https://github.com/anysearch-ai/anysearch-skill)
  > Unified real-time search engine skill for AI agents. 
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [xbtlin/ai-berkshire](https://github.com/xbtlin/ai-berkshire)
  > AI 时代的伯克希尔：基于 Claude Code / Codex 的价值投资研究框架。巴菲特·芒格·段永平·李录四大师方法论 + 多Agent并行研究。| AI-era Berkshire: a v...
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [virattt/ai-hedge-fund](https://github.com/virattt/ai-hedge-fund)
  > An AI Hedge Fund Team
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [microsoft/SkillOpt](https://github.com/microsoft/SkillOpt)
  > SkillOpt is a text-space optimizer that trains reusable natural-language skills for frozen LLM agent...
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [ginobefun/BestBlogs](https://github.com/ginobefun/BestBlogs)
  > bestblogs.dev - 汇集顶级编程、人工智能、产品、科技文章，大语言模型摘要评分辅助阅读，探索编程和技术未来
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [DIYgod/RSSHub](https://github.com/DIYgod/RSSHub)
  > 🧡 Everything is RSSible
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [juliangarnier/anime](https://github.com/juliangarnier/anime)
  > JavaScript animation engine
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [microsoft/qlib](https://github.com/microsoft/qlib)
  > Qlib is an AI-oriented Quant investment platform that aims to use AI tech to empower Quant Research,...

### [fanzhidongyzby](https://github.com/fanzhidongyzby)
- 🌟 👤 [fanzhidongyzby](https://github.com/fanzhidongyzby) Starred [multica-ai/multica](https://github.com/multica-ai/multica)
  > The open-source managed agents platform. Turn coding agents into real teammates — assign tasks, trac...

### [feicong](https://github.com/feicong)
- 🍴 👤 [feicong](https://github.com/feicong) Forked [eltavine/Duck-Detector-Refactoring](https://github.com/eltavine/Duck-Detector-Refactoring) to [feicong/Duck-Detector-Refactoring](https://github.com/feicong/Duck-Detector-Refactoring)
  > Android environment integrity inspection tool for root, hook, bootloader, SELinux, virtualization, a...

### [fengchenzxc](https://github.com/fengchenzxc)
- 🌟 👤 [fengchenzxc](https://github.com/fengchenzxc) Starred [WhiteNightShadow/camoufox-reverse-mcp](https://github.com/WhiteNightShadow/camoufox-reverse-mcp)
  > 🔧 反检测浏览器 MCP Server | 35 个 JS 逆向工具 + C++ 引擎层 JSVMP 指纹追踪
- 🌟 👤 [fengchenzxc](https://github.com/fengchenzxc) Starred [HackTricks-wiki/hacktricks](https://github.com/HackTricks-wiki/hacktricks)
  > Welcome to the page where you will find each trick/technique/whatever I have learnt in CTFs, real li...
- 🌟 👤 [fengchenzxc](https://github.com/fengchenzxc) Starred [swisskyrepo/InternalAllTheThings](https://github.com/swisskyrepo/InternalAllTheThings)
  > Active Directory and Internal Pentest Cheatsheets
- 🌟 👤 [fengchenzxc](https://github.com/fengchenzxc) Starred [chaitin/OctoBus](https://github.com/chaitin/OctoBus)
  > A secure local gateway for AI agents to reliably call approved enterprise APIs, tools, and services.
- 🌟 👤 [fengchenzxc](https://github.com/fengchenzxc) Starred [chaitin/chaitin-cli](https://github.com/chaitin/chaitin-cli)
  > Chaitin CLI for products

### [frog-game](https://github.com/frog-game)
- 🌟 👤 [frog-game](https://github.com/frog-game) Starred [xiaoyuanda666-ship-it/BaiLongma](https://github.com/xiaoyuanda666-ship-it/BaiLongma)

### [fvrmatteo](https://github.com/fvrmatteo)
- 🌟 👤 [fvrmatteo](https://github.com/fvrmatteo) Starred [par274/sharpemu](https://github.com/par274/sharpemu)
  > An experimental PlayStation 5 emulator project.

### [fynch3r](https://github.com/fynch3r)
- 🌟 👤 [fynch3r](https://github.com/fynch3r) Starred [laoma2053/awesome-zhuiju-free](https://github.com/laoma2053/awesome-zhuiju-free)
  > 人工精选的免费无广告追剧资源导航，每日自动检测可用性。收录在线影视、影视APP、网盘搜索、磁力 BT、字幕、TVBox / 影视仓配置地址、IPTV 订阅源。开源，社区共同维护。

### [gamelaster](https://github.com/gamelaster)
- 🌟 👤 [gamelaster](https://github.com/gamelaster) Starred [dpks2003/bio-core](https://github.com/dpks2003/bio-core)
  > BIO Coprocessor on GF 180

### [gmh5225](https://github.com/gmh5225)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [xp987/symbridge](https://github.com/xp987/symbridge) to [gmh5225/symbridge](https://github.com/gmh5225/symbridge)
  > Live IDA ↔ x64dbg annotation and type synchronization bridge
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [ioncodes/gpui-cpp](https://github.com/ioncodes/gpui-cpp) to [gmh5225/gpui-cpp](https://github.com/gmh5225/gpui-cpp)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [Astharot15/COMLoaderAstharot](https://github.com/Astharot15/COMLoaderAstharot) to [gmh5225/COMLoaderAstharot](https://github.com/gmh5225/COMLoaderAstharot)
- 🌟 👤 [gmh5225](https://github.com/gmh5225) Starred [pcaversaccio/tornado-governance-lock](https://github.com/pcaversaccio/tornado-governance-lock)
  > A terminal Tornado Cash governance proposal.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [pcaversaccio/tornado-governance-lock](https://github.com/pcaversaccio/tornado-governance-lock) to [gmh5225/tornado-governance-lock](https://github.com/gmh5225/tornado-governance-lock)
  > A terminal Tornado Cash governance proposal.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [weak1337/DetectTpmSpoofing](https://github.com/weak1337/DetectTpmSpoofing) to [gmh5225/DetectTpmSpoofing](https://github.com/gmh5225/DetectTpmSpoofing)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [otter-sec/rctf](https://github.com/otter-sec/rctf) to [gmh5225/rctf-](https://github.com/gmh5225/rctf-)
  > A CTF platform for all
- 🌟 👤 [gmh5225](https://github.com/gmh5225) Starred [AsahiLinux/muvm](https://github.com/AsahiLinux/muvm)
  > muvm - run programs from your system in a microVM
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [hyuunnn/ida-slides](https://github.com/hyuunnn/ida-slides) to [gmh5225/ida-slides](https://github.com/gmh5225/ida-slides)
  > Present live reverse-engineering with Marp/Slidev decks docked inside IDA Pro
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [aman-bhaskar-codes/ATLAS](https://github.com/aman-bhaskar-codes/ATLAS) to [gmh5225/ATLAS-ai](https://github.com/gmh5225/ATLAS-ai)
  > ATLAS — Autonomous Task & Learning Agent System. Production-grade agentic AI runtime: safety-first, ...
- 🌟 👤 [gmh5225](https://github.com/gmh5225) Starred [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki)
  > OpenWiki is a CLI that writes and maintains agent documentation for your codebase.
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki) to [gmh5225/openwiki](https://github.com/gmh5225/openwiki)
  > OpenWiki is a CLI that writes and maintains agent documentation for your codebase.

### [halfkiss](https://github.com/halfkiss)
- 🌟 👤 [halfkiss](https://github.com/halfkiss) Starred [TapXWorld/ChinaTextbook](https://github.com/TapXWorld/ChinaTextbook)
  > 所有小初高、大学PDF教材。

### [hhhaiai](https://github.com/hhhaiai)
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [sums001/Windows-Copilot-API](https://github.com/sums001/Windows-Copilot-API)
  > Reverse engineered Windows Copilot into an OpenAI-compatible API. Access GPT-4 and GPT-5 models thro...

### [hoochanlon](https://github.com/hoochanlon)
- 🌟 👤 [hoochanlon](https://github.com/hoochanlon) Starred [sivan/heti](https://github.com/sivan/heti)
  > 赫蹏（hètí）是专为中文内容展示设计的排版样式增强。它基于通行的中文排版规范而来，可以为网站的读者带来更好的文章阅读体验。

### [iamtorsten](https://github.com/iamtorsten)
- 🍴 👤 [iamtorsten](https://github.com/iamtorsten) Forked [linzj/fanqie-dl](https://github.com/linzj/fanqie-dl) to [iamtorsten/fanqie-dl](https://github.com/iamtorsten/fanqie-dl)
  > 番茄小说命令行下载器 — 纯 Rust 实现，通过逆向 libmetasec_ml.so 还原 API 签名算法 (SHA-256 + AES)
- 🍴 👤 [iamtorsten](https://github.com/iamtorsten) Forked [armxe/tiktok-api](https://github.com/armxe/tiktok-api) to [iamtorsten/tiktok-api](https://github.com/iamtorsten/tiktok-api)
  > Overview of TikTok's Anti-Bot Encrypted Values - X-Argus, X-Bogus and more.
- 🍴 👤 [iamtorsten](https://github.com/iamtorsten) Forked [retardedjames/direct_api](https://github.com/retardedjames/direct_api) to [iamtorsten/direct_api](https://github.com/iamtorsten/direct_api)
  > Pure-HTTP TikTok Lite scraper — Frida-signed MSSDK requests, no phone/Waydroid/mitmproxy
- 🍴 👤 [iamtorsten](https://github.com/iamtorsten) Forked [2vN9/TikTok-Devices-Register](https://github.com/2vN9/TikTok-Devices-Register) to [iamtorsten/TikTok-Devices-Register](https://github.com/iamtorsten/TikTok-Devices-Register)
  > Generation Devices Register TikTok Xlog  

### [imlk0](https://github.com/imlk0)
- 🌟 👤 [imlk0](https://github.com/imlk0) Starred [mattpocock/skills](https://github.com/mattpocock/skills)
  > Skills for Real Engineers. Straight from my .claude directory.
- 🍴 👤 [imlk0](https://github.com/imlk0) Forked [veraison/rust-ear](https://github.com/veraison/rust-ear) to [inclavare-containers/rust-ear](https://github.com/inclavare-containers/rust-ear)
  > Rust implementation of EAT Attestation Results

### [ioncodes](https://github.com/ioncodes)
- 🌟 👤 [ioncodes](https://github.com/ioncodes) Starred [par274/sharpemu](https://github.com/par274/sharpemu)
  > An experimental PlayStation 5 emulator project.

### [ivellioscolin](https://github.com/ivellioscolin)
- 🍴 👤 [ivellioscolin](https://github.com/ivellioscolin) Forked [numz/ComfyUI-SeedVR2_VideoUpscaler](https://github.com/numz/ComfyUI-SeedVR2_VideoUpscaler) to [ivellioscolin/ComfyUI-SeedVR2_VideoUpscaler](https://github.com/ivellioscolin/ComfyUI-SeedVR2_VideoUpscaler)
  > Official SeedVR2 Video Upscaler for ComfyUI

### [jas502n](https://github.com/jas502n)
- 🌟 👤 [jas502n](https://github.com/jas502n) Starred [SickleSec/GhostWolf](https://github.com/SickleSec/GhostWolf)
  > 从内存中提取浏览器和Todesk用户凭证

### [jianchang512](https://github.com/jianchang512)
- 🌟 👤 [jianchang512](https://github.com/jianchang512) Starred [CloakHQ/CloakBrowser](https://github.com/CloakHQ/CloakBrowser)
  > Stealth Chromium that passes every bot detection test. Drop-in Playwright replacement with source-le...

### [jianglin-code](https://github.com/jianglin-code)
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [iree-org/iree](https://github.com/iree-org/iree)
  > A retargetable MLIR-based machine learning compiler and runtime toolkit.
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [google/flax](https://github.com/google/flax)
  > Flax is a neural network library for JAX that is designed for flexibility.
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [google-deepmind/dm-haiku](https://github.com/google-deepmind/dm-haiku)
  > JAX-based neural network library
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [aigc17/Chinese-PhoenixCrown](https://github.com/aigc17/Chinese-PhoenixCrown)
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [mermaid-js/mermaid](https://github.com/mermaid-js/mermaid)
  > Generation of diagrams like flowcharts or sequence diagrams from text in a similar manner as markdow...
- 🌟 👤 [jianglin-code](https://github.com/jianglin-code) Starred [sebaubuntu-python/aospdtgen](https://github.com/sebaubuntu-python/aospdtgen)
  > LineageOS device tree generator

### [jiji262](https://github.com/jiji262)
- 🌟 👤 [jiji262](https://github.com/jiji262) Starred [lanyeeee/bilibili-video-downloader](https://github.com/lanyeeee/bilibili-video-downloader)
  > 哔哩哔哩 bilibili B站 视频 下载器，图形界面 + nfo刮削 + 广告标记 + 字幕下载 + 弹幕下载，轻松将视频加入emby等媒体库

### [jixiaoyong](https://github.com/jixiaoyong)
- 🌟 👤 [jixiaoyong](https://github.com/jixiaoyong) Starred [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)
  > Sub2API 一站式开源中转服务，让 Claude、Openai 、Gemini、Grok订阅统一接入，支持拼车共享，更高效分摊成本，原生工具无缝使用。

### [jweny](https://github.com/jweny)
- 🌟 👤 [jweny](https://github.com/jweny) Starred [ArcReel/ArcReel](https://github.com/ArcReel/ArcReel)
  > AI Agent 驱动的开源视频生成工作台 — 小说→角色/场景/道具设计→剧本→分镜图→视频，跨镜头角色与场景一致 | Open-source AI video workspace powered ...

### [k273811702](https://github.com/k273811702)
- 🌟 👤 [k273811702](https://github.com/k273811702) Starred [zhutingxf/InfinityHookPro](https://github.com/zhutingxf/InfinityHookPro)
  > InfinityHook 支持Win7 到 Win11 最新版本，虚拟机环境及物理机环境

### [kingking888](https://github.com/kingking888)
- 🌟 👤 [kingking888](https://github.com/kingking888) Starred [congminh1254/shopee-sdk](https://github.com/congminh1254/shopee-sdk)
  > A lightweight, fully-featured TypeScript SDK for the Shopee Open API v2. Features automatic request ...

### [kkent030315](https://github.com/kkent030315)
- 🌟 👤 [kkent030315](https://github.com/kkent030315) Starred [Yusukedao-0610/ArduinoMouseMoving](https://github.com/Yusukedao-0610/ArduinoMouseMoving)
  > Arduino leonardo + USB HOST SHIELD 2.0 is required. you can from amazon.

### [kost](https://github.com/kost)
- 🌟 👤 [kost](https://github.com/kost) Starred [OpenCut-app/OpenCut](https://github.com/OpenCut-app/OpenCut)
  > The open-source CapCut alternative
- 🌟 👤 [kost](https://github.com/kost) Starred [JustVugg/colibri](https://github.com/JustVugg/colibri)
  > Run GLM-5.2 (744B MoE) on a 25GB-RAM consumer machine — pure C, zero deps, experts streamed from dis...
- 🌟 👤 [kost](https://github.com/kost) Starred [pwnlabmx/RFSAM](https://github.com/pwnlabmx/RFSAM)
  > Radio Frequency Security Assessment Methodology
- 🌟 👤 [kost](https://github.com/kost) Starred [simbajigege/book2skills](https://github.com/simbajigege/book2skills)
  > Create best skills based on best books
- 🌟 👤 [kost](https://github.com/kost) Starred [garrytan/gstack](https://github.com/garrytan/gstack)
  > Use Garry Tan's exact Claude Code setup: 23 opinionated tools that serve as CEO, Designer, Eng Manag...
- 🌟 👤 [kost](https://github.com/kost) Starred [BlackSnufkin/BYOVD](https://github.com/BlackSnufkin/BYOVD)
  > BYOVD research use cases featuring vulnerable driver discovery and reverse engineering methodology. ...
- 🌟 👤 [kost](https://github.com/kost) Starred [0x4D31/stinger](https://github.com/0x4D31/stinger)
  > Unprivileged endpoint deception for macOS and Linux workstations. Uses FIFO baits, other local traps...

### [kurogai](https://github.com/kurogai)
- 🌟 👤 [kurogai](https://github.com/kurogai) Starred [antonioCoco/RunasCs](https://github.com/antonioCoco/RunasCs)
  > RunasCs - Csharp and open version of windows builtin runas.exe
- 🌟 👤 [kurogai](https://github.com/kurogai) Starred [strikoder/NagoyaSpray](https://github.com/strikoder/NagoyaSpray)
  > CTF-focused password spray wordlist generator.  Perfected for OSCP/PNPT/CPTS. Generates realistic se...
- 🌟 👤 [kurogai](https://github.com/kurogai) Starred [dirkjanm/BloodHound.py](https://github.com/dirkjanm/BloodHound.py)
  > A Python based ingestor for BloodHound

### [lightsummer233](https://github.com/lightsummer233)
- 🍴 👤 [lightsummer233](https://github.com/lightsummer233) Forked [mpv-player/mpv](https://github.com/mpv-player/mpv) to [lightsummer233/mpv](https://github.com/lightsummer233/mpv)
  > 🎥 Command line media player

### [linxid](https://github.com/linxid)
- 🌟 👤 [linxid](https://github.com/linxid) Starred [qingfengtommy/SpineAgent](https://github.com/qingfengtommy/SpineAgent)

### [ltlly](https://github.com/ltlly)
- 🌟 👤 [ltlly](https://github.com/ltlly) Starred [trzsz/trzsz](https://github.com/trzsz/trzsz)
  > trzsz is a simple file transfer tools, similar to lrzsz ( rz / sz ), and compatible with tmux.

### [luoyesiqiu](https://github.com/luoyesiqiu)
- 🌟 👤 [luoyesiqiu](https://github.com/luoyesiqiu) Starred [lenzarchive/UnityBundleExtractor](https://github.com/lenzarchive/UnityBundleExtractor)
  > A robust Python script for extracting various assets from Unity3D bundle files. Features intelligent...

### [marlkiller](https://github.com/marlkiller)
- 🍴 👤 [marlkiller](https://github.com/marlkiller) Forked [SharzyL/pastebin-worker](https://github.com/SharzyL/pastebin-worker) to [marlkiller/pastebin-worker](https://github.com/marlkiller/pastebin-worker)
  > Pastebin on Cloudflare worker, with friendly CLI usage and rich features
- 🍴 👤 [marlkiller](https://github.com/marlkiller) Forked [kadidalax/cf-vps-monitor](https://github.com/kadidalax/cf-vps-monitor) to [marlkiller/cf-vps-monitor](https://github.com/marlkiller/cf-vps-monitor)
  > 用cloudflare worker搭建的vps探针+网站检测 面板，部署简单，不需要服务器。

### [mav8557](https://github.com/mav8557)
- 🌟 👤 [mav8557](https://github.com/mav8557) Starred [littledivy/mimic](https://github.com/littledivy/mimic)
  > Intercept any app, then call it from Python like a library

### [miunasu](https://github.com/miunasu)
- 🌟 👤 [miunasu](https://github.com/miunasu) Starred [JustVugg/colibri](https://github.com/JustVugg/colibri)
  > Run GLM-5.2 (744B MoE) on a 25GB-RAM consumer machine — pure C, zero deps, experts streamed from dis...

### [mizzy](https://github.com/mizzy)
- 🌟 👤 [mizzy](https://github.com/mizzy) Starred [masawada/mdp](https://github.com/masawada/mdp)

### [moyada](https://github.com/moyada)
- 🌟 👤 [moyada](https://github.com/moyada) Starred [LoseNine/ruyipage](https://github.com/LoseNine/ruyipage)
  > 下一代Python的web自动化过检测框架，通过一切网站检测的Firefox内核浏览器。RuyiPage is a Python-based Firefox automation framework ...

### [mrexodia](https://github.com/mrexodia)
- 🍴 👤 [mrexodia](https://github.com/mrexodia) Forked [crackmesone/crackmes-re-dataset](https://github.com/crackmesone/crackmes-re-dataset) to [mrexodia/crackmes-re-dataset](https://github.com/mrexodia/crackmes-re-dataset)
  > Labeled reverse-engineering dataset over 4,598 crackmes: flags, verifier scripts, and normalized obf...

### [mzfr](https://github.com/mzfr)
- 🌟 👤 [mzfr](https://github.com/mzfr) Starred [ish-app/ish](https://github.com/ish-app/ish)
  > Linux shell for iOS

### [netxfly](https://github.com/netxfly)
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [P4nda0s/reverse-skills](https://github.com/P4nda0s/reverse-skills)
  > Reverse engineering skills for Claude Code | 逆向工程 Claude Code Skills 插件
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [callstack/agent-device](https://github.com/callstack/agent-device)
  > CLI to control iOS and Android devices for AI agents
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [plateaukao/einkbro](https://github.com/plateaukao/einkbro)
  > A small, fast web browser based on Android WebView. It's tailored for E-Ink devices but also works g...
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [wechat-article/wechat-article-exporter](https://github.com/wechat-article/wechat-article-exporter)
  > 一款在线的 微信公众号文章批量下载 工具，支持导出阅读量与评论数据，无需搭建任何环境，可通过 在线网站 使用，支持 docker 私有化部署和 Cloudflare 部署。  支持下载各种文件格式，其...
- 🌟 👤 [netxfly](https://github.com/netxfly) Starred [Shubhamsaboo/awesome-llm-apps](https://github.com/Shubhamsaboo/awesome-llm-apps)
  > 100+ AI Agent & RAG apps you can actually run — clone, customize, ship.

### [nil0x42](https://github.com/nil0x42)
- 🌟 👤 [nil0x42](https://github.com/nil0x42) Starred [nil0x42/firefox-privacy-containers](https://github.com/nil0x42/firefox-privacy-containers)
  > Firefox extension for routing Containers through dedicated proxies, request headers, and host rules—...

### [null-dev](https://github.com/null-dev)
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [kivikakk/comrak](https://github.com/kivikakk/comrak)
  > CommonMark + GFM compatible Markdown parser and renderer
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [jeanp413/open-remote-ssh](https://github.com/jeanp413/open-remote-ssh)
  > VSCode Remote Development: Open any folder on a remote machine using SSH.
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [ahmed-el-halawani/debugoverlay-android](https://github.com/ahmed-el-halawani/debugoverlay-android)
  > Zero-config runtime diagnostics overlay for Android—CPU, memory, FPS, logs, network, and one-tap bug...
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [block/radiography](https://github.com/block/radiography)
  > Text-ray goggles for your Android UI.
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [composablehorizons/compose-unstyled](https://github.com/composablehorizons/compose-unstyled)
  > The missing Design System layer for Compose UI
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [jetpack-compose/jetpack-compose-awesome](https://github.com/jetpack-compose/jetpack-compose-awesome)
  > 📝 A curated list of awesome Jetpack Compose libraries, projects, articles and resources
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [popovanton0/Blueprint](https://github.com/popovanton0/Blueprint)
  > 📐 Visualize the dimensions of your composables on a blueprint!
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [AndroidPoet/Dropdown](https://github.com/AndroidPoet/Dropdown)
  > 💧A Powerful and customizable Compose Multiplatform dropdown menu with cascade and animations.
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [mlm-games/yadaw](https://github.com/mlm-games/yadaw)
  > An sfx creation tool and midi player that doesn't crash often (web can boot and play audio, preview)
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [Rve27/RvSystem-Monitor](https://github.com/Rve27/RvSystem-Monitor)
  > Modern Android system monitoring app using Jetpack Compose and Rust
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [iodn/android-usb-device-info](https://github.com/iodn/android-usb-device-info)
  > Android app to inspect USB devices with full descriptor and HID details
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [DigitallyRefined/android-ip-camera](https://github.com/DigitallyRefined/android-ip-camera)
  > An Android MJPEG / H.264 IP Camera app
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [zhanghai/ComposePreference](https://github.com/zhanghai/ComposePreference)
  > Preference implementation for Jetpack Compose Material 3
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [RyanJK5/rjk-duck](https://github.com/RyanJK5/rjk-duck)
  > Zero boilerplate generic type erasure, powered by C++26 reflection.
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [juggler-ai/juggler](https://github.com/juggler-ai/juggler)
  > The Juggler Code Agent

### [oblique](https://github.com/oblique)
- 🌟 👤 [oblique](https://github.com/oblique) Starred [DarkFlippers/flipperzero-subbrute](https://github.com/DarkFlippers/flipperzero-subbrute)
  > SubGHz key checker

### [odzhan](https://github.com/odzhan)
- 🌟 👤 [odzhan](https://github.com/odzhan) Starred [weak1337/DetectTpmSpoofing](https://github.com/weak1337/DetectTpmSpoofing)

### [orangbus](https://github.com/orangbus)
- 🌟 👤 [orangbus](https://github.com/orangbus) Starred [666ghj/BettaFish](https://github.com/666ghj/BettaFish)
  > 微舆：人人可用的多Agent舆情分析助手，打破信息茧房，还原舆情原貌，预测未来走向，辅助决策！从0实现，不依赖任何框架。

### [paradiseduo](https://github.com/paradiseduo)
- 🌟 👤 [paradiseduo](https://github.com/paradiseduo) Starred [Lakr233/AssppWeb](https://github.com/Lakr233/AssppWeb)
- 🌟 👤 [paradiseduo](https://github.com/paradiseduo) Starred [toeverything/AFFiNE](https://github.com/toeverything/AFFiNE)
  > There can be more than Notion and Miro. AFFiNE(pronounced [ə‘fain]) is a next-gen knowledge base tha...

### [pen4uin](https://github.com/pen4uin)
- 🌟 👤 [pen4uin](https://github.com/pen4uin) Starred [exploitbench/exploitbench](https://github.com/exploitbench/exploitbench)
  > ExploitBench measures how far AI agents climb, from reaching vulnerable code, to triggering the bug,...

### [penguin-wwy](https://github.com/penguin-wwy)
- 🌟 👤 [penguin-wwy](https://github.com/penguin-wwy) Starred [facebookresearch/tensor-layouts](https://github.com/facebookresearch/tensor-layouts)
  > A pure-Python implementation of the Nvidia CuTe layout algebra intended to be approachable and easy ...

### [plusls](https://github.com/plusls)
- 🌟 👤 [plusls](https://github.com/plusls) Starred [lmacken/pyrasite](https://github.com/lmacken/pyrasite)
  > Inject code into running Python processes
- 🌟 👤 [plusls](https://github.com/plusls) Starred [giannisanni/neutronstar](https://github.com/giannisanni/neutronstar)
  > Giant MoE models on a single consumer GPU by streaming experts from SSD. CUDA fork of antirez/ds4: r...

### [poorevil](https://github.com/poorevil)
- 🌟 👤 [poorevil](https://github.com/poorevil) Starred [OpenMOSS/MOSS-TTS-Nano](https://github.com/OpenMOSS/MOSS-TTS-Nano)
  > MOSS-TTS-Nano is an open-source multilingual tiny speech generation model from MOSI.AI and the OpenM...

### [pplux](https://github.com/pplux)
- 🌟 👤 [pplux](https://github.com/pplux) Starred [catppuccin/nvim](https://github.com/catppuccin/nvim)
  > 🍨 Soothing pastel theme for Neovim

### [progmboy](https://github.com/progmboy)
- 🌟 👤 [progmboy](https://github.com/progmboy) Starred [gigioneggiando/argo](https://github.com/gigioneggiando/argo)
  > 👁️ LLM-native static vulnerability detection. An LLM reads your source like a human auditor, point i...

### [qazbnm456](https://github.com/qazbnm456)
- 🌟 👤 [qazbnm456](https://github.com/qazbnm456) Starred [fuji-mak/Capsomnia](https://github.com/fuji-mak/Capsomnia)
  > A tiny macOS app that turns Caps Lock into a physical keep-awake switch for closed-lid MacBook work.

### [quangnh89](https://github.com/quangnh89)
- 🌟 👤 [quangnh89](https://github.com/quangnh89) Starred [theteatoast/local-vuln-research-pipeline](https://github.com/theteatoast/local-vuln-research-pipeline)
  > Fully local vulnerability research pipeline - 14B code-specialized LLM reviews every source file exh...

### [rand-tech](https://github.com/rand-tech)
- 🌟 👤 [rand-tech](https://github.com/rand-tech) Starred [telepresenceio/telepresence](https://github.com/telepresenceio/telepresence)
  > Local development against a remote Kubernetes or OpenShift cluster

### [re-zero001](https://github.com/re-zero001)
- 🌟 👤 [re-zero001](https://github.com/re-zero001) Starred [Kanscape/Hana](https://github.com/Kanscape/Hana)
  > Hanime1 第三方客户端

### [realoriginal](https://github.com/realoriginal)
- 🌟 👤 [realoriginal](https://github.com/realoriginal) Starred [caprinux/rel-fuscate](https://github.com/caprinux/rel-fuscate)
  > A novel obfuscation technique for ELF programs by abusing symbol resolution metadata to break disass...

### [redbearder](https://github.com/redbearder)
- 🌟 👤 [redbearder](https://github.com/redbearder) Starred [Zackriya-Solutions/meetily](https://github.com/Zackriya-Solutions/meetily)
  > Privacy first, AI meeting assistant with 4x faster Parakeet/Whisper live transcription, speaker diar...
- 🌟 👤 [redbearder](https://github.com/redbearder) Starred [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian)
  > Self-organizing AI second brain for Obsidian + Claude Code. Drop any source and Claude reads, links,...
- 🌟 👤 [redbearder](https://github.com/redbearder) Starred [nashsu/llm_wiki](https://github.com/nashsu/llm_wiki)
  > LLM Wiki is a cross-platform desktop application that turns your documents into an organized, interl...
- 🌟 👤 [redbearder](https://github.com/redbearder) Starred [garrytan/gbrain](https://github.com/garrytan/gbrain)
  > Garry's Opinionated OpenClaw/Hermes Agent Brain

### [ret2ldz](https://github.com/ret2ldz)
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [scottyab/rootbeer](https://github.com/scottyab/rootbeer)
  > Simple to use root checking Android library and sample app
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [open-obfuscator/o-mvll](https://github.com/open-obfuscator/o-mvll)
  > :electron: O-MVLL is a code obfuscation tool based on LLVM for native code (Android and iOS).
- 🌟 👤 [ret2ldz](https://github.com/ret2ldz) Starred [ret2ldz/Florida](https://github.com/ret2ldz/Florida)
  > 基础反检测 frida-server / Basic anti-detection frida-server

### [revercc](https://github.com/revercc)
- 🌟 👤 [revercc](https://github.com/revercc) Starred [shi-yan/H264Naked](https://github.com/shi-yan/H264Naked)
  > A H264 frame data viewer
- 🌟 👤 [revercc](https://github.com/revercc) Starred [shenbengit/WebRTCExtension](https://github.com/shenbengit/WebRTCExtension)
  > Android WebRTC extension function.

### [riba2534](https://github.com/riba2534)
- 🌟 👤 [riba2534](https://github.com/riba2534) Starred [iOfficeAI/OfficeCLI](https://github.com/iOfficeAI/OfficeCLI)
  > OfficeCLI is the first and best Office suite  purpose-built for AI agents to read, edit, and automat...

### [riusksk](https://github.com/riusksk)
- 🌟 👤 [riusksk](https://github.com/riusksk) Starred [ffuf/ffuf](https://github.com/ffuf/ffuf)
  > Fast web fuzzer written in Go

### [roadwy](https://github.com/roadwy)
- 🌟 👤 [roadwy](https://github.com/roadwy) Starred [NirvanaOn/AntiVE-BehaviorWatch](https://github.com/NirvanaOn/AntiVE-BehaviorWatch)
  > AntiVE-BehaviorWatch is an advanced behavioral analysis malware that detects automated analysis syst...
- 🌟 👤 [roadwy](https://github.com/roadwy) Starred [crackmesone/crackmes-re-dataset](https://github.com/crackmesone/crackmes-re-dataset)
  > Labeled reverse-engineering dataset over 4,598 crackmes: flags, verifier scripts, and normalized obf...
- 🌟 👤 [roadwy](https://github.com/roadwy) Starred [zjunlp/EasyEdit](https://github.com/zjunlp/EasyEdit)
  > [ACL 2024] An Easy-to-use Knowledge Editing Framework for LLMs.
- 🌟 👤 [roadwy](https://github.com/roadwy) Starred [Tencent/VulnGym](https://github.com/Tencent/VulnGym)
  > VulnGym: A Real-World, Project-Level Vulnerability Benchmark for White-Box Vulnerability-Hunting Age...
- 🌟 👤 [roadwy](https://github.com/roadwy) Starred [OpenCut-app/OpenCut](https://github.com/OpenCut-app/OpenCut)
  > The open-source CapCut alternative

### [rogertan30](https://github.com/rogertan30)
- 🌟 👤 [rogertan30](https://github.com/rogertan30) Starred [hypeapps/FixMath](https://github.com/hypeapps/FixMath)
  > Android game: FixMath it's mathematical game in which your task is to fix math calculations.
- 🌟 👤 [rogertan30](https://github.com/rogertan30) Starred [SimonSchubert/Braincup](https://github.com/SimonSchubert/Braincup)
  > Memory, focus & math trainer
- 🌟 👤 [rogertan30](https://github.com/rogertan30) Starred [th3pi/Math-Tank](https://github.com/th3pi/Math-Tank)
  > A fun android game to train your brain with some quick math quizes.
- 🌟 👤 [rogertan30](https://github.com/rogertan30) Starred [kapaha/math-castle](https://github.com/kapaha/math-castle)
  > An educational castle defence game made for The Odin Project's Game Jam.
- 🌟 👤 [rogertan30](https://github.com/rogertan30) Starred [SebastianGalan76/Multiplication-Math-Games](https://github.com/SebastianGalan76/Multiplication-Math-Games)
  > Android Studio, Java, Android App
- 🌟 👤 [rogertan30](https://github.com/rogertan30) Starred [nividata-consultancy/math-riddle-free](https://github.com/nividata-consultancy/math-riddle-free)
  > Open source repo of math-riddle game launched on Playstore

### [rogxo](https://github.com/rogxo)
- 🌟 👤 [rogxo](https://github.com/rogxo) Starred [NVIDIA/FasterTransformer](https://github.com/NVIDIA/FasterTransformer)
  > Transformer related optimization, including BERT, GPT
- 🌟 👤 [rogxo](https://github.com/rogxo) Starred [MicrosoftDocs/Virtualization-Documentation](https://github.com/MicrosoftDocs/Virtualization-Documentation)
  > Place to store our documentation, code samples, etc for public consumption.

### [sin5678](https://github.com/sin5678)
- 🌟 👤 [sin5678](https://github.com/sin5678) Starred [hagezi/dns-blocklists](https://github.com/hagezi/dns-blocklists)
  > DNS-Blocklists: For a better internet - keep the internet clean!

### [smallevilbeast](https://github.com/smallevilbeast)
- 🌟 👤 [smallevilbeast](https://github.com/smallevilbeast) Starred [peetzweg/opendisplay](https://github.com/peetzweg/opendisplay)
  > Free, open-source Sidecar/Duet alternative — use your iPhone or iPad as a true second monitor for yo...

### [smallzhong](https://github.com/smallzhong)
- 🌟 👤 [smallzhong](https://github.com/smallzhong) Starred [mahmoudimus/ida-sigmaker](https://github.com/mahmoudimus/ida-sigmaker)
  > sigmaker is a zero-dependency IDA Pro 9.0+ cross-platform signature maker plugin with optional SIMD ...
- 🍴 👤 [smallzhong](https://github.com/smallzhong) Forked [mahmoudimus/ida-sigmaker](https://github.com/mahmoudimus/ida-sigmaker) to [smallzhong/ida-sigmaker](https://github.com/smallzhong/ida-sigmaker)
  > sigmaker is a zero-dependency IDA Pro 9.0+ cross-platform signature maker plugin with optional SIMD ...

### [solohsu](https://github.com/solohsu)
- 🍴 👤 [solohsu](https://github.com/solohsu) Forked [GCB2023/iptvorganization_TV](https://github.com/GCB2023/iptvorganization_TV) to [solohsu/iptvorganization_TV](https://github.com/solohsu/iptvorganization_TV)

### [ssfdust](https://github.com/ssfdust)
- 🍴 👤 [ssfdust](https://github.com/ssfdust) Forked [romus204/tree-sitter-manager.nvim](https://github.com/romus204/tree-sitter-manager.nvim) to [ssfdust/tree-sitter-manager.nvim](https://github.com/ssfdust/tree-sitter-manager.nvim)
  > A lightweight Tree-sitter parser manager for Neovim.

### [std-microblock](https://github.com/std-microblock)
- 🍴 👤 [std-microblock](https://github.com/std-microblock) Forked [AyuGram/AyuGramDesktop](https://github.com/AyuGram/AyuGramDesktop) to [bqqnt-dev/AyuGramDesktop](https://github.com/bqqnt-dev/AyuGramDesktop)
  > Desktop Telegram client with good customization and Ghost mode.

### [swisskyrepo](https://github.com/swisskyrepo)
- 🌟 👤 [swisskyrepo](https://github.com/swisskyrepo) Starred [0x4D31/endpoint-ai-agent-abuse](https://github.com/0x4D31/endpoint-ai-agent-abuse)
  > EAA is a curated catalog of techniques and real-world cases involving abuse of local AI agents throu...

### [tamlok](https://github.com/tamlok)
- 🌟 👤 [tamlok](https://github.com/tamlok) Starred [typst/typst](https://github.com/typst/typst)
  > A markup-based typesetting system that is powerful and easy to learn.

### [taviso](https://github.com/taviso)
- 🌟 👤 [taviso](https://github.com/taviso) Starred [knqyf263/pet](https://github.com/knqyf263/pet)
  > Simple command-line snippet manager

### [thelicato](https://github.com/thelicato)
- 🌟 👤 [thelicato](https://github.com/thelicato) Starred [JustVugg/colibri](https://github.com/JustVugg/colibri)
  > Run GLM-5.2 (744B MoE) on a 25GB-RAM consumer machine — pure C, zero deps, experts streamed from dis...

### [uforever](https://github.com/uforever)
- 🌟 👤 [uforever](https://github.com/uforever) Starred [osm0sis/PlayIntegrityFork](https://github.com/osm0sis/PlayIntegrityFork)
  > Fix Play Integrity <A13 verdicts, allowing custom fields and props

### [wh1te4ever](https://github.com/wh1te4ever)
- 🌟 👤 [wh1te4ever](https://github.com/wh1te4ever) Starred [bin456789/reinstall](https://github.com/bin456789/reinstall)
  > 一键DD/重装脚本 (One-click reinstall OS on VPS)

### [winezer0](https://github.com/winezer0)
- 🌟 👤 [winezer0](https://github.com/winezer0) Starred [owl234/ARL-Next](https://github.com/owl234/ARL-Next)
  > 🚀 自动化资产侦察与漏洞监控平台 (ARL-Next)。重构自 ARL 经典版，升级为 Puppeteer + Nuclei 引擎，打通“天眼查/ICP ➔ 资产发现 ➔ 漏洞扫描➔ 威胁情报追踪”全...
- 🍴 👤 [winezer0](https://github.com/winezer0) Forked [owl234/ARL-Next](https://github.com/owl234/ARL-Next) to [winezer0/ARL-Next](https://github.com/winezer0/ARL-Next)
  > 🚀 自动化资产侦察与漏洞监控平台 (ARL-Next)。重构自 ARL 经典版，升级为 Puppeteer + Nuclei 引擎，打通“天眼查/ICP ➔ 资产发现 ➔ 漏洞扫描➔ 威胁情报追踪”全...
- 🌟 👤 [winezer0](https://github.com/winezer0) Starred [ast-grep/ast-grep](https://github.com/ast-grep/ast-grep)
  > ⚡A CLI tool for code structural search, lint and rewriting. Written in Rust

### [winmin](https://github.com/winmin)
- 🌟 👤 [winmin](https://github.com/winmin) Starred [google/mantis](https://github.com/google/mantis)
  > A modular, stack-agnostic toolkit of security review skills for AI coding agents to autonomously fin...

### [wonderkun](https://github.com/wonderkun)
- 🌟 👤 [wonderkun](https://github.com/wonderkun) Starred [crackmesone/crackmes-re-dataset](https://github.com/crackmesone/crackmes-re-dataset)
  > Labeled reverse-engineering dataset over 4,598 crackmes: flags, verifier scripts, and normalized obf...

### [wszxl516](https://github.com/wszxl516)
- 🌟 👤 [wszxl516](https://github.com/wszxl516) Starred [Awesome-Embedded-Learning-Studio/Cinux](https://github.com/Awesome-Embedded-Learning-Studio/Cinux)
  > Cinux, C/C++'s version of X64 Operating System, using modern feature in C/C++!
- 🌟 👤 [wszxl516](https://github.com/wszxl516) Starred [WayfireWM/wayfire](https://github.com/WayfireWM/wayfire)
  > A modular and extensible wayland compositor
- 🌟 👤 [wszxl516](https://github.com/wszxl516) Starred [rustp2p/rustp2p](https://github.com/rustp2p/rustp2p)
  > A decentralized p2p library powered by Rust, which is devoted to simple use
- 🌟 👤 [wszxl516](https://github.com/wszxl516) Starred [restsend/flowdb](https://github.com/restsend/flowdb)
  > A high-performance embedded time-series + JSON document storage engine (LSM-tree), with built-in Ind...

### [wy876](https://github.com/wy876)
- 🌟 👤 [wy876](https://github.com/wy876) Starred [mtvpls/MoonTVPlus](https://github.com/mtvpls/MoonTVPlus)
  > MoonTVPlus 是基于 MoonTV v100 二次开发的增强版影视聚合播放器。它在原版基础上新增了外部播放器支持、视频超分、弹幕系统、评论抓取等实用功能，提供更强大的观影体验。

### [x7dbg](https://github.com/x7dbg)
- 🍴 👤 [x7dbg](https://github.com/x7dbg) Forked [KSwordDEV/KSword](https://github.com/KSwordDEV/KSword) to [x7dbg/KSword](https://github.com/x7dbg/KSword)
  > KSword 5.1 is an open-source Windows toolkit for ARK, kernel debugging, and system forensics.  中文：KS...
- 🌟 👤 [x7dbg](https://github.com/x7dbg) Starred [KSwordDEV/KswordFrame](https://github.com/KSwordDEV/KswordFrame)
  > Windows C++ GUI Work Frame

### [xiaoweigege](https://github.com/xiaoweigege)
- 🌟 👤 [xiaoweigege](https://github.com/xiaoweigege) Starred [CircuitSavage/hcaptcha-hsj-hsw-reversed](https://github.com/CircuitSavage/hcaptcha-hsj-hsw-reversed)
  > hCaptcha hsj/hsw master-key extraction — byte-accurate per build
- 🍴 👤 [xiaoweigege](https://github.com/xiaoweigege) Forked [CircuitSavage/hcaptcha-hsj-hsw-reversed](https://github.com/CircuitSavage/hcaptcha-hsj-hsw-reversed) to [xiaoweigege/hcaptcha-hsj-hsw-reversed](https://github.com/xiaoweigege/hcaptcha-hsj-hsw-reversed)
  > hCaptcha hsj/hsw master-key extraction — byte-accurate per build

### [xsser](https://github.com/xsser)
- 🌟 👤 [xsser](https://github.com/xsser) Starred [UKGovernmentBEIS/inspect_ai](https://github.com/UKGovernmentBEIS/inspect_ai)
  > Inspect: A framework for large language model evaluations

### [yahch](https://github.com/yahch)
- 🌟 👤 [yahch](https://github.com/yahch) Starred [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md)
  > A collection of DESIGN.md files analysis by popular brand design systems. Drop one into your project...

### [yhy0](https://github.com/yhy0)
- 🌟 👤 [yhy0](https://github.com/yhy0) Starred [Jakubantalik/border-beam](https://github.com/Jakubantalik/border-beam)
  > Animated border beam effect for React
- 🌟 👤 [yhy0](https://github.com/yhy0) Starred [JustVugg/colibri](https://github.com/JustVugg/colibri)
  > Run GLM-5.2 (744B MoE) on a 25GB-RAM consumer machine — pure C, zero deps, experts streamed from dis...

### [yossizap](https://github.com/yossizap)
- 🍴 👤 [yossizap](https://github.com/yossizap) Forked [linux-msm/qdl](https://github.com/linux-msm/qdl) to [yossizap/qdl](https://github.com/yossizap/qdl)
  > Command-line tool for flashing firmware on Qualcomm-based devices via USB Emergency Download (EDL) m...

### [zacbre](https://github.com/zacbre)
- 🌟 👤 [zacbre](https://github.com/zacbre) Starred [safijari/openpuck](https://github.com/safijari/openpuck)
  > Opensource firmware for NRF52840 Pro Micro that copycats the Steam Controller 2 Puck and adds an Xbo...

### [zenghongtu](https://github.com/zenghongtu)
- 🌟 👤 [zenghongtu](https://github.com/zenghongtu) Starred [threerocks/hand-drawn-styles](https://github.com/threerocks/hand-drawn-styles)
  > Claude Code skill：把内容套进内置手绘画风配方,产出可直接复制的生图提示词。内置儿童涂色/极简线条/蜡笔童涂/吉卜力/小豆人涂鸦 5 种已验证画风。

### [zhanghai](https://github.com/zhanghai)
- 🌟 👤 [zhanghai](https://github.com/zhanghai) Starred [bitsycore/compose-desktop-native](https://github.com/bitsycore/compose-desktop-native)
  > Compose Desktop for native target using SDL3

### [zhangyoufu](https://github.com/zhangyoufu)
- 🌟 👤 [zhangyoufu](https://github.com/zhangyoufu) Starred [verygenericname/SSHRD_Script](https://github.com/verygenericname/SSHRD_Script)
  > Shell script that creates a ssh ramdisk

### [zhanlan123](https://github.com/zhanlan123)
- 🌟 👤 [zhanlan123](https://github.com/zhanlan123) Starred [afollestad/vvalidator](https://github.com/afollestad/vvalidator)
  > 🤖 An easy to use form validator for Kotlin & Android.

### [zhaotianff](https://github.com/zhaotianff)
- 🌟 👤 [zhaotianff](https://github.com/zhaotianff) Starred [Raphire/Win11Debloat](https://github.com/Raphire/Win11Debloat)
  > A simple, lightweight PowerShell script that allows you to remove pre-installed apps, disable teleme...

### [zu1k](https://github.com/zu1k)
- 🌟 👤 [zu1k](https://github.com/zu1k) Starred [wess/guise](https://github.com/wess/guise)
  > Mantine inspired components for Zed's GPUI library.
- 🌟 👤 [zu1k](https://github.com/zu1k) Starred [alibaba/zvec](https://github.com/alibaba/zvec)
  > A lightweight, lightning-fast, in-process vector database


---
*最后更新于 2026-07-14 18:03:02 UTC*
*历史记录保存在 `archive` 目录中。*
