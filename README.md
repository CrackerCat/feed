# 每日 GitHub 动态 (2026-07-04)

我关注用户的今日公开动态 (每15分钟更新)。

## 今日动态

### [0xSh4dy](https://github.com/0xSh4dy)
- 🌟 👤 [0xSh4dy](https://github.com/0xSh4dy) Starred [gingrspacecadet/coda](https://github.com/gingrspacecadet/coda)
  > A fresh attempt at a systems language, without all the clutter!

### [5ec1cff](https://github.com/5ec1cff)
- 🌟 👤 [5ec1cff](https://github.com/5ec1cff) Starred [rr-debugger/rr](https://github.com/rr-debugger/rr)
  > Record and Replay Framework

### [AnthonyCalandra](https://github.com/AnthonyCalandra)
- 🌟 👤 [AnthonyCalandra](https://github.com/AnthonyCalandra) Starred [apple/container](https://github.com/apple/container)
  > A tool for creating and running Linux containers using lightweight virtual machines on a Mac. It is ...

### [C0untFloyd](https://github.com/C0untFloyd)
- 🌟 👤 [C0untFloyd](https://github.com/C0untFloyd) Starred [ingui-n/musescore-downloader](https://github.com/ingui-n/musescore-downloader)
  > Browser extension allowing to download music sheets, audio and midi files for free

### [CHYbeta](https://github.com/CHYbeta)
- 🌟 👤 [CHYbeta](https://github.com/CHYbeta) Starred [anysearch-ai/anysearch-skill](https://github.com/anysearch-ai/anysearch-skill)
  > Unified real-time search engine skill for AI agents. 
- 🌟 👤 [CHYbeta](https://github.com/CHYbeta) Starred [openhackai/OpenHack](https://github.com/openhackai/OpenHack)
  > Open Source Agentic Security Scanner

### [ColdWindScholar](https://github.com/ColdWindScholar)
- 🍴 👤 [ColdWindScholar](https://github.com/ColdWindScholar) Forked [zhouziyang/libndk_translation](https://github.com/zhouziyang/libndk_translation) to [Nredroid/libndk_translation](https://github.com/Nredroid/libndk_translation)

### [Cracked5pider](https://github.com/Cracked5pider)
- 🌟 👤 [Cracked5pider](https://github.com/Cracked5pider) Starred [l00sy4/LuciSocks](https://github.com/l00sy4/LuciSocks)
  > Versatile SOCKS5 proxying tool

### [Dump-GUY](https://github.com/Dump-GUY)
- 🍴 👤 [Dump-GUY](https://github.com/Dump-GUY) Forked [HotCakeX/Harden-Windows-Security](https://github.com/HotCakeX/Harden-Windows-Security) to [Dump-GUY/Harden-Windows-Security](https://github.com/Dump-GUY/Harden-Windows-Security)
  > Harden Windows Safely, Securely using Official Supported Microsoft methods and proper explanation | ...

### [Evian-Zhang](https://github.com/Evian-Zhang)
- 🍴 👤 [Evian-Zhang](https://github.com/Evian-Zhang) Forked [apache/arrow-rs](https://github.com/apache/arrow-rs) to [Evian-Zhang/arrow-rs](https://github.com/Evian-Zhang/arrow-rs)
  > Official Rust implementation of Apache Arrow

### [FldBudin](https://github.com/FldBudin)
- 🌟 👤 [FldBudin](https://github.com/FldBudin) Starred [browser-use/video-use](https://github.com/browser-use/video-use)
  > Edit videos with coding agents
- 🌟 👤 [FldBudin](https://github.com/FldBudin) Starred [coding-chimera/chimera](https://github.com/coding-chimera/chimera)
  > AI coding agent CLI with built-in CodeGraph-powered impact analysis and propagation audit, harness t...

### [GaoYuCan](https://github.com/GaoYuCan)
- 🌟 👤 [GaoYuCan](https://github.com/GaoYuCan) Starred [fishjar/kiss-translator](https://github.com/fishjar/kiss-translator)
  > A simple, open source bilingual translation extension & Greasemonkey script (一个简约、开源的 双语对照翻译扩展 & 油猴脚...

### [Gracker](https://github.com/Gracker)
- 🌟 👤 [Gracker](https://github.com/Gracker) Starred [Fenng/Tech-Doc-Style-Chinese](https://github.com/Fenng/Tech-Doc-Style-Chinese)
  > Yet another reusable writing skill for Chinese technical documentation and product copy.

### [Grigory-Rylov](https://github.com/Grigory-Rylov)
- 🌟 👤 [Grigory-Rylov](https://github.com/Grigory-Rylov) Starred [korwin86/vk-hermes-bot](https://github.com/korwin86/vk-hermes-bot)

### [H3d9](https://github.com/H3d9)
- 🌟 👤 [H3d9](https://github.com/H3d9) Starred [P4nda0s/reverse-skills](https://github.com/P4nda0s/reverse-skills)
  > Reverse engineering skills for Claude Code | 逆向工程 Claude Code Skills 插件

### [HSSkyBoy](https://github.com/HSSkyBoy)
- 🌟 👤 [HSSkyBoy](https://github.com/HSSkyBoy) Starred [7723mod/NPatch](https://github.com/7723mod/NPatch)
  > NPatch是一个复刻自LSPatch，以LSPosed为基础的免root的Xposed框架

### [HdShare](https://github.com/HdShare)
- 🌟 👤 [HdShare](https://github.com/HdShare) Starred [debugtheworldbot/keyStats](https://github.com/debugtheworldbot/keyStats)
  > Keyboard & Mouse Stats for macOS/Windows

### [Howard20181](https://github.com/Howard20181)
- 🌟 👤 [Howard20181](https://github.com/Howard20181) Starred [Nekogram/Nekogram](https://github.com/Nekogram/Nekogram)
  > Open-source third-party Telegram client with not many but useful modifications.

### [Keno](https://github.com/Keno)
- 🍴 👤 [Keno](https://github.com/Keno) Forked [MakieOrg/Makie.jl](https://github.com/MakieOrg/Makie.jl) to [KenoAIStaging/Makie.jl](https://github.com/KenoAIStaging/Makie.jl)
  > Interactive data visualizations and plotting in Julia

### [KunMinX](https://github.com/KunMinX)
- 🌟 👤 [KunMinX](https://github.com/KunMinX) Starred [snownico0722/PaperTodo](https://github.com/snownico0722/PaperTodo)
  > 极简 Windows 桌面便签工具。让桌面上有几张安静、可用、不会打扰人的纸。WPF 原生，支持待办与 Markdown。——A minimalist Windows desktop sticky n...

### [LunFengChen](https://github.com/LunFengChen)
- 🌟 👤 [LunFengChen](https://github.com/LunFengChen) Starred [marsinator358/luajit-decompiler-v2](https://github.com/marsinator358/luajit-decompiler-v2)
  > LuaJIT bytecode decompiler
- 🍴 👤 [LunFengChen](https://github.com/LunFengChen) Forked [marsinator358/luajit-decompiler-v2](https://github.com/marsinator358/luajit-decompiler-v2) to [LunFengChen/luajit-decompiler-v2](https://github.com/LunFengChen/luajit-decompiler-v2)
  > LuaJIT bytecode decompiler

### [MartinKayJr](https://github.com/MartinKayJr)
- 🌟 👤 [MartinKayJr](https://github.com/MartinKayJr) Starred [vibe-motion/skills](https://github.com/vibe-motion/skills)
  > agent skills for vibe motion

### [Moe-hacker](https://github.com/Moe-hacker)
- 🌟 👤 [Moe-hacker](https://github.com/Moe-hacker) Starred [superturtlee/anland](https://github.com/superturtlee/anland)
  > Protocol For Wayland run on android
- 🌟 👤 [Moe-hacker](https://github.com/Moe-hacker) Starred [superturtlee/gbl_root_canoe](https://github.com/superturtlee/gbl_root_canoe)
  > No TrickyStore on Canoe
- 🌟 👤 [Moe-hacker](https://github.com/Moe-hacker) Starred [lfdevs/anland-termux](https://github.com/lfdevs/anland-termux)
  > Use Anland in Termux (especially PRoot and Chroot)

### [MorvanZhou](https://github.com/MorvanZhou)
- 🌟 👤 [MorvanZhou](https://github.com/MorvanZhou) Starred [h4ckf0r0day/obscura](https://github.com/h4ckf0r0day/obscura)
  > The headless browser for AI agents and web scraping

### [Mr-xn](https://github.com/Mr-xn)
- 🌟 👤 [Mr-xn](https://github.com/Mr-xn) Starred [curlconverter/curlconverter](https://github.com/curlconverter/curlconverter)
  > Transpile curl commands into Python, JavaScript and 27 other languages

### [MuggleWei](https://github.com/MuggleWei)
- 🌟 👤 [MuggleWei](https://github.com/MuggleWei) Starred [hikariming/ghfind](https://github.com/hikariming/ghfind)
  > Discover the best developers — and become one. Drop a GitHub handle for a 0–100 value & trust score ...

### [N3rdL0rd](https://github.com/N3rdL0rd)
- 🌟 👤 [N3rdL0rd](https://github.com/N3rdL0rd) Starred [N3rdL0rd/webhl](https://github.com/N3rdL0rd/webhl)
  > Turn Hashlink bytecode into WASM
- 🌟 👤 [N3rdL0rd](https://github.com/N3rdL0rd) Starred [N3rdL0rd/ModDocCE](https://github.com/N3rdL0rd/ModDocCE)
- 🌟 👤 [N3rdL0rd](https://github.com/N3rdL0rd) Starred [N3rdL0rd/hlsteam](https://github.com/N3rdL0rd/hlsteam)
  > Haxe native extension for the Steam API
- 🌟 👤 [N3rdL0rd](https://github.com/N3rdL0rd) Starred [N3rdL0rd/hlmod-hl](https://github.com/N3rdL0rd/hlmod-hl)
  > A virtual machine for Haxe
- 🌟 👤 [N3rdL0rd](https://github.com/N3rdL0rd) Starred [N3rdL0rd/hxbit.py](https://github.com/N3rdL0rd/hxbit.py)
- 🌟 👤 [N3rdL0rd](https://github.com/N3rdL0rd) Starred [N3rdL0rd/hldiscord](https://github.com/N3rdL0rd/hldiscord)
- 🌟 👤 [N3rdL0rd](https://github.com/N3rdL0rd) Starred [N3rdL0rd/hlchroma](https://github.com/N3rdL0rd/hlchroma)

### [No-Github](https://github.com/No-Github)
- 🌟 👤 [No-Github](https://github.com/No-Github) Starred [Esonhugh/claude-code](https://github.com/Esonhugh/claude-code)
  > self maintain claude code

### [P001water](https://github.com/P001water)
- 🌟 👤 [P001water](https://github.com/P001water) Starred [nyu-mlab/iot-inspector-client](https://github.com/nyu-mlab/iot-inspector-client)
  > IoT Inspector: capturing and analyzing your smart home network traffic

### [S3cur3Th1sSh1t](https://github.com/S3cur3Th1sSh1t)
- 🌟 👤 [S3cur3Th1sSh1t](https://github.com/S3cur3Th1sSh1t) Starred [provos/ironcurtain](https://github.com/provos/ironcurtain)
  > A secure* runtime for autonomous AI agents. Policy from plain-English constitutions. (*https://ironc...

### [SakuraKoi](https://github.com/SakuraKoi)
- 🌟 👤 [SakuraKoi](https://github.com/SakuraKoi) Starred [hajidishu/oosPlusfix](https://github.com/hajidishu/oosPlusfix)
  > 一个适用于 ColorOS 的优化模块

### [Seyud](https://github.com/Seyud)
- 🌟 👤 [Seyud](https://github.com/Seyud) Starred [qwq233/OhMyKeymint](https://github.com/qwq233/OhMyKeymint)
- 🌟 👤 [Seyud](https://github.com/Seyud) Starred [z1298808165/Storage-redirection-X-Public](https://github.com/z1298808165/Storage-redirection-X-Public)
  > 存储重定向 X

### [SherlockChiang](https://github.com/SherlockChiang)
- 🌟 👤 [SherlockChiang](https://github.com/SherlockChiang) Starred [adithya-s-k/omniparse](https://github.com/adithya-s-k/omniparse)
  > Ingest, parse, and optimize any data format ➡️ from documents to multimedia ➡️ for enhanced compatib...
- 🌟 👤 [SherlockChiang](https://github.com/SherlockChiang) Starred [opendatalab/MinerU](https://github.com/opendatalab/MinerU)
  > Transforms complex documents like PDFs and Office docs into LLM-ready markdown/JSON for your Agentic...
- 🌟 👤 [SherlockChiang](https://github.com/SherlockChiang) Starred [retorquere/zotero-better-bibtex](https://github.com/retorquere/zotero-better-bibtex)
  > Make Zotero effective for us LaTeX holdouts

### [Silentsoul04](https://github.com/Silentsoul04)
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [security-attack/BrowserFUZZ](https://github.com/security-attack/BrowserFUZZ)
  > Multi-browser Windows fuzzer by NINJAS Security Research for Chrome, Edge, and Firefox. Generates HT...
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [security-attack/BrowserFUZZ](https://github.com/security-attack/BrowserFUZZ) to [Silentsoul04/BrowserFUZZ](https://github.com/Silentsoul04/BrowserFUZZ)
  > Multi-browser Windows fuzzer by NINJAS Security Research for Chrome, Edge, and Firefox. Generates HT...
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [MozillaSecurity/grizzly](https://github.com/MozillaSecurity/grizzly)
  > A cross-platform browser fuzzing framework
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [MozillaSecurity/grizzly](https://github.com/MozillaSecurity/grizzly) to [Silentsoul04/grizzly](https://github.com/Silentsoul04/grizzly)
  > A cross-platform browser fuzzing framework
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [Correia-jpv/fucking-Awesome-Hacking](https://github.com/Correia-jpv/fucking-Awesome-Hacking)
  > A collection of various awesome lists for hackers, pentesters and security researchers. With reposit...
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [Correia-jpv/fucking-Awesome-Hacking](https://github.com/Correia-jpv/fucking-Awesome-Hacking) to [Silentsoul04/fucking-Awesome-Hacking](https://github.com/Silentsoul04/fucking-Awesome-Hacking)
  > A collection of various awesome lists for hackers, pentesters and security researchers. With reposit...
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [cpuu/awesome-fuzzing](https://github.com/cpuu/awesome-fuzzing)
  > A curated list of awesome Fuzzing(or Fuzz Testing) for software security
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [cpuu/awesome-fuzzing](https://github.com/cpuu/awesome-fuzzing) to [Silentsoul04/awesome-fuzzing](https://github.com/Silentsoul04/awesome-fuzzing)
  > A curated list of awesome Fuzzing(or Fuzz Testing) for software security
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [forestfoxx/awesome-hardware-fuzzing](https://github.com/forestfoxx/awesome-hardware-fuzzing) to [Silentsoul04/awesome-hardware-fuzzing](https://github.com/Silentsoul04/awesome-hardware-fuzzing)
  > A curated list of research and repositories on the novel technique of hardware fuzzing
- 🍴 👤 [Silentsoul04](https://github.com/Silentsoul04) Forked [strongcourage/awesome-directed-fuzzing](https://github.com/strongcourage/awesome-directed-fuzzing) to [Silentsoul04/awesome-directed-fuzzing](https://github.com/Silentsoul04/awesome-directed-fuzzing)
  > A curated list of awesome directed fuzzing research papers
- 🌟 👤 [Silentsoul04](https://github.com/Silentsoul04) Starred [gmelodie/awesome-wordlists](https://github.com/gmelodie/awesome-wordlists)
  > A curated list of wordlists for bruteforcing and fuzzing

### [Skorpion96](https://github.com/Skorpion96)
- 🌟 👤 [Skorpion96](https://github.com/Skorpion96) Starred [khanhduytran0/coruna](https://github.com/khanhduytran0/coruna)
  > The leaked exploit toolkit for various iOS versions
- 🌟 👤 [Skorpion96](https://github.com/Skorpion96) Starred [prdgmshift/usbliter8](https://github.com/prdgmshift/usbliter8)
  > An A12/A13 SecureROM exploit
- 🌟 👤 [Skorpion96](https://github.com/Skorpion96) Starred [halal-beef/houston-pub](https://github.com/halal-beef/houston-pub)
  > We had a problem - and now, publicly, a solution :)

### [SodaMilk233](https://github.com/SodaMilk233)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [sjshb57/Pairip-Patches](https://github.com/sjshb57/Pairip-Patches)
- 🌟 👤 [SodaMilk233](https://github.com/SodaMilk233) Starred [xiaoguzuiniu/gpt-free-register](https://github.com/xiaoguzuiniu/gpt-free-register)
  > 纯协议层 ChatGPT 账号自动注册工具。不开浏览器、不用 selenium——直接用 curl_cffi 模拟 Chrome TLS 指纹打 HTTP 接口，单进程多线程并发，注册成功后自动跑 C...

### [Super12138](https://github.com/Super12138)
- 🌟 👤 [Super12138](https://github.com/Super12138) Starred [PhoenixNil/XrayUI-dev](https://github.com/PhoenixNil/XrayUI-dev)
  > Lightweight native Windows GUI client for Xray core, built with WinUI 3
- 🌟 👤 [Super12138](https://github.com/Super12138) Starred [XingHeYuZhuan/shiguangschedule](https://github.com/XingHeYuZhuan/shiguangschedule)
  > 一款开源、无广告、极简的课程表 APP，支持教务导入

### [T5ive](https://github.com/T5ive)
- 🌟 👤 [T5ive](https://github.com/T5ive) Starred [Younesfdj/gitfut](https://github.com/Younesfdj/gitfut)
  > Your GitHub stats, turned into a World-Cup-style player card 

### [TheNetAdmin](https://github.com/TheNetAdmin)
- 🌟 👤 [TheNetAdmin](https://github.com/TheNetAdmin) Starred [ARahim3/mlx-dspark](https://github.com/ARahim3/mlx-dspark)
  > DeepSeek's DSpark and z-lab's DFlash speculative decoding — running natively on Apple Silicon via ML...

### [WAY29](https://github.com/WAY29)
- 🍴 👤 [WAY29](https://github.com/WAY29) Forked [bagpack/poe2-pob-copy-button](https://github.com/bagpack/poe2-pob-copy-button) to [WAY29/poe2-pob-copy-button](https://github.com/WAY29/poe2-pob-copy-button)
  > PoE2 trade results add-on that copies PoB2 Create Custom item text (works on non-English sites).

### [WangWei-CM](https://github.com/WangWei-CM)
- 🌟 👤 [WangWei-CM](https://github.com/WangWei-CM) Starred [fishlabs/EASY-HWID-SPOOFER](https://github.com/fishlabs/EASY-HWID-SPOOFER)
  > 基于内核模式的硬件信息欺骗工具
- 🌟 👤 [WangWei-CM](https://github.com/WangWei-CM) Starred [FiYHer/EASY-HWID-SPOOFER](https://github.com/FiYHer/EASY-HWID-SPOOFER)
  > 基于内核模式的硬件信息欺骗工具

### [Xieansecn](https://github.com/Xieansecn)
- 🍴 👤 [Xieansecn](https://github.com/Xieansecn) Forked [RikkaApps/Shizuku](https://github.com/RikkaApps/Shizuku) to [Xieansecn/Shizuku](https://github.com/Xieansecn/Shizuku)
  > Using system APIs directly with adb/root privileges from normal apps through a Java process started ...
- 🌟 👤 [Xieansecn](https://github.com/Xieansecn) Starred [vkmark/vkmark](https://github.com/vkmark/vkmark)
  > Vulkan benchmark

### [Yricky](https://github.com/Yricky)
- 🌟 👤 [Yricky](https://github.com/Yricky) Starred [LeonardNJU/VocoType-linux](https://github.com/LeonardNJU/VocoType-linux)
  > 高性能 Linux 离线中文语音输入法，基于 Ali FunASR(VocoType-cli).  ~0.1s 瞬时上屏，输入法级稳定性， 极高中文准确率、低资源占用(CPU Only).支持 IBu...

### [Yu2erer](https://github.com/Yu2erer)
- 🌟 👤 [Yu2erer](https://github.com/Yu2erer) Starred [karpathy/nanoGPT](https://github.com/karpathy/nanoGPT)
  > The simplest, fastest repository for training/finetuning medium-sized GPTs.
- 🌟 👤 [Yu2erer](https://github.com/Yu2erer) Starred [karpathy/nn-zero-to-hero](https://github.com/karpathy/nn-zero-to-hero)
  > Neural Networks: Zero to Hero

### [ZephrFish](https://github.com/ZephrFish)
- 🌟 👤 [ZephrFish](https://github.com/ZephrFish) Starred [RyanCodrai/turbovec](https://github.com/RyanCodrai/turbovec)
  > A vector index built on TurboQuant, written in Rust with Python bindings

### [aalansehaiyang](https://github.com/aalansehaiyang)
- 🌟 👤 [aalansehaiyang](https://github.com/aalansehaiyang) Starred [anomalyco/opencode](https://github.com/anomalyco/opencode)
  > The open source coding agent.
- 🌟 👤 [aalansehaiyang](https://github.com/aalansehaiyang) Starred [DietrichGebert/ponytail](https://github.com/DietrichGebert/ponytail)
  > Makes your AI agent think like the laziest senior dev in the room. The best code is the code you nev...

### [alternoegraha](https://github.com/alternoegraha)
- 🌟 👤 [alternoegraha](https://github.com/alternoegraha) Starred [microg/GmsCore](https://github.com/microg/GmsCore)
  > Free implementation of Play Services

### [ant4g0nist](https://github.com/ant4g0nist)
- 🌟 👤 [ant4g0nist](https://github.com/ant4g0nist) Starred [antiwork/chromeless](https://github.com/antiwork/chromeless)
  > The browser that isn't there — zero-chrome macOS browser for clean screenshots and fullscreen video....
- 🌟 👤 [ant4g0nist](https://github.com/ant4g0nist) Starred [dougburks/so-crates](https://github.com/dougburks/so-crates)
  > SO-CRATES: Security Onion Containerized Rapid Analysis of Threats, Evil, and Sus!

### [apkunpacker](https://github.com/apkunpacker)
- 🌟 👤 [apkunpacker](https://github.com/apkunpacker) Starred [AldazActivator/usbliter8-c](https://github.com/AldazActivator/usbliter8-c)
  > usbliter8 boot for windows and macos without dependencies
- 🌟 👤 [apkunpacker](https://github.com/apkunpacker) Starred [milankovo/struct-compare](https://github.com/milankovo/struct-compare)

### [b3x206](https://github.com/b3x206)
- 🌟 👤 [b3x206](https://github.com/b3x206) Starred [erincatto/box3d](https://github.com/erincatto/box3d)
  > Box3D is a 3D physics engine for games

### [bigsinger](https://github.com/bigsinger)
- 🌟 👤 [bigsinger](https://github.com/bigsinger) Starred [getgrav/grav](https://github.com/getgrav/grav)
  > Modern, Crazy Fast, Ridiculously Easy and Amazingly Powerful Flat-File CMS powered by PHP, Markdown,...

### [cctv18](https://github.com/cctv18)
- 🌟 👤 [cctv18](https://github.com/cctv18) Starred [sapientinc/HRM-Text](https://github.com/sapientinc/HRM-Text)
  > HRM-Text is a 1B text generation model based on the HRM architecture, strengthened by task completio...
- 🌟 👤 [cctv18](https://github.com/cctv18) Starred [openpgpjs/openpgpjs](https://github.com/openpgpjs/openpgpjs)
  > OpenPGP implementation for JavaScript
- 🌟 👤 [cctv18](https://github.com/cctv18) Starred [gitpod-io/openvscode-server](https://github.com/gitpod-io/openvscode-server)
  > Run upstream VS Code on a remote machine with access through a modern web browser from any device, a...

### [cetfor](https://github.com/cetfor)
- 🌟 👤 [cetfor](https://github.com/cetfor) Starred [zoogies/struct-entropy](https://github.com/zoogies/struct-entropy)
  > Data obfuscation anti-cheat for Unity DOTS

### [cristeigabriela](https://github.com/cristeigabriela)
- 🌟 👤 [cristeigabriela](https://github.com/cristeigabriela) Starred [cristeigabriela/bg-be-gone](https://github.com/cristeigabriela/bg-be-gone)
  > Claude's attempt at making me a local, GPU-backed background remover using BiRefNet. Pretty good!
- 🌟 👤 [cristeigabriela](https://github.com/cristeigabriela) Starred [nagadomi/waifu2x](https://github.com/nagadomi/waifu2x)
  > Image Super-Resolution for Anime-Style Art

### [ctkqiang](https://github.com/ctkqiang)
- 🌟 👤 [ctkqiang](https://github.com/ctkqiang) Starred [postgres/postgres](https://github.com/postgres/postgres)
  > Mirror of the official PostgreSQL GIT repository. Note that this is just a *mirror* - we don't work ...

### [cwuom](https://github.com/cwuom)
- 🌟 👤 [cwuom](https://github.com/cwuom) Starred [tomakino/lyricon](https://github.com/tomakino/lyricon)
  > 为 Android 系统扩展状态栏歌词显示功能的增强工具

### [cxxzhang](https://github.com/cxxzhang)
- 🍴 👤 [cxxzhang](https://github.com/cxxzhang) Forked [Devolutions/IronRDP](https://github.com/Devolutions/IronRDP) to [cxxzhang/IronRDP](https://github.com/cxxzhang/IronRDP)
  > Rust implementation of the Microsoft Remote Desktop Protocol (RDP)

### [defuse](https://github.com/defuse)
- 🍴 👤 [defuse](https://github.com/defuse) Forked [zcash/zallet](https://github.com/zcash/zallet) to [defuse/zallet](https://github.com/defuse/zallet)
  > Repo for replacing the zcashd wallet

### [doodlewind](https://github.com/doodlewind)
- 🌟 👤 [doodlewind](https://github.com/doodlewind) Starred [slint-ui/slint](https://github.com/slint-ui/slint)
  > Slint is an open-source declarative GUI toolkit to build native user interfaces for Rust, C++, JavaS...

### [dwisiswant0](https://github.com/dwisiswant0)
- 🌟 👤 [dwisiswant0](https://github.com/dwisiswant0) Starred [LemmyNet/lemmy](https://github.com/LemmyNet/lemmy)
  > 🐀 A decentralised discussion platform for communities.

### [dzxpert](https://github.com/dzxpert)
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [audiamus/BookLibConnect](https://github.com/audiamus/BookLibConnect)
  > A standalone Audible downloader and decrypter
- 🌟 👤 [dzxpert](https://github.com/dzxpert) Starred [rstewa/Audibly](https://github.com/rstewa/Audibly)
  > Desktop audiobook player for Windows 10/11 using WinUI 3

### [exp-sky](https://github.com/exp-sky)
- 🌟 👤 [exp-sky](https://github.com/exp-sky) Starred [openai/codex](https://github.com/openai/codex)
  > Lightweight coding agent that runs in your terminal

### [extremecoders-re](https://github.com/extremecoders-re)
- 🌟 👤 [extremecoders-re](https://github.com/extremecoders-re) Starred [crynta/terax-ai](https://github.com/crynta/terax-ai)
  > Lightweight (7MB) Terminal-first AI-native dev workspace

### [feicong](https://github.com/feicong)
- 🍴 👤 [feicong](https://github.com/feicong) Forked [davidprush/RemoveAsahi](https://github.com/davidprush/RemoveAsahi) to [feicong/RemoveAsahi](https://github.com/feicong/RemoveAsahi)
  > Script to remove Asahi linux

### [fjh658](https://github.com/fjh658)
- 🌟 👤 [fjh658](https://github.com/fjh658) Starred [cherepavel/VPN-Detector](https://github.com/cherepavel/VPN-Detector)
  > Android app for detecting VPN usage and network tunneling signals.

### [gamelaster](https://github.com/gamelaster)
- 🍴 👤 [gamelaster](https://github.com/gamelaster) Forked [bouffalolab/bouffalo_sdk](https://github.com/bouffalolab/bouffalo_sdk) to [openbouffalo/bouffalo_sdk_bl808](https://github.com/openbouffalo/bouffalo_sdk_bl808)
  > BouffaloSDK is the IOT and MCU software development kit provided by the Bouffalo Lab Team, supports ...

### [gmh5225](https://github.com/gmh5225)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [n0qword/win32k-callback-detouring](https://github.com/n0qword/win32k-callback-detouring) to [gmh5225/win32k-callback-detouring](https://github.com/gmh5225/win32k-callback-detouring)
  > Abusing the win32k.sys kernel callback mechanism for arbitrary code execution
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [Ape-xCV/Nika-Read-Only](https://github.com/Ape-xCV/Nika-Read-Only) to [gmh5225/Nika-Read-Only](https://github.com/gmh5225/Nika-Read-Only)
- 🍴 👤 [gmh5225](https://github.com/gmh5225) Forked [WhiteNightShadow/firefox-reverse](https://github.com/WhiteNightShadow/firefox-reverse) to [gmh5225/firefox-reverse](https://github.com/gmh5225/firefox-reverse)
  > 🦊 内置 AI 逆向 Agent 的 Firefox — 通用 JS/JSVMP/WASM/签名逆向工作站，SpiderMonkey 引擎层非侵入 trace，把加密参数从黑盒还原成不依赖浏览器的纯算...

### [guiying712](https://github.com/guiying712)
- 🌟 👤 [guiying712](https://github.com/guiying712) Starred [kritishmohapatra/100_Days_100_IoT_Projects](https://github.com/kritishmohapatra/100_Days_100_IoT_Projects)
  > A 100-day challenge exploring IoT and embedded systems using ESP32, ESP8266, and Raspberry Pi Pico w...

### [hanc00l](https://github.com/hanc00l)
- 🌟 👤 [hanc00l](https://github.com/hanc00l) Starred [owu/wsl-dashboard](https://github.com/owu/wsl-dashboard)
  > A GUI manager for WSL featuring a modern UI — a lightweight, low‑memory, high‑performance dashboard ...

### [hhhaiai](https://github.com/hhhaiai)
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [tashfeenahmed/freellmapi](https://github.com/tashfeenahmed/freellmapi)
  > OpenAI-compatible proxy that stacks the free tiers of 16 LLM providers (~1.7B tokens/month) behind o...
- 🍴 👤 [hhhaiai](https://github.com/hhhaiai) Forked [chengtx007/fuckoai](https://github.com/chengtx007/fuckoai) to [openChatGpts/fuckoai](https://github.com/openChatGpts/fuckoai)
  > Linux container registration control panel
- 🌟 👤 [hhhaiai](https://github.com/hhhaiai) Starred [Simprint/simprint](https://github.com/Simprint/simprint)
  > [指纹浏览器] Simprint is an open-source fingerprint browser and programmable browser workspace designed f...
- 🍴 👤 [hhhaiai](https://github.com/hhhaiai) Forked [Simprint/simprint](https://github.com/Simprint/simprint) to [hhhaiai/simprint](https://github.com/hhhaiai/simprint)
  > [指纹浏览器] Simprint is an open-source fingerprint browser and programmable browser workspace designed f...

### [hujiayucc](https://github.com/hujiayucc)
- 🌟 👤 [hujiayucc](https://github.com/hujiayucc) Starred [thedjchi/Shizuku](https://github.com/thedjchi/Shizuku)
  > Using system APIs directly with adb/root privileges from normal apps through a Java process started ...

### [hzqst](https://github.com/hzqst)
- 🍴 👤 [hzqst](https://github.com/hzqst) Forked [WangWei-CM/KSword](https://github.com/WangWei-CM/KSword) to [hzqst/KSword](https://github.com/hzqst/KSword)
  > 最强的开源ARK
- 🌟 👤 [hzqst](https://github.com/hzqst) Starred [Zhenyu98/codex-chatgpt-bridge](https://github.com/Zhenyu98/codex-chatgpt-bridge)
  > A safe bridge for Codex and ChatGPT to hand off coding work, save tokens, and keep local execution u...
- 🍴 👤 [hzqst](https://github.com/hzqst) Forked [Zhenyu98/codex-chatgpt-bridge](https://github.com/Zhenyu98/codex-chatgpt-bridge) to [hzqst/codex-chatgpt-bridge](https://github.com/hzqst/codex-chatgpt-bridge)
  > A safe bridge for Codex and ChatGPT to hand off coding work, save tokens, and keep local execution u...

### [igorbrigadir](https://github.com/igorbrigadir)
- 🌟 👤 [igorbrigadir](https://github.com/igorbrigadir) Starred [HotCakeX/Official-IANA-IP-blocks](https://github.com/HotCakeX/Official-IANA-IP-blocks)
  > Official IANA IP blocks for each country | IPV4 and IPV6 | TEXT | CSV | JSON

### [jaywcjlove](https://github.com/jaywcjlove)
- 🌟 👤 [jaywcjlove](https://github.com/jaywcjlove) Starred [HMAKT99/UnTouchID](https://github.com/HMAKT99/UnTouchID)
  > Use your phone's Face ID or fingerprint to approve sudo and unlock your Mac. QR pairing, encrypted B...

### [jiji262](https://github.com/jiji262)
- 🌟 👤 [jiji262](https://github.com/jiji262) Starred [kaieye/Livo](https://github.com/kaieye/Livo)

### [joydo](https://github.com/joydo)
- 🌟 👤 [joydo](https://github.com/joydo) Starred [a9gent/mindfs](https://github.com/a9gent/mindfs)
  > Access your personal AI agents and workstation data anywhere, anytime through MindFS.

### [jpacg](https://github.com/jpacg)
- 🌟 👤 [jpacg](https://github.com/jpacg) Starred [diegosouzapw/OmniRoute](https://github.com/diegosouzapw/OmniRoute)
  > Never stop coding. Free AI gateway: one endpoint, 231+ providers (50+ free), connect Claude Code, Co...
- 🌟 👤 [jpacg](https://github.com/jpacg) Starred [Cactusinhand/filter-repo-rs](https://github.com/Cactusinhand/filter-repo-rs)
  > 🦀 A high-performance Rust implementation of git-filter-repo for efficiently rewriting Git repository...
- 🌟 👤 [jpacg](https://github.com/jpacg) Starred [11happy/cpx](https://github.com/11happy/cpx)
  > cp reimagined

### [jpillora](https://github.com/jpillora)
- 🍴 👤 [jpillora](https://github.com/jpillora) Forked [cloudflare/cloudflared](https://github.com/cloudflare/cloudflared) to [jpillora/cloudflared](https://github.com/jpillora/cloudflared)
  > Cloudflare Tunnel client

### [killvxk](https://github.com/killvxk)
- 🌟 👤 [killvxk](https://github.com/killvxk) Starred [teamchong/pxpipe](https://github.com/teamchong/pxpipe)
  > cut Fable 5 token usage by rendering text context as images

### [kin4stat](https://github.com/kin4stat)
- 🌟 👤 [kin4stat](https://github.com/kin4stat) Starred [prdgmshift/usbliter8](https://github.com/prdgmshift/usbliter8)
  > An A12/A13 SecureROM exploit

### [lich4](https://github.com/lich4)
- 🌟 👤 [lich4](https://github.com/lich4) Starred [TikHub/TikHub-API-Python-SDK](https://github.com/TikHub/TikHub-API-Python-SDK)
  > High-performance asynchronous Douyin(抖音) TikTok Xiaohongshu(小红书) Kuaishou(快手) Weibo(微博) Instagram Yo...

### [lightsummer233](https://github.com/lightsummer233)
- 🍴 👤 [lightsummer233](https://github.com/lightsummer233) Forked [imren0x/kernel_techpack_xiaomi-titanium](https://github.com/imren0x/kernel_techpack_xiaomi-titanium) to [lightsummer233/kernel_techpack_xiaomi-titanium](https://github.com/lightsummer233/kernel_techpack_xiaomi-titanium)
  > Techpack Module for Mi-Titanium (Vince/YSL/Mido/tiffany) wip unified for vince/ysl/mido

### [llsc12](https://github.com/llsc12)
- 🌟 👤 [llsc12](https://github.com/llsc12) Starred [pound-emu/ballistic](https://github.com/pound-emu/ballistic)
  > The Ballistic JIT Engine: On track to become the worlds fastest ARM recompiler.

### [lyming99](https://github.com/lyming99)
- 🌟 👤 [lyming99](https://github.com/lyming99) Starred [lyming99/autoplan](https://github.com/lyming99/autoplan)
  > 一款支持24小时执行编程任务的开源工具，支持项目管理、需求管理、反馈管理、计划任务队列。

### [madeye](https://github.com/madeye)
- 🌟 👤 [madeye](https://github.com/madeye) Starred [pliron-org/pliron](https://github.com/pliron-org/pliron)
  > An Extensible Compiler IR Framework

### [mborgerson](https://github.com/mborgerson)
- 🌟 👤 [mborgerson](https://github.com/mborgerson) Starred [jamesob/local-llm](https://github.com/jamesob/local-llm)
  > Everything I know about running LLMs locally

### [mdcgqq](https://github.com/mdcgqq)
- 🌟 👤 [mdcgqq](https://github.com/mdcgqq) Starred [vnpy/vnpy](https://github.com/vnpy/vnpy)
  > 基于Python的开源量化交易平台开发框架
- 🍴 👤 [mdcgqq](https://github.com/mdcgqq) Forked [vnpy/vnpy](https://github.com/vnpy/vnpy) to [mdcgqq/vnpy](https://github.com/mdcgqq/vnpy)
  > 基于Python的开源量化交易平台开发框架
- 🌟 👤 [mdcgqq](https://github.com/mdcgqq) Starred [microsoft/qlib](https://github.com/microsoft/qlib)
  > Qlib is an AI-oriented Quant investment platform that aims to use AI tech to empower Quant Research,...
- 🍴 👤 [mdcgqq](https://github.com/mdcgqq) Forked [microsoft/qlib](https://github.com/microsoft/qlib) to [mdcgqq/qlib](https://github.com/mdcgqq/qlib)
  > Qlib is an AI-oriented Quant investment platform that aims to use AI tech to empower Quant Research,...

### [mistrmochov](https://github.com/mistrmochov)
- 🍴 👤 [mistrmochov](https://github.com/mistrmochov) Forked [osm0sis/AnyKernel3](https://github.com/osm0sis/AnyKernel3) to [mistrmochov/AnyKernel3](https://github.com/mistrmochov/AnyKernel3)
  > AnyKernel, Evolved

### [moonD4rk](https://github.com/moonD4rk)
- 🌟 👤 [moonD4rk](https://github.com/moonD4rk) Starred [tafia/quick-xml](https://github.com/tafia/quick-xml)
  > Rust high performance xml reader and writer

### [mq1n](https://github.com/mq1n)
- 🍴 👤 [mq1n](https://github.com/mq1n) Forked [can1357/oh-my-pi](https://github.com/can1357/oh-my-pi) to [mq1n/oh-my-pi](https://github.com/mq1n/oh-my-pi)
  > ⌥  AI Coding agent for the terminal — hash-anchored edits, optimized tool harness, LSP, Python, brow...

### [mrexodia](https://github.com/mrexodia)
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [jscott3201/llm-tuning](https://github.com/jscott3201/llm-tuning)
  > Serving and fine-tuning for the Gemma 4 and Qwen3.6 model families on Modal (SGLang/vLLM) — solo and...

### [npjohnson](https://github.com/npjohnson)
- 🍴 👤 [npjohnson](https://github.com/npjohnson) Forked [LineageOS/android_kernel_motorola_exynos9610](https://github.com/LineageOS/android_kernel_motorola_exynos9610) to [npjohnson/android_kernel_motorola_exynos9610](https://github.com/npjohnson/android_kernel_motorola_exynos9610)

### [null-dev](https://github.com/null-dev)
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [security-union/videocall-rs](https://github.com/security-union/videocall-rs)
  > media streaming framework and teleconferencing system written in rust

### [nzcv](https://github.com/nzcv)
- 🌟 👤 [nzcv](https://github.com/nzcv) Starred [rr-debugger/rr](https://github.com/rr-debugger/rr)
  > Record and Replay Framework
- 🌟 👤 [nzcv](https://github.com/nzcv) Starred [hickory-dns/hickory-dns](https://github.com/hickory-dns/hickory-dns)
  > A Rust based DNS client, server, and resolver

### [oblique](https://github.com/oblique)
- 🌟 👤 [oblique](https://github.com/oblique) Starred [HalFrgrd/flyline](https://github.com/HalFrgrd/flyline)
  > Flyline: a Bash plugin to replace readline for a modern line editing experience: syntax highlighting...

### [progmboy](https://github.com/progmboy)
- 🌟 👤 [progmboy](https://github.com/progmboy) Starred [hyzhang24/DuplexSLA](https://github.com/hyzhang24/DuplexSLA)
  > DuplexSLA: A Full-Duplex Spoken Language Model with Synchronized Speech, Language, and Action

### [pwh-pwh](https://github.com/pwh-pwh)
- 🌟 👤 [pwh-pwh](https://github.com/pwh-pwh) Starred [noobexon1/XposedFakeLocation](https://github.com/noobexon1/XposedFakeLocation)
  > Android application and Xposed module that allows you to spoof your device's location globally or fo...

### [rmuxnet](https://github.com/rmuxnet)
- 🍴 👤 [rmuxnet](https://github.com/rmuxnet) Forked [ps5-linux/ps5-linux-patches](https://github.com/ps5-linux/ps5-linux-patches) to [rmuxnet/ps5](https://github.com/rmuxnet/ps5)
  > Linux patches and configuration to run on PS5

### [rogertan30](https://github.com/rogertan30)
- 🌟 👤 [rogertan30](https://github.com/rogertan30) Starred [r4ghu/iOS-CoreML-MNIST](https://github.com/r4ghu/iOS-CoreML-MNIST)
  > Real-time Number Recognition using Apple's CoreML 2.0 and MNIST -

### [s5uishida](https://github.com/s5uishida)
- 🌟 👤 [s5uishida](https://github.com/s5uishida) Starred [IBM/AssetOpsBench](https://github.com/IBM/AssetOpsBench)
  > AssetOpsBench - Industry 4.0: A unified benchmark and framework for building, orchestrating, and eva...

### [scarsty](https://github.com/scarsty)
- 🌟 👤 [scarsty](https://github.com/scarsty) Starred [ned14/quickcpplib](https://github.com/ned14/quickcpplib)
  > Eliminate all the tedious hassle when making state-of-the-art C++ 14 - 23 libraries!

### [shadow3aaa](https://github.com/shadow3aaa)
- 🌟 👤 [shadow3aaa](https://github.com/shadow3aaa) Starred [coding-chimera/chimera](https://github.com/coding-chimera/chimera)
  > AI coding agent CLI with built-in CodeGraph-powered impact analysis and propagation audit, harness t...

### [sin5678](https://github.com/sin5678)
- 🌟 👤 [sin5678](https://github.com/sin5678) Starred [ophub/amlogic-s9xxx-armbian](https://github.com/ophub/amlogic-s9xxx-armbian)
  > Supports running Armbian on Amlogic, Allwinner, and Rockchip devices. Support a311d, s922x, s905x3, ...

### [smallevilbeast](https://github.com/smallevilbeast)
- 🌟 👤 [smallevilbeast](https://github.com/smallevilbeast) Starred [iOS-Auth/iOSAuthRuntime](https://github.com/iOS-Auth/iOSAuthRuntime)
  > iOS Authorize runtime
- 🌟 👤 [smallevilbeast](https://github.com/smallevilbeast) Starred [fennaraOfficial/fennara-godot-ai](https://github.com/fennaraOfficial/fennara-godot-ai)
  > AI chat and agent tooling for Godot, with MCP support, local runtime, and native editor integration.
- 🌟 👤 [smallevilbeast](https://github.com/smallevilbeast) Starred [Kerrbty/IpaInstall](https://github.com/Kerrbty/IpaInstall)
  > 手机 apple Id 授权，然后安装ipa文件demo，包含服务端和客户端

### [smartdone](https://github.com/smartdone)
- 🌟 👤 [smartdone](https://github.com/smartdone) Starred [nevillegrech/gigahorse-toolchain](https://github.com/nevillegrech/gigahorse-toolchain)
  > A binary lifter and analysis framework for Ethereum smart contracts

### [stuxnet147](https://github.com/stuxnet147)
- 🌟 👤 [stuxnet147](https://github.com/stuxnet147) Starred [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)
  > Sub2API 一站式开源中转服务，让 Claude、Openai 、Gemini、Grok订阅统一接入，支持拼车共享，更高效分摊成本，原生工具无缝使用。

### [surendrajat](https://github.com/surendrajat)
- 🌟 👤 [surendrajat](https://github.com/surendrajat) Starred [MarcellM01/TinySearch](https://github.com/MarcellM01/TinySearch)
  > Shrink the web for your local LLMs!
- 🚀 👤 [surendrajat](https://github.com/surendrajat) Made [surendrajat/cfwarpd](https://github.com/surendrajat/cfwarpd) public
  > Cloudflare WARP tunnel with custom DNS (macOS)

### [veaba](https://github.com/veaba)
- 🌟 👤 [veaba](https://github.com/veaba) Starred [hikariming/ghfind](https://github.com/hikariming/ghfind)
  > Discover the best developers — and become one. Drop a GitHub handle for a 0–100 value & trust score ...

### [wangshub](https://github.com/wangshub)
- 🍴 👤 [wangshub](https://github.com/wangshub) Forked [fastrepl/anarlog](https://github.com/fastrepl/anarlog) to [wangshub/anarlog](https://github.com/wangshub/anarlog)
  > Open source Granola AI Alternative

### [winmin](https://github.com/winmin)
- 🌟 👤 [winmin](https://github.com/winmin) Starred [huangzheng2016/eTerm](https://github.com/huangzheng2016/eTerm)
  > 终端 SSH/SFTP 客户端：支持跳板与 ProxyCommand、HTTP(S)/SOCKS 代理、本地/远程/动态端口转发、命令片段、多标签与双栏 SFTP 文件管理。 A terminal S...

### [xiaolai](https://github.com/xiaolai)
- 🍴 👤 [xiaolai](https://github.com/xiaolai) Forked [lst97/claude-code-sub-agents](https://github.com/lst97/claude-code-sub-agents) to [xiaolai/claude-code-sub-agents](https://github.com/xiaolai/claude-code-sub-agents)
  > Collection of specialized AI subagents for Claude Code for personal use (full-stack development).
- 🍴 👤 [xiaolai](https://github.com/xiaolai) Forked [matt1398/claude-devtools](https://github.com/matt1398/claude-devtools) to [xiaolai/claude-devtools](https://github.com/xiaolai/claude-devtools)
  > The missing DevTools for Claude Code — inspect session logs, tool calls, token usage, subagents, and...
- 🍴 👤 [xiaolai](https://github.com/xiaolai) Forked [greensock/gsap-skills](https://github.com/greensock/gsap-skills) to [xiaolai/gsap-skills](https://github.com/xiaolai/gsap-skills)
  > Official AI skills for GSAP. These skills teach AI coding agents how to correctly use GSAP (GreenSoc...

### [xiaoweigege](https://github.com/xiaoweigege)
- 🌟 👤 [xiaoweigege](https://github.com/xiaoweigege) Starred [GitKittys/AndroidPrybar](https://github.com/GitKittys/AndroidPrybar)
  > Android Native Trace 与逆向分析工具，用于函数包装执行、指令跟踪、JNI 调用观测与外部跳转控制

### [xypron](https://github.com/xypron)
- 🍴 👤 [xypron](https://github.com/xypron) Forked [torvalds/linux](https://github.com/torvalds/linux) to [xypron/linux](https://github.com/xypron/linux)
  > Linux kernel source tree

### [yunsmall](https://github.com/yunsmall)
- 🌟 👤 [yunsmall](https://github.com/yunsmall) Starred [JonaNorman/WebViewPackage](https://github.com/JonaNorman/WebViewPackage)
  > A comprehensive archive of Android WebView APKs (Google, Huawei, Amazon) for compatibility testing a...

### [yynxxxxx](https://github.com/yynxxxxx)
- 🌟 👤 [yynxxxxx](https://github.com/yynxxxxx) Starred [yynxxxxx/Codex-X](https://github.com/yynxxxxx/Codex-X)
  > Codex Switch & Instruct desktop manager

### [zetaloop](https://github.com/zetaloop)
- 🌟 👤 [zetaloop](https://github.com/zetaloop) Starred [paper-design/shaders](https://github.com/paper-design/shaders)
  > Zero-dependency canvas shaders that can be installed from npm or designed in Paper

### [zhaotianff](https://github.com/zhaotianff)
- 🌟 👤 [zhaotianff](https://github.com/zhaotianff) Starred [dragosniamtu/WPF-Liquid-Glass-Effect](https://github.com/dragosniamtu/WPF-Liquid-Glass-Effect)
  >  WPF Liquid Glass Effect  A WPF demo that recreates an Apple-inspired Liquid Glass window effect on ...
- 🌟 👤 [zhaotianff](https://github.com/zhaotianff) Starred [undebel/NoFuserEx](https://github.com/undebel/NoFuserEx)
  > Free deobfuscator for ConfuserEx.

### [zsxsoft](https://github.com/zsxsoft)
- 🍴 👤 [zsxsoft](https://github.com/zsxsoft) Forked [pandada8/opencode-axonhub](https://github.com/pandada8/opencode-axonhub) to [zsxsoft/opencode-axonhub](https://github.com/zsxsoft/opencode-axonhub)
  > OpenCode plugin for AxonHub that dynamically discovers available models, caches them daily, and conf...


---
*最后更新于 2026-07-04 17:05:44 UTC*
*历史记录保存在 `archive` 目录中。*
