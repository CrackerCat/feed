# 每日 GitHub 动态 (2026-09-05)

我关注用户的今日公开动态 (每15分钟更新)。

## 今日动态

### [0x1306a94](https://github.com/0x1306a94)
- 🌟 👤 [0x1306a94](https://github.com/0x1306a94) Starred [canmi21/rdm](https://github.com/canmi21/rdm)
  > A refined download manager.

### [0xBakeer](https://github.com/0xBakeer)
- 🌟 👤 [0xBakeer](https://github.com/0xBakeer) Starred [0xBakeer/deepseek-v4-flash-spark](https://github.com/0xBakeer/deepseek-v4-flash-spark)
  > DeepSeek V4 Flash on a DGX Spark: 384k context, thinking, tools, OpenAI API — one command to install

### [34306](https://github.com/34306)
- 🚀 👤 [34306](https://github.com/34306) Made [34306/vphone-web](https://github.com/34306/vphone-web) public
  > vphone-cli but you can use your mac as a host and control it over the web

### [Alinshans](https://github.com/Alinshans)
- 🌟 👤 [Alinshans](https://github.com/Alinshans) Starred [rosemarycox5334-debug/PA_Agent](https://github.com/rosemarycox5334-debug/PA_Agent)

### [Aniketh01](https://github.com/Aniketh01)
- 🌟 👤 [Aniketh01](https://github.com/Aniketh01) Starred [Graphify-Labs/graphify](https://github.com/Graphify-Labs/graphify)
  > Turn any codebase, with its docs, SQL schemas, configs, and PDFs, into a queryable knowledge graph. ...

### [Brinmon](https://github.com/Brinmon)
- 🌟 👤 [Brinmon](https://github.com/Brinmon) Starred [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness)
  > DeepSeek Harness: Everything is a Plugin.

### [CYM-Lab](https://github.com/CYM-Lab)
- 🌟 👤 [CYM-Lab](https://github.com/CYM-Lab) Starred [manyuegong33/r0crawl_skills](https://github.com/manyuegong33/r0crawl_skills)
  > r0crawl_skills is a massive reverse-engineering skill arsenal for web, Android, iOS, Frida, native b...
- 🌟 👤 [CYM-Lab](https://github.com/CYM-Lab) Starred [zhaoxuya520/reverse-skill](https://github.com/zhaoxuya520/reverse-skill)
  > Reverse Engineering / Authorized Penetration Testing / Security Research Skill Router Pack AI-powere...

### [Chordp](https://github.com/Chordp)
- 🌟 👤 [Chordp](https://github.com/Chordp) Starred [allthingsida/idasql](https://github.com/allthingsida/idasql)
  > Vibe Reverse Engineer with IDA SQL: An interface for IDA in SQL via live virtual tables
- 🌟 👤 [Chordp](https://github.com/Chordp) Starred [Cec1c/PVF-MCP](https://github.com/Cec1c/PVF-MCP)
  > 一个用于pvfUtility 的MCP服务 使用GO语言编写
- 🌟 👤 [Chordp](https://github.com/Chordp) Starred [Autumn-27/ARTEX](https://github.com/Autumn-27/ARTEX)
  > AI 自主渗透测试系统

### [ChrisTheCoolHut](https://github.com/ChrisTheCoolHut)
- 🌟 👤 [ChrisTheCoolHut](https://github.com/ChrisTheCoolHut) Starred [aramosf/CVE-2026-64468](https://github.com/aramosf/CVE-2026-64468)
  > Linux Binder binder_free_transaction() process-lifetime use-after-free (CVE-2026-64468): unprivilege...

### [Col-E](https://github.com/Col-E)
- 🌟 👤 [Col-E](https://github.com/Col-E) Starred [mrphrazer/msynth](https://github.com/mrphrazer/msynth)
  > Code deobfuscation framework to simplify Mixed Boolean-Arithmetic (MBA) expressions

### [Cossack9989](https://github.com/Cossack9989)
- 🌟 👤 [Cossack9989](https://github.com/Cossack9989) Starred [p-e-w/heretic](https://github.com/p-e-w/heretic)
  > Fully automatic censorship removal for language models
- 🌟 👤 [Cossack9989](https://github.com/Cossack9989) Starred [elder-plinius/OBLITERATUS](https://github.com/elder-plinius/OBLITERATUS)
  > OBLITERATE THE CHAINS THAT BIND YOU

### [Cxk4ng](https://github.com/Cxk4ng)
- 🌟 👤 [Cxk4ng](https://github.com/Cxk4ng) Starred [DevelopCubeLab/CellularInfo](https://github.com/DevelopCubeLab/CellularInfo)
  > iOS TrollStore App Cellular information. All your device Cellular Information

### [DerekSelander](https://github.com/DerekSelander)
- 🌟 👤 [DerekSelander](https://github.com/DerekSelander) Starred [mowisec/macos-to-ios](https://github.com/mowisec/macos-to-ios)

### [Dy-Baby](https://github.com/Dy-Baby)
- 🌟 👤 [Dy-Baby](https://github.com/Dy-Baby) Starred [Mattiwatti/EfiGuard](https://github.com/Mattiwatti/EfiGuard)
  > Disable PatchGuard and Driver Signature Enforcement at boot time

### [Konano](https://github.com/Konano)
- 🌟 👤 [Konano](https://github.com/Konano) Starred [hakavlad/nohang](https://github.com/hakavlad/nohang)
  > A sophisticated low memory handler for Linux
- 🌟 👤 [Konano](https://github.com/Konano) Starred [sindresorhus/Gifski](https://github.com/sindresorhus/Gifski)
  > 🌈 Convert videos to high-quality GIFs on your Mac

### [L4ys](https://github.com/L4ys)
- 🌟 👤 [L4ys](https://github.com/L4ys) Starred [cellebrite-labs/ida-bridge](https://github.com/cellebrite-labs/ida-bridge)
  > Bridge between agents and IDA Pro.

### [Lake1059](https://github.com/Lake1059)
- 🌟 👤 [Lake1059](https://github.com/Lake1059) Starred [sixiaolong1117/RTX-Encode-Toolkit](https://github.com/sixiaolong1117/RTX-Encode-Toolkit)
  > 基于 NVEnc 的 NVIDIA RTX 视频编码工具，支持 RTX VSR 超分、RTX HDR、NVOF FRUC 插帧

### [LasCC](https://github.com/LasCC)
- 🌟 👤 [LasCC](https://github.com/LasCC) Starred [chrisgreg/glance](https://github.com/chrisgreg/glance)
  > A tiny, self-hosted web analytics service. 

### [Monoceros406](https://github.com/Monoceros406)
- 🌟 👤 [Monoceros406](https://github.com/Monoceros406) Starred [DErDYAST1R/KRNLDriver](https://github.com/DErDYAST1R/KRNLDriver)
  > My personal / private P2C Project undetected for 8- Months Straight leaked & resold so I am publishi...
- 🌟 👤 [Monoceros406](https://github.com/Monoceros406) Starred [memN0ps/illusion-rs](https://github.com/memN0ps/illusion-rs)
  > Rusty Hypervisor - Windows UEFI Blue Pill Type-1 Hypervisor in Rust (Codename: Illusion)

### [Mrmiaomrzh](https://github.com/Mrmiaomrzh)
- 🌟 👤 [Mrmiaomrzh](https://github.com/Mrmiaomrzh) Starred [lingqiqi5211/EzHookTool](https://github.com/lingqiqi5211/EzHookTool)
  > 使用 EzXHelper 的思路封装反射以及 Hook API, 并提供 反射/Hook 工具

### [No-Github](https://github.com/No-Github)
- 🌟 👤 [No-Github](https://github.com/No-Github) Starred [Autumn-27/ARTEX](https://github.com/Autumn-27/ARTEX)
  > AI 自主渗透测试系统

### [People-11](https://github.com/People-11)
- 🍴 👤 [People-11](https://github.com/People-11) Forked [ppy/osu](https://github.com/ppy/osu) to [People-11/osu](https://github.com/People-11/osu)
  > rhythm is just a *click* away!
- 🍴 👤 [People-11](https://github.com/People-11) Forked [ppy/osu-framework](https://github.com/ppy/osu-framework) to [People-11/osu-framework](https://github.com/People-11/osu-framework)
  > A game framework written with osu! in mind.

### [PhoenixHairpin](https://github.com/PhoenixHairpin)
- 🌟 👤 [PhoenixHairpin](https://github.com/PhoenixHairpin) Starred [yzddmr6/PrismSpace](https://github.com/yzddmr6/PrismSpace)
  > 基于 Android 工作资料的应用双开管理器。用系统级隔离创建独立空间，安装、运行和管理应用分身。
- 🍴 👤 [PhoenixHairpin](https://github.com/PhoenixHairpin) Forked [yzddmr6/PrismSpace](https://github.com/yzddmr6/PrismSpace) to [PhoenixHairpin/PrismSpace](https://github.com/PhoenixHairpin/PrismSpace)
  > 基于 Android 工作资料的应用双开管理器。用系统级隔离创建独立空间，安装、运行和管理应用分身。
- 🍴 👤 [PhoenixHairpin](https://github.com/PhoenixHairpin) Forked [SorarinX/SorarinBot](https://github.com/SorarinX/SorarinBot) to [PhoenixHairpin/SorarinBot](https://github.com/PhoenixHairpin/SorarinBot)
  > 微信 AI 智能助手 · WeChat AI Assistant powered by LLM
- 🍴 👤 [PhoenixHairpin](https://github.com/PhoenixHairpin) Forked [eatmoreapple/openwechat](https://github.com/eatmoreapple/openwechat) to [PhoenixHairpin/openwechat](https://github.com/PhoenixHairpin/openwechat)
  > golang微信SDK

### [SPuerBRead](https://github.com/SPuerBRead)
- 🌟 👤 [SPuerBRead](https://github.com/SPuerBRead) Starred [uiverse-io/galaxy](https://github.com/uiverse-io/galaxy)
  > The largest Open-Source UI Library! Community-made and free to use. Made with either CSS or Tailwind...

### [SeeFlowerX](https://github.com/SeeFlowerX)
- 🌟 👤 [SeeFlowerX](https://github.com/SeeFlowerX) Starred [Radiant303/SpringNote](https://github.com/Radiant303/SpringNote)
  > SpringNote is a note-taking tool for the lazy. Just jot things down, and AI tidies up the scattered ...

### [SimonTheCoder](https://github.com/SimonTheCoder)
- 🌟 👤 [SimonTheCoder](https://github.com/SimonTheCoder) Starred [RyuDTakano/Sunshine-moonlight-add-USB-Connect](https://github.com/RyuDTakano/Sunshine-moonlight-add-USB-Connect)

### [Sndav](https://github.com/Sndav)
- 🌟 👤 [Sndav](https://github.com/Sndav) Starred [shorthorn-project/One-Core-API-Source](https://github.com/shorthorn-project/One-Core-API-Source)
  > A complete layer to get compatibility on XP/2003 for newer applications
- 🌟 👤 [Sndav](https://github.com/Sndav) Starred [SnailSploit/Claude-Red](https://github.com/SnailSploit/Claude-Red)
  > claude-red is a curated library of offensive security skills designed for the Claude skills system. ...
- 🌟 👤 [Sndav](https://github.com/Sndav) Starred [utoni/ptunnel-ng](https://github.com/utoni/ptunnel-ng)
  > Tunnel TCP connections through ICMP.
- 🌟 👤 [Sndav](https://github.com/Sndav) Starred [Jia-Ethan/codex-keysmith](https://github.com/Jia-Ethan/codex-keysmith)
  > Versioned Codex instruction deployment with preview, ownership manifests, hook isolation, scenario e...

### [Soulghost](https://github.com/Soulghost)
- 🌟 👤 [Soulghost](https://github.com/Soulghost) Starred [Coding-Solo/godot-mcp](https://github.com/Coding-Solo/godot-mcp)
  > MCP server for interfacing with Godot game engine. Provides tools for launching the editor, running ...

### [T5ive](https://github.com/T5ive)
- 🌟 👤 [T5ive](https://github.com/T5ive) Starred [IcySnex/SkeleKit](https://github.com/IcySnex/SkeleKit)
  > A C# UI framework for native iOS: real UIKit controls, no storyboards or constraints, zero boilerpla...
- 🌟 👤 [T5ive](https://github.com/T5ive) Starred [alirezanet/Gridify](https://github.com/alirezanet/Gridify)
  > Easy and optimized way to apply Filtering, Sorting, and Pagination using text-based data.

### [TonyChen56](https://github.com/TonyChen56)
- 🌟 👤 [TonyChen56](https://github.com/TonyChen56) Starred [markrussinovich/refchecker](https://github.com/markrussinovich/refchecker)
  > A tool that validates academic paper references
- 🌟 👤 [TonyChen56](https://github.com/TonyChen56) Starred [ChenLiu-1996/figures4papers](https://github.com/ChenLiu-1996/figures4papers)
  > My Python scripts to make high-quality figures for publications in top AI conferences and journals.

### [Twometer](https://github.com/Twometer)
- 🌟 👤 [Twometer](https://github.com/Twometer) Starred [minecraft-dev/MinecraftDev](https://github.com/minecraft-dev/MinecraftDev)
  > Plugin for IntelliJ IDEA that gives special support for Minecraft modding projects.

### [VonChange](https://github.com/VonChange)
- 🌟 👤 [VonChange](https://github.com/VonChange) Starred [LiveContainer/LiveContainer](https://github.com/LiveContainer/LiveContainer)
  > Run iOS apps without actually installing them!

### [XMuli](https://github.com/XMuli)
- 🌟 👤 [XMuli](https://github.com/XMuli) Starred [pangju666/file-viewer](https://github.com/pangju666/file-viewer)
  > 基于 Vue 3 和 Naive UI 打造的高集成度文件预览组件。

### [XadillaX](https://github.com/XadillaX)
- 🌟 👤 [XadillaX](https://github.com/XadillaX) Starred [Fyrgo8/meihua-divination-skill](https://github.com/Fyrgo8/meihua-divination-skill)
  > 面向 Codex 的公开梅花易数解卦 Skill：支持动静定体用、月令与卦气结合看旺衰，以及结构化逐步断卦输出。

### [ZeroMemoryEx](https://github.com/ZeroMemoryEx)
- 🌟 👤 [ZeroMemoryEx](https://github.com/ZeroMemoryEx) Starred [ADScanPro/adscan](https://github.com/ADScanPro/adscan)
  > Free Active Directory pentesting tool for Linux. Automates AD and LDAP enumeration, Kerberoasting, A...

### [alex193a](https://github.com/alex193a)
- 🍴 👤 [alex193a](https://github.com/alex193a) Forked [coolzyd9107/GKI_ReSukiSU_SUSFS](https://github.com/coolzyd9107/GKI_ReSukiSU_SUSFS) to [alex193a/GKI_ReSukiSU_SUSFS](https://github.com/alex193a/GKI_ReSukiSU_SUSFS)

### [ant4g0nist](https://github.com/ant4g0nist)
- 🌟 👤 [ant4g0nist](https://github.com/ant4g0nist) Starred [mowisec/macos-to-ios](https://github.com/mowisec/macos-to-ios)

### [arkup](https://github.com/arkup)
- 🚀 👤 [arkup](https://github.com/arkup) Made [arkup/remora](https://github.com/arkup/remora) public
  > Win64 API monitor using EAT/IAT hooking - no API body patching or debugger attachment

### [bet4it](https://github.com/bet4it)
- 🌟 👤 [bet4it](https://github.com/bet4it) Starred [usetrmnl/trmnl-koreader](https://github.com/usetrmnl/trmnl-koreader)
  > Turn KOReader into a TRMNL display for Kindle and other devices.
- 🌟 👤 [bet4it](https://github.com/bet4it) Starred [softmutiny/kindle-ai-quota-dashboard](https://github.com/softmutiny/kindle-ai-quota-dashboard)
  > 把吃灰的 Kindle 变成 AI 额度监控屏。跨 WiFi 实时显示 Claude / Codex / Kimi / DeepSeek 用量，外加天气和每日一语。

### [bibinocode](https://github.com/bibinocode)
- 🌟 👤 [bibinocode](https://github.com/bibinocode) Starred [Arsylk/Deep-Live-Cam](https://github.com/Arsylk/Deep-Live-Cam)
  > Deep-Live-Cam: Arch slot client, native manager, phone native-portrait transport, puppet assembler
- 🌟 👤 [bibinocode](https://github.com/bibinocode) Starred [yanliudesign/mono-color-skill](https://github.com/yanliudesign/mono-color-skill)
  > One-ink editorial print image skill — warm paper, halftone photography, active negative space, and r...

### [bytesFighting](https://github.com/bytesFighting)
- 🍴 👤 [bytesFighting](https://github.com/bytesFighting) Forked [zee295/oneplus8-kernelsu-susfs](https://github.com/zee295/oneplus8-kernelsu-susfs) to [bytesFighting/oneplus8-kernelsu-susfs](https://github.com/bytesFighting/oneplus8-kernelsu-susfs)

### [carlonluca](https://github.com/carlonluca)
- 🌟 👤 [carlonluca](https://github.com/carlonluca) Starred [nextcloud/notes](https://github.com/nextcloud/notes)
  > ✎ Distraction-free notes and writing
- 🌟 👤 [carlonluca](https://github.com/carlonluca) Starred [nextcloud/notes-android](https://github.com/nextcloud/notes-android)
  > ✎ Android client for Nextcloud Notes app.

### [chsbuffer](https://github.com/chsbuffer)
- 🌟 👤 [chsbuffer](https://github.com/chsbuffer) Starred [buffcow/ollvm-project-android](https://github.com/buffcow/ollvm-project-android)
  > ollvm project base on llvm-android of google

### [cwuom](https://github.com/cwuom)
- 🌟 👤 [cwuom](https://github.com/cwuom) Starred [NexAlloy/NexAlloy](https://github.com/NexAlloy/NexAlloy)
  > ChsBuffer's LSPosed module, powered by Morphe, ReVanced, and beyond. (formerly ReVanced Xposed)
- 🌟 👤 [cwuom](https://github.com/cwuom) Starred [cwuom/KNHV](https://github.com/cwuom/KNHV)
  > Type-2 Hypervisor implementation for Intel Processors based on VT-x technology. (Demo)

### [feicong](https://github.com/feicong)
- 🌟 👤 [feicong](https://github.com/feicong) Starred [zibo-chen/SubnetDesk](https://github.com/zibo-chen/SubnetDesk)
  > LAN-only remote desktop based on RustDesk

### [fengjixuchui](https://github.com/fengjixuchui)
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [Qmeimei10086/pte-dbg](https://github.com/Qmeimei10086/pte-dbg) to [fengjixuchui/pte-dbg](https://github.com/fengjixuchui/pte-dbg)
  > 基于pte-hook的调试器保护工具
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [memN0ps/illusion-rs](https://github.com/memN0ps/illusion-rs) to [fengjixuchui/illusion-rs](https://github.com/fengjixuchui/illusion-rs)
  > Rusty Hypervisor - Windows UEFI Blue Pill Type-1 Hypervisor in Rust (Codename: Illusion)
- 🍴 👤 [fengjixuchui](https://github.com/fengjixuchui) Forked [DeathShotXD/0xM0nCrush](https://github.com/DeathShotXD/0xM0nCrush) to [fengjixuchui/0xM0nCrush](https://github.com/fengjixuchui/0xM0nCrush)
  > Kernel-mode process terminator using a signed BYOVD driver. Works on all Windows 10/11. No offsets, ...

### [fjh658](https://github.com/fjh658)
- 🌟 👤 [fjh658](https://github.com/fjh658) Starred [34306/vphone-web](https://github.com/34306/vphone-web)
  > vphone-cli but you can use your mac as a host and control it over the web

### [fulu2778](https://github.com/fulu2778)
- 🚀 👤 [fulu2778](https://github.com/fulu2778) Made [fulu2778/Gemini-For-HyperOS4](https://github.com/fulu2778/Gemini-For-HyperOS4) public
  > Gemini For HyperOS4: power-key Gemini Overlay + white-bar Circle to Search (libxposed API102)

### [gal2xy](https://github.com/gal2xy)
- 🌟 👤 [gal2xy](https://github.com/gal2xy) Starred [Perkins-Fund/Malware-Bible](https://github.com/Perkins-Fund/Malware-Bible)
  > Free educational courses in cybersecurity, reverse engineering, malware analysis, and programming de...

### [gh0stkey](https://github.com/gh0stkey)
- 🌟 👤 [gh0stkey](https://github.com/gh0stkey) Starred [ArvinQi/dsh-mcp](https://github.com/ArvinQi/dsh-mcp)
  > DeepSeek Harness 的 MCP 服务器管理插件：可视化界面管理 + 按需热注入，OAuth 鉴权支持，环境变量配置。

### [heyhu](https://github.com/heyhu)
- 🌟 👤 [heyhu](https://github.com/heyhu) Starred [earendil-works/pi](https://github.com/earendil-works/pi)
  > AI agent toolkit: unified LLM API, agent loop, TUI, coding agent CLI

### [hmtheboy154](https://github.com/hmtheboy154)
- 🍴 👤 [hmtheboy154](https://github.com/hmtheboy154) Forked [OpenMouse-Project/openmouse](https://github.com/OpenMouse-Project/openmouse) to [hmtheboy154/openmouse](https://github.com/hmtheboy154/openmouse)
- 🍴 👤 [hmtheboy154](https://github.com/hmtheboy154) Forked [OpenMouse-Project/mouse-protocol](https://github.com/OpenMouse-Project/mouse-protocol) to [hmtheboy154/mouse-protocol](https://github.com/hmtheboy154/mouse-protocol)

### [hyuunnn](https://github.com/hyuunnn)
- 🌟 👤 [hyuunnn](https://github.com/hyuunnn) Starred [persiyanov/herdr-reviewr](https://github.com/persiyanov/herdr-reviewr)
  > A code review + file viewer sidebar for herdr. Comment on a diff and send back to agent. Inspect dif...

### [iBotPeaches](https://github.com/iBotPeaches)
- 🌟 👤 [iBotPeaches](https://github.com/iBotPeaches) Starred [cOzInABox/dashLaunch](https://github.com/cOzInABox/dashLaunch)
  > The One, The Only, The Xbox 360 Dash Launcher!

### [imj01y](https://github.com/imj01y)
- 🌟 👤 [imj01y](https://github.com/imj01y) Starred [jiyin004-jpg/ommega](https://github.com/jiyin004-jpg/ommega)
  > Ommega three-end system: A/B-side Magisk modules, relay server, and B-side Android app
- 🌟 👤 [imj01y](https://github.com/imj01y) Starred [zhaoxuya520/reverse-skill](https://github.com/zhaoxuya520/reverse-skill)
  > Reverse Engineering / Authorized Penetration Testing / Security Research Skill Router Pack AI-powere...
- 🌟 👤 [imj01y](https://github.com/imj01y) Starred [XiaoTong6666/FuseHide](https://github.com/XiaoTong6666/FuseHide)
  > 面向 Android 12+ 的 Xposed 模块，用于在 MediaProvider 中按运行时配置隐藏普通存储路径，并提供 Android/data 相关 Unicode 场景的调试与修复能力。
- 🌟 👤 [imj01y](https://github.com/imj01y) Starred [hqw700/ScrcpyOverWebRTC](https://github.com/hqw700/ScrcpyOverWebRTC)
  > A high-performance, web-based Android remote control solution powered by scrcpy and WebRTC. Control ...

### [ioncodes](https://github.com/ioncodes)
- 🌟 👤 [ioncodes](https://github.com/ioncodes) Starred [reindertpelsma/nvkvm-pv](https://github.com/reindertpelsma/nvkvm-pv)
  > Paravirtual NVIDIA GPU for KVM guests — run unmodified CUDA, PyTorch and Vulkan inside a VM at host ...

### [israel-dryer](https://github.com/israel-dryer)
- 🌟 👤 [israel-dryer](https://github.com/israel-dryer) Starred [DietrichGebert/ponytail](https://github.com/DietrichGebert/ponytail)
  > Makes your AI agent think like the laziest senior dev in the room. The best code is the code you nev...

### [itm4n](https://github.com/itm4n)
- 🌟 👤 [itm4n](https://github.com/itm4n) Starred [garatc/BitUnlocker](https://github.com/garatc/BitUnlocker)
  > Downgrade attack for CVE-2025-48804

### [j0nk0](https://github.com/j0nk0)
- 🌟 👤 [j0nk0](https://github.com/j0nk0) Starred [rajivm1991/DroidDock](https://github.com/rajivm1991/DroidDock)
  > A sleek macOS desktop application for browsing Android device files via ADB

### [jiushill](https://github.com/jiushill)
- 🌟 👤 [jiushill](https://github.com/jiushill) Starred [optiscaler/OptiScaler](https://github.com/optiscaler/OptiScaler)
  > OptiScaler bridges upscaling/frame gen across GPUs. Supports DLSS2+/XeSS/FSR2+ inputs, replaces nati...

### [jiyulany](https://github.com/jiyulany)
- 🌟 👤 [jiyulany](https://github.com/jiyulany) Starred [Orbmu2k/nvidiaProfileInspector](https://github.com/Orbmu2k/nvidiaProfileInspector)

### [kernelzeroday](https://github.com/kernelzeroday)
- 🚀 👤 [kernelzeroday](https://github.com/kernelzeroday) Made [kernelzeroday/careye-poc-1788584611](https://github.com/kernelzeroday/careye-poc-1788584611) public

### [kmlgping](https://github.com/kmlgping)
- 🌟 👤 [kmlgping](https://github.com/kmlgping) Starred [DErDYAST1R/KRNLDriver](https://github.com/DErDYAST1R/KRNLDriver)
  > My personal / private P2C Project undetected for 8- Months Straight leaked & resold so I am publishi...
- 🌟 👤 [kmlgping](https://github.com/kmlgping) Starred [2akouwu/reverify](https://github.com/2akouwu/reverify)
  > Stop your AI from making things up — it proposes, deterministic tools decide, every claim checked ag...

### [kujian](https://github.com/kujian)
- 🌟 👤 [kujian](https://github.com/kujian) Starred [yaojingang/GEOFlow](https://github.com/yaojingang/GEOFlow)
  > Open-source GEO content engineering and multi-site distribution platform with AI quality inspection,...

### [kyle-sylvestre](https://github.com/kyle-sylvestre)
- 🍴 👤 [kyle-sylvestre](https://github.com/kyle-sylvestre) Forked [mmozeiko/aseprite-bin](https://github.com/mmozeiko/aseprite-bin) to [kyle-sylvestre/aseprite-bin](https://github.com/kyle-sylvestre/aseprite-bin)
  > Aseprite binary build for 64-bit Windows

### [kylesmile1103](https://github.com/kylesmile1103)
- 🌟 👤 [kylesmile1103](https://github.com/kylesmile1103) Starred [OpenMouse-Project/openmouse](https://github.com/OpenMouse-Project/openmouse)

### [le31ei](https://github.com/le31ei)
- 🌟 👤 [le31ei](https://github.com/le31ei) Starred [lennney/stop-that-shit](https://github.com/lennney/stop-that-shit)
  > Stop That Shit（别再造史了）｜面向 Codex/GPT 场景的多平台 Hook + Skill Guard：拦截 AI coding agent 无需求的哈希、校验和与任务范围膨胀。 A...

### [lefex](https://github.com/lefex)
- 🌟 👤 [lefex](https://github.com/lefex) Starred [typst/typst](https://github.com/typst/typst)
  > A markup-based typesetting system that is powerful and easy to learn.

### [lidongyooo](https://github.com/lidongyooo)
- 🌟 👤 [lidongyooo](https://github.com/lidongyooo) Starred [tomgly/Android-SIM-Unlock](https://github.com/tomgly/Android-SIM-Unlock)
  > Notes and helper scripts for a Pixel-specific SIM/carrier-lock bypass method

### [liuzjMr](https://github.com/liuzjMr)
- 🍴 👤 [liuzjMr](https://github.com/liuzjMr) Forked [yilujian/easy-writing](https://github.com/yilujian/easy-writing) to [liuzjMr/easy-writing](https://github.com/liuzjMr/easy-writing)
  > 易创：纯本地、开源的 AI 网文写作桌面软件，支持小说创作、AI 辅助写作、BYOK 与自定义提示词。
- 🍴 👤 [liuzjMr](https://github.com/liuzjMr) Forked [ahujasid/camera-to-blender](https://github.com/ahujasid/camera-to-blender) to [liuzjMr/camera-to-blender](https://github.com/liuzjMr/camera-to-blender)
  > Take a photo of real objects, and paste them in Blender
- 🍴 👤 [liuzjMr](https://github.com/liuzjMr) Forked [0x-IHRR/Xiaobai-Skills](https://github.com/0x-IHRR/Xiaobai-Skills) to [liuzjMr/Xiaobai-Skills](https://github.com/liuzjMr/Xiaobai-Skills)
  >  Open-source skill
- 🍴 👤 [liuzjMr](https://github.com/liuzjMr) Forked [lumingya/universal-web-api](https://github.com/lumingya/universal-web-api) to [liuzjMr/universal-web-api](https://github.com/liuzjMr/universal-web-api)
  > 🤖通用性反代程序，通过模拟人类操作支持将任意网站转化为标准的OpenAi格式的api。
- 🍴 👤 [liuzjMr](https://github.com/liuzjMr) Forked [eternityspring/shuohao-skills](https://github.com/eternityspring/shuohao-skills) to [liuzjMr/shuohao-skills](https://github.com/liuzjMr/shuohao-skills)
  > AI 短剧制作的 skill 集合：拆角色、排大纲、出场景与道具设定、写剧本、切分镜 | Agent skills for AI short-drama production — character ...
- 🍴 👤 [liuzjMr](https://github.com/liuzjMr) Forked [NVIDIA/Personal-AI-Router](https://github.com/NVIDIA/Personal-AI-Router) to [liuzjMr/Personal-AI-Router](https://github.com/liuzjMr/Personal-AI-Router)
  > Router that virtually distributes inference across connected devices in the home.

### [llsc12](https://github.com/llsc12)
- 🌟 👤 [llsc12](https://github.com/llsc12) Starred [SwiftTUI/swift-tui](https://github.com/SwiftTUI/swift-tui)
  > SwiftUI semantics, drawn in terminal cells — macOS/Linux/Windows/WASI
- 🌟 👤 [llsc12](https://github.com/llsc12) Starred [amethystsoft/vein](https://github.com/amethystsoft/vein)
  > Cross platform declarative DB framework for apps, inspired by SwiftData, refined by us.

### [lz6688](https://github.com/lz6688)
- 🍴 👤 [lz6688](https://github.com/lz6688) Forked [DietrichGebert/ponytail](https://github.com/DietrichGebert/ponytail) to [lz6688/ponytail](https://github.com/lz6688/ponytail)
  > Makes your AI agent think like the laziest senior dev in the room. The best code is the code you nev...
- 🍴 👤 [lz6688](https://github.com/lz6688) Forked [vercel-labs/skills](https://github.com/vercel-labs/skills) to [lz6688/skills](https://github.com/lz6688/skills)
  > The open agent skills tool - npx skills

### [mgeeky](https://github.com/mgeeky)
- 🌟 👤 [mgeeky](https://github.com/mgeeky) Starred [Ignitetechnologies/Windows-Privilege-Escalation](https://github.com/Ignitetechnologies/Windows-Privilege-Escalation)

### [mo-xiaoxi](https://github.com/mo-xiaoxi)
- 🌟 👤 [mo-xiaoxi](https://github.com/mo-xiaoxi) Starred [can4hou6joeng4/boss-agent-cli](https://github.com/can4hou6joeng4/boss-agent-cli)
  > 🤖 Local-assist BOSS Zhipin CLI for AI agents — search, welfare filtering, shortlist, JSON-envelope o...

### [mq1n](https://github.com/mq1n)
- 🍴 👤 [mq1n](https://github.com/mq1n) Forked [SecretiveShell/Awesome-llms-txt](https://github.com/SecretiveShell/Awesome-llms-txt) to [mq1n/Awesome-llms-txt](https://github.com/mq1n/Awesome-llms-txt)
  > This list contains an index of llms.txt files hosted on various websites, to be used as seed data.
- 🍴 👤 [mq1n](https://github.com/mq1n) Forked [thedaviddias/llms-txt-hub](https://github.com/thedaviddias/llms-txt-hub) to [mq1n/llms-txt-hub](https://github.com/mq1n/llms-txt-hub)
  > 🤖 The largest directory for AI-ready documentation and tools implementing the proposed llms.txt stan...

### [nahueldsanchez](https://github.com/nahueldsanchez)
- 🌟 👤 [nahueldsanchez](https://github.com/nahueldsanchez) Starred [Graphify-Labs/graphify](https://github.com/Graphify-Labs/graphify)
  > Turn any codebase, with its docs, SQL schemas, configs, and PDFs, into a queryable knowledge graph. ...
- 🌟 👤 [nahueldsanchez](https://github.com/nahueldsanchez) Starred [google/mantis](https://github.com/google/mantis)
  > A modular, stack-agnostic toolkit of security review skills for AI coding agents to autonomously fin...

### [nek0y4nsu](https://github.com/nek0y4nsu)
- 🌟 👤 [nek0y4nsu](https://github.com/nek0y4nsu) Starred [ViRb3/wgcf](https://github.com/ViRb3/wgcf)
  > 🚤 Cross-platform, unofficial CLI for Cloudflare Warp
- 🌟 👤 [nek0y4nsu](https://github.com/nek0y4nsu) Starred [fscarmen/warp-sh](https://github.com/fscarmen/warp-sh)
  > WARP script is move to: https://gitlab.com/fscarmen/warp

### [nex121](https://github.com/nex121)
- 🌟 👤 [nex121](https://github.com/nex121) Starred [amd2g2zz/kunglao-agent](https://github.com/amd2g2zz/kunglao-agent)
  > The reverse-engineering expert agent: plans its own analysis path, derives every fact from raw evide...

### [null-dev](https://github.com/null-dev)
- 🌟 👤 [null-dev](https://github.com/null-dev) Starred [LINBIT/drbd](https://github.com/LINBIT/drbd)
  > LINBIT DRBD kernel module

### [omeraydindev](https://github.com/omeraydindev)
- 🌟 👤 [omeraydindev](https://github.com/omeraydindev) Starred [KytyPS5/KytyPS5](https://github.com/KytyPS5/KytyPS5)
  > PlayStation 5 emulator for Windows, Linux and MacOS

### [playGitboy](https://github.com/playGitboy)
- 🌟 👤 [playGitboy](https://github.com/playGitboy) Starred [jondkinney/omapop](https://github.com/jondkinney/omapop)
  > Select text and act on it: an Omarchy popup with PopClip-compatible extensions.

### [polygraphene](https://github.com/polygraphene)
- 🍴 👤 [polygraphene](https://github.com/polygraphene) Forked [snothin/ghostlock-s26](https://github.com/snothin/ghostlock-s26) to [polygraphene/ghostlock-s26](https://github.com/polygraphene/ghostlock-s26)
  > GhostLock (CVE-2026-43499) for the Galaxy S26

### [rabbitstack](https://github.com/rabbitstack)
- 🌟 👤 [rabbitstack](https://github.com/rabbitstack) Starred [BlackSnufkin/LitterBox](https://github.com/BlackSnufkin/LitterBox)
  > A self-hosted sandbox for red teams to test payloads against modern detection before deployment. MCP...
- 🌟 👤 [rabbitstack](https://github.com/rabbitstack) Starred [FalkorDB/FalkorDB](https://github.com/FalkorDB/FalkorDB)
  > A super fast Graph Database uses GraphBLAS under the hood for its sparse adjacency matrix graph repr...

### [rankalpha](https://github.com/rankalpha)
- 🌟 👤 [rankalpha](https://github.com/rankalpha) Starred [dengyie/zcode2api](https://github.com/dengyie/zcode2api)
  > ZCode 账号运营 + Anthropic 反代网关一体机 —— 账号池轮询、额度监控、活动套餐领取、浏览器过码转发

### [ravindu644](https://github.com/ravindu644)
- 🌟 👤 [ravindu644](https://github.com/ravindu644) Starred [cromaguy/Rhythm](https://github.com/cromaguy/Rhythm)
  > Your Music, Your Rhythm. An open-source, privacy-first Android music player featuring a beautiful Ma...

### [sjshb57](https://github.com/sjshb57)
- 🌟 👤 [sjshb57](https://github.com/sjshb57) Starred [crimera/piko-newx](https://github.com/crimera/piko-newx)
  > New X on Android compatible piko builds
- 🌟 👤 [sjshb57](https://github.com/sjshb57) Starred [eternalfuture-e38299/TEFManager](https://github.com/eternalfuture-e38299/TEFManager)
  > Cross-platform Terraria mod management tool · Built with Kotlin Multiplatform · Complete package typ...
- 🌟 👤 [sjshb57](https://github.com/sjshb57) Starred [shanmaomaoymmm/hiwara](https://github.com/shanmaomaoymmm/hiwara)
  > Iwara for Tauri, Apply mobile and desktop 

### [szcf-weiya](https://github.com/szcf-weiya)
- 🌟 👤 [szcf-weiya](https://github.com/szcf-weiya) Starred [harbor-framework/terminal-bench-science](https://github.com/harbor-framework/terminal-bench-science)
  > Terminal-Bench-Science: Evaluating AI agents on research workflows across scientific domains

### [tijme](https://github.com/tijme)
- 🌟 👤 [tijme](https://github.com/tijme) Starred [davi-1337/chrome-rce-extension](https://github.com/davi-1337/chrome-rce-extension)
  > chrome said that this is not a bug (intended behavior)

### [totravel](https://github.com/totravel)
- 🌟 👤 [totravel](https://github.com/totravel) Starred [laoma2053/awesome-zhuiju-free](https://github.com/laoma2053/awesome-zhuiju-free)
  > 免费无广告的追剧资源指南，人工精选资源、每天检测资源有效性。收录在线影视、影视APP、网盘搜索、磁力BT、字幕、TVBox / 影视仓空壳软件/配置地址、IPTV直播源、会员拼团、影视相关开源项目。开...

### [vilebbit](https://github.com/vilebbit)
- 🌟 👤 [vilebbit](https://github.com/vilebbit) Starred [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)
  > Wrap Antigravity, ChatGPT Codex, Claude Code, Grok Build as an OpenAI/Gemini/Claude/Codex compatible...

### [vipstone](https://github.com/vipstone)
- 🌟 👤 [vipstone](https://github.com/vipstone) Starred [HisMax/RedInk](https://github.com/HisMax/RedInk)
  > Red Ink - A one-stop Xiaohongshu image-and-text generator based on the 🍌Nano Banana Pro🍌, "One Sente...

### [wuhan005](https://github.com/wuhan005)
- 🍴 👤 [wuhan005](https://github.com/wuhan005) Forked [HansZ8/EncoreLightSticks](https://github.com/HansZ8/EncoreLightSticks) to [wuhan005/EncoreLightSticks](https://github.com/wuhan005/EncoreLightSticks)
  > ✨ 荧光棒复活计划 

### [wwh1004](https://github.com/wwh1004)
- 🌟 👤 [wwh1004](https://github.com/wwh1004) Starred [wwh1004/rdp-mcp](https://github.com/wwh1004/rdp-mcp)

### [xingguangcuican6666](https://github.com/xingguangcuican6666)
- 🚀 👤 [xingguangcuican6666](https://github.com/xingguangcuican6666) Made [MeowArch-open/MeowArchMoblie_Manifest](https://github.com/MeowArch-open/MeowArchMoblie_Manifest) public
  > MeowArchMoblie Manifest
- 🍴 👤 [xingguangcuican6666](https://github.com/xingguangcuican6666) Forked [archlinux/linux](https://github.com/archlinux/linux) to [MeowArch-open/MeowArchMoblie_Kernel](https://github.com/MeowArch-open/MeowArchMoblie_Kernel)
  > Arch Linux kernel sources, with patches

### [xixiaobei-bei](https://github.com/xixiaobei-bei)
- 🍴 👤 [xixiaobei-bei](https://github.com/xixiaobei-bei) Forked [AviderMin/ofrp_device_xiaomi_piano](https://github.com/AviderMin/ofrp_device_xiaomi_piano) to [xixiaobei-bei/ofrp_device_xiaomi_piano](https://github.com/xixiaobei-bei/ofrp_device_xiaomi_piano)

### [yahch](https://github.com/yahch)
- 🌟 👤 [yahch](https://github.com/yahch) Starred [Esdex/Arcanum](https://github.com/Esdex/Arcanum)
  > VeraCrypt-compatible encrypted vault manager for Android

### [yanglikai0806](https://github.com/yanglikai0806)
- 🌟 👤 [yanglikai0806](https://github.com/yanglikai0806) Starred [GCWing/BitFun](https://github.com/GCWing/BitFun)
  > BitFun combines a high-performance agent runtime written in Rust with a polished desktop application...

### [zer0yu](https://github.com/zer0yu)
- 🌟 👤 [zer0yu](https://github.com/zer0yu) Starred [omacom/omarchy](https://github.com/omacom/omarchy)
  > Beautiful, Modern & Opinionated Linux

### [zjw-swun](https://github.com/zjw-swun)
- 🌟 👤 [zjw-swun](https://github.com/zjw-swun) Starred [zjw-swun/xi_caipiao](https://github.com/zjw-swun/xi_caipiao)
  > 囍相逢网页刮刮乐演示Demo

### [zodiacon](https://github.com/zodiacon)
- 🍴 👤 [zodiacon](https://github.com/zodiacon) Forked [microsoft/winget-pkgs](https://github.com/microsoft/winget-pkgs) to [zodiacon/winget-pkgs](https://github.com/zodiacon/winget-pkgs)
  > The Microsoft community Windows Package Manager manifest repository


---
*最后更新于 2026-09-05 20:08:21 UTC*
*历史记录保存在 `archive` 目录中。*
